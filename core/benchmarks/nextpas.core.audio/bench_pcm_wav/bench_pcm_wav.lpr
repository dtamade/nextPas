...
var
  GSmall: IStream;
  GLarge: IStream;
  GWriteStream: IStream;
  GWriteSamples: array of SmallInt;
  GSink: UInt64;
...
begin
  GWriteStream.Position := 0;
  WritePcmWavStream(GWriteStream, 44100, 1, GWriteSamples);
  GSink := GSink xor UInt64(GWriteStream.Size);
end;
...
begin
  ...
  SetLength(GWriteSamples, (1024 * 1024) div SizeOf(SmallInt));
  for I := 0 to High(GWriteSamples) do
    GWriteSamples[I] := SmallInt((I * 37) and $FFFF);
  GWriteStream := BytesStream(1024 * 1024 + 64);
  GSink := 0;
