...
begin
  Check(TVec2f.Cross2D(TVec2f.Create(1.0, 0.0), TVec2f.Create(0.0, 1.0)) = 1.0,
    'TVec2f.Cross2D i x j = 1');
  Check(TVec2f.Cross2D(TVec2f.Create(0.0, 1.0), TVec2f.Create(1.0, 0.0)) = -1.0,
    'TVec2f.Cross2D j x i = -1');
  Check(TVec2f.Cross2D(TVec2f.Create(3.0, 4.0), TVec2f.Create(3.0, 4.0)) = 0.0,
    'TVec2f.Cross2D parallel = 0');
  Check(FloatEquals(TVec2f.Cross2D(TVec2f.Create(2.0, 3.0), TVec2f.Create(5.0, 7.0)),
    Single(-1.0), Single(1e-6)), 'TVec2f.Cross2D (2,3)x(5,7) = -1');
end;

procedure TestVec2dCross2D;
begin
  Check(TVec2d.Cross2D(TVec2d.Create(1.0, 0.0), TVec2d.Create(0.0, 1.0)) = 1.0,
    'TVec2d.Cross2D i x j = 1');
  Check(TVec2d.Cross2D(TVec2d.Create(0.0, 1.0), TVec2d.Create(1.0, 0.0)) = -1.0,
...
begin
  ...
  Check(TVec3f.Max(LA, LB).PerfectlyEquals(TVec3f.Create(4.0, 5.0, 6.0)),
    'TVec3f.Max component-wise');
  Check(TVec3f.Min(LA, LB).PerfectlyEquals(TVec3f.Create(1.0, 2.0, 3.0)),
    'TVec3f.Min component-wise');

  Check(TVec2f.Max(TVec2f.Create(1.0, 5.0), TVec2f.Create(4.0, 2.0)).PerfectlyEquals(
    TVec2f.Create(4.0, 5.0)), 'TVec2f.Max component-wise');
  Check(TVec4f.Max(TVec4f.Create(1.0, 5.0, 3.0, 7.0), TVec4f.Create(4.0, 2.0, 6.0, 1.0)).PerfectlyEquals(
...
begin
  ...
  Check(TVec3d.Max(LA, LB).PerfectlyEquals(TVec3d.Create(4.0, 5.0, 6.0)),
    'TVec3d.Max component-wise');
  Check(TVec3d.Min(LA, LB).PerfectlyEquals(TVec3d.Create(1.0, 2.0, 3.0)),
