...
procedure RaiseVec2fComponentDivZero;
begin
  Vec2fSink := TVec2f.Create(1.0, 2.0).ComponentDiv(TVec2f.Create(1.0, 0.0));
end;

procedure RaiseVec3fComponentDivNaN;
begin
  Vec3fSink := TVec3f.Create(1.0, 2.0, 3.0).ComponentDiv(TVec3f.Create(1.0, SingleNaN, 1.0));
end;

procedure RaiseVec4fComponentDivInfinity;
begin
  Vec4fSink := TVec4f.Create(1.0, 2.0, 3.0, 4.0).ComponentDiv(TVec4f.Create(1.0, 1.0, SingleInfinity, 1.0));
end;

procedure RaiseVec4fComponentDivNegativeInfinity;
begin
  Vec4fSink := TVec4f.Create(1.0, 2.0, 3.0, 4.0).ComponentDiv(TVec4f.Create(1.0, 1.0, 1.0, SingleNegativeInfinity));
end;

procedure RaiseVec2dComponentDivZero;
begin
  Vec2dSink := TVec2d.Create(1.0, 2.0).ComponentDiv(TVec2d.Create(0.0, 1.0));
end;

procedure RaiseVec2dComponentDivNegativeZero;
begin
  Vec2dSink := TVec2d.Create(1.0, 2.0).ComponentDiv(TVec2d.Create(1.0, DoubleNegativeZero));
end;

procedure RaiseVec3dComponentDivNaN;
begin
  Vec3dSink := TVec3d.Create(1.0, 2.0, 3.0).ComponentDiv(TVec3d.Create(1.0, 1.0, DoubleNaN));
end;

procedure RaiseVec4dComponentDivInfinity;
begin
  Vec4dSink := TVec4d.Create(1.0, 2.0, 3.0, 4.0).ComponentDiv(TVec4d.Create(DoubleInfinity, 1.0, 1.0, 1.0));
end;

procedure RaiseVec3fComponentDivNaNDividendByZeroAndNaNDivisors;
begin
  Vec3fSink := TVec3f.Create(SingleNaN, 2.0, 3.0).ComponentDiv(TVec3f.Create(0.0, SingleNaN, 1.0));
end;

procedure RaiseVec4dComponentDivFiniteDividendByZeroAndInfinityDivisors;
begin
  Vec4dSink := TVec4d.Create(1.0, 2.0, 3.0, 4.0).ComponentDiv(TVec4d.Create(1.0, 0.0, DoubleInfinity, 1.0));
end;
...
begin
  ...
  CheckVec2f(3.0, 8.0, A.ComponentMul(B), 'TVec2f component multiply');
  CheckVec2f(3.0, 2.0, A.ComponentDiv(B), 'TVec2f component divide');
  CheckNear(11.0, A.Dot(B), 0.000001, 'TVec2f dot');
  ...
  CheckVec2f(2.0, 3.0, B.Lerp(A, Single(0.5)), 'TVec2f lerp');
  Check(A.Equals(TVec2f.Create(3.0000001, 4.0000001), Single(0.000001)),
    'TVec2f equals epsilon');
  Check(not A.Equals(B, Single(0.000001)), 'TVec2f not equals');
  Check(not A.Equals(A, Single(-0.000001)), 'TVec2f Equals rejects negative epsilon');
end;
...
begin
  ...
  CheckVec3f(4.0, 10.0, 18.0, A.ComponentMul(B), 'TVec3f component multiply');
  CheckVec3f(4.0, 2.5, 2.0, B.ComponentDiv(A), 'TVec3f component divide');
  CheckNear(32.0, A.Dot(B), 0.000001, 'TVec3f dot');
  CheckVec3f(0.0, 0.0, 1.0, TVec3f.Create(1.0, 0.0, 0.0).Cross(TVec3f.Create(0.0, 1.0, 0.0)), 'TVec3f cross');
  ...
  CheckVec3f(2.5, 3.5, 4.5, A.Lerp(B, Single(0.5)), 'TVec3f lerp');
  Check(A.Equals(TVec3f.Create(1.0000001, 2.0000001, 3.0000001), Single(0.000001)),
    'TVec3f equals epsilon');
  Check(not A.Equals(A, Single(-0.000001)), 'TVec3f Equals rejects negative epsilon');
end;
...
begin
  ...
  CheckVec4f(5.0, 12.0, 21.0, 32.0, A.ComponentMul(B), 'TVec4f component multiply');
  CheckVec4f(5.0, 3.0, 7.0 / 3.0, 2.0, B.ComponentDiv(A), 'TVec4f component divide');
  CheckNear(70.0, A.Dot(B), 0.000001, 'TVec4f dot');
  ...
  CheckVec4f(3.0, 4.0, 5.0, 6.0, A.Lerp(B, Single(0.5)), 'TVec4f lerp');
  Check(A.Equals(TVec4f.Create(1.0000001, 2.0000001, 3.0000001, 4.0000001), Single(0.000001)), 'TVec4f equals epsilon');
  Check(not A.Equals(A, Single(-0.000001)), 'TVec4f Equals rejects negative epsilon');
end;
...
begin
  ...
  CheckVec2d(3.0, 8.0, V2.ComponentMul(TVec2d.Create(1.0, 2.0)),
    'TVec2d component multiply');
  CheckVec2d(3.0, 2.0, V2.ComponentDiv(TVec2d.Create(1.0, 2.0)),
    'TVec2d component divide');
  CheckNear(11.0, V2.Dot(TVec2d.Create(1.0, 2.0)), 0.000000000001, 'TVec2d dot');
  ...
  CheckVec2d(2.0, 3.0, TVec2d.Create(1.0, 2.0).Lerp(V2, 0.5), 'TVec2d lerp');
  Check(V2.Equals(TVec2d.Create(3.0000000000001, 4.0000000000001), 0.000000000001),
    'TVec2d equals epsilon');
  Check(not V2.Equals(V2, -0.000000000001), 'TVec2d Equals rejects negative epsilon');
  ...
  CheckVec3d(1.0, 0.5, Double(1.0) / Double(3.0),
    TVec3d.Create(1.0, 1.0, 1.0).ComponentDiv(V3),
    'TVec3d component divide');
  CheckVec3d(4.0, 10.0, 18.0, V3.ComponentMul(TVec3d.Create(4.0, 5.0, 6.0)),
    'TVec3d component multiply');
  CheckNear(32.0, V3.Dot(TVec3d.Create(4.0, 5.0, 6.0)), 0.000000000001,
    'TVec3d dot');
  CheckVec3d(0.0, 0.0, 1.0, TVec3d.Create(1.0, 0.0, 0.0).Cross(TVec3d.Create(0.0, 1.0, 0.0)), 'TVec3d cross');
  ...
  CheckVec3d(2.5, 3.5, 4.5, V3.Lerp(TVec3d.Create(4.0, 5.0, 6.0), 0.5),
    'TVec3d lerp');
  Check(V3.Equals(TVec3d.Create(1.0000000000001, 2.0000000000001, 3.0000000000001), 0.000000000001), 'TVec3d equals epsilon');
  Check(not V3.Equals(V3, -0.000000000001), 'TVec3d Equals rejects negative epsilon');
  ...
  CheckVec4d(1.0, 0.5, Double(1.0) / Double(3.0), 0.25,
    TVec4d.Create(1.0, 1.0, 1.0, 1.0).ComponentDiv(V4),
    'TVec4d component divide');
  CheckVec4d(5.0, 12.0, 21.0, 32.0, V4.ComponentMul(TVec4d.Create(5.0, 6.0, 7.0, 8.0)),
    'TVec4d component multiply');
  CheckNear(70.0, V4.Dot(TVec4d.Create(5.0, 6.0, 7.0, 8.0)), 0.000000000001,
    'TVec4d dot');
  ...
  CheckVec4d(3.0, 4.0, 5.0, 6.0, V4.Lerp(TVec4d.Create(5.0, 6.0, 7.0, 8.0), 0.5),
    'TVec4d lerp');
  Check(V4.Equals(TVec4d.Create(1.0000000000001, 2.0000000000001, 3.0000000000001,
    4.0000000000001), 0.000000000001), 'TVec4d equals epsilon');
  Check(not V4.Equals(V4, -0.000000000001), 'TVec4d Equals rejects negative epsilon');
end;
...
begin
  CheckSinglePositiveInfinity(TVec2f.Create(Single(3.0e20), Single(4.0e20)).Dot(TVec2f.Create(Single(3.0e20), Single(4.0e20))),
    'TVec2f huge finite Dot positive overflow returns +Inf');
  CheckSingleNegativeInfinity(TVec3f.Create(Single(3.0e20), Single(4.0e20), 0.0).Dot(TVec3f.Create(Single(-3.0e20), Single(-4.0e20), 0.0)),
    'TVec3f huge finite Dot negative overflow returns -Inf');
  CheckNear(0.0, TVec4f.Create(Single(3.0e20), Single(4.0e20), Single(-3.0e20), Single(-4.0e20)).Dot(TVec4f.Create(Single(3.0e20), Single(4.0e20), Single(3.0e20), Single(4.0e20))),
    0.0, 'TVec4f huge finite Dot cancellation stays finite zero');

  CheckDoublePositiveInfinity(TVec2d.Create(3.0e200, 4.0e200).Dot(TVec2d.Create(3.0e200, 4.0e200)),
    'TVec2d huge finite Dot positive overflow returns +Inf');
  CheckDoubleNegativeInfinity(TVec3d.Create(3.0e200, 4.0e200, 0.0).Dot(TVec3d.Create(-3.0e200, -4.0e200, 0.0)),
    'TVec3d huge finite Dot negative overflow returns -Inf');
  CheckNear(0.0, TVec4d.Create(3.0e200, 4.0e200, -3.0e200, -4.0e200).Dot(TVec4d.Create(3.0e200, 4.0e200, 3.0e200, 4.0e200)),
    0.0, 'TVec4d huge finite Dot cancellation stays finite zero');
  CheckNear(0.0, TVec2d.Create(1.0e-300, -1.0e-300).Dot(TVec2d.Create(1.0e-300, 1.0e-300)),
...
begin
  ...
  try
    ...
    CheckSingleNaNValue(TVec2f.Create(0.0, 1.0).Dot(TVec2f.Create(SingleInfinity, 2.0)),
      'TVec2f Dot zero times infinity returns NaN');
    CheckSingleNaNValue(TVec4f.Create(0.0, 1.0, 2.0, 3.0).Dot(TVec4f.Create(SingleInfinity, 2.0, 3.0, 4.0)),
      'TVec4f Dot zero times infinity returns NaN');
    CheckDoublePositiveInfinity(TVec3d.Create(DoubleInfinity, 2.0, 0.0).Dot(TVec3d.Create(1.0, 3.0, 0.0)),
      'TVec3d Dot infinite component returns +Inf');

    C3f := TVec3f.Create(0.0, 1.0, 0.0).Cross(TVec3f.Create(0.0, 0.0, SingleInfinity));
    CheckSinglePositiveInfinity(C3f.X,
    ...
    C3d := TVec3d.Create(0.0, 1.0, 0.0).Cross(TVec3d.Create(0.0, 0.0, DoubleInfinity));
    CheckDoublePositiveInfinity(C3d.X,
...
begin
  ...
  try
    ...
    CheckSinglePositiveInfinity(TVec3f.Create(SingleInfinity, 2.0, 0.0).Dot(TVec3f.Create(1.0, 3.0, 0.0)),
      'TVec3f Dot infinite component returns +Inf');
    CheckDoubleNaNValue(TVec2d.Create(0.0, 1.0).Dot(TVec2d.Create(DoubleInfinity, 2.0)),
      'TVec2d Dot zero times infinity returns NaN');
    CheckDoubleNaNValue(TVec4d.Create(0.0, 1.0, 2.0, 3.0).Dot(TVec4d.Create(DoubleInfinity, 2.0, 3.0, 4.0)),
...
begin
  ...
  try
    ...
    CheckDoubleNaNValue(TVec3d.Create(DoubleNaN, 2.0, 0.0).Dot(TVec3d.Create(1.0, 3.0, 0.0)),
      'TVec3d Dot NaN operand returns NaN');

    C3d := TVec3d.Create(DoubleNaN, 1.0, 0.0).Cross(TVec3d.Create(0.0, 0.0, 1.0));
...
begin
  ...
  try
    ...
    V3d := TVec3d.Create(0.0, DoubleNegativeZero, 2.0).ComponentMul(TVec3d.Create(DoubleInfinity, 3.0, DoubleNegativeZero));
...
begin
  ...
  CheckDoublePositiveZero(TVec4d.Create(1.0, -1.0, 0.0, 0.0).Dot(TVec4d.Create(1.0, 1.0, 0.0, 0.0)),
    'TVec4d exact zero Dot returns +0');

  C3f := TVec3f.Create(1.0, 0.0, 0.0).Cross(TVec3f.Create(2.0, 0.0, 0.0));
  ...
  Check(TVec2f.Create(SingleNegativeZero, 0.0).Equals(TVec2f.Create(0.0, SingleNegativeZero), Single(0.0)),
...
begin
  ...
  CheckSinglePositiveZero(TVec2f.Create(1.0, -1.0).Dot(TVec2f.Create(1.0, 1.0)),
    'TVec2f exact zero Dot returns +0');

  C3d := TVec3d.Create(1.0, 0.0, 0.0).Cross(TVec3d.Create(2.0, 0.0, 0.0));
...
begin
  ...
  ExpectArgumentErrorMessage('TVec2f.ComponentDiv: divisor vector must be finite and non-zero',
    'TVec2f component divide zero', @RaiseVec2fComponentDivZero);
  ExpectArgumentErrorMessage('TVec3f.ComponentDiv: divisor vector must be finite and non-zero',
    'TVec3f component divide NaN', @RaiseVec3fComponentDivNaN);
  ExpectArgumentErrorMessage('TVec4f.ComponentDiv: divisor vector must be finite and non-zero',
    'TVec4f component divide infinity', @RaiseVec4fComponentDivInfinity);
  ExpectArgumentErrorMessage('TVec4f.ComponentDiv: divisor vector must be finite and non-zero',
    'TVec4f component divide negative infinity', @RaiseVec4fComponentDivNegativeInfinity);
  ExpectArgumentErrorMessage('TVec2d.ComponentDiv: divisor vector must be finite and non-zero',
    'TVec2d component divide zero', @RaiseVec2dComponentDivZero);
  ExpectArgumentErrorMessage('TVec2d.ComponentDiv: divisor vector must be finite and non-zero',
    'TVec2d component divide negative zero', @RaiseVec2dComponentDivNegativeZero);
  ExpectArgumentErrorMessage('TVec3d.ComponentDiv: divisor vector must be finite and non-zero',
    'TVec3d component divide NaN', @RaiseVec3dComponentDivNaN);
  ExpectArgumentErrorMessage('TVec4d.ComponentDiv: divisor vector must be finite and non-zero',
    'TVec4d component divide infinity', @RaiseVec4dComponentDivInfinity);
end;
...
begin
  ...
  ExpectArgumentErrorMessage('TVec3f.ComponentDiv: divisor vector must be finite and non-zero',
    ...
    @RaiseVec3fComponentDivNaNDividendByZeroAndNaNDivisors);
  ExpectArgumentErrorMessage('TVec4d.ComponentDiv: divisor vector must be finite and non-zero',
    'TVec4d component divide finite dividend by zero and infinity divisors',
    @RaiseVec4dComponentDivFiniteDividendByZeroAndInfinityDivisors);
end;

procedure TestVectorEqualsNonFiniteComparisonContracts;
begin
  Check(not TVec2f.Create(SingleNaN, 1.0).Equals(TVec2f.Create(SingleNaN, 1.0), Single(0.0)), 'TVec2f Equals rejects NaN components');
  Check(TVec2f.Create(SingleInfinity, 1.0).Equals(TVec2f.Create(SingleInfinity, 1.0), Single(0.0)), 'TVec2f Equals accepts matching positive infinity');
  Check(not TVec2f.Create(SingleInfinity, 1.0).Equals(TVec2f.Create(SingleNegativeInfinity, 1.0), Single(0.0)),
    'TVec2f Equals rejects opposite infinities');
  Check(not TVec2f.Create(1.0, 2.0).Equals(TVec2f.Create(1.0, 2.0), SingleNaN), 'TVec2f Equals rejects NaN epsilon');

  Check(not TVec3f.Create(1.0, SingleInfinity, 3.0).Equals(TVec3f.Create(1.0, 2.0, 3.0), Single(0.0)),
    'TVec3f Equals rejects finite vs infinity');
  Check(not TVec3f.Create(1.0, 2.0, 3.0).Equals(TVec3f.Create(1.0, 2.0, 3.0), SingleInfinity),
    'TVec3f Equals rejects infinite epsilon');
  Check(not TVec4f.Create(1.0, 2.0, 3.0, 4.0).Equals(TVec4f.Create(1.0, 2.0, 3.0, 4.0), SingleNegativeInfinity),
    'TVec4f Equals rejects negative infinite epsilon');

  Check(not TVec2d.Create(DoubleNaN, 1.0).Equals(TVec2d.Create(DoubleNaN, 1.0), 0.0), 'TVec2d Equals rejects NaN components');
  Check(TVec2d.Create(DoubleNegativeInfinity, 1.0).Equals(TVec2d.Create(DoubleNegativeInfinity, 1.0), 0.0),
    'TVec2d Equals accepts matching negative infinity');
  Check(not TVec2d.Create(DoubleInfinity, 1.0).Equals(TVec2d.Create(DoubleNegativeInfinity, 1.0), 0.0),
    'TVec2d Equals rejects opposite infinities');
  Check(not TVec2d.Create(1.0, 2.0).Equals(TVec2d.Create(1.0, 2.0), DoubleNaN), 'TVec2d Equals rejects NaN epsilon');

  Check(not TVec3d.Create(1.0, DoubleInfinity, 3.0).Equals(TVec3d.Create(1.0, 2.0, 3.0), 0.0),
    'TVec3d Equals rejects finite vs infinity');
  Check(not TVec3d.Create(1.0, 2.0, 3.0).Equals(TVec3d.Create(1.0, 2.0, 3.0), DoubleInfinity),
    'TVec3d Equals rejects infinite epsilon');
  Check(not TVec4d.Create(1.0, 2.0, 3.0, 4.0).Equals(TVec4d.Create(1.0, 2.0, 3.0, 4.0), DoubleNegativeInfinity),
    'TVec4d Equals rejects negative infinite epsilon');
end;

procedure TestVec2fCross2D;
begin
  Check(TVec2f.Create(1.0, 0.0).Cross2D(TVec2f.Create(0.0, 1.0)) = 1.0,
    'TVec2f.Cross2D i x j = 1');
  Check(TVec2f.Create(0.0, 1.0).Cross2D(TVec2f.Create(1.0, 0.0)) = -1.0,
    'TVec2f.Cross2D j x i = -1');
  Check(TVec2f.Create(3.0, 4.0).Cross2D(TVec2f.Create(3.0, 4.0)) = 0.0,
    'TVec2f.Cross2D parallel = 0');
  Check(FloatEquals(TVec2f.Create(2.0, 3.0).Cross2D(TVec2f.Create(5.0, 7.0)),
    Single(-1.0), Single(1e-6)), 'TVec2f.Cross2D (2,3)x(5,7) = -1');
end;

procedure TestVec2dCross2D;
begin
  Check(TVec2d.Create(1.0, 0.0).Cross2D(TVec2d.Create(0.0, 1.0)) = 1.0,
    'TVec2d.Cross2D i x j = 1');
  Check(TVec2d.Create(0.0, 1.0).Cross2D(TVec2d.Create(1.0, 0.0)) = -1.0,
    'TVec2d.Cross2D j x i = -1');
end;

procedure TestVecMaxMinComponent;
var
  LA, LB: TVec3f;
begin
  LA := TVec3f.Create(1.0, 5.0, 3.0);
  LB := TVec3f.Create(4.0, 2.0, 6.0);
  Check(LA.Max(LB).PerfectlyEquals(TVec3f.Create(4.0, 5.0, 6.0)),
    'TVec3f.Max component-wise');
  Check(LA.Min(LB).PerfectlyEquals(TVec3f.Create(1.0, 2.0, 3.0)),
    'TVec3f.Min component-wise');

  Check(TVec2f.Create(1.0, 5.0).Max(TVec2f.Create(4.0, 2.0)).PerfectlyEquals(
    TVec2f.Create(4.0, 5.0)), 'TVec2f.Max component-wise');
  Check(TVec4f.Create(1.0, 5.0, 3.0, 7.0).Max(TVec4f.Create(4.0, 2.0, 6.0, 1.0)).PerfectlyEquals(
    TVec4f.Create(4.0, 5.0, 6.0, 7.0)), 'TVec4f.Max component-wise');
end;

procedure TestVecMaxMinDouble;
var
  LA, LB: TVec3d;
begin
  LA := TVec3d.Create(1.0, 5.0, 3.0);
  LB := TVec3d.Create(4.0, 2.0, 6.0);
  Check(LA.Max(LB).PerfectlyEquals(TVec3d.Create(4.0, 5.0, 6.0)),
    'TVec3d.Max component-wise');
  Check(LA.Min(LB).PerfectlyEquals(TVec3d.Create(1.0, 2.0, 3.0)),
    'TVec3d.Min component-wise');
end;

procedure TestVecAbs;
begin
  Check(TVec2f.Create(-1.0, 2.0).Abs.PerfectlyEquals(TVec2f.Create(1.0, 2.0)),
    'TVec2f.Abs');
  Check(TVec3f.Create(-1.0, 2.0, -3.0).Abs.PerfectlyEquals(TVec3f.Create(1.0, 2.0, 3.0)),
    'TVec3f.Abs');
  Check(TVec4f.Create(-1.0, 2.0, -3.0, 0.0).Abs.PerfectlyEquals(
    TVec4f.Create(1.0, 2.0, 3.0, 0.0)), 'TVec4f.Abs');
  Check(TVec2d.Create(-1.0, 2.0).Abs.PerfectlyEquals(TVec2d.Create(1.0, 2.0)),
    'TVec2d.Abs');
end;

procedure TestVecIsZero;
begin
  Check(TVec2f.Create(0.0, 0.0).IsZero, 'TVec2f.IsZero true');
  Check(not TVec2f.Create(0.0, 1.0).IsZero, 'TVec2f.IsZero false');
  Check(TVec3f.Create(0.0, 0.0, 0.0).IsZero, 'TVec3f.IsZero true');
  Check(not TVec3f.Create(0.0, 0.0, 0.001).IsZero, 'TVec3f.IsZero false');
  Check(TVec4f.Create(0.0, 0.0, 0.0, 0.0).IsZero, 'TVec4f.IsZero true');
  Check(not TVec4f.Create(0.0, 0.0, 0.0, 1.0).IsZero, 'TVec4f.IsZero false');
  Check(TVec3d.Create(0.0, 0.0, 0.0).IsZero, 'TVec3d.IsZero true');
end;

procedure TestVecPerfectlyEquals;
begin
  Check(TVec2f.Create(1.0, 2.0).PerfectlyEquals(TVec2f.Create(1.0, 2.0)),
    'TVec2f.PerfectlyEquals same');
  Check(not TVec2f.Create(1.0, 2.0).PerfectlyEquals(TVec2f.Create(1.0, 2.001)),
    'TVec2f.PerfectlyEquals different');
  Check(TVec3f.Create(1.0, 2.0, 3.0).PerfectlyEquals(TVec3f.Create(1.0, 2.0, 3.0)),
    'TVec3f.PerfectlyEquals same');
  Check(TVec4f.Create(1.0, 2.0, 3.0, 4.0).PerfectlyEquals(TVec4f.Create(1.0, 2.0, 3.0, 4.0)),
    'TVec4f.PerfectlyEquals same');
  Check(not TVec4f.Create(1.0, 2.0, 3.0, 4.0).PerfectlyEquals(
    TVec4f.Create(1.0, 2.0, 3.0, 4.0001)),
    'TVec4f.PerfectlyEquals different');
end;

procedure TestVec3fAverage;
var
  LA, LB, LR: TVec3f;
begin
  LA := TVec3f.Create(2.0, 4.0, 6.0);
  LB := TVec3f.Create(4.0, 8.0, 12.0);
  LR := LA.Average(LB);
  Check(LR.PerfectlyEquals(TVec3f.Create(3.0, 6.0, 9.0)), 'TVec3f.Average');
end;

procedure TestVec3fAdjustToLength;
var
  LDir, LR: TVec3f;
begin
  LDir := TVec3f.Create(3.0, 4.0, 0.0);
  LR := LDir.AdjustToLength(10.0);
  Check(FloatEquals(LR.Length, Single(10.0), Single(0.001)),
    'TVec3f.AdjustToLength length = 10');
  Check(TVec3f.Create(0.0, 0.0, 0.0).AdjustToLength(5.0).IsZero,
    'TVec3f.AdjustToLength zero vec returns zero');
end;

procedure TestVec4fToPosition;
begin
  Check(TVec4f.Create(2.0, 4.0, 6.0, 2.0).ToPosition.PerfectlyEquals(
    TVec3f.Create(1.0, 2.0, 3.0)), 'TVec4f.ToPosition perspective divide');
  Check(TVec4f.Create(1.0, 2.0, 3.0, 0.0).ToPosition.PerfectlyEquals(
    TVec3f.Create(1.0, 2.0, 3.0)), 'TVec4f.ToPosition w=0 no divide');
  Check(TVec4d.Create(2.0, 4.0, 6.0, 2.0).ToPosition.PerfectlyEquals(
