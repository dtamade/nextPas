...
{ S17 gate: async SFTP via TAsyncLoop + loopback TCP.
  Reuses session_asyc's loopback host key/kex/cipher, server adds
...
begin
  if Length(A)=0 then Exit(Copy(B,0,Length(B))); if Length(B)=0 then Exit(Copy(A,0,Length(A)));
  SetLength(Result, SizeUInt(Length(A)+Length(B))); Move(A[0], Result[0], SizeUInt(Length(A))); Move(B[0], Result[SizeUInt(Length(A))], SizeUInt(Length(B)));
end;
...
begin
  ...
  repeat if not RecvRaw(LLine,8000) then begin Fail('no version'); Exit; end; LText:=BytesToText(LLine); LNl:=Pos(#10, LText); if LNl>0 then begin if SizeUInt(Length(LLine))>SizeUInt(LNl) then FBuf:=ConcatBytes(Copy(LLine,LNl,Length(LLine)-LNl), FBuf); LText:=Trim(Copy(LText,1,LNl)); if Copy(LText,1,4)='SSH-' then LVc:=StringToBytes(LText); end; until Length(LVc)>0;
