program test_sfx;
{$mode objfpc}{$H+}
uses nextpas.core.exception, nextpas.core.base, nextpas.core.test, nextpas.core.audio.base, nextpas.core.audio.sfx.intf, nextpas.core.audio.sfx, nextpas.core.audio.device.null, nextpas.core.audio;
...
var G: ISfxAudio; S: TSfxId; V: TVoiceId;
begin
  G:=CreateSfxAudioForFormat(CreateNullAudioProvider, AudioFormatCreate(48000,2,sfF32), 8);
...
procedure T.TestPan;
var G: ISfxAudio; S: TSfxId; Buf: TAudioBuffer; P: PSingle;
begin
  G:=CreateSfxAudioForFormat(CreateNullAudioProvider, AudioFormatCreate(48000,2,sfF32), 8);
...
var G: ISfxAudio; S: TSfxId; Buf: TAudioBuffer; P: PSingle; Fmt: TAudioFormat;
begin
  Fmt:=AudioFormatCreate(48000,1,sfF32);
  G:=CreateSfxAudioForFormat(CreateNullAudioProvider, Fmt, 8);
...
var G: ISfxAudio; S: TSfxId; Buf: TAudioBuffer;
begin
  G:=CreateSfxAudioForFormat(CreateNullAudioProvider, AudioFormatCreate(48000,1,sfF32), 8);
...
var G: ISfxAudio; S: TSfxId;
begin
  G:=CreateSfxAudioForFormat(CreateNullAudioProvider, AudioFormatCreate(48000,1,sfF32), 2);
...
var G: ISfxAudio; OK: Boolean; Buf: TAudioBuffer;
begin
  G:=CreateSfxAudioForFormat(CreateNullAudioProvider, AudioFormatCreate(48000,2,sfF32), 8);
...
var G: ISfxAudio; GG: IGameAudio;
begin
  G:=nextpas.core.audio.CreateSfxAudioForFormat(CreateNullAudioProvider, AudioFormatCreate(48000,2,sfF32), 4);
  CheckTrue(Assigned(G),'facade sfx');
  GG:=nextpas.core.audio.CreateGameAudioForFormat(CreateNullAudioProvider, AudioFormatCreate(48000,2,sfF32), 4);
  CheckTrue(Assigned(GG),'facade game deprecated still works');
end;
...
begin
  C:=T.Create;
  S:=TTestSuite.Create('nextpas.core.audio.sfx');
