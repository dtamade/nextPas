...
for f in "$SRC/nextpas.core.audio.base.pas" "$SRC/nextpas.core.audio.pas" "$SRC/nextpas.core.audio.intf.pas" "$SRC/nextpas.core.audio.codec.intf.pas" "$SRC/nextpas.core.audio.codec.wav.pas" "$SRC/nextpas.core.audio.codec.aiff.pas" "$SRC/nextpas.core.audio.codec.meta.pas" "$SRC/nextpas.core.audio.codec.registry.pas" "$SRC/nextpas.core.audio.pcm.pas" "$SRC/nextpas.core.audio.pcm_wav.pas" "$SRC/nextpas.core.audio.resample.pas" "$SRC/nextpas.core.audio.resample.sinc.pas" "$SRC/nextpas.core.audio.mix.pas" "$SRC/nextpas.core.audio.dsp.filters.pas" "$SRC/nextpas.core.audio.dsp.dynamics.pas" "$SRC/nextpas.core.audio.dsp.fft.pas" "$SRC/nextpas.core.audio.device.intf.pas" "$SRC/nextpas.core.audio.device.null.pas" "$SRC/nextpas.core.audio.graph.intf.pas" "$SRC/nextpas.core.audio.graph.pas" "$SRC/nextpas.core.audio.player.pas" "$SRC/nextpas.core.audio.game.intf.pas" "$SRC/nextpas.core.audio.game.pas" "$SRC/nextpas.core.audio.timeline.intf.pas" "$SRC/nextpas.core.audio.timeline.pas" "$SRC/nextpas.core.audio.errors.pas"; do if [ ! -f "$f" ]; then echo "[FAIL] missing $f"; fail=1; continue; fi; check_no_ffi "$f"; echo "[OK] no ffi/vendor in $(basename "$f")"; done
...
# 实时纪律全量检查：实时实现必须含 FillRealtime
for rf in "$SRC"/nextpas.core.audio.device.null.pas "$SRC"/nextpas.core.audio.graph.pas "$SRC"/nextpas.core.audio.timeline.pas; do
  if ! grep -q "FillRealtime" "$rf"; then echo "[FAIL] $rf missing FillRealtime"; fail=1; fi
done
echo "[OK] realtime impl FillRealtime present"
