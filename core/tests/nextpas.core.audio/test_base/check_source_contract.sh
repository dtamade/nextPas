...
# audio source-contract gate — 69 files (core 26 + extension 43, unique 67+2 bus facade) — 契约 1.4
# Header: 56→69 sync — enumeration includes bank/resource/event/spatial.impl + 5 base + playlist 4件套 + codec 3×3 decoder/sse, for loop must include test_automation
echo "== audio source-contract gate (69 files: core 26 + extension 43) =="
for f in \
...
echo "[OK] 69 files (core 26 + candidate 43) no ffi/vendor — 契约 1.4 已对齐实盘（unique 67+2 bus facade）"
...
# 裸露桩白名单：flac/mp3/vorbis OpenStreaming not implemented 必须带 STUB 注释，gate 允许过渡桩
for codec in flac mp3 vorbis; do
  if grep -q "OpenStreaming.*not implemented" "$SRC/nextpas.core.audio.codec.$codec.pas"; then
    if ! grep -q "STUB: OpenStreaming" "$SRC/nextpas.core.audio.codec.$codec.pas"; then echo "[FAIL] $codec OpenStreaming stub missing STUB marker"; fail=1; else echo "[OK] $codec OpenStreaming STUB marker present"; fi
  fi
done
...
if [ ! -f "$SRC/nextpas.core.audio.bus.base.pas" ]; then echo "[FAIL] bus.base missing (69 enumeration)"; fail=1; else echo "[OK] bus.base present (base豁免 GUID)"; fi
if [ ! -f "$SRC/nextpas.core.audio.bus.impl.pas" ]; then echo "[FAIL] bus.impl missing (69 enumeration)"; fail=1; else echo "[OK] bus.impl present (impl复用 intf GUID豁免)"; fi
...
echo "source-contract gate PASSED — 69 files (core 26 + extension 43) + 23 GUID + test_automation — 契约 1.4"
