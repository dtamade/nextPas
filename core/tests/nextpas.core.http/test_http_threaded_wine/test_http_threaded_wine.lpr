program test_http_threaded_wine;
{**
 * @desc HTTP Wine runtime smoke (Win64 cross + wine).
 *       Proves product default backend (tsbThreaded) + HTTP/1.1 wire under Wine.
 *       Uses net.server.threaded + minimal wire (not full http facade/TLS chain):
 *       full nextpas.core.http Win64 cross currently blocked on TLS→system.sysutils
 *       FPC internal exception — residual documented in CONTRACT.
 *       truth=wine-runtime-smoke — NOT real-Windows, NOT Windows scale-ready.
 *       Multi-OS host smoke: test_http_threaded_host.
 *}
...
begin
  ...
  Check(AServer.IsRunning, 'threaded TCP server started under Wine');
...
begin
  ...
  Check(LOpts.Backend = TCP_SERVER_BACKEND_THREADED,
    'HTTP Default backend remains threaded (Wine path)');
end;
...
begin
  WriteLn('not Windows runtime evidence on this host; use test_http_threaded_host or wine-runtime-smoke');
...
begin
  T := TTestSuite.Create('nextpas.core.http.threaded_wine_smoke');
  {$IFDEF NEXTPAS_WINDOWS}
  T.Test('HTTP Default backend is threaded', @TestDefaultHttpBackendIsThreaded);
  T.Test('threaded HTTP/1.1 wire GET under Wine', @TestThreadedHttpWireGetOk);
  {$ELSE}
