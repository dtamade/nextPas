...
begin
  Result := CreateJsRuntime(jsbkJs888);
end;
...
begin
  RT := TJsJs888Runtime.Create(jsbkJs888, TJsRuntimeOptions.WithTimeout(10));
  Ctx := RT.NewContext;
  Raised := False;
  try
    Ctx.Eval('while(true) {}');
...
var RT: IJsRuntime; Ctx: IJsContext; Raised: Boolean;
begin
  RT := TJsJs888Runtime.Create(jsbkJs888, TJsRuntimeOptions.WithMemoryLimit(512));
  Ctx := RT.NewContext;
  Raised := False;
  try
    Ctx.Eval('1+2');
...
begin
  Check(JsBackendAvailable(jsbkJs888), 'fake avail');
...
begin
  Result := JsUndefinedValue;
  raise EJsError.Create('host boom', jecUnknown, 'Error', '', jsbkJs888);
end;
...
begin
  ...
  Check(RT.Kind = jsbkJs888, 'kind');
end;

procedure TestRuntimeOptionsPropagate;
var RT: IJsRuntime; Ctx: IJsContext;
begin
  RT := TJsJs888Runtime.Create(jsbkJs888, TJsRuntimeOptions.WithTimeout(123));
...
begin
  ...
  RT := CreateJsRuntime(jsbkJs888, TJsRuntimeOptions.Default);
  Check(RT.Kind = jsbkJs888, 'explicit js888');
end;
...
begin
  T := TTestSuite.Create('nextpas.core.js.js888');
  // 值语义 6
