...
begin
  RT := TJsV8Runtime.Create(jsbkV8, TJsRuntimeOptions.WithTimeout(10));
  Ctx := RT.NewContext;
  Raised := False;
  try
    Ctx.Eval('while(true) {}');
...
var RT: IJsRuntime; Ctx: IJsContext; Raised: Boolean;
begin
  RT := TJsV8Runtime.Create(jsbkV8, TJsRuntimeOptions.WithMemoryLimit(512));
  Ctx := RT.NewContext;
  Raised := False;
  try
    Ctx.Eval('1+2');
...
begin
  Check(JsBackendAvailable(jsbkV8), 'fake avail');
...
begin
  Result := JsUndefinedValue;
  raise EJsError.Create('host boom', jecUnknown, 'Error', '', jsbkV8);
end;
...
begin
  ...
  Check(RT.Kind = jsbkV8, 'kind');
end;

procedure TestRuntimeOptionsPropagate;
var RT: IJsRuntime; Ctx: IJsContext;
begin
  RT := TJsV8Runtime.Create(jsbkV8, TJsRuntimeOptions.WithTimeout(123));
...
begin
  ...
  RT := CreateJsRuntime(jsbkV8, TJsRuntimeOptions.Default);
  Check(RT.Kind = jsbkV8, 'explicit js888');
end;
...
begin
  T := TTestSuite.Create('nextpas.core.js.v8');
  // 值语义 6
