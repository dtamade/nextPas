...
begin
  T := TTestSuite.Create('nextpas.core.regex');
  T.Test('Literal', @TestLiteral);
  T.Test('Dot', @TestDot);
  T.Test('Star', @TestStar);
  T.Test('Plus', @TestPlus);
  T.Test('Question', @TestQuestion);
  T.Test('Alternation', @TestAlternation);
  T.Test('CharClass', @TestCharClass);
  T.Test('Shorthand (\d \w \s)', @TestShorthand);
  T.Test('Anchors (^ $)', @TestAnchors);
  T.Test('Find', @TestFind);
  T.Test('FindAll', @TestFindAll);
  T.Test('Replace', @TestReplace);
  T.Test('Split', @TestSplit);
  T.Test('Repetition {n,m}', @TestRepetition);
  T.Test('TryCompile', @TestTryCompile);
  T.Test('Convenience functions', @TestConvenience);
  T.Test('No exponential blowup', @TestNoExponentialBlowup);
  T.Test('Capture groups', @TestCaptureGroups);
  T.Test('Word boundary (\b)', @TestWordBoundary);
  T.Test('Non-capturing group', @TestNonCapturingGroup);
  T.Test('Edge cases', @TestEdgeCases);
  T.Test('Escapes', @TestEscapes);
  T.Test('StartClass prefilter', @TestStartClassPrefilter);
  T.Test('Named groups', @TestNamedGroups);
  T.Test('Negated shorthand (\D \W \S)', @TestNegatedShorthand);
  T.Test('FindAt', @TestFindAt);
  T.Test('IsFullMatch', @TestIsFullMatch);
  T.Test('QuoteMeta', @TestQuoteMeta);
  T.Test('Split with limit', @TestSplitLimit);
  T.Test('ReplaceFunc', @TestReplaceFunc);
  T.Test('ReplaceExpand', @TestReplaceExpand);
  T.Test('Zero repeat {0}', @TestZeroRepeat);
  T.Test('Alternation priority', @TestAlternationPriority);
  T.Test('Split zero', @TestSplitZero);
  T.Test('Malformed template', @TestMalformedTemplate);
  T.Test('FindAt boundary', @TestFindAtBoundary);
  T.Test('Large repeat {100}', @TestLargeRepeat);
  T.Test('Memory stress', @TestMemoryStress);
  T.Test('Compile limits', @TestCompileLimits);
  T.Test('Malformed patterns', @TestMalformedPatterns);
  T.Test('Case insensitive', @TestCaseInsensitive);
  T.Test('Regex flags public contract', @TestRegexFlagsPublicContract);
  { --- New comprehensive tests --- }
  T.Test('Greedy vs Non-Greedy', @TestGreedyVsNonGreedy);
  T.Test('Capture edge cases', @TestCaptureEdgeCases);
  T.Test('FindAll edge cases', @TestFindAllEdgeCases);
  T.Test('Split edge cases', @TestSplitEdgeCases);
  T.Test('Char class edge cases', @TestCharClassEdgeCases);
  T.Test('Anchor edge cases', @TestAnchorEdgeCases);
  T.Test('Escape edge cases', @TestEscapeEdgeCases);
  T.Test('IsFullMatch edge cases', @TestIsFullMatchEdgeCases);
  T.Test('Case insensitive edge cases', @TestCaseInsensitiveEdgeCases);
  T.Test('Performance regression', @TestPerformanceRegression);
  T.Test('QuoteMeta thorough', @TestQuoteMetaThorough);
  T.Test('ReplaceFunc edge cases', @TestReplaceFuncEdgeCases);
  T.Test('ReplaceFunc nil callback', @TestReplaceFuncNilCallback);
  { --- 13 new test areas --- }
  T.Test('Multiple captures', @TestMultipleCaptures);
  T.Test('Overlapping patterns', @TestOverlappingPatterns);
  T.Test('Special inputs', @TestSpecialInputs);
  T.Test('Quantifier boundaries', @TestQuantifierBoundaries);
  T.Test('Complex patterns', @TestComplexPatterns);
  T.Test('FindAll progress', @TestFindAllProgress);
  T.Test('ReplaceAllExpand advanced', @TestReplaceAllExpandAdvanced);
  T.Test('Split advanced', @TestSplitAdvanced);
  T.Test('Case insensitive advanced', @TestCaseInsensitiveAdvanced);
  T.Test('Backtracking immunity', @TestBacktrackingImmunity);
  T.Test('GroupByName advanced', @TestGroupByNameAdvanced);
  T.Test('IsMatch vs Find', @TestIsMatchVsFind);
  T.Test('Compile reuse', @TestCompileReuse);
  { --- 13 semantic conformance tests --- }
  T.Test('Find positions', @TestFindPositions);
  T.Test('FindAll positions', @TestFindAllPositions);
  T.Test('Capture positions', @TestCapturePositions);
  T.Test('Replace positional', @TestReplacePositional);
  T.Test('Split positional', @TestSplitPositional);
  T.Test('Long patterns', @TestLongPatterns);
  T.Test('Empty match behavior', @TestEmptyMatchBehavior);
  T.Test('Word boundary detailed', @TestWordBoundaryDetailed);
  T.Test('Quantifier interaction', @TestQuantifierInteraction);
  T.Test('Anchored search', @TestAnchoredSearch);
  T.Test('Dot behavior', @TestDotBehavior);
  T.Test('Char class ranges', @TestCharClassRanges);
  T.Test('Regex reuse', @TestRegexReuse);
  { --- 13 adversarial stress tests --- }
  T.Test('ADV: Adversarial patterns', @TestAdversarialPatterns);
  T.Test('ADV: Binary input', @TestBinaryInput);
  T.Test('ADV: Repeated compile', @TestRepeatedCompile);
  T.Test('ADV: Maximal input', @TestMaximalInput);
  T.Test('ADV: TryCompile errors', @TestTryCompileErrors);
  T.Test('ADV: FindAll consistency', @TestFindAllConsistency);
  T.Test('ADV: ReplaceExpand consistency', @TestReplaceExpandConsistency);
  T.Test('ADV: Pattern syntax coverage', @TestPatternSyntaxCoverage);
  T.Test('ADV: IsFullMatch thorough', @TestIsFullMatchThorough);
  T.Test('ADV: Group interaction with replace', @TestGroupInteractionWithReplace);
  ...
  T.Test('ENC: Control chars', @TestControlChars);
  T.Test('ENC: Byte exactness', @TestByteExactness);
  T.Test('ENC: Word boundary with encoding', @TestWordBoundaryWithEncoding);
  T.Test('ENC: Char class with high bytes', @TestCharClassWithHighBytes);
  T.Test('ENC: Newline handling', @TestNewlineHandling);
  T.Test('ENC: Mixed encoding input', @TestMixedEncodingInput);
  T.Test('ENC: Pattern with high bytes', @TestPatternWithHighBytes);
  T.Test('ENC: Off-by-one positions', @TestOffByOnePositions);
  T.Test('ENC: Case folding boundary', @TestCaseFoldingBoundary);
  T.Test('ENC: Shorthand with encoding', @TestShorthandWithEncoding);
  T.Test('ENC: Value extraction', @TestValueExtraction);
  { --- DFA v2 assertion correctness tests --- }
  T.Test('DFA: Assertion correctness', @TestDfaAssertionCorrectness);
  T.Test('DFA: FindAll correctness', @TestDfaFindAllCorrectness);
  T.Test('DFA: NFA cross-validation', @TestDfaNfaCrossValidation);
  T.Test('DFA: Overflow fallback', @TestDfaOverflowFallback);
  { --- ScanFindSubstring tests --- }
  T.Test('ScanFindSubstring', @TestScanFindSubstring);
  { --- FindAll AMaxMatches tests --- }
  T.Test('FindAll MaxMatches', @TestFindAllMaxMatches);
  T.Test('FindAll MaxMatches=0', @TestFindAllMaxMatchesZero);
  T.Test('FindAll MaxMatches=-1', @TestFindAllMaxMatchesNegative);
  T.Test('FindAll MaxMatches>total', @TestFindAllMaxMatchesExceedsTotal);
  { --- Phase 4 API + Edge Cases --- }
  T.Test('P4: FindIter', @TestFindIter);
  T.Test('P4: FindIter empty', @TestFindIterEmpty);
  T.Test('P4: FindIter zero-len', @TestFindIterZeroLen);
  T.Test('P4: FindIter single', @TestFindIterSingle);
  T.Test('P4: FindIter program snapshot', @TestFindIterKeepsProgramSnapshot);
  T.Test('P4: SubexpNames', @TestSubexpNames);
  T.Test('P4: SubexpNames empty', @TestSubexpNamesEmpty);
  T.Test('P4: Longest match', @TestLongestMatch);
  T.Test('P4: Replace edge cases', @TestReplaceEdgeCasesP4);
  T.Test('P4: Split edge cases', @TestSplitEdgeCases2);
  T.Test('P4: Large input 100KB', @TestLargeInput);
  T.Test('P4: Unicode basic', @TestUnicodeBasic);
  T.Test('P4: Error patterns', @TestErrorPatterns);
  T.Test('P4: Iter/FindAll consistency', @TestIterConsistencyWithFindAll);
