...
### [QA-006] mem.utils.pas 1313 行 / 71 个导出函数 — ⚠️ 真实质量债务
- **严重度**: major (保持)
- **维度**: maintainability
- **文件**: `nextpas.core.mem.utils.pas`
- **描述**: 此文件包含内存操作工具（Copy/Fill/Zero/Compare/AlignUp）、IsOverlap、IsPowerOfTwo、memcpy/memmove FFI 声明、以及大量 8/16/32 位宽度的变体。1313 行，71 个函数。超过 800 行软阈值。
- **影响**: 同一工具域内聚性仍在，`test_mem_utils` 已可跑通，不适合为了行数机械拆分。
- **状态**: 保持 P3，后续继续动 `mem.utils` 时按 seam 拆成 `core ops` / `align+bit helpers` / `ffi backend`。
...
### [QA-008] utils.pas 中 Copy/Zero/Compare 与 FPC System 单元同名
- **严重度**: major
- **维度**: quality
- **文件**: `nextpas.core.mem.utils.pas`
- **描述**: `Copy`, `Zero`, `Compare`, `Fill` 都是 FPC System 单元中已有函数的同名或近似名。在 uses 子句中引入 `nextpas.core.mem.utils` 后，如果不加限定符，可能遮蔽 System 的同名函数。
- **影响**: 隐蔽的遮蔽 bug，特别是 `Copy` 函数（System.Copy 是字符串切片）。
- **建议**: 考虑使用更具辨识度的前缀（如 `MemCopy`, `MemZero`, `MemCompare`），或在文档中强调必须使用限定符。
...
### [QA-015] blockpool.sharded.pas 1424 行 — 过于复杂
- **严重度**: minor
- **维度**: maintainability
- **文件**: `nextpas.core.mem.blockpool.sharded.pas`
- **描述**: 包含 TShardedBlockPool + TThreadCacheNode + 路由表 + GC 队列 + 全局 TLS 管理。1424 行。
- **影响**: 认知负荷极高，审查困难。
- **建议**: 考虑拆分为：路由逻辑（routing.pas）、线程缓存（thread_cache.pas）、主类。
