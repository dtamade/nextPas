#!/usr/bin/env python3
"""Guard SIMD metadata-query helpers from leaking past intended surfaces."""
...
SUMMARY_PREFIX = "METADATA_QUERY_SCOPE"
SCAN_GLOBS = (
    "src/nextpas.core.simd*.pas",
    "src/nextpas.core.simd*.inc",
)
SYMBOL_ALLOWED_FILES = {
    "GetBackendInfo": {
        "src/nextpas.core.simd.dispatch.pas",
        "src/nextpas.core.simd.runtime.pas",
        "src/nextpas.core.simd.public_abi.impl.inc",
        "src/nextpas.core.simd.backend.adapter.pas",
    },
    "TryGetRegisteredBackendDispatchTable": {
        "src/nextpas.core.simd.dispatch.pas",
        "src/nextpas.core.simd.public_abi.impl.inc",
        "src/nextpas.core.simd.backend.adapter.pas",
    },
    "GetBackendNameTextPtr": {
        "src/nextpas.core.simd.dispatch.pas",
        "src/nextpas.core.simd.public_abi.impl.inc",
    },
    "GetBackendDescriptionTextPtr": {
        "src/nextpas.core.simd.dispatch.pas",
        "src/nextpas.core.simd.public_abi.impl.inc",
    },
}
SYMBOL_RES = {
    l_symbol: re.compile(rf"\b{re.escape(l_symbol)}\b")
    for l_symbol in SYMBOL_ALLOWED_FILES
}
...
def parse_args() -> argparse.Namespace:
    l_parser = argparse.ArgumentParser(
        description=(
            "Check that SIMD metadata-query helpers only stay in "
            "dispatch/runtime/public ABI/adapter internals."
        )
    )
...
def scan_file(a_root: Path, a_path: Path) -> list[Hit]:
    ...
    for l_line_no, (l_original, l_code) in enumerate(
        ...
        for l_symbol, l_re in SYMBOL_RES.items():
            if l_re.search(l_code):
                l_hits.append(
                    Hit(
                        symbol=l_symbol,
                        path=l_rel,
...
def build_report(a_root: Path) -> Report:
    ...
    for l_path in l_files:
        for l_hit in scan_file(a_root, l_path):
            if l_hit.path in SYMBOL_ALLOWED_FILES[l_hit.symbol]:
    ...
    return Report(
        ...
        allowed_files={
            l_symbol: sorted(l_files)
            for l_symbol, l_files in SYMBOL_ALLOWED_FILES.items()
        },
...
def print_human_report(a_report: Report, a_verbose: bool) -> None:
    if a_report.forbidden_hits:
        for l_hit in a_report.forbidden_hits:
            print(
                "[METADATA-SCOPE] FORBIDDEN "
                f"{l_hit.path}:{l_hit.line}: {l_hit.symbol}: {l_hit.text}"
            )
    elif a_verbose:
        print("[METADATA-SCOPE] No forbidden metadata-query helper usage found.")
    ...
    if a_report.ok:
        print("[METADATA-SCOPE] OK")
    else:
        print(
            "[METADATA-SCOPE] FAILED: metadata-query helpers must stay inside "
            "dispatch/runtime/public ABI/adapter internals so facade and "
            "companion surfaces do not grow a second metadata truth path."
        )
...
def render_summary_line(a_report: Report) -> str:
    return (
        ...
        f"symbols={len(SYMBOL_ALLOWED_FILES)} "
...
def main() -> int:
    ...
    if not l_root.exists():
        print(f"[METADATA-SCOPE] Missing root: {l_root}")
        return 2
    ...
    if l_args.json_file:
        ...
        print(f"[METADATA-SCOPE] JSON snapshot: {l_json_path}")
