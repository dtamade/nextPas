...
begin
  ...
  Check(m = TMask16($0001), 'Vec16CmpEq value=0: got $' + IntToHex(m, 4));
  m := Vec16CmpEq(@data[0], 255);
  Check(m = TMask16(0), 'Vec16CmpEq value=255 no match: got $' + IntToHex(m, 4));
  for i := 0 to 15 do allSame[i] := 42;
  m := Vec16CmpEq(@allSame[0], 42);
  Check(m = TMask16($FFFF), 'Vec16CmpEq all same: got $' + IntToHex(m, 4));
  m := Vec16CmpEq(@allSame[0], 99);
  Check(m = TMask16(0), 'Vec16CmpEq no match: got $' + IntToHex(m, 4));
  m := Vec16CmpEq(@data[0], 7);
  Check(m = TMask16($0080), 'Vec16CmpEq value=7: got $' + IntToHex(m, 4));
end;
...
begin
  ...
  Check(m = TMask16($FFFF), 'Vec16CmpEq2 identical: got $' + IntToHex(m, 4));
  for i := 0 to 15 do c[i] := i + 100;
  m := Vec16CmpEq2(@a[0], @c[0]);
  Check(m = TMask16(0), 'Vec16CmpEq2 all different: got $' + IntToHex(m, 4));
  for i := 0 to 15 do begin
    a[i] := i;
    if (i mod 2) = 0 then b[i] := i else b[i] := 200;
  end;
  m := Vec16CmpEq2(@a[0], @b[0]);
  Check(m = TMask16($5555), 'Vec16CmpEq2 partial (even): got $' + IntToHex(m, 4));
end;
...
begin
  ...
  Check(m = TMask16(0), 'Vec16CmpLtU threshold=0: got $' + IntToHex(m, 4));
  m := Vec16CmpLtU(@data[0], 1);
  Check(m = TMask16($0001), 'Vec16CmpLtU threshold=1: got $' + IntToHex(m, 4));
  m := Vec16CmpLtU(@data[0], 255);
  Check(m = TMask16($FFFF), 'Vec16CmpLtU threshold=255 sequential: got $' + IntToHex(m, 4));
  m := Vec16CmpLtU(@allZero[0], 255);
  Check(m = TMask16($FFFF), 'Vec16CmpLtU allZero thr=255: got $' + IntToHex(m, 4));
  m := Vec16CmpLtU(@all255[0], 255);
  Check(m = TMask16(0), 'Vec16CmpLtU all255 thr=255: got $' + IntToHex(m, 4));
  m := Vec16CmpLtU(@data[0], 5);
  Check(m = TMask16($001F), 'Vec16CmpLtU threshold=5: got $' + IntToHex(m, 4));
end;
...
begin
  ...
  Check(m = TMask16($FFFE), 'Vec16CmpGtU threshold=0: got $' + IntToHex(m, 4));
  m := Vec16CmpGtU(@data[0], 14);
  Check(m = TMask16($8000), 'Vec16CmpGtU threshold=14: got $' + IntToHex(m, 4));
  m := Vec16CmpGtU(@data[0], 255);
  Check(m = TMask16(0), 'Vec16CmpGtU threshold=255: got $' + IntToHex(m, 4));
  m := Vec16CmpGtU(@allZero[0], 0);
  Check(m = TMask16(0), 'Vec16CmpGtU allZero thr=0: got $' + IntToHex(m, 4));
  m := Vec16CmpGtU(@all255[0], 254);
  Check(m = TMask16($FFFF), 'Vec16CmpGtU all255 thr=254: got $' + IntToHex(m, 4));
  m := Vec16CmpGtU(@all255[0], 255);
  Check(m = TMask16(0), 'Vec16CmpGtU all255 thr=255: got $' + IntToHex(m, 4));
end;
...
begin
  ...
  Check(m = TMask16($FFFF), 'Vec16CmpRange 0..255: got $' + IntToHex(m, 4));
  m := Vec16CmpRange(@data[0], 7, 7);
  Check(m = TMask16($0080), 'Vec16CmpRange 7..7: got $' + IntToHex(m, 4));
  m := Vec16CmpRange(@data[0], 10, 5);
  Check(m = TMask16(0), 'Vec16CmpRange 10..5 (inverted): got $' + IntToHex(m, 4));
  m := Vec16CmpRange(@data[0], 3, 7);
  Check(m = TMask16($00F8), 'Vec16CmpRange 3..7: got $' + IntToHex(m, 4));
end;
...
begin
  ...
  Check(m = TMask32($00000001), 'Vec32CmpEq value=0: got $' + IntToHex(m, 8));
  m := Vec32CmpEq(@data[0], 255);
  Check(m = TMask32(0), 'Vec32CmpEq value=255 no match: got $' + IntToHex(m, 8));
  for i := 0 to 31 do allSame[i] := 42;
  m := Vec32CmpEq(@allSame[0], 42);
  Check(m = TMask32($FFFFFFFF), 'Vec32CmpEq all same: got $' + IntToHex(m, 8));
  m := Vec32CmpEq(@allSame[0], 99);
  Check(m = TMask32(0), 'Vec32CmpEq no match: got $' + IntToHex(m, 8));
  m := Vec32CmpEq(@data[0], 7);
  Check(m = TMask32($00000080), 'Vec32CmpEq value=7: got $' + IntToHex(m, 8));
end;
...
begin
  ...
  Check(m = TMask32($FFFFFFFF), 'Vec32CmpEq2 identical: got $' + IntToHex(m, 8));
  for i := 0 to 31 do c[i] := i + 100;
  m := Vec32CmpEq2(@a[0], @c[0]);
  Check(m = TMask32(0), 'Vec32CmpEq2 all different: got $' + IntToHex(m, 8));
  for i := 0 to 31 do begin
    a[i] := i;
    if (i mod 2) = 0 then b[i] := i else b[i] := 200;
  end;
  m := Vec32CmpEq2(@a[0], @b[0]);
  Check(m = TMask32($55555555), 'Vec32CmpEq2 partial (even): got $' + IntToHex(m, 8));
end;
...
begin
  ...
  Check(m = TMask32(0), 'Vec32CmpLtU threshold=0: got $' + IntToHex(m, 8));
  m := Vec32CmpLtU(@data[0], 1);
  Check(m = TMask32($00000001), 'Vec32CmpLtU threshold=1: got $' + IntToHex(m, 8));
  m := Vec32CmpLtU(@data[0], 255);
  Check(m = TMask32($FFFFFFFF), 'Vec32CmpLtU threshold=255 sequential: got $' + IntToHex(m, 8));
  m := Vec32CmpLtU(@allZero[0], 255);
  Check(m = TMask32($FFFFFFFF), 'Vec32CmpLtU allZero thr=255: got $' + IntToHex(m, 8));
  m := Vec32CmpLtU(@all255[0], 255);
  Check(m = TMask32(0), 'Vec32CmpLtU all255 thr=255: got $' + IntToHex(m, 8));
  m := Vec32CmpLtU(@data[0], 5);
  Check(m = TMask32($0000001F), 'Vec32CmpLtU threshold=5: got $' + IntToHex(m, 8));
end;
...
begin
  ...
  Check(m = TMask32($FFFFFFFE), 'Vec32CmpGtU threshold=0: got $' + IntToHex(m, 8));
  m := Vec32CmpGtU(@data[0], 30);
  Check(m = TMask32($80000000), 'Vec32CmpGtU threshold=30: got $' + IntToHex(m, 8));
  m := Vec32CmpGtU(@data[0], 255);
  Check(m = TMask32(0), 'Vec32CmpGtU threshold=255: got $' + IntToHex(m, 8));
  m := Vec32CmpGtU(@allZero[0], 0);
  Check(m = TMask32(0), 'Vec32CmpGtU allZero thr=0: got $' + IntToHex(m, 8));
  m := Vec32CmpGtU(@all255[0], 254);
  Check(m = TMask32($FFFFFFFF), 'Vec32CmpGtU all255 thr=254: got $' + IntToHex(m, 8));
  m := Vec32CmpGtU(@all255[0], 255);
  Check(m = TMask32(0), 'Vec32CmpGtU all255 thr=255: got $' + IntToHex(m, 8));
end;
...
begin
  ...
  Check(m = TMask32($FFFFFFFF), 'Vec32CmpRange 0..255: got $' + IntToHex(m, 8));
  m := Vec32CmpRange(@data[0], 7, 7);
  Check(m = TMask32($00000080), 'Vec32CmpRange 7..7: got $' + IntToHex(m, 8));
  m := Vec32CmpRange(@data[0], 10, 5);
  Check(m = TMask32(0), 'Vec32CmpRange 10..5 (inverted): got $' + IntToHex(m, 8));
  m := Vec32CmpRange(@data[0], 3, 7);
  Check(m = TMask32($000000F8), 'Vec32CmpRange 3..7: got $' + IntToHex(m, 8));
end;
...
begin
  ...
  Check(m = TMask64(QWord($0000000000000001)), 'Vec64CmpEq value=0: got $' + IntToHex(m, 16));
  m := Vec64CmpEq(@data[0], 255);
  Check(m = TMask64(0), 'Vec64CmpEq value=255 no match: got $' + IntToHex(m, 16));
  for i := 0 to 63 do allSame[i] := 42;
  m := Vec64CmpEq(@allSame[0], 42);
  Check(m = TMask64(QWord($FFFFFFFFFFFFFFFF)), 'Vec64CmpEq all same: got $' + IntToHex(m, 16));
  m := Vec64CmpEq(@allSame[0], 99);
  Check(m = TMask64(0), 'Vec64CmpEq no match: got $' + IntToHex(m, 16));
  m := Vec64CmpEq(@data[0], 7);
  Check(m = TMask64(QWord($0000000000000080)), 'Vec64CmpEq value=7: got $' + IntToHex(m, 16));
end;
...
begin
  ...
  Check(m = TMask64(QWord($FFFFFFFFFFFFFFFF)), 'Vec64CmpEq2 identical: got $' + IntToHex(m, 16));
  for i := 0 to 63 do c[i] := i + 100;
  m := Vec64CmpEq2(@a[0], @c[0]);
  Check(m = TMask64(0), 'Vec64CmpEq2 all different: got $' + IntToHex(m, 16));
  for i := 0 to 63 do begin
    a[i] := i;
    if (i mod 2) = 0 then b[i] := i else b[i] := 200;
  end;
  m := Vec64CmpEq2(@a[0], @b[0]);
  Check(m = TMask64(QWord($5555555555555555)), 'Vec64CmpEq2 partial (even): got $' + IntToHex(m, 16));
end;
...
begin
  ...
  Check(m = TMask64(0), 'Vec64CmpLtU threshold=0: got $' + IntToHex(m, 16));
  m := Vec64CmpLtU(@data[0], 1);
  Check(m = TMask64(QWord($0000000000000001)), 'Vec64CmpLtU threshold=1: got $' + IntToHex(m, 16));
  m := Vec64CmpLtU(@data[0], 255);
  Check(m = TMask64(QWord($FFFFFFFFFFFFFFFF)), 'Vec64CmpLtU threshold=255 sequential: got $' + IntToHex(m, 16));
  m := Vec64CmpLtU(@allZero[0], 255);
  Check(m = TMask64(QWord($FFFFFFFFFFFFFFFF)), 'Vec64CmpLtU allZero thr=255: got $' + IntToHex(m, 16));
  m := Vec64CmpLtU(@all255[0], 255);
  Check(m = TMask64(0), 'Vec64CmpLtU all255 thr=255: got $' + IntToHex(m, 16));
  m := Vec64CmpLtU(@data[0], 5);
  Check(m = TMask64(QWord($000000000000001F)), 'Vec64CmpLtU threshold=5: got $' + IntToHex(m, 16));
end;
...
begin
  ...
  Check(m = TMask64(QWord($FFFFFFFFFFFFFFFE)), 'Vec64CmpGtU threshold=0: got $' + IntToHex(m, 16));
  m := Vec64CmpGtU(@data[0], 62);
  Check(m = TMask64(QWord($8000000000000000)), 'Vec64CmpGtU threshold=62: got $' + IntToHex(m, 16));
  m := Vec64CmpGtU(@data[0], 255);
  Check(m = TMask64(0), 'Vec64CmpGtU threshold=255: got $' + IntToHex(m, 16));
  m := Vec64CmpGtU(@allZero[0], 0);
  Check(m = TMask64(0), 'Vec64CmpGtU allZero thr=0: got $' + IntToHex(m, 16));
  m := Vec64CmpGtU(@all255[0], 254);
  Check(m = TMask64(QWord($FFFFFFFFFFFFFFFF)), 'Vec64CmpGtU all255 thr=254: got $' + IntToHex(m, 16));
  m := Vec64CmpGtU(@all255[0], 255);
  Check(m = TMask64(0), 'Vec64CmpGtU all255 thr=255: got $' + IntToHex(m, 16));
end;
...
begin
  ...
  Check(m = TMask64(QWord($FFFFFFFFFFFFFFFF)), 'Vec64CmpRange 0..255: got $' + IntToHex(m, 16));
  m := Vec64CmpRange(@data[0], 7, 7);
  Check(m = TMask64(QWord($0000000000000080)), 'Vec64CmpRange 7..7: got $' + IntToHex(m, 16));
  m := Vec64CmpRange(@data[0], 10, 5);
  Check(m = TMask64(0), 'Vec64CmpRange 10..5 (inverted): got $' + IntToHex(m, 16));
  m := Vec64CmpRange(@data[0], 3, 7);
  Check(m = TMask64(QWord($00000000000000F8)), 'Vec64CmpRange 3..7: got $' + IntToHex(m, 16));
end;
...
begin
  ...
  WriteLn(Format('--- %d tests, %d passed, 0 failed ---', [GTestCount, GPassCount]));
end.
