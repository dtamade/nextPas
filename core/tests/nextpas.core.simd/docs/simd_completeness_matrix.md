...
- `gate`：通过（simd + cpuinfo + unified x86/global suite）
...
- 机器检查：`check_interface_implementation_completeness.py --strict` 通过（`dispatch=616, P0=0/P1=0/P2=0`）
- 机器检查产物：
...
- 2026-06-06 机器检查快照：`dispatch=616, scalar=616, sse2=520, sse3=10, ssse3=0, sse41=28, sse42=2, avx2=416, avx512=232, neon=458, riscvv=529, P0=0/P1=0/P2=0`
- 2026-05-26 最新 backend benchmark summary：`tests/nextpas.core.simd/logs/backend-bench-20260526-105300/summary.md`
- 关键性能结论：
  - `VecI16x32Add`：`2.24x`；raw：`2.23x`
  - `VecU32x16Mul`：`0.93x`；raw：`0.95x`
  - `VecU64x8Add`：`0.72x`；raw：`0.76x`
  - `VecU8x64Max`：`5.95x`；raw：`5.07x`
  - `VecF32x4Add`：`0.88x`；raw：`0.93x`
...
- 快照结果：
  - dispatch slots total：`616`
  - backend slot counts：`scalar=616, sse2=520, sse3=10, ssse3=0, sse41=28, sse42=2, avx2=416, avx512=232, neon=458, riscvv=529`
  - severity：`P0=0 / P1=0 / P2=0`
  - 说明：机器检查为 token/赋值启发式扫描，不等同于完整语义正确性证明。
- 结论：
  - 当前 non-x86 在机器检查口径下保持 `P0/P1/P2=0`，backend assignment count 不等于满覆盖或语义证明。
