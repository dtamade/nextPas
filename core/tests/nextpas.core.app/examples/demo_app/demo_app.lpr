...
procedure Stamp(var P:TDateTime; out M:Int64); var N:TDateTime; begin N:=DateTimeNow; M:=DateTimeMillisecondsBetween(N,P); P:=N; end;
...
procedure TDemoApp.Log(const S:string); begin WriteLn(FormatDateTime('[%H:%M:%S]',DateTimeNow),' ',S); end;
...
procedure TDemoApp.Run; var W2:IWebviewWindow; begin FStarted:=DateTimeNow; FApp:=TAppBuilder.New.Title('nextPas App Demo').Size(900,600).Kind(wvGtk).MountEmbedded('',TDemoPage.Create).RegisterInvoke('demo.sum',@HSum).RegisterInvoke('demo.report',@HReport).Build; FApp.MainWindow.OnNavigationFinished(@OnNav); FApp.MainWindow.OnWindowClosed(procedure begin FApp.Quit; end); FApp.MainWindow.Show; Log('app up count='+IntToStr(FApp.WindowCount));
