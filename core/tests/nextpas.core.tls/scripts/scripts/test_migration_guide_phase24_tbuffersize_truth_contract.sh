...
require_fixed "$guide" '当前 `nextpas.ssl` 的 TLS context / factory / direct-library 路径并没有单独的 `WithBufferSize(...)` / `SetBuffer(...)` public 入口。' \
...
require_fixed "$guide" '下面这个组合示意应理解为“你自己的 typed wrapper / policy boundary”，不是当前 `nextpas.ssl` 直接提供的单一 SSL 配置入口。' \
