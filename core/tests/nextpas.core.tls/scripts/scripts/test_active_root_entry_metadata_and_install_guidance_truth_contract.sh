...
if ! rg -n --quiet 'nextpas\.core\.tls\.pas\s+# 主门面 / 当前普通入口' README.md; then
  fail "README architecture entry must publish nextpas.core.tls.pas as the ordinary public entry"
fi

if ! rg -n --quiet 'nextpas\.core\.tls\.context\.builder\.pas\s+# 推荐 context builder 入口' README.md; then
  fail "README architecture entry must publish nextpas.core.tls.context.builder.pas as the recommended builder entry"
fi

if rg -n --quiet 'nextpas\.core\.tls\.factory\.pas\s+# 工厂模式入口' README.md; then
...
if rg -n --quiet 'fafafa\.ssl,\s*nextpas\.core\.tls\.base' docs/zh/安装配置.md; then
  fail "zh install guide must not require split import nextpas.ssl + nextpas.core.tls.base for ordinary setup"
fi

if ! rg -n --quiet '^\s*fafafa\.ssl;\s*$' docs/zh/安装配置.md; then
  fail "zh install guide must show nextpas.ssl as the ordinary public import"
fi
pass "zh install guide ordinary import aligned"

if rg -n --quiet 'nextpas\.core\.tls\.base;\s*// 或直接使用 base 单元' docs/zh/编译指南.md; then
...
if ! rg -n --quiet '^\s*fafafa\.ssl;\s*// 包含 ProtocolVersionToString' docs/zh/编译指南.md; then
  fail "zh compile guide must show nextpas.ssl as the source of ProtocolVersionToString"
fi
