...
assert_not_contains "$DOC" \
  "https://github.com/your-org/nextpas.ssl/issues" \
...
assert_contains "$DOC" \
  "能力矩阵最早在 nextpas.ssl v1.2.0 引入；本文当前内容对齐 nextpas.ssl v1.5.0 shipped truth。" \
...
assert_contains "$DOC" \
  '普通 capability / native-handle 查询不必再拆分回 `uses nextpas.core.tls.base` / `nextpas.core.tls.factory`；`nextpas.ssl` 已 re-export 当前所需的 capability helper surface。' \
