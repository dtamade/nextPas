# Findings: expect interim-100 truncated trailer section EOF proof

## Scope

- 上一刀已经锁住了 `Expect: 100-continue` 在 interim `100` 发出后，
  truncated trailer section CR EOF 的 after-interim truth。
- 本轮继续补更贴近 EOF 邻接的小缺口：
  interim `100` 发出后，如果 chunked trailer section 在结束空行前直接 EOF 截断，
  server 仍应返回 final `400 Bad Request`，且不应进入 handler，
  也不应误补 synthetic `500`。

## Confirmed truths

### 1. 现有 after-interim helper 已足够表达 section EOF 截断
...
- 因此这轮不需要再改 helper 形状，只要复用同一路径，把请求体切成：
  - `5\r\nhello\r\n`
  - `0\r\n`
  - partial trailer field line `X-Test: value\r\n`
  - 然后客户端 write-half-close
- 这使得本轮真正保持为纯测试补证，而不是再做测试基础设施扩展。

### 2. after-interim trailer section EOF focused tests 直接 GREEN
...
- 在
  ...
  - threaded `Expect chunked truncated trailer section EOF rejects after interim 100`
...
- 在
  ...
  - threaded `Expect: chunked truncated trailer section EOF rejects after interim response`
  - epoll `Expect: chunked truncated trailer section EOF rejects after interim response`
- 这 4 条 tests 直接锁住：
  - 先收到单条 `HTTP/1.1 100 Continue`
  - partial trailer field line + peer write-half-close 后最终返回 `400 Bad Request`
...
- 非 `Expect` 路径此前已经有 parser / security / server 三层
  `truncated trailer section at EOF -> 400` 证据。
...
- 现有 helper 现在已经能表达 `field-name` / `separator` / `field line` / `field CR` / `section` / `section CR` 六个 after-interim
  trailer EOF 邻接形状，后续可以继续复用。
- 下一刀更自然的是继续挑一个 after-interim trailer EOF 邻接 case：
  - `truncated trailer empty-value EOF after interim 100`
  - 或 `truncated trailer whitespace EOF after interim 100`
