# Findings: HTTP terminal chunk ending CR EOF proof
...
- 但 terminal `0` chunk 如果自己不带 extension、只是最终空 trailer section
...
- server focused tests 证明：
  - peer half-close 暴露出 terminal chunk ending CR EOF 截断时，server 返回显式 `400`；
  - handler 不会被调用。
...
- malformed chunk framing 审计仍可继续细化其他 terminal/trailer grammar 子类，但这轮已经把
  terminal chunk 无 extension 时空 trailer section 的 CR-only EOF truth 锁实。
