# Findings: expect interim-100 truncated trailer separator EOF proof
...
- 本轮继续补更贴近 EOF 邻接的小缺口：
  interim `100` 发出后，如果 chunked trailer 的 separator 在 EOF 前被截断，
  server 仍应返回 final `400 Bad Request`，且不应进入 handler，
  也不应误补 synthetic `500`。

## Confirmed truths

### 1. 现有 after-interim helper 已足够表达 separator EOF 截断
...
- 因此这轮不需要再改 helper 形状，只要复用同一路径，把请求体切成：
  - `5\r\nhello\r\n`
  - `0\r\n`
  - partial trailer separator `X-Test:`
  - 然后客户端 write-half-close
- 这使得本轮真正保持为纯测试补证，而不是再做测试基础设施扩展。

### 2. after-interim trailer separator EOF focused tests 直接 GREEN
...
- 这 4 条 tests 直接锁住：
  ...
  - partial trailer separator + peer write-half-close 后最终返回 `400 Bad Request`
...
- 非 `Expect` 路径此前已经有 parser / security / server 三层
  `truncated trailer separator at EOF -> 400` 证据。
- 本轮把同一 truth 延伸到 after-interim `100 Continue` 路径后，
  request-side runtime truth 再补一格：
  - 请求已进入 `Expect` body phase 也不会放松 trailer separator EOF 校验
...
- 现有 helper 现在已经能表达 `field-name` / `separator` 两个 after-interim
  trailer EOF 邻接形状，后续可以继续复用。
- 下一刀更自然的是继续挑一个 after-interim trailer EOF 邻接 case：
  - `truncated trailer field line at EOF after interim 100`
  - 或 `truncated trailer field CR at EOF after interim 100`
