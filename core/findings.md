# Findings: HTTP truncated chunk-extension EOF proof
...
- `malformed chunk extension` 已覆盖空 extension token 这类直接语法错误。
- `truncated chunked request at EOF` 已覆盖 chunk body 中途截断。
- `chunk-size line EOF truncation`、`chunk-data CRLF EOF truncation`、
  `terminal 0 chunk ending EOF truncation`、`truncated trailer section at EOF`
  都已有单独 focused proof。
- 但 chunk-extension line 如果自身被 EOF 截断，仍没有把
  `...5;sig=abc` 与 `...5;sig=abc\r` 两类输入单独锁住。

## New Evidence From This Batch

- parser focused tests 证明：
  - 当输入结束在 `...5;sig=abc` 时，`Finish` 后会进入 parser error；
  - 当输入结束在 `...5;sig=abc\r` 时，`Finish` 后同样会进入 parser error；
  - 两类输入都保持 not-complete，不会被误判成合法完成。
- server focused tests 证明：
  - peer half-close 暴露出 chunk-extension line EOF 截断时，server 返回显式 `400`；
  - handler 不会被调用。
...
- 现在 chunked EOF 截断的边界陈述又细了一层：
  ...
  - chunk-extension line EOF 截断也已单独证明；
...
- malformed chunk framing 审计仍可继续细化其他 terminal/trailer grammar 子类，但这轮已经把
  chunk-extension line 的 EOF truth 锁实。
