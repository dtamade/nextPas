# Findings: expect interim-100 truncated trailer empty-value CR EOF proof

## Scope

- 上一刀已经锁住了 `Expect: 100-continue` 在 interim `100` 发出后，
  truncated trailer whitespace section CR EOF 的 after-interim truth。
- 本轮继续补更贴近 EOF 邻接的小缺口：
  interim `100` 发出后，如果 chunked trailer field 已经到 empty-value 行尾，
  但字段行结尾只收到单个 `CR` 就直接 EOF 截断，
  server 仍应返回 final `400 Bad Request`，且不应进入 handler，
  也不应误补 synthetic `500`。

## Confirmed truths

### 1. 现有 after-interim helper 已足够表达 empty-value CR EOF 截断
...
- 因此这轮不需要再改 helper 形状，只要复用同一路径，把请求体切成：
  - `5\r\nhello\r\n`
  - `0\r\n`
  - partial trailer empty-value CR EOF `X-Test:\r`
  - 然后客户端 write-half-close
- 这使得本轮真正保持为纯测试补证，而不是再做测试基础设施扩展。

### 2. after-interim trailer empty-value CR EOF focused tests 直接 GREEN
...
- 在
  ...
  - threaded `Expect chunked truncated trailer empty-value CR EOF rejects after interim 100`
...
- 在
  ...
  - threaded `Expect: chunked truncated trailer empty-value CR EOF rejects after interim response`
  - epoll `Expect: chunked truncated trailer empty-value CR EOF rejects after interim response`
- 这 4 条 tests 直接锁住：
...
- 现有 helper 现在已经能表达 `field-name` / `separator` / `empty-value CR` / `empty-value` / `empty-value section CR` / `whitespace` / `whitespace section` / `whitespace section CR` / `field line` / `field CR` / `section` / `section CR` 十二个 after-interim
  trailer EOF 邻接形状，后续可以继续复用。
- 下一刀更自然的是继续挑一个 after-interim trailer EOF 邻接 case：
  - `truncated trailer whitespace CR EOF after interim 100`
