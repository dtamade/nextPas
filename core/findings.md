# Findings: expect interim-100 malformed chunk-size proof

## Scope

- 上一刀已经锁住了 `Expect: 100-continue` 发出 interim `100` 后的
  timeout 邻接 truth。
- 本轮继续补更贴近 malformed chunk framing security 的相邻缺口：
  interim `100` 发出后，如果 chunked body 以 invalid chunk-size 开头，
  server 仍应返回 final `400 Bad Request`，不会进入 handler，也不会误补
  synthetic `500`。

## Confirmed truths

### 1. `Expect + chunked invalid chunk-size after interim 100` 之前确实缺双层 proof
...
- 但缺少更直接的 malformed-after-interim truth：
  - interim `100` 已发出后，invalid chunk-size 仍应落到 final `400`
...
- 这 4 条都直接锁住：
  ...
  - malformed chunk-size 到达前不会误回 `200`
  - malformed chunk-size 到达后最终返回 `400 Bad Request`
  - 不会重复 interim `100`
  - 不会追加 synthetic `500`
  - 不会误回成功响应
  - handler 永远不会进入
...
### 3. `Expect` request-side contract 的 malformed chunk-size 邻接分支已闭环

- security 层：
  - 直接 raw-wire truth
  - 证明 threaded / epoll 两条 live path 在 invalid chunk-size 情况下
    都返回 final `400`，不补 `500`
- server 层：
  - 更贴近 public API / contract 的 focused live proof
  - 证明 handler 不进入、不会误发成功响应
- 因此 `Expect after interim 100 + invalid chunk-size -> final 400` 这条
