...
- 既然现在代表性的 `400/501/431`、partial-next-line bridge、partial follow-up headers、same-write pipelining 都没有 backend 差异，再继续机械复制同型 epoll case 的收益已经明显下降。
- 更值的下一刀有两个方向：
...
- 这轮补的是一个 epoll request-tail raw-wire 缺口，不是把所有 parser/server request-tail 契约都复制一遍到 security。
