# Findings: expect interim-100 truncated trailer section CR EOF proof

## Scope

- 上一刀已经锁住了 `Expect: 100-continue` 在 interim `100` 发出后，
  truncated trailer field CR EOF 的 after-interim truth。
- 本轮继续补更贴近 EOF 邻接的小缺口：
  interim `100` 发出后，如果 chunked trailer section 的最终 `CR` 在 EOF 前被截断，
  server 仍应返回 final `400 Bad Request`，且不应进入 handler，
  也不应误补 synthetic `500`。

## Confirmed truths

### 1. 现有 after-interim helper 已足够表达 section CR EOF 截断
...
- 因此这轮不需要再改 helper 形状，只要复用同一路径，把请求体切成：
  - `5\r\nhello\r\n`
  - `0\r\n`
  - partial trailer section CR `X-Test: value\r\n\r`
  - 然后客户端 write-half-close
- 这使得本轮真正保持为纯测试补证，而不是再做测试基础设施扩展。

### 2. after-interim trailer section CR EOF focused tests 直接 GREEN
...
- 在
  ...
  - threaded `Expect chunked truncated trailer section CR EOF rejects after interim 100`
...
- 在
  ...
  - threaded `Expect: chunked truncated trailer section CR EOF rejects after interim response`
  - epoll `Expect: chunked truncated trailer section CR EOF rejects after interim response`
- 这 4 条 tests 直接锁住：
  - 先收到单条 `HTTP/1.1 100 Continue`
  - partial trailer section CR + peer write-half-close 后最终返回 `400 Bad Request`
...
- 非 `Expect` 路径此前已经有 parser / security / server 三层
  `truncated trailer section CR at EOF -> 400` 证据。
- 本轮把同一 truth 延伸到 after-interim `100 Continue` 路径后，
  request-side runtime truth 再补一格：
  - 请求已进入 `Expect` body phase 也不会放松 trailer section CR EOF 校验
...
- 现有 helper 现在已经能表达 `field-name` / `separator` / `field line` / `field CR` / `section CR` 五个 after-interim
  trailer EOF 邻接形状，后续可以继续复用。
- 下一刀更自然的是继续挑一个 after-interim trailer EOF 邻接 case：
  - `truncated trailer section EOF after interim 100`
  - 或 `truncated trailer empty-value EOF after interim 100`
