# Task Plan: header field over max-header explicit 431

## Goal

继续留在 `3/6 H1 正确性加固` 主线，沿上一刀的 header-budget
邻接方向推进：把普通 header field over `MaxHeaderSize` 从 broad
safe-handling 收紧成 explicit `431` proof，并补齐 server/security 两层的
threaded / epoll 直接证据。

- 复用 request-target over `MaxHeaderSize` 已收紧的 focused `431` 口径
...
- [x] 审阅 `test_http_server` / `test_http_security` 现有 oversized header /
  `MaxHeaderSize` 口径
- [x] 在 `test_http_server` 补 threaded explicit `431` + handler 不进入 proof
...
- server/security 两层对普通 header field over `MaxHeaderSize` 明确锁住：
  - 在受控小 `MaxHeaderSize` 下，oversized header field 直接返回显式 `431`
  - threaded / Linux `epoll` 两条路径都给出 focused 证据
- server / security 两层口径重新对齐：
  - server 锁 `431` 且 handler 不进入
  - security 锁 raw-wire `431`
- 证据要求：
  - 新增四条 header field `431` tests GREEN
