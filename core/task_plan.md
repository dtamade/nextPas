# Task Plan: HTTP terminal chunk ending CR EOF proof

## Goal

补齐 terminal `0` chunk 无 extension 且最终空 trailer section 只收到单个 `CR` 后 EOF 时的
...
`terminal 0 chunk ending`、`terminal chunk ending CR`、`terminal chunk ending after extension`、`terminal chunk ending after extension CR`、`trailer section` 这些 EOF 边界子类逐个拆开锁实，而不是停留在宽泛的
...
- [x] 确认当前缺口是 terminal chunk 无 extension 时最终空 trailer section 只收到单个 `CR` 的 EOF 子类，
  且现有 case 还没单独覆盖 `...0\r\n\r`。
- [x] 新增 parser focused test：
  `Chunked request truncated terminal chunk ending CR at EOF`。
- [x] 新增 server focused test：
  `Malformed chunked request truncated terminal chunk ending CR at EOF -> 400`。
- [x] 新增 security focused test：
  `Truncated terminal chunk ending CR at EOF -> 400`。
...
| Decision | Rationale |
| --- | --- |
| 本轮优先补 terminal chunk 无 extension 时最终空 trailer section 的 CR-only EOF 子类 | 这是紧邻既有 `...0\r\n` proof 的自然相邻边界。 |
| 只锁 `...0\r\n\r` 这个子类 | 它语义上不同于 `...0\r\n` 的整段缺失，也不同于 extension/trailer-field-started 相关截断。 |
