# Task Plan: http chunked partial follow-up request-line epoll bridge proof
...
- 在 `test_http_security` 里补 plain chunked partial follow-up request-line can-complete-later 的 Linux `epoll` raw-wire bridge proof
- 锁定“首个 chunked request 先返回 `200 / echo:5`，后续把半截下一请求行补全后，第二个请求仍返回 `200 / ok`”的 live truth
...
- [x] 做 parser/server/security 矩阵筛查，确认这轮最值的缺口是 plain chunked partial follow-up request-line bridge 的 epoll raw-wire proof
...
- 把 `test_http_security` 从已存在的 threaded plain chunked partial-next-line bridge truth，再往前补到 epoll live backend
