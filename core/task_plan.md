# Task Plan: expect interim-100 truncated trailer field line EOF proof
...
在 interim `100` 发出后，如果 trailer field line 在 EOF 前被截断，
应返回 final `400 Bad Request`。

- 补 `test_http_security` 与 `test_http_server` 的
  after-interim trailer field line EOF focused proof
- 锁住 `interim 100` 已发出后：
...
- [x] 审阅 `test_http_security` / `test_http_server` 现有 `Expect` / malformed
  trailer EOF 覆盖，确认 after-interim `truncated trailer field line EOF`
  仍是空缺
- [x] 复用现有 `shutdown-after-body` helper，不再扩新 helper 形状
- [x] 在 `test_http_security` 与 `test_http_server` 新增 threaded / epoll
  两组 `Expect + chunked truncated trailer field line EOF after interim 100`
  focused proof
