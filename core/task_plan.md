# Task Plan: expect interim-100 truncated trailer empty-value CR EOF proof

## Goal

继续停留在 `3/6 H1 正确性加固` 主线，承接上一刀已经完成的
`Expect: 100-continue` trailer whitespace section CR EOF after-interim proof，继续补
相邻的小缺口：`Expect + Transfer-Encoding: chunked`
在 interim `100` 发出后，如果 trailer field 已到 empty-value 行尾，
但字段行结尾只收到单个 `CR` 就 EOF 截断，
应返回 final `400 Bad Request`。

- 补 `test_http_security` 与 `test_http_server` 的
  after-interim trailer empty-value CR EOF focused proof
- 锁住 `interim 100` 已发出后：
  - partial trailer empty-value CR EOF + write-half-close 会返回 explicit `400 Bad Request`
  - 不会重复发 `100 Continue`
  - 不会误回 `200`
  - handler 不会进入
...
- [x] 审阅 `test_http_security` / `test_http_server` 现有 `Expect` / malformed
  trailer EOF 覆盖，确认 after-interim `truncated trailer empty-value CR EOF`
  仍是空缺
- [x] 复用现有 `shutdown-after-body` helper，不再扩新 helper 形状
- [x] 在 `test_http_security` 与 `test_http_server` 新增 threaded / epoll
  两组 `Expect + chunked truncated trailer empty-value CR EOF after interim 100`
  focused proof
...
- `Expect: 100-continue` 的 trailer EOF 邻接 truth 在 security/server 两层收口：
  - interim `100` 先发出
  - partial trailer empty-value CR EOF 后 peer write-half-close 会返回 final `400 Bad Request`
