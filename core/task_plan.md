# Task Plan: WebSocket RSV bit rejection

## Goal

继续推进 `HttpServer 完成` 主线中的 WebSocket negative frame coverage。RFC 6455
要求 RSV1/RSV2/RSV3 只能在协商扩展后使用；当前实现没有 negotiated extensions，
server-side `ReadFrame` 必须拒绝 RSV bit 非零的 client frame。

要求：

- 先 RED：masked text first byte `$C1` 当前会被当普通 text echo。
- GREEN：`ReadFrame` 在 opcode 处理前检查 RSV bits，非零时抛 `EHttpError`。
...
- [x] 从 `docs/http/API_COVERAGE.md` 选择 WebSocket RSV bit 缺口。
- [x] 在 `test_http_websocket` 写 RED：RSV1 text frame 应被 protocol close。
- [x] 在 `nextpas.core.http.websocket.ReadFrame` 增加 RSV bit guard。
...
- RSV1/RSV2/RSV3 非零的 frame 不再被 handler 当作正常 text。
