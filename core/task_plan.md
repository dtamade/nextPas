# Task Plan: http content-length truncated follow-up headers epoll raw-wire proof
...
- 在 `test_http_security` 里补 `Content-Length` truncated follow-up headers 的 Linux `epoll` raw-wire proof
- 锁定“首个 `Content-Length` request 先返回 `200 / echo:5`，follow-up 半截请求头在 EOF 后返回 `400`”的 live truth
...
- [x] 做 parser/server/security 矩阵筛查，确认这轮最值的缺口是 `Content-Length` truncated follow-up headers 的 epoll raw-wire proof
...
- 把 `test_http_security` 从已存在的 threaded `Content-Length` truncated follow-up headers truth，再往前补到 epoll live backend
