...
begin
  ...
  try
    ...
    // 3 accepts, 1 policy reject, 1 replay
    Obs.Decide(Id, Early, Sess, True);
    Early[0] := $78; Obs.Decide(Id, Early, Sess, True);
    Early[0] := $79; Obs.Decide(Id, Early, Sess, True);
    SetLength(Early, 0); Obs.Decide(Id, Early, Sess, True); // policy
    SetLength(Early, 20); FillChar(Early[0], 20, $77); Obs.Decide(Id, Early, Sess, True); // replay
...
begin
  WriteLn('=== tlspas 0-RTT Early Data Demo (S15 final) ===');
  WriteLn('L2 async TLS 1.3 | X25519/P-256/P-384 | HRR 0xFE | 0-RTT EarlyData | Replay LRU/KV | ServerDecide | Observer | Adaptive');
  WriteLn;
  ...
  WriteLn;
  WriteLn('Demo done: all paths 0 warnings, 5 dimensions verified.');
end.
