...
{** @desc GTK 后端：自 webview.gtk.win 机械抽取 + 事件信号挂接。
       依托 window.gtk.ffi/.loader（platform.dl 装载），实现 IWindow
       全量方法与 g_idle_add_full 驱动的 IWindowDispatcher。
...
begin
  ...
  if not TryLoadWindowGtk(LInfo) or not LInfo.Loaded then
