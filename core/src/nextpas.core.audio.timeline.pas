...
var I, LCap: Integer;
begin
  // ensure FSnapshotClips outer capacity geometric
  if AMaxClips <= 0 then Exit;
  // caller ensures avail already, we ensure inner arrays for existing avail
end;
...
begin
  ...
  begin
    ...
    begin
      LCap:=Length(FSnapshotClips[i]); if LCap < MaxClips then begin InterlockedExchangeAdd64(FViolations, 1); AudioEnsureCapacity(LCap, MaxClips, 4); if Length(FSnapshotClips[i]) <> LCap then SetLength(FSnapshotClips[i], LCap); end;
      LCap:=Length(FSnapshotLGains[i]); if LCap < MaxClips then begin InterlockedExchangeAdd64(FViolations, 1); AudioEnsureCapacity(LCap, MaxClips, 4); if Length(FSnapshotLGains[i]) <> LCap then SetLength(FSnapshotLGains[i], LCap); end;
      LCap:=Length(FSnapshotRGains[i]); if LCap < MaxClips then begin InterlockedExchangeAdd64(FViolations, 1); AudioEnsureCapacity(LCap, MaxClips, 4); if Length(FSnapshotRGains[i]) <> LCap then SetLength(FSnapshotRGains[i], LCap); end;
    end;
  end;
