...
{ Inc 内容直写单源：header+声明+body+尾零拷贝写入，bytes.ops.BytesCopy inline 单 Move，HexEncodeDollarCommaBulkUpper 4-wide 单源，外联守 I-Cache（外层不 inline 避热点复制膨胀，内层 WriteStr/BytesCopy inline），writer.builder 单源收敛；供 IncSource/IncUnit 单次 SetLength 直写复用，消除 4MiB 内二次整包拷贝 }
...
var
  ...
  begin
    ...
    if L = 0 then
      Exit;
    BytesCopy(AW, PAnsiChar(S), SizeUInt(L));
    Inc(AW, L);
  end;
