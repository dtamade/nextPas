...
{ Not inline: FPC trunk -O2 REGVAR keeps a stale register for Cap across the
  nested inline var-param chain (CheckedAdd); stale 0 yields empty
...
begin
  Cap := 0;
  CheckedAdd(Cap, SizeUInt(Length('{ generated by rp_pack / nextpas.core.respack embed toolchain. }' + #10 + '{ do not edit; regenerate instead. deterministic output is    }' + #10 + '{ locked by the test_respack_embed golden gate.               }' + #10)), EMBED_CAP_OVERFLOW_MSG);
  CheckedAdd(Cap, SizeUInt(Length('const'#10'  ')), EMBED_CAP_OVERFLOW_MSG);
  CheckedAdd(Cap, SizeUInt(Length(AName)), EMBED_CAP_OVERFLOW_MSG);
  CheckedAdd(Cap, SizeUInt(Length('_SIZE = ')), EMBED_CAP_OVERFLOW_MSG);
  CheckedAdd(Cap, SizeUInt(Length(ANStr)), EMBED_CAP_OVERFLOW_MSG);
  CheckedAdd(Cap, SizeUInt(Length(';'#10'  ')), EMBED_CAP_OVERFLOW_MSG);
  CheckedAdd(Cap, SizeUInt(Length(AName)), EMBED_CAP_OVERFLOW_MSG);
  CheckedAdd(Cap, SizeUInt(Length(': array[0..')), EMBED_CAP_OVERFLOW_MSG);
  CheckedAdd(Cap, SizeUInt(Length(AName)), EMBED_CAP_OVERFLOW_MSG);
  CheckedAdd(Cap, SizeUInt(Length('_SIZE - 1] of Byte = ('#10)), EMBED_CAP_OVERFLOW_MSG);
  ...
  if Lines > 1 then
  begin
    CheckedMul(Lines - 1, 3, LTmp, EMBED_CAP_OVERFLOW_MSG);
    CheckedAdd(BodyLen, LTmp, EMBED_CAP_OVERFLOW_MSG);
  end;
  CheckedAdd(BodyLen, 1, EMBED_CAP_OVERFLOW_MSG);
  CheckedAdd(Cap, BodyLen, EMBED_CAP_OVERFLOW_MSG);
  CheckedAdd(Cap, SizeUInt(Length(#10'  );'#10)), EMBED_CAP_OVERFLOW_MSG);
  Result := Cap;
end;
...
begin
  ...
  if APerLine <= 0 then
    APerLine := RESPACK_INC_DEFAULT_BYTES_PER_LINE;
  AN := ABlob.Size;
...
begin
  ...
  Total := 0;
  CheckedAdd(Total, SizeUInt(Length('unit ')), EMBED_CAP_OVERFLOW_MSG);
  CheckedAdd(Total, SizeUInt(Length(AUnitName)), EMBED_CAP_OVERFLOW_MSG);
  CheckedAdd(Total, SizeUInt(Length(';' + #10 + '{$mode objfpc}{$H+}' + #10 + #10 + 'interface' + #10 + #10)), EMBED_CAP_OVERFLOW_MSG);
  CheckedAdd(Total, CapInc, EMBED_CAP_OVERFLOW_MSG);
  CheckedAdd(Total, SizeUInt(Length(#10 + 'implementation' + #10 + #10 + 'end.' + #10)), EMBED_CAP_OVERFLOW_MSG);
