...
const
  ...
  { .inc 头三行字面量单源：EmitIncContent 与 CalcIncCapacity 共用，防漂移 }
  INC_HDR_LINE1 = '{ generated by rp_pack / nextpas.core.respack embed toolchain. }';
  INC_HDR_LINE2 = '{ do not edit; regenerate instead. deterministic output is    }';
  INC_HDR_LINE3 = '{ locked by the test_respack_embed golden gate.               }';
...
begin
  WriteStrTo(AW, AEnd, INC_HDR_LINE1 + #10);
  WriteStrTo(AW, AEnd, INC_HDR_LINE2 + #10);
  WriteStrTo(AW, AEnd, INC_HDR_LINE3 + #10);
...
begin
  ...
  Cap := 0;
  if not TryAddSizeUInt(Cap, SizeUInt(Length(INC_HDR_LINE1 + #10 + INC_HDR_LINE2 + #10 + INC_HDR_LINE3 + #10)), Cap) then
    raise EResPackTooLarge.Create(EMBED_CAP_OVERFLOW_MSG);
