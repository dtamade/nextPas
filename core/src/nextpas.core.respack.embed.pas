...
{** @desc 嵌入工具链库（S4）：目录 → 过滤/映射 → 打包 blob；blob → .inc typed const
  ...
  rust-embed derive 属性与 asar unpack-dir；文件系统接触点收口 dirsource
  seam（输入枚举 + 输出解包 ResPackExtractToDir），本单元仅依赖 dirsource +
  fs.glob 纯匹配（GlobMatch 无 IO），不直接依赖 fs 全量模块。
...
begin
  ...
  for I := 0 to SizeUInt(Length(AList)) - 1 do
    if Length(AList[I]) = 0 then
      raise EResPackError.CreateCtx('embed', '', 'respack.embed: empty ' + AWhat
        + ' glob pattern');
end;
...
begin
  ...
  if not ResPackValidPath(AOut, True) then
    raise EResPackInvalidPath.CreateCtx('embed', AOut, 'respack.embed: mapped path invalid "'
      + AOut + '"');
  Result := True;
end;
...
begin
  if (AOpts.StripPrefix <> '') and (not StartsSlash(AOpts.StripPrefix)) then
    raise EResPackError.CreateCtx('embed', AOpts.StripPrefix, 'respack.embed: StripPrefix must be empty or ' +
      'end with "/" ("' + AOpts.StripPrefix + '")');
  if (AOpts.AddPrefix <> '') and (not StartsSlash(AOpts.AddPrefix)) then
    raise EResPackError.CreateCtx('embed', AOpts.AddPrefix, 'respack.embed: AddPrefix must be empty or ' +
  ...
  if SizeUInt(Length(Kept)) = 0 then
    raise EResPackError.CreateCtx('embed', ASourceDir, 'respack.embed: no entries matched after ' +
...
var
  ...
  begin
    ...
    Builder.AppendBytes(PByte(PAnsiChar(S)), SizeUInt(Length(S)));
  end;

  procedure EmitCh(const C: AnsiChar); inline;
  begin
    Builder.AppendByte(Byte(C));
  end;

begin
  Name := AOpts.ConstName;
  if not ResPackValidIdent(Name) then
    raise EResPackError.CreateCtx('inc', Name, 'respack.embed: ConstName must be a Pascal ' +
      'identifier ("' + Name + '")');
  if ABlob.Data = nil then
    raise EResPackError.CreateCtx('inc', '', 'respack.embed: blob is nil');
  if ABlob.Size = 0 then
    raise EResPackError.CreateCtx('inc', '', 'respack.embed: blob is empty');
  PerLine := AOpts.BytesPerLine;
  if PerLine <= 0 then
    PerLine := RESPACK_INC_DEFAULT_BYTES_PER_LINE;
  N := ABlob.Size;
  { 单源容量：复用 bytes.builder Grow 单源（同 writer AlignUp 单源），
    初始 hint = 原手工 Cap 上界，Builder 按需倍增零手工指针。 }
  Builder := CreateBytesBuilder(256 + SizeUInt(Length(Name)) * 4 + 40
...
begin
  if not ResPackValidIdent(AUnitName) then
    raise EResPackError.CreateCtx('inc', AUnitName, 'respack.embed: UnitName must be a Pascal ' +
      'identifier ("' + AUnitName + '")');
  if IdentEqualCI(AUnitName, AOpts.ConstName) then
    raise EResPackError.CreateCtx('inc', AUnitName, 'respack.embed: UnitName must differ from ' +
      'ConstName ("' + AUnitName + '")');
