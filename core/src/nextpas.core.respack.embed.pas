...
procedure RequireIncSpace(const ACur, AEnd: PByte; const ANeed: SizeUInt); inline;
begin
  if (ACur = nil) or (AEnd = nil) or (ACur > AEnd) or
    (SizeUInt(AEnd - ACur) < ANeed) then
    raise EResPackTooLarge.Create(EMBED_CAP_OVERFLOW_MSG);
end;

procedure WriteStrTo(var ADst: PByte; const AEnd: PByte; const S: string); inline;
var
  L: SizeInt;
begin
  L := Length(S);
  if L = 0 then
    Exit;
  RequireIncSpace(ADst, AEnd, SizeUInt(L));
  BytesCopy(ADst, PAnsiChar(S), SizeUInt(L));
  Inc(ADst, L);
end;

{ Inc 直写单源：header+body+尾零拷贝，Hex 4-wide 单源，外联守 I-Cache }
procedure EmitIncContent(var AW: PByte; const AEnd: PByte; const ABlob: TResPackBlob; const AName, ANStr: string; const APerLine: Integer; const AN: SizeUInt);
var
  I: SizeInt;
  LineCols: Integer;
  PData: PByte;
  LChunk, LNeed, LTmp: SizeUInt;

begin
  WriteStrTo(AW, AEnd, '{ generated by rp_pack / nextpas.core.respack embed toolchain. }' + #10);
  WriteStrTo(AW, AEnd, '{ do not edit; regenerate instead. deterministic output is    }' + #10);
  WriteStrTo(AW, AEnd, '{ locked by the test_respack_embed golden gate.               }' + #10);
  WriteStrTo(AW, AEnd, 'const'#10'  ');
  WriteStrTo(AW, AEnd, AName);
  WriteStrTo(AW, AEnd, '_SIZE = ');
  WriteStrTo(AW, AEnd, ANStr);
  WriteStrTo(AW, AEnd, ';'#10'  ');
  WriteStrTo(AW, AEnd, AName);
  WriteStrTo(AW, AEnd, ': array[0..');
  WriteStrTo(AW, AEnd, AName);
  WriteStrTo(AW, AEnd, '_SIZE - 1] of Byte = ('#10);
  LineCols := 0;
  PData := ABlob.Data;
  I := 0;
  ...
  begin
    ...
    begin
      ...
      begin
        LChunk := SizeUInt(SizeInt(AN) - I - 1);
        if not TryMulSizeUInt(LChunk, 4, LTmp) then
          raise EResPackTooLarge.Create(EMBED_CAP_OVERFLOW_MSG);
        if not TryAddSizeUInt(LTmp, 3, LNeed) then
          raise EResPackTooLarge.Create(EMBED_CAP_OVERFLOW_MSG);
        RequireIncSpace(AW, AEnd, LNeed);
        HexEncodeDollarCommaBulkUpper(@PData[I], LChunk, AW);
        Inc(AW, SizeInt(LChunk * 4));
        AW^ := Byte('$');
        Inc(AW);
    ...
    begin
      LChunk := SizeUInt(APerLine - LineCols);
      if not TryMulSizeUInt(LChunk, 4, LTmp) then
        raise EResPackTooLarge.Create(EMBED_CAP_OVERFLOW_MSG);
      if not TryAddSizeUInt(LTmp, 1, LNeed) then
        raise EResPackTooLarge.Create(EMBED_CAP_OVERFLOW_MSG);
      RequireIncSpace(AW, AEnd, LNeed);
      HexEncodeDollarCommaBulkUpper(@PData[I], LChunk, AW);
      Inc(AW, SizeInt(LChunk * 4));
...
begin
  ...
  begin
    W := @OutBuf[0];
    EndBuf := PByte(PtrUInt(W) + Cap);
  end;
  Start := W;
  EmitIncContent(W, EndBuf, ABlob, Name, NStr, PerLine, N);
...
begin
  ...
  Dst := @Result[0];
  Start := Dst;
  EndBuf := PByte(PtrUInt(Start) + Total);
  WriteStrTo(Dst, EndBuf, 'unit ');
  WriteStrTo(Dst, EndBuf, AUnitName);
  WriteStrTo(Dst, EndBuf, ';' + #10 + '{$mode objfpc}{$H+}' + #10 + #10 + 'interface' + #10 + #10);
  EmitIncContent(Dst, EndBuf, ABlob, Name, NStr, PerLine, N); // 单次 SetLength 后直写，bytes.ops/hex 单源 inline 零拷贝，消除 4MiB 内二次整包 BytesCopy
  WriteStrTo(Dst, EndBuf, #10 + 'implementation' + #10 + #10 + 'end.' + #10);
