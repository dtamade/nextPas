...
{ Helper: read XClientMessageEvent data.l[0]. }
...
function x11_event_window(const AEvent: TX11Event): TX11Window;
begin
  Result := AEvent.FWindow;
end;

function x11_key_event_state(const AEvent: TX11Event): UInt32;
begin
  Result := PUInt32(@PByte(@AEvent)[X11_KEY_EVENT_STATE_OFFSET])^;
end;

function x11_key_event_keycode(const AEvent: TX11Event): UInt32;
begin
  Result := PUInt32(@PByte(@AEvent)[X11_KEY_EVENT_KEYCODE_OFFSET])^;
end;

function x11_button_event_button(const AEvent: TX11Event): UInt32;
begin
  Result := PUInt32(@PByte(@AEvent)[X11_BUTTON_EVENT_BUTTON_OFFSET])^;
end;

function x11_button_event_x(const AEvent: TX11Event): Int32;
begin
  Result := PInt32(@PByte(@AEvent)[X11_BUTTON_EVENT_X_OFFSET])^;
end;

function x11_button_event_y(const AEvent: TX11Event): Int32;
begin
  Result := PInt32(@PByte(@AEvent)[X11_BUTTON_EVENT_Y_OFFSET])^;
end;

function x11_motion_event_x(const AEvent: TX11Event): Int32;
begin
  Result := PInt32(@PByte(@AEvent)[X11_MOTION_EVENT_X_OFFSET])^;
end;

function x11_motion_event_y(const AEvent: TX11Event): Int32;
begin
  Result := PInt32(@PByte(@AEvent)[X11_MOTION_EVENT_Y_OFFSET])^;
end;

function x11_configure_event_width(const AEvent: TX11Event): Int32;
begin
  Result := PInt32(@PByte(@AEvent)[X11_CONFIGURE_EVENT_WIDTH_OFFSET])^;
end;

function x11_configure_event_height(const AEvent: TX11Event): Int32;
begin
  Result := PInt32(@PByte(@AEvent)[X11_CONFIGURE_EVENT_HEIGHT_OFFSET])^;
end;

function x11_client_message_l0(const AEvent: TX11Event): TX11Atom;
begin
  Result := PTX11Atom(@PByte(@AEvent)[X11_CLIENT_MESSAGE_DATA_OFFSET])^;
end;
