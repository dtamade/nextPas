...
function TJsJs888Context.TryEval(const ACode: string; out AValue: TJsValue): Boolean; begin EnsureNotClosed; try AValue:=Eval(ACode); Result:=True; except AValue:=JsUndefinedValue; Result:=False; end; end;
...
function TJsJs888Context.Global: TJsValue; begin EnsureNotClosed; Result:=FGlobal; end;
...
function TJsJs888Context.NewObject: TJsValue; begin EnsureNotClosed; Result:=JsPureHeapNewObject(FHeap); end;
function TJsJs888Context.NewArray: TJsValue; begin EnsureNotClosed; Result:=JsPureHeapNewArray(FHeap); end;
...
function TJsJs888Context.HasProp(const AObj: TJsValue; const AName: string): Boolean; begin EnsureNotClosed; Result:=JsPureHeapHasProp(FHeap, AObj, AName); end;
function TJsJs888Context.DeleteProp(const AObj: TJsValue; const AName: string): Boolean; begin EnsureNotClosed; Result:=JsPureHeapDeleteProp(FHeap, AObj, AName); end;
function TJsJs888Context.GetKeys(const AObj: TJsValue): TJsStringArray; begin EnsureNotClosed; Result:=JsPureHeapGetKeys(FHeap, AObj); end;
function TJsJs888Context.NewError(const AMessage: string; ACategory: TJsErrorCategory): TJsValue; begin EnsureNotClosed; Result:=JsErrorValue(AMessage); end;
function TJsJs888Context.NewFunction(const AName: string; AHandler: TJsHostFunction): TJsValue; begin EnsureNotClosed; if Assigned(AHandler) then SetHostFunction(AName,AHandler); Result:=JsFunctionValue(AName); end;
function TJsJs888Context.NewFunction(const AName: string; AHandler: TJsHostMethod): TJsValue; begin EnsureNotClosed; if Assigned(AHandler) then SetHostFunction(AName,AHandler); Result:=JsFunctionValue(AName); end;
function TJsJs888Context.NewFunction(const AName: string; AHandler: TJsHostProc): TJsValue; begin EnsureNotClosed; if Assigned(AHandler) then SetHostFunction(AName,AHandler); Result:=JsFunctionValue(AName); end;
function TJsJs888Context.GetProp(const AObj: TJsValue; const AName: string): TJsValue; begin EnsureNotClosed; Result:=JsPureHeapGetProp(FHeap, AObj, AName); end;
procedure TJsJs888Context.SetProp(const AObj: TJsValue; const AName: string; const AVal: TJsValue); begin EnsureNotClosed; JsPureHeapSetProp(FHeap, AObj, AName, AVal); end;
function TJsJs888Context.Call(const AFunc: TJsValue; const AThis: TJsValue; const AArgs: array of TJsValue): TJsValue; begin EnsureNotClosed; EnsureThreadAffinity; Result:=JsPureCall(Self, FHostFuncs, AFunc, AThis, AArgs, jsbkJs888); end;
