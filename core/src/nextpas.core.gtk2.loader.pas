unit nextpas.core.gtk2.loader;

{** @desc GTK2 窗口子集动态装载（家族内唯一触 platform.dl 的单元）。
       原语一律来自 nextpas.core.platform.dl，禁用 FPC DynLibs。

       探测仅装载 GTK2 栈（libgtk-x11-2.0 / libgobject-2.0 / libglib-2.0），
       不触 WebKit。能力以符号存在性判定，装载结果进程级幂等。
       gtk_widget_get_scale_factor 在 GTK2 上不存在，以 BindOpt 可选绑定。 *}
...
function TryLoadGtk2(out AInfo: TGtk2LoadInfo): Boolean;
  ...
  begin
    Result :=
      ...
      BindOpt(@gtk_widget_get_scale_factor, 'gtk_widget_get_scale_factor') and
...
begin
  ...
  GLoading := True;
  try
    if not (TryDlOpen(GGtkLib, ['libgtk-x11-2.0.so.0','libgtk-x11-2.0.so']) and
