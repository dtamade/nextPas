...
{** @desc webview 家族：IVfs → IWebviewAssetProvider 适配器。
       ...
       - 前缀兼容：bridge 不剥离 mount 前缀（最长前缀匹配后仍透传全路径），
         ...
         先试全路径，未命中再试剥首段（"index.html"），两试均 ExistsView O(log n)
         二分预检（命中单次 VfsReadAllBytesView Open/Read，404 零重度 ExistsView 收敛 False），
         避免手工路径猜测与跨分配器陷阱；剥段分支 TStringView 零拷贝 Slice 直通
         IVfsView（memtree/embedded 真零拷贝视图二分，os 单次物化兜底，无 Copy 临时串、无 MaxInt 扫描），
         TryRead/TryReadView inline 零额外调用，零拷贝 Move 透传，单请求至多一次重度 I/O
...
{ perf: inline Exists 预检 + 单次 O(log n) 二分直达 VfsReadAllBytes（memtree/embedded 二分、os Open 单次）；
  命中 SetLength+Move 单次分配零拷贝透传；404/非法路径/目录 Exists 零重度 I/O 快速收敛 False；
  单请求至多一次重度 Open/Read（404 零重度，命中/回退命中单次 VfsReadAllBytes，Exists 二分廉价）；
  稳定性：VfsReadAllBytes 内 S.Close 于 finally 释放，竞态/异常统一 False；
  单源复用：TryResolve 剥段复用 nextpas.core.text.view 零拷贝 Slice
  （与 bytes.ops VecGrowCapacity 同源单源策略，避免跨单元重复切片）。 }
...
begin
  ...
  ABytes := nil;
  AMime := '';
  if not VfsExistsView(FVfs, TStringView.FromStr(APath)) then
    Exit;
  try
...
{ perf: inline ExistsView 零拷贝视图预检 + 单次 O(log n) 视图二分直达 VfsReadAllBytesView
  （memtree/embedded 真零拷贝视图二分、os 单次物化兜底）；命中 SetLength+Move 单次分配零拷贝透传；
  404/非法路径/目录 ExistsView 零重度 I/O 快速收敛 False；单请求至多一次重度 Open/Read；
...
begin
  ...
  if LNorm = '' then
    Exit(False);
  { 先试全路径（mount 前缀保留的形态）；TryRead 视图二分直达 VfsReadAllBytes }
  ...
  { 回退：剥首段（兼容 mount '' + URL 含 "app/" 前缀的常见形态）。
    ...
    IVfsView（memtree/embedded 真零拷贝视图二分，os 单次物化兜底，无 Copy 临时串、无 MaxInt 扫描）；
    单请求至多一次重度 I/O（404 零重度，命中单次 VfsReadAllBytesView）；
    ...
    TryReadView inline 零额外调用，零拷贝 Move 透传。 }
