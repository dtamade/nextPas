...
{** @desc webview 家族：IVfs → IWebviewAssetProvider 适配器。
       ...
       - 前缀兼容：bridge 不剥离 mount 前缀（最长前缀匹配后仍透传全路径），
         ...
         先试全路径，未命中再试剥首段（"index.html"），两试均单次 O(log n)
         探测直达 VfsReadAllBytes(View)（命中单次 Open/Read，404 抛 EVfs* 零额外二分收敛），
         避免手工路径猜测与跨分配器陷阱；剥段分支 TStringView 零拷贝 Slice 直通
         IVfsView（memtree/embedded 真零拷贝视图二分，os 单次物化兜底，无 Copy 临时串、无 MaxInt 扫描），
         TryRead/TryReadView inline 单次探测零额外二分，零拷贝 Move 透传，单请求至多一次重度 I/O
...
{ perf: inline 单次探测 O(log n) 直达 VfsReadAllBytes（memtree/embedded 单次二分、os 单次 Open）；
  命中 SetLength+Move 单次分配零拷贝透传；404/非法路径/目录 OpenRead 抛 EVfs* 直接 False 零额外二分；
  单请求至多一次重度 Open/Read（命中/回退命中单次 VfsReadAllBytes，404 零重度抛异常收敛）；
  稳定性：VfsReadAllBytes 内 S.Close 于 finally 释放，竞态/异常统一 False；
  单源复用：TryResolve 剥段复用 nextpas.core.text.view 零拷贝 Slice
  （与 bytes.ops VecGrowCapacity 同源单源策略，避免跨单元重复切片）；命中路径较旧双重 O(log n) 省一次二分。 }
...
{ perf: inline 单次探测 O(log n) 视图二分直达 VfsReadAllBytesView
  （memtree/embedded 真零拷贝视图二分、os 单次物化兜底）；命中 SetLength+Move 单次分配零拷贝透传；
  404/非法路径/目录 OpenReadView 抛 EVfs* 直接 False 零额外二分；单请求至多一次重度 Open/Read；
  ...
  命中路径较旧 ExistsView+VfsReadAllBytesView 双重 O(log n) 省一次二分 }
...
begin
  ...
  if LNorm = '' then
    Exit(False);
  { 先试全路径（mount 前缀保留的形态）；TryRead 单次 O(log n) 探测直达 VfsReadAllBytes }
  ...
  { 回退：剥首段（兼容 mount '' + URL 含 "app/" 前缀的常见形态）。
    ...
    IVfsView（memtree/embedded 真零拷贝视图二分，os 单次物化兜底，无 Copy 临时串、无 MaxInt 扫描）；
    单请求至多一次重度 I/O（404 抛 EVfs* 零额外二分收敛，命中单次 VfsReadAllBytesView）；
    ...
    TryReadView inline 单次探测零额外二分，零拷贝 Move 透传。 }
