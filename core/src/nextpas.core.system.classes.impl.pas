...
type
  ...
  public
    ...
    destructor Destroy; override;
    function Read(var Buffer; Count: LongInt): LongInt; override; inline;
    function Write(const Buffer; Count: LongInt): LongInt; override; inline;
    function Seek(const Offset: Int64; Origin: TSeekOrigin): Int64; override; inline;
  end;
  TList = class(TObject)
  private
...
begin
  ...
  if (AMode and fmCreate) = fmCreate then
    Result := [fmRead, fmWrite, fmCreate, fmTruncate]
  else
    case AMode and $0003 of
      fmOpenWrite: Result := [fmWrite];
      fmOpenReadWrite: Result := [fmRead, fmWrite];
      else Result := [fmRead];
    end;
...
begin
  ...
  // perf: inline thin forward to fs owner FsOpenFile single source (platform_file_open_ex zero-copy), no extra buffer
  try
    FFile := nextpas.core.fs.FsOpenFile(AFileName, LMode, nextpas.core.fs.base.PermDefault);
  except
    FFile := nil;
...
function TFileStream.GetSize: Int64; inline;
begin
  if FFile = nil then Exit(0);
  // perf: inline thin forward to IFile.GetSize single source (platform_file_fstat zero-copy)
  try
...
function TFileStream.Read(var Buffer; Count: LongInt): LongInt; inline;
begin
  if (Count <= 0) or (FFile = nil) then Exit(0);
  // perf: inline thin forward to fs IFile.Read single source platform_file_read zero-copy (no Move)
  try
...
function TFileStream.Write(const Buffer; Count: LongInt): LongInt; inline;
begin
  if (Count <= 0) or (FFile = nil) then Exit(0);
  // perf: inline thin forward to fs IFile.Write single source platform_file_write zero-copy (no Move)
  try
...
function TFileStream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64; inline;
begin
  if FFile = nil then Exit(-1);
  // perf: inline thin forward to IFile.Seek single source platform_file_seek zero-copy
  try
