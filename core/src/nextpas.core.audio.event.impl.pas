unit nextpas.core.audio.event.impl;
...
begin
  ...
  if FEof and not FLoop then begin AudioSilentFill(ABuffer, FFormat, AFrames); Exit(0); end;
...
begin
  ...
  // control-plane preallocate scratch (realtime must not GetMem)
  LCap := 4096 * FFormat.BlockAlign;
  if LCap > 0 then SetLength(FScratch, LCap);
  // preallocate snapshot voices to MaxVoices geometric start
  LCap := 0; AudioEnsureCapacity(LCap, FMaxVoices, 8);
  if LCap > 0 then SetLength(FSnapshotVoices, LCap);
end;
...
begin
  ...
  try
    ...
    for I:=0 to High(FEvents) do SetLength(FEvents[I].Desc.Buffer.Data, 0);
    SetLength(FEvents, 0);
...
begin
  ...
  if Length(FScratch) < LNeed then EnsureScratch(LNeed);
  ...
  // snapshot scratch reuse: geometric growth via AudioEnsureCapacity, steady zero alloc
  if Length(FSnapshotVoices) < LCount then
  begin
    LCap2 := Length(FSnapshotVoices); AudioEnsureCapacity(LCap2, LCount, 8); if Length(FSnapshotVoices) <> LCap2 then SetLength(FSnapshotVoices, LCap2);
  end;
  if LCount > 0 then
  begin
    FLock.Acquire;
  ...
  begin
    AudioSilentFill(LSrc, FFormat, AFrames);
