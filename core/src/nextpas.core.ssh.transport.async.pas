 ...
 * 复用 cipher/compress/kex 的同步加解密逻辑, I/O 事件化。经 ssh.net.ffi
 * 单缝隙复用 IAsyncTcpStream（L2→L2 net 唯一缝隙，inline 零拷贝，外层
 * Move 单源复用 bytes.ops），不再直连 net.async.tcp；try-finally 释放不丢。 *}
