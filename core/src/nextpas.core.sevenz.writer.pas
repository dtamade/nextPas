...
begin
  ...
  begin
    ...
    SevenZFilterConvert(LStage, AFilters[LI],
      AFolder.Specs[LI].Props, True);
  end;
  if AHasForced then
  begin
    case AForcedId of
        ...
        begin
          ...
          begin
            if FEntries[LI].Source = esReader then
              ReadFully(FEntries[LI].Reader,
                LFolders[LFolderIdx].RawSolid[LCursor],
                SizeUInt(EntrySize(FEntries[LI])))
            else
              Move(FEntries[LI].Data[0],
                LFolders[LFolderIdx].RawSolid[LCursor],
                SizeInt(EntrySize(FEntries[LI])));
            { 子流 CRC 基于物化后的 RawSolid 切片计算，保证 reader 与 bytes 路径一致 }
            LFolders[LFolderIdx].SubCrcs[LJ] :=
              Crc32Of(LFolders[LFolderIdx].RawSolid[LCursor],
                SizeUInt(EntrySize(FEntries[LI])));
