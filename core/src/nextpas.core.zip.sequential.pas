...
procedure StoreLE32(var ADst: TBytes; AOff: SizeUInt; AValue: LongWord); inline;
begin
  ADst[AOff] := Byte(AValue);
  ADst[AOff + 1] := Byte(AValue shr 8);
  ADst[AOff + 2] := Byte(AValue shr 16);
  ADst[AOff + 3] := Byte(AValue shr 24);
end;
...
function IsDescriptorSig(const AData: TBytes; APos: SizeUInt): Boolean; inline;
begin
  Result := LE32At(AData, APos) = C_ZIP_DESCRIPTOR_SIG;
end;

{ Guard 与 Decompress 的本地复刻：与 reader.pas 等价语义，保证校验层一致 }
