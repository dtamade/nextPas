unit nextpas.core.db.factory.facade.odbc;

{**
 * @desc L3 工厂门面分治：odbc 薄转发单源（3 ConnectOdbc）。
...
const
  FACTORY_FACADE_ODBC_BYTES_SINGLE_SOURCE = BYTES_OPS_SINGLE_SOURCE;
  FACTORY_FACADE_ODBC_BYTES_SINGLE_SOURCE_VERSION = BYTES_OPS_SINGLE_SOURCE_VERSION;

{$IF not BYTES_OPS_SINGLE_SOURCE}
  {$FATAL 'bytes.ops single source drift: factory.facade.odbc must reuse bytes.ops'}
{$IFEND}

{$I nextpas.core.bytes.ops.single_source.inc}

{ ---- odbc facade pure re-export inline thin forward via Kind-driven table, zero adapter hard link ---- }

function ConnectOdbc(const ADsn: string): IDbConnection; inline;
begin
  // perf: inline thin forward Kind-driven, zero-copy string COW, bytes.ops single source, interface refcount auto
  Result := DbOpen(dbkOdbc, ADsn, TDbConnectOptions.Default, 64);
end;
...
begin
  // perf: inline thin forward Kind-driven, zero-copy string COW, bytes.ops single source, interface refcount auto
  Result := DbOpen(dbkOdbc, ADsn, AOptions, 64);
end;
...
begin
  // perf: inline thin forward Kind-driven, zero-copy string COW, bytes.ops single source, interface refcount auto
  Result := DbOpen(dbkOdbc, ADsn, AOptions, AStmtCacheCapacity);
end;
