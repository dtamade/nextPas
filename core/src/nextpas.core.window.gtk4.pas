...
{** @desc GTK4 后端（薄适配层，消费独立 L2 gtk4 家族 + 共享实现单元）。
       原 786 行 `{$I}` 共享实现已显式化为
...
var
  L: TGtk4LoadInfo;
begin
  Result := TryLoadGtk4(L);
  ALoaded := L.Loaded;
end;

function BuildGtk4Ops: TGtkOps; inline;
begin
...
begin
  ...
  GContext := TGtkContext.Create(BuildGtk4Ops);
  Result := GContext;
end;
...
procedure RegisterGtk4Backend;
var LDesc: TBackendDesc;
begin
  LDesc.Kind := wkGtk4;
  LDesc.Probe := @WindowGtk4IsAvailable;
  LDesc.Create := @CreateWindowGtk4;
  LDesc.Live := @Gtk4LiveWindowCount;
  LDesc.Run := @WindowGtk4RunMainLoop;
  LDesc.Quit := @WindowGtk4QuitMainLoop;
  LDesc.Pump := @GtkPumpOnce;
  LDesc.Sonames := GTK4_SONAMES;
