...
begin
  ...
  LD := GetDirectDispatchTable;
  LD^.BatchF32.ArrayAdd(aSrc1, aSrc2, aDst, aCount);
end;
...
begin
  if (aSrc1 = nil) or (aSrc2 = nil) or (aDst = nil) or (aCount = 0) then Exit;
  LD := GetDirectDispatchTable;
  LD^.BatchF32.ArrayMul(aSrc1, aSrc2, aDst, aCount);
end;
...
begin
  ...
  LD := GetDirectDispatchTable;
  LD^.BatchF32.ArrayMulScalar(aSrc, aDst, aCount, aScalar);
end;
...
begin
  ...
  LD := GetDirectDispatchTable;
  LD^.BatchF32.ArrayAxpy(aAlpha, aX, aY, aDst, aCount);
end;
...
begin
  ...
  LD := GetDirectDispatchTable;
  Result := LD^.BatchF32.ReduceSum(aSrc, aCount);
end;
...
begin
  ...
  LD := GetDirectDispatchTable;
  Result := LD^.BatchF32.ReduceDot(aSrc1, aSrc2, aCount);
end;
...
begin
  if (aSrc = nil) or (aCount = 0) then begin Result := 0; Exit; end;
  LD := GetDirectDispatchTable;
  Result := LD^.BatchF32.ReduceMin(aSrc, aCount);
end;
...
begin
  if (aSrc = nil) or (aCount = 0) then begin Result := 0; Exit; end;
  LD := GetDirectDispatchTable;
  Result := LD^.BatchF32.ReduceMax(aSrc, aCount);
end;
