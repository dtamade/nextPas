...
{** @desc GTK3 后端（薄适配层，消费独立 L2 gtk3 家族）。
       自 window.gtk 扭转后，本单元改为 uses nextpas.core.gtk2.base,
  nextpas.core.gtk2.ffi/loader，
...
var
  LInfo: TGtk2LoadInfo;
begin
  Result := TryLoadGtk2(LInfo) and LInfo.Loaded;
end;
...
begin
  ...
  if not TryLoadGtk2(LInfo) or not LInfo.Loaded then
