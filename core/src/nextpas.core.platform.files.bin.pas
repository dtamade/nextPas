{**
 * nextpas.core.platform.files.bin - 二进制文件整体读写（高层）
 *
 * 职责：整个文件内容 ↔ TBytes 的场景（图像、序列化产物、压缩包）。
 * 层次：建立在 files.pas（fd 级）与 fs.pas（文件系统操作）之上，
 *       不直接依赖 FPC RTL（SysUtils/Classes 等）。
 *}

unit nextpas.core.platform.files.bin;
...
function FileReadAllBytes(const APath: AnsiString; out AData: TBytes): Boolean;

{** @desc 写整个文件（创建或截断覆盖）
    @param APath 文件路径（空路径返回 False）
    @param AData 待写入内容（空数组 = 创建空文件）
    @return True 成功；路径不可写等失败返回 False *}
function FileWriteAllBytes(const APath: AnsiString; const AData: TBytes): Boolean;
