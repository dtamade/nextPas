...
{** @desc db.pool 并发桶子模块（L2 基础设施，CONTRACT §2.7）。
       ...
       复用 bytes.ops 单源（PoolGrowCap 经 BytesCalcGrowCapWithMin 单 Move 零拷贝，min 4 与 idle/leak 归一），性能 inline 零 I-Cache 膨胀。 *}
...
const
  POOL_CONCURRENCY_BYTES_SINGLE_SOURCE = BYTES_OPS_SINGLE_SOURCE;
...
function PoolGrowCap(const AOld, ARequired: SizeUInt): SizeUInt; inline;
begin
  { 单源直连 bytes.ops：容量增长零拷贝单源，idle/leak/concurrency 三面收敛至 BytesCalcGrowCapWithMin(min 4)，零额外分配，inline 零 I-Cache 膨胀 }
