unit nextpas.core.audio.bank.impl;
...
begin
  ...
  FData := SpanClone(TByteSpan.FromBytes(ABuffer.Data)); // perf: bytes.ops single source — inline SetLength+Move, zero-copy view, unified heap path
...
begin
  ...
  try
    ...
    { deep copy isolation for snapshot safety — bytes.ops single source }
    ABuffer := FEntries[Idx].Buffer;
    ABuffer.Data := SpanClone(TByteSpan.FromBytes(FEntries[Idx].Buffer.Data)); // perf: inline SetLength+Move, zero-copy view, unified heap path
    Result := True;
  finally
    FLock.Release;
  end;
...
begin
  ...
  try
    ...
    { deep copy TAudioBuffer Data for SoundBank preload — bytes.ops single source }
    FEntries[Idx].Buffer := ABuffer;
    FEntries[Idx].Buffer.Data := SpanClone(TByteSpan.FromBytes(ABuffer.Data)); // perf: inline SetLength+Move, zero-copy view, unified heap path
