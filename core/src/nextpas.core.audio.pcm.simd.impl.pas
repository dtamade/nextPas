unit nextpas.core.audio.pcm.simd.impl;
...
uses
  ...
  nextpas.core.bytes.ops; // single source BytesCopy for F32 raw zero-copy, inline — no Move bypass
...
// pcm.simd.impl is thin inline forwarding single source to simd owner via audio.simd SimdConvert*,
// no duplicate 4-way unroll, no secondary caps dispatch, zero extra branch, inline + zero-copy.
// Raw F32 block copy stays single source via bytes.ops BytesCopy → bytes.ops (see audio.pcm).

procedure PcmConvertBlockS16ToF32(const ASrc: PSmallInt; ADst: PSingle; ACount: Integer); inline;
begin
  if (ASrc = nil) or (ADst = nil) or (ACount <= 0) then Exit;
  // thin inline forward single source to Owner simd; inline + no extra caps branch, zero-copy pointers, bytes.ops single source for raw F32 path
