...
// MIGRATION: future impl: TResourceLoadTask = class(TInterfacedObject, IAsyncTask) with TAsyncLoop.Schedule; Execute via Loop.Queue
  ...
  private
    type
      ...
      TResourceItem = record
        ...
        Tags: TAudioTags;
        Alive: Boolean;
        Thread: Pointer; // REUSE-TODO: migrate TResourceLoadThread → nextpas.core.async IAsyncTask/TaskGroup when async lane ready — keep impl-internal Classes shim
      end;
  private
...
uses
  Classes, // REUSE-TODO: migrate TResourceLoadThread → nextpas.core.async IAsyncTask/TaskGroup when async lane ready — keep impl-internal Classes shim
  nextpas.core.fs,
  nextpas.core.io.intf;

type
  TResourceLoadThread = class(TThread) // REUSE-TODO: migrate TResourceLoadThread → nextpas.core.async IAsyncTask/TaskGroup when async lane ready — keep impl-internal Classes shim
  private
  ...
  try
    ...
    for I := 0 to High(FItems) do
      ...
      begin
        ...
        LThreads[High(LThreads)] := TThread(FItems[I].Thread);
      end;
  finally
    FLock.Release;
  end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        LOld := TThread(FItems[Idx].Thread);
...
begin
  ...
  try
    Idx := FindIndexLocked(AId);
    if Idx < 0 then Exit;
    LThread := TThread(FItems[Idx].Thread);
    FItems[Idx].Thread := nil;
