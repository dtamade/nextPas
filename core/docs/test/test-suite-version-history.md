...
| `test_mock` | TMock 录制/验证/返回值/参数匹配/typed 返回/ResetAll/CalledInOrder/VerifyInOrder/GetCallHistory | 106 |
...
- **v8.41**: B78 密度 tranche 8 — **mock MatchingCallCount 跨轨 hash 门修复**（string 查询 hash 在 MockStr 域 vs 记录时 typed-kind 域 ArgHash 恒不等 → 提前 Continue 错杀 legacy Args 桥接；RecordCallTyped 后 string CalledWith 必失败=P2 #6 hash 加速引入的真回归；TMockCall 增 StrArgHash 双 hash 修复，typed 轨 kind-strict 不变）+ test_mock.lpr 头注释 case-sensitivity 纠正（宣称 CompareText、实为精确 `=`）+ test_mock 四张契约表 54 行（22 fp）消息全 exact：verify count-message 矩阵 14 行（qualifier 词汇/detail 三态 calls-to·all-recorded·无）+ CalledWith 双轨 16 行（b-bridge-int/bool 锁修复、kind-strict、消息不对称 first-actual vs total-only、xw times 措辞+xw0 never-with、arity-blind）+ call-order 12 行（Before/After 三失败态 exact index、同名退化边、InOrder 贪心/`<start>`）+ When/Returns 分派 12 行（后注册胜出、miss 回退、kind 回退 bool→0、SameText、Reset/ResetAll）；scale 8085→8140（test_mock 1430→1485）
