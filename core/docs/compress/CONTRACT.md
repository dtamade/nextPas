...
**模块路径**：`core/src/nextpas.core.compress*.pas`（10 个有效源；tar 单源已收敛至 `nextpas.core.tar`，`compress.tar` 已删除）
**层级**：L2（门面 `nextpas.core.compress` 依赖 L0-L1: base/io；`compress.tar` 已删除，单源收敛完成，新增请直接 uses `nextpas.core.tar`）
**Owner**：Claude（AI 负责）
**最后更新**：2026-09-02
**版本**：1.3
...
| compress.tar | 已删除（空存根已移除；单源收敛至 `nextpas.core.tar`，`EntryDataSlice/OpenEntryStream` + `bytes.ops` 单源 `Move` + `inline` + `try-finally` 能力由 `nextpas.core.tar` 单源承载） |
| compress.pas | 门面 |
...
| test_compress_tar | Tar 容器回归（`nextpas.core.tar` 单源全量；`compress.tar` 已删除，回归仍绿） |
| **合计** | **6 个测试目录** |
