...
**复核 2026-08-29**：`main b60771353` 基线上复跑 `test_text 33 / redis 12 / pool 21 / mysql 7 heaptrc0` 与 `bench_kv 10 validate 0 allocs`（在册 `129/277/1102 ns`，当前 `123/273/1074 ns` 紧贴在册，`b607` 含 db 锚点 `769c69cbd` + redis Trim 单源化），`make hygiene pass` / `diff-check 0` / `Trim( 2 行 text.utils 单源`，证据见 `benchmarks.md 验证锚点` 与 `{SCRATCH}`。
