...
| **EVIDENCE** | **W1 done 2026-07-26**: audit 逐步实测(22 步)失败 **13→8**(PASS 14/FAIL 8,余者全 rc=1 契约漂移、0 崩溃/rc=2)。路径遗孤清零:①`contract_roadmap` 最大头——9ab2e83fc 删除的 14 份文档(GOAL_TREE/checklist/closeout/cpuinfo/architecture-guide/publicabi/publicabi.stability/quickref/map/backend-truth/architecture-impl/pipeline.quickstart/layering-implementation/intrinsics.neon)零 token 再宿主,其 21 个死函数/死腿删除(逐项论证于模块 docstring+行内注释),x86.sse2 腿重指向 e42ac7416 重命名后的 intrinsics.sse2.pas(3/3 token 命中),canonical 616 从 closeout.md 重定向到 tests docs/interface_implementation_completeness.md,脚本从 FileNotFoundError 崩溃恢复为可运行并暴露 **43 条**活文件 W2 内容漂移;②`intrinsics_experimental_status` 幻影实验项目(fc8f2520a 只带 checker 未带项目)测试腿全删、src AES/SHA fail-close 契约全保留→**GREEN**;③`public_abi_signature` smoke header C 腿删除(载体从未存在),余 12/13 段落匹配、仅 simd_backend_enum sha 漂移(W2);④`wiring_sync` checklist 标记腿删除(W2 余 grouped 正则);⑤前批已修 GREEN:shell_runner(45 checks)/windows_publicabi(13)/riscvv_native/linux_evidence/windows_freeze/cpuinfo_runner。gates:focused **1764**/0(65 suites,26.2s);hygiene pass;`git diff --check` clean。残余 8 失败=既定 backlog:simdgen(W3)、avx512/roadmap 43 条/helper_semantics fragment/sse2_structure/dispatch sha/public_abi enum sha(W2)、cpuinfo_boundary hash-tls(W4)。**W2 done 2026-07-26**: audit 逐步实测(22 步)失败 **8→3**(PASS 19)。基线有意更新六件:①`wiring_sync` 双修——slot 正则允许分组前缀(`aTable.CoreVectors.X`)+`extract_routine_block` 取最后 header 匹配(修 interface forward 声明抽错体的根因)→60/60/60 三方一致 GREEN;②`public_abi_signature` simd_backend_enum sha 更新至 14 后端(e9fe7b0b7 LASX/1124ad877 VSX+MSA 特性提交有意扩展,12/13 段原本已匹配)+overall 派生更新→GREEN;③`dispatch_contract_signature` 表 sha 更新(401540bad Phase19-25 嵌套分组/1fbab101d I8U8/4947a7e32 U32 有意演进)→GREEN;④`avx512_alignment` 契约短语在 base.pas L174-175 被注释换行拆开,checker 子串匹配加空白归一化(检查强度不变)→GREEN;⑤`sse2_structure` raw leaf 腿重指向 intrinsics.sse2.pas(role marker 3/3 在场);backend-truth/intrinsics-disposition/sse2-migration-map 三文档 `git log --all --diff-filter=AD` 全仓零命中(never-landed,fc8f2520a 同族)→对比腿+EXPECTED_*表+解析器删除,src 侧结构/边界检查全保留;旧「root 禁依赖 intrinsics.sse2」检查的目标单元已因 e42ac7416 重命名消亡→检查反转为「必须委派」(fail-close),src 头注释同步改为委派事实→GREEN;⑥`helper_semantics` 84 条扁平 fragment 由脚本按各 routine 真实源码解析分组形态批量改写(83 映射+1 重复,0 无解,`CoreVectors.` 等)→774 checks GREEN。gates:focused **1764**/0(clean rebuild);hygiene pass(清 tools/simdgen pycache);`git diff --check` clean。残余 3 失败=既定 backlog:simdgen 3 DRIFT(W3)、roadmap 活文档内容回填(下一波)、cpuinfo hash-tls 跨 lane token(W4)。**W-roadmap(活文档内容回填)done 2026-07-26**: audit 逐步实测(22 步)失败 **3→2**(PASS 20),roadmap checker 43(W1 后)→**0** PASS。考古定谳:`git log --all -S 'x86_64 backend family'` 全仓唯一命中 fc8f2520a(2026-06-13 单亲 squash)且短语仅存于 checker——被钉内容从未出现在任何已提交文档,配套文档半边随已删除的 codex/core-simd worktree 消亡;故本波=按 checker 规格补齐 partial merge 的文档半边,**零检查放宽**。①README 以 9ab2e83fc^ 旧版(1352 行)为基底、经断言校验脚本 25 组替换重建(1398 行):死链清理、概述+四 topline bullets 换必需行、6×通用 **优化** 注记、Utf8Validate/BitsetPopCount/UTF‑8 矩阵注记、x86/AArch64 当前+规划支持行、LEVEL_1/2/3 行、bench 锚重指现存 backend-bench-20260526-105300、558→canonical 616、Batch 控制面/高层 API 5 段限定化、公开 façade 边界四小节(gather-scatter/F16 future ABI boundary/transpose 双路径/对齐内存 20 token)回填;②api.md 4 处:runtime 绑定语义段、gather/scatter 语义契约(duplicate indices/lane order/nil-base)、canonical runtime 控制面入口置前+dispatch 层标注、14 后端 enum+支持的后端表+sbNEON activation truth+façade 边界;③maintenance.md:已知技术债表(LoongArch/SVE/SVE2 行自旧版 L522 逐字采矿)+façade 边界提醒;④src 措辞对齐:neon/riscvv register.inc 与 public_abi.impl.inc 的 Description 补 default scalar fallback / asm opt-in / experimental opt-in 事实。全部文档契约主张对照 src 核实(runtime canonical 六入口、base.pas 14 后端、alloc saAuto 64/32/16 profile、size+header+alignment 溢出检查、SimdFree(nil) no-op、memutils EArgumentError/EOutOfMemory)。gates:focused **1764**/0(65 suites,27.2s);hygiene pass;`git diff --check` clean。残余 2 失败=simdgen 3 DRIFT(W3)、cpuinfo hash-tls 跨 lane(W4)。**W3(simdgen 同步)done 2026-07-26**: audit 逐步实测(22 步)失败 **2→1**(PASS 21),simdgen --verify DRIFT→**0**(rc=0)。两个债根:①dispatch_slots.json 停在 616 slots(post-json 特性提交从未注册 slot),从活 baseline.inc 重挖 **675** slots(inc 序;59 新增=BatchF64 33+BatchF32 16+CoreVectors 宽 Cmp 10;元数据自 types.inc 字段声明解析+同 section 邻项继承);②生成器无 Phase19 分组模型——新增 `_load_group_fields`(table.inc+types.inc 解析出 group→字段集)+`_resolve_slot_group`(精确匹配→F32/F64 后缀→BatchF32/F64→Mem 前缀→Memory→MemSet→Fill 别名,无解 fail-close),生成 792 行与线上 baseline.inc **byte-identical**(diff 0)。内核模板收编:Add/Mul `fast_path` 入模板(`emit_elementwise_2input_fastpath`:<64 tail 梯/8x YMM 展开/≥1024 prefetchnta 双流预取,401540bad 手工形态);Add 内核 @loop64_prefetch 尾部一条可证明死代码 `jmp @tail32` 删除(未跳转即顺序落入 @tail32;唯一生产 .inc 改动,收敛 Add/Mul 至同一规范形态,focused 全量重验);Clamp NaN-safe 入模板(`_clamp_nan_safe_block`:vcmpunordps 掩码/min 替换/clamp/vblendvps 原 NaN 回填,6835a2aec 形态)→verify-avx2-batch **20/20 OK**。`simdgen_verify_slots` 假绿门歼灭:原 main() 恒 return 0+正则在分组布局下实际匹配数 0——重写为分组感知(复用 `_resolve_slot_group`),scalar fallback 缺失=硬失败 rc=1(**675/675** GREEN),backend batch 覆盖率降信息层(AVX2 115/123、SSE2 119/123 真实数字首次曝光)。gates:focused **1764**/0(65 suites,26.18s,clean rebuild);hygiene pass;`git diff --check` clean。残余 1 失败=cpuinfo hash-tls 跨 lane(W4)。**W4(cpuinfo 跨 lane 边界+key_slot 对账)done 2026-07-26**: audit 逐步实测 **1→0**,**22/22 全绿**(DoD 主体达成,初始 13 失败清零)。cpuinfo_boundary 考古定谳:`git log --all -S` 证明期望 token(`GChaCha20HasAVX2 := HasAVX2`/`HasAVX2 and LCPUInfo.X86.HasBMI2`)全仓仅存于 checker 自身(fc8f2520a 单亲 squash 带入)——hash/tls 消费者迁移半边从未 land,checker 生来即红;且 tls13.chacha20poly1305 已重构为纯转发 shim(实现居 crypto.chacha20poly1305,该单元亦 raw cpuid)。处置(OUT_OF_SCOPE 合规,**零跨 lane 源码改动**):①虚构的「已迁移」断言修正为 `KNOWN_RAW_PROBE_DEBT` 显式债务登记(sha256 pin=2、crypto.chacha20poly1305 pin=2,去注释后计数),pin 双向 fail-close(增=禁止新 raw 探测;减=还债必须更新/退休登记项);②非 CPUID 执行门模式(GenericRaw/raw X86 AVX gate)对债务单元保持全禁;③tls shim 转入空 token 契约=必须保持无 CPU 探测;④static.avx2 本 lane 契约保留;目标态(经 simd.cpuinfo usable view 统一派发)在 docstring 申明。**Needs Review**:建议总控为 hash/crypto lane 排 cpuinfo 迁移卡(去债归属方)。key_slot 对账:check_nonx86_key_slot_audit 为非门诊断(rc=0,不在 22 步),NEON 58/RISCVV 38 findings 与 main 一致、其 truth=测试断言(OUT_OF_SCOPE 禁改),深度对账留待 NEON/RISCVV 后端专项波。gates:checker-only 波(0 生产源码改动),focused 沿用 W3 clean rebuild **1764**/0;hygiene pass;`git diff --check` clean。**W5(audit no-fail-fast)done 2026-07-26**: audit target 改造为逐步全跑+末尾聚合退出(shell step 函数,每步 `[audit] PASS/FAIL <名>` 标记+`failures: N/22` 汇总+`test $fails -eq 0` 收口)——单步损坏不再掩盖后续(M3.4 起因:第一步 simdgen 失败掩盖 13 步同红)。验证:正常 **22 PASS rc=0**;注入 `PYTHON=false` **22 FAIL 全列出+rc=2**(no-fail-fast+聚合非零退出双性质证明)。rtl_isolation 步去重复 `-B`(`PYTHON ?= python3 -B` 已含)。**M3.4 DoD 达成:audit 22/22 GREEN(13→8→3→2→1→0)**;被删检查逐条论证在案(W1 21 死腿、W2 never-landed 文档腿、W4 never-landed 迁移断言→债务登记);focused 1764/0 不漂;hygiene pass |
