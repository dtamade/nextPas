...
| `compressed` | `transform` + `compress.gzip` + `bytes.ops` | L3 薄门面（单缝寄居正名，Registry 白名单过渡，L7 聚合拆分为 decorator，接口层字面量对齐无直接 compress.base 依赖，同 vfs.base 数值对齐纪律）：仅策略（`daAuto/daGzip`、`IsGzipHeaderPred` 4K 头部谓词 `bytes.ops` inline 零拷贝单源、直接复用 `TRANSFORM_HEADER_PEEK` 无本地别名、`VFS_DECOMPRESS_MAX_BYTES` 字面量 32MiB 数值对齐 GZIP_MAX canonical 单源），模板复用 `transform` 单源决策器，32MiB 防 bomb 由 transform 统一承载（泛型/压缩一致，输入/输出双阈值） |
...
| 压缩/加密不进 vfs 内核（ADR 0003） | `vfs` 保持 `STORE` 零拷贝；压缩/加密由 `L3` 装饰器 `CreateTransformingVfs` 通用模板承载（`CreateDecompressingVfs` 为其 gzip 特化薄门面，`CreateDecryptingVfs` 同构可直接复用；`http Content-Encoding` 另选承载面），避免 `L2→L2` 闭环与 `solid block` 随机访问劣化；`GZIP_MAX_DECOMPRESS_BYTES` canonical 单源于 `compress.base`，`vfs` 侧字面量 32MiB 数值对齐（漂移由 source-contract 锁定，无直接 `compress.base` 接口依赖），32MiB 防 bomb 与 `ContentHash=0/ETag` 禁用一致性由 `transform` 统一承载 |
