...
>
> 如果你只想查公开 API，请优先看 `docs/simd/api.md`；如果你只想落地维护，请优先看 `docs/simd/maintenance.md` 与 `docs/simd/roadmap.md`。
...
- 因此，如果你这次只是从入口文档重新接手模块，不要先重开 SIMD 泛审查；先看 `docs/simd/roadmap.md` 与 `docs/simd/maintenance.md`，并把当前状态理解成 `code-green / cross-ready`

### 设计目标

- **性能优化**：在 FPC 缺少稳定 SIMD 内建支持的现实下，以“手写汇编微内核 + 运行时派发 + 标量回退”的方式，为常见热点提供可选加速
- **API 兼容性**：不改变调用方 API 语义；任何平台/构建环境下均可运行；只有当 runtime 把当前 `dispatchable` SIMD backend 绑定为 `active backend` 时才走加速路径，否则保持 `scalar fallback`
- **跨平台支持**：支持 x86_64 backend family（`SSE2 / SSE3 / SSSE3 / SSE4.1 / SSE4.2 / AVX2 / AVX-512`）与 AArch64（default scalar fallback + NEON asm opt-in）架构

### 设计原则

- **接口稳定、低耦合**：对外只暴露语义清晰的函数；不暴露 ISA 细节
- **运行时派发**：初始化时由 runtime/control-plane 从当前 `dispatchable` 集合绑定 `active backend`；支持 `TrySetCurrentBackend` / `ResetCurrentBackendSelection` 显式控制与环境/宏强制降级
- **平滑回退**：`scalar fallback` 永远可用；如果当前没有可绑定的 `dispatchable` SIMD backend，或 `runtime/control-plane` 没有把 `active backend` 绑定到汇编路径，执行仍保持标量语义
...
#### x86_64 架构
- **当前支持**：x86_64 backend family 当前覆盖 `SSE2 / SSE3 / SSSE3 / SSE4.1 / SSE4.2 / AVX2 / AVX-512`；实际 runtime 仍从当前 `dispatchable` 集合里绑定 `active backend`，`AVX-512F` / `POPCNT` 属于 CPU capability 或 backend 内部实现细节，不是独立 façade backend 名称
- **规划支持**：`AVX-512` family 的后续 capability frontier 当前聚焦 `AVX-512VL / AVX-512BW`（部分子集）；它们属于现有 `AVX-512` backend 的子特性扩展，不是新增独立 façade backend 名称

#### AArch64 架构
- **当前支持**：NEON（default scalar fallback；仅在显式启用 NEON asm opt-in 且满足编译器/平台条件时才走原生 AArch64 汇编路径）
- **规划支持**：AArch64 capability / crypto extension frontier 当前聚焦 `CRC32 / AES / PMULL`；更宽 vector frontier 当前观察 `SVE / SVE2`。这两类都属于 capability qualification、opt-in / experimental intrinsics 方向与后续 backend frontier，不是已接纳的稳定 façade backend 名称
...
| LEVEL_1 | SSE2 | NEON asm opt-in（default scalar fallback） | 基线 SIMD 支持 |
| LEVEL_2 | AVX2 | NEON asm opt-in + `CRC32 / AES / PMULL` capability qualification | 128-bit AArch64 SIMD 增强层；只有显式启用 NEON asm 且满足对应 capability / crypto extension 条件时才成立 |
| LEVEL_3 | AVX-512 | `SVE / SVE2` experimental intrinsics frontier | 更宽 vector frontier；当前仍停留在 intrinsics / capability qualification 方向，不是稳定 façade backend 等级 |
...
**当前补充说明（2026-03-11）**：
- 当前稳定 benchmark 证据见 `tests/nextpas.core.simd/logs/backend-bench-20260526-105300/summary.md`。
...
- 2026-03-11 轮 summary 的关键行（历史口径；最新数字以上面链接的 summary 为准）：
...
**优化**：x86_64 会经 façade `dispatch table` 从当前 `dispatchable` 集合里绑定 `active backend`；常见候选包括 SSE2/AVX2，无可派发 SIMD 时继续走 `scalar fallback`。AArch64 默认仍是 scalar fallback，只有显式启用 NEON asm opt-in 时才会走 NEON 快路径
...
**优化**：x86_64 会经 façade `dispatch table` 从当前 `dispatchable` 集合里绑定 `active backend`；当前可落到 SSE2 / AVX2，若 `active backend` 为 AVX-512，则 `Utf8Validate` 继续复用 cloned AVX2 slot；无可派发 SIMD 时回到 `scalar fallback`。AArch64 当前仍走 `scalar fallback`；现阶段即使启用 NEON asm opt-in，也没有独立 `Utf8Validate` NEON slot
...
- AArch64：遵守 AAPCS64；必要时保存/恢复 V8–V15；默认 public 路径是 scalar fallback，NEON asm 仅在显式 opt-in 后参与。
...
- x86_64：
  ...
  - BitsetPopCount：x86_64 会经 façade `dispatch table` 绑定当前 `active backend`；可落到 `SSE2 / AVX2 / AVX-512`，其中 `AVX2 / AVX-512` 内部使用 `POPCNT`，`SSE2` 保留 SWAR 计数；无可派发 SIMD 时回到 `scalar fallback`
  - UTF‑8：`Utf8Validate` 会经 façade `dispatch table` 绑定当前 `active backend`；可落到 `SSE2 / AVX2`，若 `active backend` 为 `AVX-512` 则继续复用 cloned `AVX2` slot；无可派发 SIMD 时回到 `scalar fallback`
...
NEON 后端已于 2026-04 完成 dispatch 落地；canonical 口径为 dispatch_slots_total=616（见 `tests/nextpas.core.simd/docs/interface_implementation_completeness.md`）。这个结论只表示 dispatch coverage，不改变默认 public scalar fallback 和 asm opt-in 契约。

## 公开 façade 边界（gather/scatter、F16/half、transpose、对齐内存）

### Gather/Scatter 与 public ABI wrapper 边界

`VecF32x4Gather` / `VecI32x4Gather` / `VecF32x4Scatter` / `VecI32x4Scatter` 及其 Select 变体属于 SIMD 原语层；它们可经 public facade 调用，但 gather/scatter 家族目前 not part of the current stable public ABI wrapper（public ABI wrapper 只覆盖已冻结的核心原语面）。duplicate indices、lane order、nil-base 等语义契约以 `docs/simd/api.md` 的 gather/scatter 契约小节与 focused tests 为准。
...
- `SimdAlloc` / `SimdRealloc`：内部按 size + header + alignment 预留空间（带溢出检查）；`saAuto` 按当前 active backend 的默认 profile 取对齐值。
...
Batch API 提供对整个数组的批量操作；对齐与尾部元素由实现内部处理，backend 绑定始终经 runtime/control-plane 完成。所有函数通过 façade dispatch table 路由：runtime 从 `GetDispatchableBackendList` 的当前 `dispatchable` 集合中绑定 `active backend`（自动模式绑定 `GetBestDispatchableBackend` 的结果），可用 `GetCurrentBackend` 查询、`TrySetCurrentBackend` 显式指定、`ResetCurrentBackendSelection` 恢复自动选择；无可派发 SIMD 时保持 scalar fallback。非 x86 backend 仍有 opt-in / experimental 边界（NEON asm opt-in；RISC-V V experimental）。
...
高层 API 让外部模块无需手动管理指针即可复用与底层一致的 runtime 路由：仍由 façade `dispatch table` 按当前 `active backend` 派发，无可派发 SIMD 时保持 `scalar fallback`。
...
var p: Pointer;
begin
  p := SimdAlloc(1024);        // `saAuto`：按当前 active backend 的默认 profile 取值 (AVX-512=64B, AVX2=32B, 其余=16B)
...
begin
  ...
  // 运算符（经 dispatch table 按当前 active backend 路由；无可派发 SIMD 时为 scalar fallback）
  C := A + B;
  C := A * 2.5;
...
begin
  ...
  // 链式操作（MulScalar+AddScalar 可被识别为 ArrayLinearF32 单 pass；仍按当前 active backend 路由）
...
- **Contiguous 快路径**: stride=1 时经底层 batch slot 走 façade dispatch table，按当前 active backend 派发；无可派发 SIMD 时保持标量语义
- **Stride fallback**: 非连续内存回退到标量循环（scalar loop）
- **Pipeline 融合**: `MulScalar+AddScalar` 可被识别为 `ArrayLinearF32` 单 pass；是否走 FMA 取决于当前 active backend
- **对齐分配**: Create/Zeros 经 `SimdAlloc`（saAuto）按当前 active backend 的默认 profile 对齐（AVX-512=64B, AVX2=32B, 其余=16B）
