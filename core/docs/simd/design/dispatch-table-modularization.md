...
3. **NEON / RVV 对齐审计（路径级）**：
   ...
   - **槽级覆盖差距是刻意的**（非路径错位）：靠 baseline 继承，不在本 phase 补全 native 实现
     - NEON：有 `CoreVectors`（~332）+ 完整 `Memory`（15/15，Phase 22）+ 完整 `Mask`（20，绑定 portable `SharedMask*`，非 `NEONMask*` 死包装）；**无** Batch* override（刻意继承 scalar → Phase 23）
...
3. Memory 15/15 已由 Phase 22 接管（ASM opt-in）；Batch* 保持 scalar 继承 + source-contract 锁定 → Phase 23
