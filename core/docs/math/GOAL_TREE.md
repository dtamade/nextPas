...
- Transform tests cover `Ortho`, `Perspective`, `LookAt`, `Translate`, `Scale`, `RotateX`,
  ...
  owner-level `LookAt`, `Translate`, and `Scale` finite-guard messages, and direct `Double`-path
  parity for projection/view/model/camera builders.
...
- Tests lock column-major translation in column 3, right-handed `LookAt`, right-handed perspective
  ...
  inputs plus `Translate` / `Scale` axis inputs, invalid projection inputs, and
