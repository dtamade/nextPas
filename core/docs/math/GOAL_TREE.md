...
- Transform tests cover `Ortho`, `Perspective`, `LookAt`, `Translate`, `Scale`, `RotateX`,
  ...
  owner-level `LookAt`, `Translate`, `Scale`, `Rotate*`, and `Camera2D` finite-guard messages, and
...
- Tests lock column-major translation in column 3, right-handed `LookAt`, right-handed perspective
  ...
  inputs plus `Translate` / `Scale` axis inputs, `RotateX/Y/Z` radian inputs, and `Camera2D`
  center/zoom inputs, invalid projection inputs, and `Translate * Rotate * Scale` local
  composition.
