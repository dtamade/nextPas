...
> 最后更新: 2026-06-07 | 本轮证据: 本地 Linux x86_64 focused gates: lockfree 44/44 + debug 44/44 + 8 stress + adjacent atomic 34/34, heaptrc 0
...
| `lockfree` | 无锁 (MPMC/SPSC/MPSC/Stack/Deque) | ✅ 完成/强化中 (44 default + 44 debug + 8 stress, SPSC publish-wake + space-release and MPMC publish-wake timeout progress runtime checks, wait helper stale-epoch guard, SPSC + MPMC batch partial-progress contracts, Close wait/timeout wake contract, MPMC single-slot sequence states, managed-type rejection cleanup, stack/deque query contract, tagged-ptr ABA) |
