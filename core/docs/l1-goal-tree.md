...
| `lockfree` | 无锁 (MPMC/SPSC/MPSC/Stack/Deque) | ✅ 完成/强化中 (43 default + 43 debug + 8 stress, SPSC publish-wake + space-release timeout progress contracts, wait helper stale-epoch guard, SPSC + MPMC batch partial-progress contracts, Close wait/timeout wake contract, MPMC single-slot sequence states, managed-type rejection cleanup, stack/deque query contract, tagged-ptr ABA) |
