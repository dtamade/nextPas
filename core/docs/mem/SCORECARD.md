...
**日期**: 2026-07-19（Era E · E3；`RELEASE=1` 本机一次全 PASS）
...
|----|---------|-------|-----|--------|------|
| SC1 | growing | 12 | — | 83.3 | ~1.8× system |
| SC1 | default_heap | 19 | — | 52.6 | Growing 单例（DefaultHeap） |
| SC1 | system | 21 | — | 47.6 | glibc via System.GetMem |
| SC2 | growing | 24 | 27 | 41.7 | sizes 16..4K |
| SC2 | system | 39 | 39 | 25.6 | |
| SC3 | growing | 108 | — | 9.3 | producer alloc / consumer free |
| SC4 | local_arena | 2 | — | 500 | AllocFast reset+reuse 64B |
| SC5 | growing | 86 | — | 11.6 | peak LiveBytes≈407KB; final≈16KB; delta Released≈501KB / 35 spans |
| SC6 | virtual_arena | 11 | — | 90.9 | 200 units × 4000 mixed nodes; peakUsed=352KB; finalUsed=0 |
| SC7 | local_arena | 92 | 77 | 10.9 | per-request scope（hdr+body+temps） |
| SC7 | system | 246 | 299 | 4.1 | 同负载 GetMem/FreeMem 对照 |
| SC8 | free_sized | 15 | — | 66.7 | `FreeMem(ptr,size)` 优选热 free |
| SC8 | free_unsized | 112 | — | 8.9 | `FreeMem(ptr)` span 扫描；~**7.5×** 更慢 |
| SC8 | try_block_size | — | — | — | 正确性：size-class ≥ 请求 |
| SC9 | hot_heap | 16 | — | 62.5 | DefaultHeap sized path |
| SC9 | plugin_ia | 128 | — | 7.8 | DefaultAllocator vtable path；~**8×** 更慢 |
...
SC7 说明：p99 是 **单次请求** 延迟（ns），不是单次 alloc；LocalArena 相对 system 约 **2.7×** mean / **3.9×** p99（本机 2026-07-19）。

SC8 说明：同 64B 负载对比 sized vs unsized free；本机 ~**7.5×** 税（15 vs 112 ns/op）。`try_block_size` 行是契约/正确性，ns/op 可为 0。

SC9 说明：同 64B 对比热路径 vs 插件面；本机 ~**8×** 税（16 vs 128 ns/op）。教学证据“为什么要双轨而不是处处 IAllocator”。`same_heap` 行锁 S5 同堆互释。

**E3 过程式门面**（`bench_arena_go_rust` B 段，SC1 平坦 200k）：DefaultHeap direct ~14 / process `GetMem` ~18 / System ~13 ns/op（同量级；旧 compare 嵌套 10M + CAS-as-load 曾假报 ~100 ns）。
