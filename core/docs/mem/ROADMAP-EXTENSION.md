...
**目标**: 每线程独享 Arena，零锁热路径，线程结束前由调用方显式归还。
...
- `TThreadArenaManager` — 管理线程 Arena 池
  ...
  - `DrainTLS` — 在线程结束前手动把当前线程 Arena 归还到池
- `TThreadArena` — 轻量 record 包装器 (一个 Pointer 大小)
  - `.Alloc` / `.AllocZeroed` / `.AllocAligned`
  - `.Reset` / `.SaveMark` / `.RestoreToMark`
- 当前实现没有自动 threadvar cleanup hook；线程结束前必须显式调用 `DrainTLS`
- 配置: Arena 容量 (默认 1MB), 最大池大小 (默认 8)
...
**依赖**: mem.intf (IAllocator), mem.arena.intf (IArena)
