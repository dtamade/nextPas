...
| `base.pas` | 区间二分查找原语 |
| `props.pas` | GC / BinaryProperty / GCB / **InCB** |
| `casefold.pas` | 大小写映射 + CaseFold |
| `normalize.pas` | NFC/NFD/NFKC/NFKD + QuickCheck |
| `segment.pas` | UAX#29 + **`GraphemeClusterByteLen` 共享内核** |
...
3. **单一真源**：`GraphemeNext` 与 `NextGraphemeCluster` 边界一致
4. **GB9c**：`InCB=Consonant … Linker … × Consonant` 已实现
...
1. 无 CLDR tailored grapheme
2. Collation 仅 DUCET（无 locale）
3. Word/Sentence/Line 未接官方 BreakTest 全量
