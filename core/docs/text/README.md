...
| `nextpas.core.text.wildmatch`| 通配引擎 owner（`* ? ** []` 含转义，零 SysUtils，bytes.ops 单源） | `WildSegment`, `WildSegmentRange`, `WildSegmentsMatch`, `HasUnescapedSlash` | gitignore/fs.glob/http 路由/tui 过滤共用；git 仅 thin shim |
