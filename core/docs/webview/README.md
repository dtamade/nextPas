...
性能基线（`core/benchmarks/nextpas.core.webview/bench_vfs`，`nextpas.core.bench` 框架，过滤均值 1.22 GB/s 级，S27 基线 → S95 复测无回归）：
...
桥协议基线（`core/benchmarks/nextpas.core.webview/bench_bridge`，过滤均值，S27 基线 → S95 复测无回归）：
