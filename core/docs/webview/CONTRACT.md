...
**最后更新**：2026-08-28
**版本**：1.31（S37—— 容量与 Fail-Fast 完整性抛光：base IsValidWebviewSchemeToken 公共抽取（builder Scheme 早期复用，零重复校验）、Builder FInvokes/FReady 容量倍增预分配（4→2×，零 O(n²) 重分配）+ Scheme/几何 early Fail-Fast（Scheme token 非法、Width/Height/Min/Max 负值及 max<min 在链式期即抛），hygiene/source-contracts 双 pass、vfs 6 + factory 13 门全绿；Production Ready 保持）
**承接**：1.30（S36—— 热点 inline 与早期互斥抛光：Builder 13 个纯赋值链路全量 inline（Title/Size/MinSize/MaxSize/Resizable/StartMaximized/DebugTools/Scheme/DataDirectory/Ephemeral/Kind/InitialUrl/InitialHtml/DevServerUrl/OnReady），DataDirectory/Ephemeral 互斥由构造期提前至 Builder 链式期早抛（Fail Fast）、AddInitScript __npw 早筛保持，hygiene/source-contracts 双 pass、vfs 6 + factory 13 门全绿；Production Ready 保持）
