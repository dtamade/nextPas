...
**最后更新**：2026-08-28
**版本**：1.33（S39—— 几何校验复用收口：base 抽取 CheckWebviewSize/MinSize/MaxSize 三公共校验（builder Size/MinSize/MaxSize 早期复用，零重复校验；CheckWebviewOptions 同源复用）+ CONTRACT 1.33 时效，承接 S38 InitScripts 倍增预分配（4→2×）；hygiene/source-contracts 双 pass、vfs 6 + factory 13 门全绿；Production Ready 保持）
