...
**最后更新**：2026-08-29
**版本**：1.56（S62—— 空闲调度表倍增收口：`gtk FIdleTags` 逐次 `+1`→ `WebviewGrowCapacity(0→4→2×)` 倍增（`GrowIdleTags` inline 与 `FPendingEvals` 同源复用，`PostIdle` 热路径零 `O(n²)`，`DropIdlePendings` 以 `FIdleCount` 精确遍历容量保留）+ 承接 S61 辅助表；hygiene/source-contracts 双 pass、vfs 6 + base 10 + factory 13 + bridge 17 全绿 heaptrc 0；Production Ready 保持）
