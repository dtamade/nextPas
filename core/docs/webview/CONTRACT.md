...
| `nextpas.core.webview.assets` | 私有索引 | 资产前缀单哈希 + Trie 路由索引（`bridge` 单源承载：`WyHash` 单源 + 0.75 负载单表 + L2 `collections.prefixrouter` 稀疏小表 Byte-Trie O(m) 最长前缀 inline 薄转发，`TStringView` 零拷贝 view 哈希/Trie 遍历双源，`VecGrowCapacity` 单源 0→4→2×，`distinctLens` 降序 + Trie 单遍，`Finalize` 全量串/接口 + Trie 递归 Dispose 不丢，已反哺 L2 通用 prefix-router） | S105→S107 Trie sparse |
