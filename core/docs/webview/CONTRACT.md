...
> 纪律：8/8 已反哺/抽离经设计评审落地，现 thin-forward/直复用不自溢（含 `metrics` L2 通用可观测单源 `METRICS_MAX_FRAME_BYTES` 1 MiB + `MetricsOversizedCount` `inline` 零拷贝 + `MetricsExpandedSize/IsOversizedExpanded*` 解析后规范化膨胀水位 `raw+payload+arena` 零拷贝视图 + `factory/probe` 探测注册 `WEBVIEW_PROBES` + `GProbeSnapshot` 热点快照）；详见 Owner 单元及单测 inline/零拷贝基线，业务以 CONTRACT 为准、缺能力先反哺 Owner（`inline/零拷贝` + `Default(T)/Dispose/FreeAndNil` 不丢，守四件套与 L0-L3）。
```
