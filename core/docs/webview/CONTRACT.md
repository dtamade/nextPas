...
**最后更新**：2026-08-28
**版本**：1.34（S40—— 会话与脚本守卫复用收口：base 抽取 CheckWebviewSession/CheckWebviewInitScript 二公共校验（builder DataDirectory/Ephemeral/AddInitScript 早期复用，零重复；CheckWebviewOptions 同源）+ 几何校验 inline 化 + CONTRACT 1.34 时效，承接 S39 几何复用；hygiene/source-contracts 双 pass、vfs 6 + factory 13 门全绿；Production Ready 保持）
