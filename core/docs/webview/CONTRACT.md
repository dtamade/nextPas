...
**最后更新**：2026-09-02
**版本**：1.86（S93—— `bytes.ops` 单源几何下沉收口：`base.WebviewGrowCapacity(0→4→2×)` inline 薄转发复用 `bytes.ops.WebviewGrowCapacityForReuse`→`BytesGrowCapacityWithMin(WithMin 0→4同源*2)` 单源几何（`BYTES_BUILDER_MIN_GROW=64` 同源 `*2` 均摊 `O(1)`，零 `O(n²)`，零私有倍增冗余，全家族 `WebviewGrowCapacity` 三级收敛至 `bytes.ops` 单源，`tls.websocket:115` 等 `Move/FillChar` 零拷贝经 `BytesCopy/BytesZero` 单源薄转发门禁冻结，`red-line 1/2 inline` 门禁脚本冻结，高级感收口）+ 承接 S92 增量冻结复核（全链 `WebviewGrowCapacity(0→4→2×)` 单源 inline 与 `Count` 精确语义持续零退化）
