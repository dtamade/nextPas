...
**最后更新**：2026-08-29
**版本**：1.70（S77—— 家族全量封口审计：全家族 `WebviewGrowCapacity(0→4→2×)` 单源 inline + `Count` 精确语义已闭环（factory `F*Count` + bridge `FEntries/FMounts` + fake `FEvalQueue/FHistory/FRing/GLiveWindows` + gtk `GLiveWindows/GRegisteredSchemeCtxs/FPendingEvals/FIdleTags` + w2 `GLiveList/FPendingEvals/FOnScale` + wk `GLiveList/FPendingEvals/FOnNav*`，`97 Grow inline` 单源复用，4 处非 Grow `SetLength` 均为定容切片/快照非扩容，零遗漏；hygiene/source-contracts 双 pass、vfs 6 + base 10 + factory 13 + bridge 17 + grow 4 全绿 heaptrc 0；Production Ready 保持）+ 承接 S76 命名同构
