...
**最后更新**：2026-08-29
**版本**：1.67（S73—— wk 事件注册表补齐同构：`wk FOnNavStarted/FOnNavFinished/FOnNavFailed/FOnWindowClosed/FOnReady` 五表逐次 `+1`→`WebviewGrowCapacity(0→4→2×)` 倍增（`GrowOn*` inline 五表同源复用，`On*` 三重载以 `Count` 精确存储/遍历，`Close` 以 `FOnWindowClosedCount` 精确触发，零 `O(n²)`，wk 与 `fake/gtk/w2` 事件家族 8 表全量闭环 parity）+ 承接 S72 缩放；hygiene/source-contracts 双 pass、vfs 6 + base 10 + factory 13 + bridge 17 全绿 heaptrc 0；Production Ready 保持）
