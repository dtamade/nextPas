...
| `nextpas.core.webview.live` | 私有索引（L1 薄别名） | 活窗紧凑 Vec 注册表（L1 `bytes.ops.TCompactLiveRegistry<T>` 薄别名单源，4 后端 has-a 复用，inline 零额外调用/零拷贝，`Default(T)` 释放不丢，见折叠层 S105） | S105→S107 已反哺 |
...
| `nextpas.core.webview.mime` | 薄门面（deprecated 薄门面冗余·兼容） | `GuessWebviewMime` 兼容 inline 转发至 L2 `nextpas.core.mime.types`（65 项 O(1) 哈希 128 槽，零分配切片，1-2 探测；与 `http.mime` 同源）；家族内 `vfs/gtk/bridge` 已改直连 L2 单源 `MimeTypeFromPath`，`webview.mime` 仅兼容保留，新代码直接 `uses mime.types`，下一步主版本移除（compat debt 下线，不再参与家族 glob/门面重导出，消除 L3 同层依赖） | S13→S106 deprecated |
...
live/assets/viewmap/pool 为家族内私有索引/池化（不经门面 re-export，可抽模块候选见 §1.2，viewmap/pool 仍家族内私有，未自动外溢，抽取需经设计评审与 Owner 反哺时序）：live 已反哺 L1 `bytes.ops` 薄别名单源但仍家族内私有评估保留（inline 零拷贝，Default(T) 释放不丢，跨家族与 window.live 同构已收敛至单源零重复，复用 bytes.ops 单源）、assets 供 bridge 单哈希+Trie 单源（已反哺 L2 `collections.prefixrouter`）、viewmap 供 gtk O(1) 哈希（L1 `collections.hashmap` 单源复用，仍私有未外溢，抽取待评审与 Owner 时序）、pool 供 gtk dispatcher 双池 Slab（L1 `sync.pool` 单源，仍私有未外溢，抽取待评审与 Owner 时序）；均 inline 零拷贝，`Finalize`/`Dispose`/`FreeAndNil` 释放不丢，见折叠层 S105，跨家族已收敛至单源，性能与稳定性证据齐备，业务以 CONTRACT 为准、缺能力先反哺 Owner，复用 bytes.ops 单源
utils/callbacks 为家族内共享辅助（不经门面 re-export，可抽模块候选显式登记，触发与 Owner 时序见 §1.2，utils/callbacks/live 评估保留，仍家族内私有，未自动外溢，抽取需经设计评审与 Owner 反哺时序）：utils 供 bridge/vfs/gtk 复用路径归一 View 单源（text.view 单源，SliceToStr 单次 SetString+Move 单源，热点 View 零拷贝，评估保留未外溢，迁移经评审不自行外溢，复用 bytes.ops 单源）、callbacks 已反哺 L0 `base.callbacks` 单源落地但家族内保留 thin-forward 评估保留（供 4 后端复用三形态归一 `TProc`/`TProc1<T>`/`TCallbackScaleHandler` inline 薄转发零拷贝闭包，仍私有未外溢，单源治理 window/webview 同构重复，跨家族单源审计通过）；L3 内单向依赖 `base/intf` + L0 `base.callbacks`，守四件套与 L0-L3，inline/零拷贝证据齐备，释放不丢
...
|------|----------|-------------------|--------------|------------|
| `live` | 家族内私有 `webview.live`（紧凑 Vec，评估保留，仍家族内私有，未自动外溢，已薄别名反哺 L1 `bytes.ops.TCompactLiveRegistry<T>` 但保留 thin-forward） | L1 `nextpas.core.bytes.ops` 泛化 Vec | 评估保留：`webview.live.TWebviewLiveRegistry<T>` 薄别名继承 `bytes.ops.TCompactLiveRegistry<T>`（VecGrow/VecRemoveSwap/VecSnapshot/VecTrim inline 零额外调用，0→4→2× 单源 bytes.ops 单源，Default(T) 释放不丢，跨家族与 `window.live` 同构已收敛至单源零重复，仍私有未外溢，抽取需经设计评审与 Owner 反哺时序） | 评估保留，经设计评审，抽取待 Owner 时序；保持 inline 零额外调用与稳定性，释放不丢 |
...
| `viewmap` | 家族内私有 `gtk.viewmap`（指针键哈希，已直接复用 L1 THashMap，仍家族内私有，未自动外溢） | L1 `collections.hashmap` THashMap<Pointer,Pointer> 指针键特化 via @PointerHash | 第二个指针键 O(1) view→window 映射需求 | **已反哺落地但仍私有（S108）**：L1 THashMap<Pointer,Pointer> 直接复用 via @PointerHash (=HashOfPointer→HashMix32) 指针专化 inline 零额外调用，`VecGrowCapacity` 0→4→2× 单源初建 bytes.ops 单源 + 0.75 负载 THashMap 单源，桶迁移/墓碑由 THashMap 单源承载零自建 VIEW_TOMBSTONE 双层循环，FreeAndNil 释放不丢；保持 inline 零 I-Cache 膨胀与稳定性，仍家族内私有，未自动外溢，抽取需经设计评审与 Owner 反哺时序 |
| `pool` | 家族内私有 `gtk.pool`（Slab 双池，已反哺 L1 `sync.pool.SyncPoolTryAcquire/Release` 薄别名，仍家族内私有，未自动外溢） | L1 `sync.pool` 通用 Slab | 评估保留（已薄转发 L1 `sync.pool` 泛型 Slab 单源 inline 零拷贝短临界 <1µs，`webview.live`/`gtk.pool` 双池薄转发复用，溢出 `Dispose` 单所有权不丢，仍私有未外溢，抽取需经设计评审与 Owner 反哺时序） | 评估保留，仍家族内私有，未自动外溢，经设计评审落地；保持 inline 零额外调用与稳定性，释放不丢 |
| `utils` | 家族内共享 `webview.utils`（路径归一，评估保留，仍家族内私有，未自动外溢） | L1 `text.view` 共享路径归一 | 第二个资源路径归一复用（window/vfs/http）且需零拷贝 View 直通 | 评估保留，仍家族内私有，未自动外溢（S52 单源收敛后评估，触发待第二处复用与设计评审，抽取需先反哺 L1 text.view Owner 再 thin-forward，经设计评审与 Owner 反哺时序）；保持 `SliceToStr SetString+Move` 单源 inline 零拷贝、热点 View 零拷贝，`Finalize` 释放不丢 |
| `callbacks` | 家族内共享 `webview.callbacks`（三形态归一，评估保留，仍家族内私有 thin-forward，未自动外溢） | L0 `base.callbacks`（`nextpas.core.base.callbacks`） | 跨家族（window/webview/async）三形态重复且需单源审计 | 评估保留，仍家族内私有 thin-forward，未自动外溢（已反哺落地 L0 `base.callbacks` 单源 inline 零拷贝 `TProc`/`TProc1<T>`/`TCallbackScaleHandler` 薄转发、reference 闭包 Move 零拷贝，但家族内保留 thin-forward 门面，抽取经设计评审与 Owner 反哺时序，跨家族单源审计通过）；保持 inline 零拷贝，释放不丢 |

> 纪律：可抽≠可自溢，viewmap/pool/utils 仍家族内私有，未自动外溢；抽取前必须先反哺目标 Owner（补单测与性能基线 inline/零拷贝，复用 bytes.ops 单源，性能加 inline/零拷贝证据，稳定性资源释放不丢），再由 webview 侧 `uses` 新 Owner 单源；守四件套 `base←intf←impl←facade` 与 L0-L3 单向依赖，抽取经设计评审与 Owner 反哺时序，受控跨模块 slice，业务以 CONTRACT 为准、缺能力先反哺 Owner。
```
