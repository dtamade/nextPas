...
live/assets/viewmap/pool 为家族内私有索引/池化（不经门面 re-export，可抽见 §1.2，仍私有未自溢）：live thin-forward L1 `bytes.ops.TCompactLiveRegistry<T>`（`VecGrow/RemoveSwap/Snapshot/Trim` inline 零额外调用，0→4→2× 单源，`Default(T)` 不丢，跨家族 `window.live` 同源零重复）、assets 单哈希+Trie（已反哺 L2 `collections.prefixrouter`）、viewmap 直复用 L1 `collections.hashmap`（`@PointerHash`）、pool thin-forward L1 `sync.pool`（双池 Slab）；均 `inline/零拷贝` + `Finalize/Dispose/FreeAndNil` 不丢，跨家族收敛至 `bytes.ops` 单源，业务以 CONTRACT 为准、缺能力先反哺 Owner
utils/callbacks 为家族内共享辅助（不经门面 re-export，可抽见 §1.2）：utils 已反哺 L1 `text.view` Owner 单源（`TrimLeftChar`/`StripLeadingSlashView`/`StripLeadingSlash`/`ViewFromPChar`，`SliceToStr`/`CStrLen` 单源，热点 `TStringView` 零拷贝 `inline` 薄转发、String 版 `SliceToStr` 单次 `SetString+Move` CoW 零分配，`Finalize` 不丢）、callbacks thin-forward L0 `base.callbacks`（三形态 `TProc`/`TProc1<T>` inline 零拷贝）；`inline/零拷贝` + `Default(T)` 不丢，守四件套与 L0-L3，跨家族单源审计通过，抽取经设计评审不自溢
...
|------|----------|-------------------|--------------|------------|
| `live` | 私有 `webview.live` · thin-forward `bytes.ops.TCompactLiveRegistry<T>` | L1 `nextpas.core.bytes.ops` 泛化 Vec | 通用 Vec 第二处复用已满足，跨家族 `window.live` 已同源 | 私有候选·已 thin-forward（`VecGrow/RemoveSwap/Snapshot/Trim` inline 零额外调用，0→4→2× 单源，`Default(T)` 不丢，零重复）· 待设计评审定 Owner 时序，`inline/零拷贝` 不丢 |
...
| `viewmap` | 私有 `gtk.viewmap` · 直复用 L1 `THashMap<Pointer,Pointer>` via `@PointerHash` | L1 `collections.hashmap` 指针键特化 | 第二个指针键 O(1) 映射需求 | 私有候选·已直复用（`HashOfPointer→HashMix32` inline 零额外调用，`VecGrowCapacity` 0→4→2× 单源 + 0.75 负载，`FreeAndNil` 不丢，零自建 `VIEW_TOMBSTONE` 双循环）· Owner 反哺时序：L1 `collections.hashmap` 为 Owner，第二处指针键映射出现时经设计评审决定是否特化外溢（补单测+`inline`零拷贝 `HashOfPointer` 基线+`FreeAndNil` 不丢评审），当前薄转发收敛不自溢；触发条件评审：第二处 O(1) 映射 + 单源审计通过（业务以 CONTRACT 为准、缺能力先反哺 Owner） |
| `pool` | 私有 `gtk.pool` · thin-forward L1 `sync.pool` | L1 `sync.pool` 通用 Slab | 通用 Slab 第二处复用 | 私有候选·已 thin-forward（泛型 Slab `SyncPoolTryAcquire/SyncPoolRelease` inline 零拷贝，短临界 <1µs 仅指针压入，双池 Slab 复用，溢出 `Dispose` 单所有权不丢，突发锁内 `VecGrow` 0→4→2× 单源扩容零竞态）· Owner 反哺时序：L1 `sync.pool` 为通用 Slab Owner，第二处 Slab 复用出现时经设计评审反哺单源（补单测+`inline`零拷贝基线+`Dispose/FreeAndNil` 不丢评审），当前 `gtk.pool` 薄转发收敛不自溢；触发条件评审：通用 Slab 第二处复用 + 评审通过（业务以 CONTRACT 为准、缺能力先反哺 Owner） |
...
| `callbacks` | 私有 `webview.callbacks` · thin-forward L0 `base.callbacks` | L0 `base.callbacks` | 跨家族三形态重复需单源审计 | 私有候选·已 thin-forward（`TProc/TProc1<T>/TCallbackScaleHandler` `Callback*MethodToRef/ProcToRef` inline 零拷贝，reference 闭包 `Move` 零拷贝，跨家族审计通过）· Owner 反哺时序：L0 `base.callbacks` 为 Owner，跨家族三形态已收敛至 Owner 单源，待设计评审确认跨家族复用规模后定外溢时序，当前薄转发收敛不自溢；触发条件评审：跨家族三形态重复单源审计通过 + `inline`零拷贝基线 + 评审通过（业务以 CONTRACT 为准、缺能力先反哺 Owner，`Finalize` 不丢） |

> 纪律：可抽≠可自溢，仍私有未自溢；抽取前先反哺 Owner（补单测与性能基线 `inline/零拷贝`，复用 `bytes.ops` 单源，`Default(T)/Dispose/FreeAndNil` 不丢），再由 webview 侧 `uses` 新 Owner 单源；守四件套 `base←intf←impl←facade` 与 L0-L3 单向依赖，经设计评审、受控跨模块 slice，业务以 CONTRACT 为准、缺能力先反哺 Owner。
```
