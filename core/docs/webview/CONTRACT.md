...
- **S106 探针闭环**：`wk.loader` 经 `platform.dl` 真探 `WebKit.framework`/`libobjc`，`inline` 零分配，`platform_dl_release` 释放不丢；真实现复用 L2 `window.cocoa` `IWindow` 组合（`WKWebView` child），见 §1/§10 与 `registry`。
- **S105 基线**：`VecGrowCapacity 0→4→2×` 单源 `inline` 零拷贝，`Default(T)` 释放不丢。
- **考古栈**：1.98←1.13 折叠至 [ROADMAP §3](ROADMAP.md#3-回溯s0-s44-已交付)。

> 单文件 650+ 行属契约完整性保留，入口为本折叠层与 §1 家族布局。
</details>
...
| `nextpas.core.webview.bridge` | 协议 | 桥协议 v1 编解码 + pending 表 + 注入脚本常量（后端无关唯一实现；帧超限计数见 `metrics` 行 `inline` 薄转发） | W1 |
| `nextpas.core.webview.metrics` | 可观测性（L3 thin-forward → L2 `nextpas.core.metrics`） | 帧超限背压计数（`WEBVIEW_MAX_FRAME_BYTES` 1 MiB 常量即契约 + `IsOversized*` `inline` 零拷贝视图 + `NoteOversized`，`Default` 释放不丢） | W1 |
| `nextpas.core.webview.eval` | 共享辅助（L3 Owner 单源） | Eval pending exactly-once Owner（INV-7 单源：`TEvalRegistry = TCompactLiveRegistry<PEvalSlot>` Snapshot `SetLength+Move` `inline` 零拷贝 + `Done` 守卫 + `EvalSettleInline` `try-finally`，复用 `bytes.ops` 单源 `VecGrowCapacity 0→4→2×` / `VecRemoveSwap` O(1)，`Default(T)` 释放不丢） | S* 匠心修复落地 (2026-09-02) |
| `nextpas.core.webview.dispatch` | 共享辅助（L3 Owner 单源） | Idle 调度 Owner（`TIdleTagRegistry = TCompactLiveRegistry<guint>` `VecGrowCapacity 0→4→2×` / `Snapshot` `inline` 零拷贝，复用 `bytes.ops`/`sync.pool` 单源，`Dispose` 释放不丢） | S* 匠心修复落地 (2026-09-02) |
| `nextpas.core.webview.live` | **removed** | 已移除（S108 物理删除；活窗注册收敛至 L1 `bytes.ops.TCompactLiveRegistry<T>` 单源 `inline` 零拷贝 `VecGrowCapacity 0→4→2×`，`Default(T)` 释放不丢） | — |
| `nextpas.core.webview.assets` | 私有索引 | 资产前缀 Trie 路由索引（`WyHash` + `TStringView` 零拷贝 + `collections.prefixrouter` 单源，`Finalize` 释放不丢） | W1 |
...
| `nextpas.core.webview.gtk.viewmap` | 私有索引 | GTK view→window O(1) 哈希（复用 L1 `hashmap` `THashMap<Pointer,Pointer>`，`inline` 零额外调用，`FreeAndNil` 释放不丢） | W1 |
| `nextpas.core.webview.gtk.pool` | 私有池化 | GTK 池化 Slab（Idle/Completion/AssetHolder 三池，复用 `sync.pool` + `bytes.ops` 单源 `inline` 零拷贝，`Dispose` 释放不丢） | W1 |
| `nextpas.core.webview.gtk.shell` | 壳缝 | GTK 窗口壳缝：活窗/scheme 上下文注册、窗口创建/嵌入、`IWindow` 委托（复用 `window.gtk3` 单源 `inline` 零拷贝，`bytes.ops` 单源，短临界 <1µs，释放不丢） | S109 |
| `nextpas.core.webview.gtk.bridge` | 桥缝 | 桥传输缝：script-message/load 信号、Scheme 资产分发、`DispatchFrame`/`Emit`（复用 `bridge`/`utils`/`viewmap`/`mime.types` 单源 `inline` 零拷贝，`GBytes` 零 `Move`） | S109 |
| `nextpas.core.webview.gtk.dispatch` | 调度缝 | 调度缝：Idle/Completion trampoline、`Eval` exactly-once 结算（复用 `pool` Slab + `bytes.ops` 单源，`try-finally` 释放不丢） | S109 |
| `nextpas.core.webview.gtk.win` | **removed** | 已移除（S106 物理删除；窗口壳收口至 L2 `window.gtk3`，`platform_dl_release`/`FreeAndNil` 释放不丢，`inline` 零拷贝） | — |
| `nextpas.core.webview.gtk` | 后端（门面聚合） | Linux 实现：**has-a `IWindow`**（L2）+ WebKit 视图挂载、scheme、idle dispatch、信号桥接（经 `gtk.shell`/`bridge`/`dispatch` 三缝薄转发，`inline` 零拷贝，800 行内聚） | W1→S109 薄门面 |
| `nextpas.core.webview.mime` | **removed** | 已移除（S107 物理删除；能力收敛至 L2 `mime.types` 单源 `inline` 零拷贝） | — |
...
| `nextpas.core.webview.webview2.win` | **removed** | 已移除（S106 物理删除；窗口壳收口至 L2 `window.win32`，释放不丢） | — |
...
| 候选 | Owner 单源 (L层) | 状态 |
|------|------------------|------|
| `live` | L1 `bytes.ops.TCompactLiveRegistry<T>` (`VecGrowCapacity` 0→4→2× / `VecRemoveSwap` O(1) / `Snapshot`/`Trim` inline 零拷贝) | 已物理删除（收敛至 `bytes.ops` 单源 `inline` 零拷贝，`Default(T)` 释放不丢） |
...
| `metrics` | L2 `nextpas.core.metrics` / `nextpas.core.metrics.base` (`METRICS_MAX_FRAME_BYTES` 1 MiB Hard Limit 常量即契约单源 + `MetricsOversizedCount/NoteOversized/IsOversized*` 单调递增 `inline` 零拷贝背压计数 + `MetricsIsOversizedExpanded*`/`MetricsExpandedSize` 解析后规范化膨胀水位 `raw+payload+arena` 零拷贝视图; L3 `webview.metrics` `WEBVIEW_MAX_FRAME_BYTES` + `WebviewMetrics*` `inline` thin-forward alias 零双写, `bridge` `TryDecodeFrame` 解析后膨胀二次校验 + `TryBuildOversizedReject` 薄转发零重复阈值/计数, `bytes.ops` 单源思想零拷贝视图) | 已反哺 L2 thin-forward (2026-09-02) — L2 通用可观测单源已落地（bench/log 正交，计数 plain UInt64 单源收敛），对应 BRIDGE_PROTOCOL §6 阈值 + INV-7 exactly-once 背压可观测闭环 |
| `factory/probe` | L3 `nextpas.core.webview.registry` (`WEBVIEW_PROBES` Probe 单表 + `GProbeSnapshot/GDefaultSnapshot` 快照 O(1) `inline` 复用零重复双检锁) | 已抽离 thin-forward (2026-09-02) — `factory` `WEBVIEW_BACKENDS` 仅 Create 单表零 Probe 耦合，探测/默认 kind 均 `inline` 薄转发至 `registry` 快照复用，热点 `WebviewBackendAvailable/DefaultWebviewKind` 零重复 `TryLoad*` 双检锁/零堆分配，`platform_dl_release` 释放不丢 |
| `eval` | L3 `nextpas.core.webview.eval` (`TEvalRegistry = TCompactLiveRegistry<PEvalSlot>` Snapshot `SetLength+Move` `inline` 零拷贝 + `Done` exactly-once 守卫 + `EvalSettleInline` `try-finally` 释放不丢，复用 `bytes.ops` 单源 `VecGrowCapacity 0→4→2×` / `VecRemoveSwap` O(1)) | 已单源化 thin-forward (2026-09-02) — INV-7 exactly-once 单源 Owner 落地，`gtk` `SettlePendingOnClose` 原锁+快照自管已薄转发至 `eval.EvalSnapshotInline` 单源 `inline` 零拷贝 `Default(T)` 释放不丢；`fake` `FPendingEvals` 直管待后续薄转发，待评审后直复用 |
| `dispatch/idle` | L3 `nextpas.core.webview.dispatch` (`TIdleTagRegistry = TCompactLiveRegistry<guint>` `VecGrowCapacity 0→4→2×` / `Snapshot` `inline` 零拷贝 + `sync.pool` Slab 单源) | 已单源化 thin-forward (2026-09-02) — Idle 回调池单源 Owner 落地，`gtk` `FIdleTags`/`DropIdlePendings` 原自管已薄转发至 `dispatch.IdleTag*Inline` 单源 `inline` 零拷贝 `Dispose` 释放不丢；`gtk.pool` Idle Slab 仍为 L1 `sync.pool` 单源，`fake.dispatcher` 环形 FIFO 私有待后续评估薄转发 |
| `scheme token` | L2 `nextpas.core.validation.IsValidSchemeToken` / `TValidator.SchemeToken` (`[a-z][a-z0-9+.-]*` 表驱动 `L1 text.char IsLower/IsDigit` `CharClassTable` 零分支零拷贝视图 `inline` thin-forward) | 已反哺 L2 thin-forward (2026-09-02) — `webview.validation.IsValidWebviewSchemeToken` `inline` thin-forward 至 L2 通用单源，手写区间漂移已消除（底层 `CharClassTable` 表驱动零分支+零拷贝 `Byte` 视图无分配，去 `inline` 循环体红线二），`TValidator.SchemeToken` 同源复用，稳定性无堆资源释放不丢，守 L0-L3 单向 |

> 纪律：8/8 原候选 + `eval`/`dispatch`/`scheme token` 已反哺/抽离落地（`eval.EvalSnapshotInline` + `dispatch.IdleTag*Inline` 均 `TCompactLiveRegistry` `inline` 零拷贝 `Default(T)/Dispose` 释放不丢，复用 `bytes.ops` 单源；`scheme token` `validation.IsValidSchemeToken` `inline` thin-forward 表驱动 `CharClassTable` 零分支零拷贝 `Byte` 视图无手写区间漂移，去 `inline` 红线二）；现 thin-forward/直复用不自溢（含 `metrics` L2 通用可观测单源 `METRICS_MAX_FRAME_BYTES` 1 MiB `IsOversized*` `inline` 零拷贝 `MetricsOversizedCount` 单调递增 `MetricsExpandedSize` 解析后膨胀水位 `raw+payload+arena` 零拷贝视图 + `factory/probe` `WEBVIEW_PROBES` + `GProbeSnapshot` 快照 + `scheme token` `IsValidSchemeToken` 单源）；`metrics`/`factory/probe`/`scheme token` 已 L2/L3 单源，业务域 §4 STA 亲和 (`BridgeRequireSTA` `inline` 零额外调用) 与 §6 1 MiB 阈值（`WEBVIEW_MAX_FRAME_BYTES` 别名零双写）已收敛，`scheme token` 表驱动零漂移已闭环；详见 Owner 单元及单测 inline/零拷贝基线，业务以 CONTRACT 为准、缺能力先反哺 Owner（`inline/零拷贝` + `Default(T)/Dispose/FreeAndNil` 不丢，守四件套与 L0-L3）。
```
...
- INV-7 用户回调恰好一次触发：Eval 二选一（含 Close 时在途 Eval 统一
  `EWebviewEvalFailed` 收尾）、invoke completion 至多一次、事件每来源每次一条。`eval`/`dispatch` pending exactly-once 单源收口于 L3 `nextpas.core.webview.eval` / `webview.dispatch` Owner（`TCompactLiveRegistry<PEvalSlot>` Snapshot `SetLength+Move` `inline` 零拷贝 + `Done` 守卫 + `EvalSettleInline` `try-finally`，`TIdleTagRegistry` `VecGrowCapacity 0→4→2×` / `VecRemoveSwap` O(1) `inline` 零拷贝，复用 `bytes.ops`/`sync.pool` 单源，`Default(T)/Dispose` 释放不丢，见 §1.2；`gtk` 原 `SettlePendingOnClose` 锁+快照自管已薄转发至 `eval` 单源，`FIdleTags`/`DropIdlePendings` 已薄转发至 `dispatch` 单源）。
  帧超限背压可观测阈值 `WEBVIEW_MAX_FRAME_BYTES` 1 MiB（BRIDGE_PROTOCOL §6）与计数单源收口于 L2 `nextpas.core.metrics` / `nextpas.core.metrics.base` Owner（L3 `webview.metrics` `inline` thin-forward alias 零双写，`bridge` `TryDecodeFrame` 解析前 `IsOversized` 快径 + 解析后 `IsOversizedExpanded*` 水位 `raw+payload+arena` 零拷贝视图二次校验 + `TryBuildOversizedReject` `inline` 薄转发零重复阈值/计数, `bytes.ops` 单源思想零拷贝视图+`Default` 释放不丢，L2 通用可观测单源已落地见 §1.2），业务域 §4 STA 亲和（`BridgeRequireSTA` `inline` 零额外调用）与 §6 1 MiB 阈值复用同单源，bridge/pool 复用符合不变量无需可抽新模块；exactly-once 语义与阈值共同闭环；fake 后端的契约测试逐条冻结此性质。
