...
**最后更新**：2026-08-30
**版本**：1.93（S101—— 热路径零调用：`FindMountByPrefix inline` 探针（`TryResolve` 单挂载外最热路径）+ `CONTRACT 1.92→1.93` 匠心收口；`fpc -vh` 0 hint、`grep TODO/FIXME` 0、`hygiene` 双 pass、13 gates 全绿 heaptrc 0；Production Ready 保持）+ 承接 S100
**承接**：1.92（S100—— 极致收口：`MountHash` 家族 `inline` 探针零额外调用 + 窗口缝 checklist 固化（`gtk.win` 对称冻结，`webview2.win` 18+3 预留 S45 单点可抽）+ `bench` 框架 `1.22GB/s / 3.26µs` 再校准；`fpc -vh` 0 hint、`grep TODO/FIXME` 0、`hygiene` 双 pass、13 gates 全绿 heaptrc 0；Production Ready 保持）+ 承接 S99
**承接**：1.91（S98—— 六维深化：窗口缝对称受控预留（`gtk.win 14 inline → window.gtk3.Raw` 已对称，`webview2.win 22 项权威壳` 保留至 `window.win32.Raw` 抽取 S45，跨平台缝同构可机械迁移）+ 桥注册表阈值 `8→16` 减哈希未命中保持线性快路径优势 + `bench_bridge/bench_vfs` 权威框架校准（`6.17µs` 手写 bench 去噪为框架均值）；`fpc -vh` 0 hint、`grep TODO/FIXME` 0、`hygiene` 双 pass、13 gates 全绿 heaptrc 0；Production Ready 保持）+ 承接 S97
**承接**：1.90（S97—— 跨平台文档与 loader 纪律冻结：`CONTRACT 1.88→1.90` 跨平台诚实收口（`linux GTK 真实现 / windows WebView2 wine可交互 / macOS WK 桩冻结 + Darwin 真实现预留`），`README/ROADMAP/BACKENDS/PARITY` 时效一致，`demo` 404 卡保留，`loader` 唯一触 `platform.dl` 纪律冻结；`fpc -vh` 0 hint、`grep TODO/FIXME` 0、`hygiene` 双 pass、13 gates 全绿 heaptrc 0；Production Ready 保持）+ 承接 S96
**承接**：1.89（S96—— 跨平台缝与稳定性：`factory` 表驱动 `CKindOrder` 循环探测（`W2>Gtk>Wk>Fake` 无 IFDEF，预留 `RegisterBackend` 缝）+ `GLive` 读路径全程 `GSchemeLock`（`LiveWindowForView/LatestLiveWebview/SchemeRequestCb` 拷贝副本 + `IInterface keep` 避免阻塞）+ `SettlePendingOnClose` 单源抽取共用 `HandleNativeDestroy/Close` exactly-once（`Done+ EWebviewEvalFailed('window closed') + heaptrc 0`）；`hygiene/source-contracts` 双 pass、13 gates 全绿；Production Ready 保持）+ 承接 S95
**承接**：1.88（S95—— 完整性冻结：`CheckWebviewOptions` 追加 `DevServerUrl` http(s) 校验（`CheckWebviewDevServerUrl` inline 与 Builder `DevServerUrl` 同源单源）+ Builder 三形态 `RegisterInvoke/RegisterAsyncInvoke/OnReady` method/proc 重载补齐（与 window.Invokes 6 形态同范式闭环）+ `NPW_MAX_FRAME_ID` 单源化（`NPW_BRIDGE_SCRIPT` 由 `BuildBridgeScript` 以 `IntToStr(NPW_MAX_FRAME_ID)` 单源生成，避免 JS 硬编码漂移）；`fpc -vh` 0 hint、`grep TODO/FIXME` 0、`hygiene` 双 pass、13 gates 全绿 heaptrc 0；Production Ready 保持）+ 承接 S94
**承接**：1.87（S94-A—— 性能极致化：`TWebviewInvokeRegistry` n≤8 线性快路径 + n>8 WyHash 开放寻址（`FHash/FHashMask/Rebuild` 单源）+ `TWebviewAssetsImpl` 挂载 Trie/哈希快路径 + `TryDecodeFrame` 零拷（`TStringBuilder` 栈缓冲 + `GDecodeDoc` 池 + `WriteNode` 直写）+ Dispatcher 池化薄转发；`hygiene` 双 pass、13 gates 全绿 heaptrc 0、bench 无回归；Production Ready 保持）+ 承接 S93
**承接**：1.86（S93—— 最强稳定性收口：`GSchemeLock` 单例声明补齐 + `GLiveWindows/GRegisteredSchemeCtxs/GtkLiveWindowCount` 统一 `GSchemeLock` 守卫（多窗并发读半更新/UAF 消除）+ `HandleNativeDestroy` 在途 Eval exactly-once 共用结算（与 Close 同口径防泄漏）+ `vfs TryResolve` 复用 `NormalizeWebviewAssetPath` 单源（手工剥离缺口闭环）+ `wk GetInvokes/GetAssets` 空实现单例（跨后端 AV 消除）；`fpc -vh` 0 hint、`grep TODO/FIXME` 0、`hygiene` 双 pass、13 gates 全绿 heaptrc 0（vfs 6 + wk 3 + factory 13 + bridge 17 等）；Production Ready 保持）+ 承接 S92
...
**承接**：1.43（S49—— 装载探针快照薄转发全 inline（GtkLoadInfo/WebView2LoadInfo/WkLoadInfo 3 路径 inline，零额外调用）+ CONTRACT 1.43 时效，承接 S48 Win32 全 inline；hygiene 双 pass、vfs 6 + base 10 + factory 13 全绿；Production Ready 保持）
