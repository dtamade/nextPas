...
| **复用收口** | `base` 校验 helper 全 `inline`：`IsValidWebviewSchemeToken/CheckInvokeCmd/CheckWebviewSize/Min/Max/Session/InitScript/CheckWebviewEventName` + `NormalizeWebviewAssetPath`/`WebviewGrowCapacity(0→4→2×)` 单源 inline（S52/S56/S59-S77 全家族 Count 精确语义，`bytes.ops` 零拷贝 span 复用，`CheckWebviewOptions` 单源校验，四件套 L0-L3 零重复） |
...
- **门禁**：`test_webview_*` vfs 6 + base 10 + factory 13 + bridge 17 + grow 4 全绿 `heaptrc 0`；`hygiene`；`source-contracts`（INV-4/INV-5）。
