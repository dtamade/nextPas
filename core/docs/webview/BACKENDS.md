...
- **S26 封版→S106 闭环**：`wk.loader` 已引入 `platform.dl` 真探（S106 前为零依赖桩以注释占位 `// Darwin 真实现将经 platform.dl`，现已落地为 `TryDlOpen` 双库探测），`wk.pas` 桩方法以 `{%H-}` 按符号定向抑制未用参数（`Navigate`/`NavigateToString`/`Eval`/`Emit` 四桩，零大段 `{$HINTS OFF}` 屏蔽，hygiene 零 warning 高级感）、`Post` 三重载加 `FClosed` 守卫（与工厂 Close 幂等一致），全量编译 `-vh` 0 hint；**S33 对齐**：本文档此前 `platform_dl_close` 旧述已校正为上述释放不丢最终态。
