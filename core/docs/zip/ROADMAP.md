...
*当前状态*：`1.0.1 @ 1.0.1`（S64—S99 收敛），`VERSION 1.0.1`，`12 门` `10→12`（原子选项透传），`zip_roundtrip 7 式` `all demos ok`，`main eae251a` 已落地（S98→S99 待落），`bench 16 项` 可编译，`` 十六单源（三路泵+双管道+EOCD双+中央循环+Zip64双+载荷定位+解压口令） + AES 零堆栈。
