...
Roadmap: [ROADMAP.md](./ROADMAP.md) — S0—S93 已落地，1.0.1 巡检（`VERSION 1.0.1`，`SECURITY.md` 五模型，12门+16项全绿，`test_zip_fs` 12 项，`bench 16项` 可编译，`Normalize/TryMethod/ResolveWithAes/ParseLocalHeader/GuardPassword/GuardIndex/FindEntry/GuardTotalAdvance/ZipPumpReader/ZipWrapEntryReader` 十单源三路泵+双管道）。
