...
| `ProcessSucceeded(AOut): Boolean` | 非 TimedOut 且 psExited 且 ExitCode=0 |
...
| `ICommand.Output: TProcessOutput` | 同步执行并捕获（含 TimedOut） |
| `ICommand.Status: Integer` | 同步执行，**只返回退出码**（不含 TimedOut；需超时信息用 Output） |
...
- **[INV-7]** ProcessSucceeded ⇔ (not TimedOut) and (Status=psExited) and (ExitCode=0)
...
| 测试目录 | 参考通过数 | 说明 |
|----------|-----------|------|
| test_process | 230 Check | API 全覆盖 + ProcessSucceeded/MustCapture |
...
| **合计** | **4 目录 / 314 项** | 2026-07-19 实测全绿 + 0 leak |
