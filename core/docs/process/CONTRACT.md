...
- **[INV-13]** **管道与 Wait/TryWait**：`IChild` 仍持有 stdout/stderr 时，`Wait` **与** `TryWait`（进程已退出）均走排水路径（`WaitWithOutput`），避免写满死锁与输出丢失。`TakeStdout`/`TakeStderr` 后由调用方排水，再 `Wait`。`Destroy`：尽力 Kill+reap（约 5s 上限），超时 abandon 再 detach，**不保证**零僵尸。
