...
**最后更新**：2026-09-02
**版本**：1.6.0（独立族·bus闭环：`87=31+56`（`31 core=26冻结+3 wav四件套+2 pcm_wav四件套`）+ 56 ext 已抽8独立L2，`nextpas.core.audio.bus` 四件套 `bus.base→bus.intf→bus.impl→bus`独立化 — `module-registry`登记L2 seam（`audio.bus` L2 `focused-runtime`+`source-contract`，8/8, 21/21，`audio`侧去L2→L2直引），`bytes.ops`单源`AudioEnsureCapacity/BytesZero/BytesCopy` + `inline`几何`EnsureScratch/EnsureSnapshotCapacity`零拷贝 + `SimdAddF32`快照（`audio.simd→simd.cpuinfo` Owner 8-wide/4-wide, `vzeroupper`），`IMutex`owner隔离 + `try..finally`释放不丢，`HEAPTRC`零泄漏；`85=29+56→87=31+56`后独立族计数87=31+56已闭环，高级感与Owner边界收敛）
