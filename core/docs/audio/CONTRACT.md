...
**最后更新**：2026-09-02
**版本**：1.5.2（codec 四件套完整 78→85 + prOggOpus 占位；85 provisional = 29 core (26+3) + 56 ext 候选待抽独立 L2（`codec.opus` 四件套 `base←intf←impl←facade` 占位桩，`base` L0 only + `intf` 仅 `IAudioDecoder` 别名 + `impl` 含 `Probe≤4KB/DecodeWhole 1024帧静音桩/OpenStreaming STUB` + `facade` 仅 `type` 别名 + `inline` 转发，守 `bytes.ops` 单源 + `Probe≤4KB` 零分配，`STUB: OpenStreaming` 白名单），23 GUID 不变）
