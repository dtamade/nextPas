...
**模块路径**：`core/src/nextpas.core.audio*.pas`（核心 26 + 扩展 43，共 69 个源文件；核心 26 + 扩展 43 = 69 实盘，56 冻结 + 13 四件套按需补齐：`bank.base/impl • resource.base/impl • event.base/impl • spatial.base/impl` + `playlist.base/intf/impl` 四件套 + `studio.base/studio.pas` 聚合门面；`codec.flac/mp3/vorbis • spatial/event • bank/resource/playlist • studio/bus/bus.base/bus.impl • simd/pcm.simd` 为可抽新模块候选）
**层级**：L2（只依赖 L0–L1；`io`/`fs` 为 L2 显式允许依赖；`bytes.ops` 为字节操作单源，禁止各 codec 自写重复 Move/SetLength）
**Owner**：audio lane
**最后更新**：2026-09-02
**版本**：1.4（四件套完整：56→65→69 文件 +23 GUID 不变；新增 `bank.impl/resource.impl/event.impl/spatial.impl` 四件套闭环，`bank/resource/event/spatial/playlist/studio` 均 `base←intf←impl←facade`；260 tests 含 test_automation）
...
| _— 扩展（43，实盘 69-26，待抽新模块）—_ | — | — |
| `audio.codec.flac` / `.decoder` / `.sse` | FLAC 纯 Pascal 解码，Probe≤4KB `prFlac`，`bytes.cursor` + `bytes.ops` 单源，不引 `ffi/vendor` | impl（候选 → `nextpas.core.audio.codec.flac` 独立模块） |
| `audio.codec.mp3` / `.decoder` / `.sse` | MP3 纯 Pascal 解码，Probe≤4KB `prMp3` | impl（候选 → 独立模块） |
| `audio.codec.vorbis` / `.decoder` / `.sse` | Vorbis/Ogg 解码，Probe `prOggVorbis`，归一 `codec.meta` 标签 | impl（候选 → 独立模块） |
...
- 禁止：任何 `*.ffi/vendor/miniaudio/mpg123/opusfile`（gate `grep -qi "\.ffi|vendor"` 强校验，69 文件全量）；`SyncObjs/Classes/SysUtils` 直引已收敛至 `nextpas.core.sync`/`nextpas.core.bytes.ops`（`device.null/graph/timeline` 已迁移，`game` 经 `sfx` 兼容层；新增代码禁止直引宿主单元，gate `grep -R "uses.*SyncObjs|Classes|SysUtils"`）；`math/trig/scalar` 允许但仅纯函数 `inline` 调用，不引入运行时分配。
...
```bash
bash core/tests/nextpas.core.audio/test_base/check_source_contract.sh  # 69 文件：核心 26 冻结 + 扩展 43 四件套完整校验（含 bank/resource/event/spatial.impl + playlist 四件套 + bus 四件套 + codec 3×3），无 ffi/vendor + 23 GUID(11+12,B前缀bus异形) + Probe≤4KB + 实时纪律 + test_automation
...
- `source-contract`：`check_source_contract.sh` 69 文件（核心 26 冻结 + 扩展 43 四件套完整，含 bank/resource/event/spatial 四件套 + playlist 四件套 + bus 四件套 + codec 3×3 decoder/sse）`无ffi/vendor` + `23 GUID`（11 核心：0001/0002/0010/0011/0020/0021/0030/0040/0041/0042/0043/0050/0060 实 13 枚按域计 + 12 扩展：0051 spatial/0052 event/0053 bank/0054 resource/0070 studio/0071 project/0072 sequencer/0080 playlist/C00001 bus/C00002 mixer + 2 预留；B 前缀 bus 为异形与 A 前缀区分）+ `TAudioEncodeOptions before IAudioDecoder` + `实时纪律（FillRealtime）` + `Probe≤4KB` + 域文件存在性 + `test_automation` gate 存活；实盘 69 文件（56 冻结 + 13 四件套完整：`bank.base/impl/resource.base/impl/event.base/impl/spatial.base/impl/playlist.base/intf/impl/studio.base/studio.pas` 均守 `base←intf←impl←facade`）；扩展候选缺失按审计阈值 FAIL（WARN 仅限 bus.base/impl 豁免注释标注的过渡桩已收敛）
