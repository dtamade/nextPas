...
- **复用与性能**：`pcm.simd` 块转换 4-wide 展开（`PcmConvertBlockS16/S32↔F32`），`SimdAdd/Mul/Peak/SumSquares/Clamp` 硬件化复用于 `mix/timeline/graph/bus/playlist`，`spatial` 4-wide 立体声 `LL/LR` 展开，`dsp.filters` Biquad 热循环缓存化，`base` 校验/填充单真相
