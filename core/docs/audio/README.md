...
- **复用与性能**：`pcm` `PcmRead/WriteS16LE/S32LE` 单源 LE 编解码（与 `S24`/`bank.WriteLE32` 对称），`pcm.simd` 块转换 4-wide，`pcm.PcmConvert` 热点 `S16/S32↔F32` 4-wide 快 path 复用 LE helpers 零分支（`PSingle` 直访），`SimdAdd/Mul/Peak/SumSquares/Clamp` 硬件化复用于 `mix/timeline/graph/bus/playlist`，`mix.ApplyGainRamp` 增量步进替代每样本除法，`spatial/timeline` 立体声 4-wide `LL/LR` 展开对齐，`dsp.filters` Biquad `LBiquads/LBase` 缓存化，`bus` `FScratch` 按需 `SetLength` 自适应，`resample` 线性增量 `PSingle` 直访 + `sinc` Kaiser 预计算，`bank` `WriteLE32/LE32` 对称复用，`sequencer` `Inc/Vel` 零分配缓存 + `FastSin` 2048 wavetable，`mix.PanLawGains` 复用 `AudioPanLawGains`（`CAudioPanLawUnity`），`base` 校验/填充单真相
