...
- 每域门禁：`本域 focused gate + HEAPTRC OK + check_source_contract.sh + hygiene`；全量 `218 tests (16门，含 deprecated test_game 兼容门，当前 36 files，理想态 45 — 9 files 预留 flac/mp3/vorbis/studio/playlist 等由 music888 以 Probe≤4KB 可插拔吸收)` 在 Final 再跑。
...
| **S9 门面+bench+文档** | `audio.pas` + bench + docs | 聚合 | `type` 别名 + `inline` 全量；`bench_pcm_wav` 扩至 `Graph/Timeline/Device/Bank/Resource` `ns/op+MB/s -O2` 10 项；`README` 8 示例对齐；`contract 36文件 16门 218 tests（理想态 45 — 9 files 预留 flac/mp3/vorbis/studio/playlist 等由 music888 以 Probe≤4KB 可插拔吸收）` 接入 CI | F-15/16/43/44 |
...
`218 tests (16门，含 deprecated test_game 兼容门)` 全量绿（`HEAPTRC OK`，36 files + 17 GUID + 实时纪律 + two-phase/snapshot mixing - lock free）已具备 `S0` 起点条件；`bench 10项 -O2` 完整；理想态 45 — 9 files 预留 `flac/mp3/vorbis/studio/playlist` 等由 `music888` 以 `Probe≤4KB` 可插拔吸收（与 DESIGN §9 同词）。
