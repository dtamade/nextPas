...
**最后更新**：2026-09-07（文件数 **104**（92+9+3）：`pool/retry/defense/tail/timeout` 五薄域 + `messages/transports/extensions` 三子facade + `gating`；umbrella 1914 行>800软阈但按五facade节奏分流且纯 re-export 豁免（`design-conventions.md:163` 软阈、13别名+40+ inline薄转发，无循环体）；Range 静态流式诚实化：`CopyFileRange`→`CopyRange(4K, inline/零拷贝)`/`io.Copy`，`bench_server ≥0.80× Go` 仅 keepalive 小响应不覆盖静态域，补无阈值 Go/Rust 回归 residual）
**版本**：3.59
...
```
http.pas                 ← 完整门面（re-export；含产品 middleware 全家桶；umbrella, 1914 行 >800 软阈但属纯聚合——见 §1.1 rhythm 与 `http.pas:1-65` 聚合注释；实现经五子facade委托（minimal/messages/transports/extensions/middlewares）聚合，thin consumers 按五facade节奏分流，用 http.minimal / messages / transports / extensions / middlewares）
...
>
> **门面瘦身（800 行软阈 `design-conventions.md:163`）**：`http.pas` 1914 行属 umbrella 纯 re-export（13 别名+40+ `inline` 薄转发，无循环/路由/SIMD 体，`bytes.ops:25/89` 单源在 owner，热点 out-of-line 在 owner，释放经 `try/finally`/`Close`/`PoolClear`）；本版已改为经五子facade委托（`minimal`/`messages`/`transports`/`extensions`/`middlewares` 分别 ~201/~420/~520/~520/~500 行），不再直连 20+ 叶 owner——按 `design-conventions.md:163` “单单元>800 考虑拆分”属**软阈、内聚可例外**：umbrella 无业务体、仅薄转发，符合内聚豁免；认知负荷已分流（thin 按需 `uses` 子facade，`uses nextpas.core.http` 仅稳定聚合入口）；**可抽子facade候选已全部兑现为五facade节奏，无新增待抽候选**（后续若再增长，优先将超阈帮助面拆至对应子facade，守行为冻结+双绿+`heaptrc 0 unfreed`+`git diff --check`/`make hygiene`）。入口见五选一 guidance。
>
...
> - 性能证据：`core/benchmarks/http` 横向 `bytes.ops` 单源复用后，`HttpIsRetrySafeRequest`/`HttpCookieSiteKey` 纳秒级（`inline` 薄转发 + `bytes.ops` 零拷贝比较）；静态路径为 `http.static:CopyRange(4K, STATIC_COPY_BUF_SIZE)` + `io.Copy` 流式（`inline` 探针 `HttpRangeHasBytesPrefix`/`HttpWeakETagEquals` 零分配、`bytes.ops:CompareMem` 单源，VFS 嵌入 `TEmbeddedSlice/Move` 零拷贝已兑现，文件路径 4K 页对齐缓冲拷贝，**非**内核 `CopyFileRange` 零拷贝）；`make focused FOCUS=core/tests/nextpas.core.http` 全域 <2s，umbrella inline 不增加热点循环。**诚实**：静态/Range 域当前仅 `test_http_static` 等 focused 功能 smoke（`heaptrc 0 unfreed`），**无** Go/Rust 对照固定阈值回归；`bench_server` keepalive `≥0.80× Go` 仅覆盖 keepalive 小响应，**不覆盖静态文件/Range 吞吐**（见 `BENCHMARKS.md:Static file / Range — honest residual`），禁止跨机排行榜宣称；固定回归待 `bench_static` + `compare_go`/`compare_rust` 同 harness 冻结阈值（`ROADMAP.md` 待办）。
>
...
| 性能不变量（诚实） | 静态/Range `inline`/`bytes.ops` 单源+零拷贝：`HttpRangeHasBytesPrefix`/`HttpWeakETagEquals` `inline` 零分配（`bytes.ops:CompareMem` 单源），`CopyRange(4K, STATIC_COPY_BUF_SIZE)` + `io.Copy` 用户态流式（VFS 嵌入切片零拷贝，文件路径 4K 缓冲拷贝，非 `CopyFileRange`），`source-contract` 锁定 `IFile`/`io.Copy` 禁止 `ReadAll` 全量缓冲；当前仅 focused 功能 smoke（`test_http_static` 等，`heaptrc 0 unfreed`），**无** Go/Rust 对照固定阈值回归；`bench_server` keepalive `≥0.80× Go` 仅覆盖 keepalive 小响应，**不覆盖静态文件/Range 吞吐**，静态域不参与跨机排行榜；固定回归待新增 `bench_static` + `compare_go`/`compare_rust` 同 harness 并冻结阈值（见 `ROADMAP.md` + `BENCHMARKS.md:Static file / Range — honest residual`） |
