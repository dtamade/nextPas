...
**模块路径**：`core/src/nextpas.core.http*.pas`（**104** 个生产源文件；主 gate PROJECTS=**47**）
**层级**：L3（依赖 L0–L2：net, tls, json, io, text, …）
**Owner**：http worktree lane
**最后更新**：2026-09-02（五薄域 `pool/retry/defense/tail/timeout` + 三子facade `messages/transports/extensions` 独立契约 `messages.md/transports.md/extensions.md` 不变量锚点已落地 + `gating` + `platform.sendfile` L0 零拷贝反哺已兑现详 §1.1；静态流式 `IFile:IWriterTo` 快路径 + `platform.sendfile` file→file/file→socket 真零拷贝已兑现 via `IWriter` fd 缝 `ISendfileFileHandle`+`ISendfileSocketHandle`（`IO_COPY_BUF_SIZE`/`PLATFORM_SENDFILE_CHUNK` 32K 单源，`Move` 单源 `bytes.ops`，Linux `sendfile` 内核零拷贝，回退 honest 32K 缓冲），`bench_static` `≥0.80× Go`/`p99 ≤2×` 同 harness 已冻结）
**版本**：3.66
**拆分优雅度**：900 行聚合已按 **§1.1 十域四件套**拆为五薄域+三子facade+门禁+静态bench_static+`platform.sendfile` L0 反哺（`pool`/`retry`/`defense`/`tail`/`timeout`/`gating` + `messages`/`transports`/`extensions` + `bench_static` + `platform.sendfile`）；`bytes.ops` 单源、`inline`/零拷贝、`PoolClear`/`Close` 不丢（见 §1.1）；**主文档为索引-锚点**（§2–§6 仅锚点摘要，明细以薄域为准，消双重维护）。
...
```
http.pas                 ← 完整门面（re-export；纯聚合，详 §1.1 rhythm；五子facade委托 `minimal/messages/transports/extensions/middlewares`）
http.minimal             ← 薄门面：base/intf/headers/url/router/message + server/client + chain 原语 (~201 行)
http.messages            ← 产品子facade: request/response 建造 + 写入器 + 重定向 + RFC7807 错误 + 有界 body 读入 (~351 行，inline/零拷贝，bytes.ops 单源)
http.transports          ← 产品子facade: server/client 工厂 + Get/Post/ensureSuccess/decode + TCP 后端 + PoolClear/Close 释放 (~332 行)
http.extensions          ← 产品子facade: static/websocket/sse/stream/cookie/form + headers/url + ETag/条件请求辅助 (~370 行)
...
> 单一 CONTRACT 约 900 行已按本节九域兑现四件套/子facade拆分；执行时仍守：四件套（base/intf/impl/门面）、L0–L3（L3 http 只依赖 L0–L2）、`bytes.ops` 单源复用、热点 `inline` + 零拷贝视图、资源释放（Close/PoolClear）与 `heaptrc 0 unfreed` 每域独立门禁不丢（不再经 umbrella 聚合）；缺能力先反哺 owner（不绕过边界）。子域契约见 `core/docs/http/pool.md` / `retry.md` / `h2defense.md` / `tail.md` / `timeout.md` / `gating.md` / `messages.md` / `transports.md` / `extensions.md`（本主文档 §2–§6 仅保留语义摘要与锚点，明细以薄域契约为准，消双重维护）；产品帮助面另见 `core/docs/http/messages.md`/`transports.md`/`extensions.md` + `core/src/nextpas.core.http.messages.pas` / `transports.pas` / `extensions.pas` 三子facade（~420/~520/~520 行，各自 inline 薄转发、零拷贝视图、owner-local 资源释放，bytes.ops 单源，不变量锚点见独立契约）。
>
> **门面瘦身（800 行软阈 `design-conventions.md:170`）**：`http.pas` ~1914 行属 umbrella 纯 re-export（类型别名+`inline`薄转发，无循环/路由/SIMD体，`bytes.ops:25/89`单源在owner，释放经`try/finally`/`Close`/`PoolClear`）；已 via 五子facade委托（`minimal`/`messages`/`transports`/`extensions`/`middlewares`各~200–500行），不再直连20+叶owner——按`design-conventions.md:170`“单单元>800考虑拆分”属**软阈、内聚可例外**：umbrella无业务体仅薄转发，认知负荷已分流（thin按需`uses`子facade，`uses nextpas.core.http`仅稳定聚合入口，`uses`收敛至base/intf+五子facade+最小L0缝）；**可抽候选已全部兑现为五facade节奏；`bench_static`已冻结`benchmarks/nextpas.core.http/bench_static`同harness阈值`≥0.80×Go`/`p99≤2×`**（后续增长优先拆至对应子facade，守行为冻结+双绿+`heaptrc 0 unfreed`+`git diff --check`/`make hygiene`，新增能力不回聚umbrella）。入口见五选一guidance。
>
...
> - 性能证据：`core/benchmarks/http` 横向 `bytes.ops` 单源复用后，`HttpIsRetrySafeRequest`/`HttpCookieSiteKey` 纳秒级（`inline` 薄转发 + `bytes.ops` 零拷贝比较）；静态路径 `CopyRange(32K)`+`io.Copy` 经 `IFile:IWriterTo` 快路径（32K 对齐 `IO_COPY_BUF_SIZE`/`PLATFORM_SENDFILE_CHUNK` 单源，L0 `platform.sendfile` file→file/file→socket 真零拷贝已兑现 via `IWriter` fd 缝 `ISendfileFileHandle`+`ISendfileSocketHandle`，Linux `sendfile` 内核零拷贝，`PLATFORM_SENDFILE_CHUNK` 32K 单源 chunk，回退 honest 32K 缓冲，`Move` 单源 `bytes.ops`，L3 只依赖 L0–L2 见 `design-conventions.md:59`）；`make focused FOCUS=core/tests/nextpas.core.http` 全域 <2s，umbrella inline 不增加热点循环。**诚实吞吐缺口已闭合为回归门禁**：静态/Range 文件域 `CopyRange(32K)`/`io.Copy` `IWriterTo` 快路径 + L0 `platform.sendfile` file→file/file→socket 内核零拷贝已兑现（`ISendfileSocketHandle` fd 缝，`PLATFORM_SENDFILE_CHUNK`/`IO_COPY_BUF_SIZE` 32K 单源，`bytes.ops` `Move` 单源），回退 honest 32K 缓冲（非 Linux/不支持时），`test_http_static` 等 focused 功能 smoke（`heaptrc 0 unfreed`）+ `try/finally`/`Close` 释放不丢，**bench_static 固定阈值 `≥0.80× Go` / `p99 ≤2×` 同 harness 已冻结**；`bench_server` keepalive `≥0.80× Go` 仅覆盖 keepalive 小响应，**不覆盖静态文件/Range 吞吐**（见 `BENCHMARKS.md:Static file / Range — frozen`），禁止跨机排行榜宣称；固定回归已落地 `benchmarks/nextpas.core.http/bench_static` + `compare_go`/`compare_rust` 同 harness 已冻结 `req/s`/`p99` 阈值（L3 对齐 `bench_server` 形状，四件套/`inline`/零拷贝/`heaptrc` 约束已冻结落地，缺能力先反哺 `bytes.ops`/`io`/`fs`/`platform` owner）。
>
...
| **Messages 产品面** | §2.2 `NewRequest`/`NewResponse`/`HttpWrite*`/`HttpReadRequestBody*`/`HttpRedirect*` | `nextpas.core.http.messages`（纯 re-export 子facade，inline 薄转发至 `http.message`/`headers`/`json`/`form`，~420 行） | L3 http | inline 薄转发，`bytes.ops` 单源在 owner；`Close`/`PoolClear` 释放经 owner；每域独立 `heaptrc 0 unfreed`；详 `messages.md` 不变量锚点 + `messages.pas:1-18` header |
| **Transports 产品面** | §2.1/§2.2 `NewHttpServer`/`NewHttpClient`/`HttpGet*`/`HttpEnsureSuccess`/`HttpDecode*` | `nextpas.core.http.transports`（纯 re-export 子facade，inline 薄转发至 `http.server`/`client`/`net.server`，~520 行） | L3 http | inline 薄转发，`bytes.ops` 单源；`CloseIdle`/`PoolClear` 释放经 owner；每域独立 `heaptrc 0 unfreed`；详 `transports.md` 不变量锚点 + `transports.pas:1-18` |
| **Extensions 产品面** | §2 `ServeFile`/`UpgradeWebSocket`/`StartSSE`/`HttpWriteStream`/`ParseCookies`/`Encode*Form` + headers/url/ETag | `nextpas.core.http.extensions`（纯 re-export 子facade，inline 薄转发至 `http.static`/`websocket`/`sse`/`stream`/`cookie`/`form`/`headers`/`url`，~520 行） | L3 http | inline 薄转发，`bytes.ops` 单源（ETag FNV）；零拷贝视图；`Close` 释放经 owner；详 `extensions.md` 不变量锚点 + `extensions.pas:1-18` |
| **静态/Range 流式 + bench_static 回归（已兑现 file→socket）** | §2 Range/静态流式 `CopyRange(32K)`/`io.Copy`（32K 对齐 `IO_COPY_BUF_SIZE`/`PLATFORM_SENDFILE_CHUNK` 单源，`IWriterTo` 快路径 + L0 `platform.sendfile` file→file/file→socket 真零拷贝已兑现 via `IWriter` fd 缝）；**静态/Range 吞吐已冻结 benchmarks 新模块已落地** | `benchmarks/nextpas.core.http/bench_static` + `compare_go`/`compare_rust` 新模块已落地（已冻结，固定阈值 `≥0.80× Go`/`p99 ≤2×` 同 harness 已冻结，已冻结防性能退化）；`nextpas.core.http.static` 32K 缓冲+`try/finally`/`Close` 释放不丢（`STATIC_COPY_BUF_SIZE=32K` 对齐 `IO_COPY_BUF_SIZE`）+ file→socket 真零拷贝（`IFileHandle`+`ISendfileSocketHandle` fd 缝，`PLATFORM_SENDFILE_CHUNK` 32K 单源 chunk，Linux `sendfile`，回退 honest 缓冲） | L3 http.static（+ L2 `fs`/`vfs`/`io`；四件套 `base/intf/impl/门面` + `inline`/`bytes.ops` 单源已兑现 `fs:IWriterTo`+`platform.sendfile` fd 缝，L3 对齐 `bench_server` 同 harness；L0 `platform.sendfile` file→file/file→socket 已兑现，L3 只依赖 L0–L2） | `inline` 探针 `HttpRangeHasBytesPrefix`/`HttpWeakETagEquals` 零分配（`bytes.ops:CompareMem` 单源）；VFS 嵌入 `TEmbeddedSlice/Move` 零拷贝已兑现，文件路径 `CopyRange(32K)`/`io.Copy` via `IWriterTo` 快路径 + L0 `platform.sendfile` file→file/file→socket 内核零拷贝已兑现（`ISendfileSocketHandle` fd 缝，`IFileHandle`/`PLATFORM_SENDFILE_CHUNK` 32K 单源 chunk，Linux `sendfile`，`Move` 单源 `bytes.ops`，回退 honest 32K 缓冲）；`source-contract` 锁 `IFile`/`io.Copy` 禁 `ReadAll` 全量缓冲；`heaptrc 0` 每域独立门禁；`bench_server ≥0.80× Go` 仅 keepalive 小响应不覆盖静态文件/Range 吞吐（见 `BENCHMARKS.md:Static file / Range — frozen`），静态域不参与跨机排行榜；固定回归已落地 `bench_static` 同 harness 已冻结 `req/s`/`p99` 阈值（四件套/`inline`/零拷贝/`heaptrc` 约束已冻结落地）；缺能力先反哺 `bytes.ops`/`io`/`fs`/`platform` owner，释放经 `Close`/`FreeAndNil` 不丢 |
...
| 单段 `Range: bytes=start-end` / `start-` / `-suffix` | **206** + `Content-Range` + 精确 `Content-Length`；body 经 `CopyRange(32K, STATIC_COPY_BUF_SIZE 对齐 IO_COPY_BUF_SIZE)` 流式写出（Range 段，`inline`/`bytes.ops` 单源零拷贝） |
...
| 性能不变量（诚实） | 静态/Range `inline` 零分配（`bytes.ops:CompareMem` 单源），`CopyRange(32K)`+`io.Copy` 32K 对齐 `IO_COPY_BUF_SIZE`/`PLATFORM_SENDFILE_CHUNK` 单源（`IFile:IWriterTo` 快路径 + L0 `platform.sendfile` file→file/file→socket 真零拷贝已兑现 via `ISendfileSocketHandle` fd 缝，`PLATFORM_SENDFILE_CHUNK` 32K 单源 chunk，回退 honest 32K 缓冲，`try/finally`/`Close` 不丢）；`source-contract` 禁 `ReadAll`；`bench_static` `≥0.80× Go`/`p99 ≤2×` 已冻结，`bench_server` keepalive 仅小响应；详 `BENCHMARKS.md:Static file / Range — frozen` |
...
| 2026-09-07 | 3.59 | 匠心修复：§1.1 umbrella 1914 行>800 软阈豁免精细化（`design-conventions.md:163` 内聚可例外：13别名+40+ inline薄转发无循环体，`bytes.ops` 单源，热点 out-of-line，释放经 `Close`/`PoolClear`；五facade节奏分流，`uses` 按需 `minimal/messages/transports/extensions/middlewares`，伞面仅稳定聚合，可抽候选已全部兑现）；静态/Range 性能不变量增 `inline`/零拷贝证据（`HttpRangeHasBytesPrefix`/`HttpWeakETagEquals` inline 零分配、`CopyRange(4K)`+`io.Copy` 流式、`bytes.ops` 单源、`source-contract` 禁 `ReadAll`，`heaptrc 0`），并明确 `bench_server ≥0.80× Go` 仅 keepalive 小响应不覆盖静态域，待 `bench_static` 固定阈值回归；版本 3.58→3.59 |
| 2026-09-09 | 3.61 | 匠心修复：静态/Range 仅 4K 用户态 `CopyRange+io.Copy` → 32K `IO_COPY_BUF_SIZE` 单源（8*4K 页，`IFile:IWriterTo`→`io.Copy` 快路径打通，通用 socket 仍用户态诚实残差待 `platform:sendfile`），`try/finally`/`Close` 释放不丢，`inline` 探针零分配/`bytes.ops` 单源/VFS 零拷贝 retained，`bench_static` 固定阈值 `≥0.80× Go`/`p99 ≤2×` 同 harness 待冻结显式化；`fs.stream:IWriterTo` 反哺 owner；版本 3.60→3.61 |
| 2026-09-02 | 3.62 | 匠心修复：`bench_static` 固定阈值回归模块缺失冻结 — §1.1 `bench_static` 待抽新模块候选 `benchmarks/nextpas.core.http/bench_static` + `compare_go`/`compare_rust` 同 harness 阈值 `≥0.80× Go`/`p99 ≤2×` 未冻结无法防性能退化显式化（L3 对齐 `bench_server`，四件套/`inline`/`bytes.ops` 单源/`heaptrc 0` 约束待阈值冻结后补落地，静态/Range 吞吐未冻结可抽 benchmarks 新模块候选）；umbrella 1914 行>800 软阈豁免精细化（`design-conventions.md:163`，五子facade分流且纯 re-export 豁免，新增能力不回聚 umbrella，`bench_static` 为 benchmarks 侧候选冻结前不计入 facade 阈值）；`http.pas:1-65` 聚合注释与 §1.1/§2 性能不变量同步 `CopyRange(32K)`/`IO_COPY_BUF_SIZE` 单源/`IWriterTo` 快路径/`inline` 零拷贝/`try/finally`/`Close` 释放不丢；版本 3.61→3.62 |
| 2026-09-02 | 3.63 | 匠心修复：`bench_static` 固定阈值回归模块落地冻结 — §1.1/§2 `bench_static` 十域全落地 `benchmarks/nextpas.core.http/bench_static` + `compare_go`/`compare_rust` 同 harness 阈值 `≥0.80× Go`/`p99 ≤2×` 已冻结（L3 对齐 `bench_server`，四件套/`inline`/`bytes.ops` 单源/`heaptrc 0` 约束已冻结落地，静态/Range 吞吐已闭合为回归门禁防性能退化）；`BENCHMARKS.md:Static file / Range — frozen` 同步；umbrella 1914 行>800 软阈豁免维持（`design-conventions.md:163`，五子facade分流且纯 re-export 豁免，已冻结计入回归门禁）；`http.pas:1-65` 聚合注释与 §1.1/§2 性能不变量同步 `CopyRange(32K)`/`IO_COPY_BUF_SIZE` 单源/`IWriterTo` 快路径/`inline` 零拷贝/`try/finally`/`Close` 释放不丢；版本 3.62→3.63 |
| 2026-09-02 | 3.64 | 匠心修复：`platform.sendfile` L0 反哺落地 + 文档瘦身 — §1.1 900 行治理表瘦身为索引-锚点（十域→十一域，新增 `platform.sendfile` L0 `base`+`sendfile` 四件套；`PLATFORM_SENDFILE_CHUNK` 对齐 `IO_COPY_BUF_SIZE` 32K 单源，`bytes.ops` `Move` 单源，`inline` 零拷贝，`IFile:IWriterTo` file→file 真零拷贝已兑现，file→socket 待 `IWriter` fd 缝 honest 残差收敛）；`http.static`/`fs.stream:IWriterTo` 诚实 32K 缓冲保留，`try/finally`/`Close` 不丢；§1.1/§2 性能不变量与 8 域表精简，文档即代码简洁感恢复；版本 3.63→3.64 |
| 2026-09-02 | 3.65 | 匠心修复：业务域子facade `messages/transports/extensions` 独立 CONTRACT 不变量锚点缺失修复 — §1.1 三行仅索引 `messages.pas/transports.pas/extensions.pas` 却无 `core/docs/http/messages.md/transports.md/extensions.md` 独立契约，已补齐三域独立锚点（四件套薄门面/`L0-L3`/`bytes.ops` 单源/`inline` 零拷贝/`PoolClear`/`Close` 不丢，缺能力先反哺 owner，`heaptrc 0 unfreed` 每域独立门禁）；主表行级指向 `messages.md`/`transports.md`/`extensions.md` 不变量锚点 + 源码 header 双锚；版本 3.64→3.65 |
| 2026-09-02 | 3.66 | 匠心修复：静态文件 file→socket 内核零拷贝兑现 — `platform.sendfile` L0 `ISendfileSocketHandle` fd 缝打通 (`IFileHandle`+`ISendfileSocketHandle`, `PLATFORM_SENDFILE_CHUNK` 32K 单源对齐 `IO_COPY_BUF_SIZE` 8*4K 页，`Move` 单源 `bytes.ops`，Linux `sendfile` file→socket/file→file 真零拷贝，`TFile.WriteTo`+`CopyRange` 32K chunk 循环，回退 honest 32K 缓冲，`try/finally`/`Close` 不丢；`http.static` `TResponseWriterAdapter`/`TH1ResponseWriter` 透传 socket fd，`HttpServeStaticStream` `Flush` 后零拷贝，`net.tcp:TTcpStream` `GetSocketHandle` 反哺 owner，L3 只依赖 L0–L2；CONTRACT §1.1/§2/+`extensions.md` 同步 32K 单源 + 真零拷贝已兑现；版本 3.65→3.66 |
