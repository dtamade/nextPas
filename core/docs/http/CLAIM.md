...
| Multi-OS HTTP host path | **Smoke only** | `test_http_threaded_host` + `test_http_iocp_wine`（IOCP wire）+ `test_http_iocp_facade_wine`（M-1：产品 `THttpServer`+`tsbIocp` GET/keep-alive；Windows host 真用例）via `scripts/http-host-ci-matrix.sh`（Linux/macOS/Windows/FreeBSD CI）；**非** scale-ready / **非** TLS-over-Windows |
...
| Windows IOCP server | **W2-1..W2-3b landed** — `net.server.iocp` completion 驱动 recv/send/deadline-wake 数据路径（零字节 recv readiness 桥 + server 自有 GQCS 循环 + writable waiter timeout 重试 + 有限 `WakeDeadline` 经 `TryCancelByContext` 取消唤醒；生产 H1 session 走完成路径）；**real-Windows host 证据**：`http.iocp_wire` 6 用例 + heaptrc 0 unfreed（windows-latest via `http-host-ci-matrix.sh`，2026-07-26 run 30196530589）；含 16MB backpressure（真机部分写语义验证）与 idle deadline wake（真机 437ms，`CancelIoEx` 语义一致）；**产品 facade 端到端（M-1）**：`test_http_iocp_facade_wine` = full facade `THttpServer`+`tsbIocp` GET/keep-alive（Wine 3 用例 + 0 unfreed；**真机证据**：windows-latest PASS `http.iocp_facade` 3 用例 + heaptrc 干净，2026-07-26 Core CI run 30200508397——经 M-3 修复 `-Sc` 宿主依赖编译失败后回填）；**禁止** scale claim（wire smoke ≠ scale） |
