...
- 本轮继续补 raw-wire malformed chunk framing proof：terminal `0` chunk 无 extension、最终空 trailer section 只收到单个 `CR` 后 EOF 的子类，现在也已在 parser/server/security 三层有 focused 证据，输入 `...0\r\n\r` 会稳定被拒绝
- 这轮没有新增生产修复；当前 parser/H1 transport 已能安全拒绝该类 terminal chunk ending CR 截断，本轮主要是把 current truth 锁进回归
...
- 这轮没有新增生产修复；当前 parser/H1 transport 已能安全拒绝该类 terminal chunk extension 截断，本轮主要是把 current truth 锁进回归
- 本轮继续补 raw-wire malformed chunk framing proof：chunk-extension line 的 EOF 截断，现在也已在 parser/server/security 三层有 focused 证据，且把 `...5;sig=abc` 与 `...5;sig=abc\r` 两个相邻子类都单独锁实
- 这轮没有新增生产修复；当前 parser/H1 transport 已能安全拒绝该类 chunk-extension line 截断，本轮主要是把 current truth 锁进回归
- 本轮继续补 raw-wire malformed chunk framing proof：chunk-data 后行尾 CRLF 的 EOF 截断，现在也已在 parser/server/security 三层有 focused 证据，且把 `...hello` 与 `...hello\r` 两个相邻子类都单独锁实
- 这轮没有新增生产修复；当前 parser/H1 transport 已能安全拒绝该类 chunk-data line-ending 截断，本轮主要是把 current truth 锁进回归
...
- `impl.h1.parser` 现在也有 `chunk-extension line EOF truncation` focused proof。
- `impl.h1.parser` 现在也有 `chunk-size line EOF truncation` focused proof。
- `impl.h1.parser` 现在也有 `terminal chunk ending CR EOF truncation` focused proof。
...
- `impl.h1.parser` 现在也有 `truncated trailer section CR EOF truncation` focused proof。
...
- `THttpServer` 现在也有 `terminal chunk ending CR EOF truncation` focused proof：peer half-close 后返回显式 `400`，且不进入 handler。
...
- `THttpServer` 现在也有 `truncated trailer section CR EOF truncation` focused proof：peer half-close 后返回显式 `400`，且不进入 handler。
...
- `test_http_security` 现在也有 `terminal chunk ending CR EOF truncation` 的 raw-wire explicit `400` proof。
...
- `test_http_security` 现在也有 `truncated trailer section CR EOF truncation` 的 raw-wire explicit `400` proof。
...
- 当前 trailer 侧最自然的下一个相邻子类，是 trailer field 行本体只收到字段内容前缀或字段行尾半截的更细粒度 EOF 场景；也可以转去 keep-alive request-tail 契约决策。
