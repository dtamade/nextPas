...
| Windows residual | **IOCP 回调错误码语义漂移**（2026-08-11 审计）：`io.reactor.iocp` 的 `IocpFail` 等 3 处传错点回传 `-Windows错误码`，而 §2 声称回调 deliver `-errno`（epoll/io_uring 后端语义）——消费者跨 host 分支会读错错误码。修复补丁（`IocpMapOsError` 全映射表 + 3 处接线）已草拟并存档 `build/stash-archive/`，**待 Windows 交叉编译环境验证后落地**（当前 Linux 无 ppcrossx64，不吸收未验证代码） |
