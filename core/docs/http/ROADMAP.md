...
| **R2-x residual** STRUCT 2.4b | **done (lane)** | `TH1FastRequestSnapshot` / body reader → `impl.h1.fast`；`NewH1FastRequestSnapshot`；`impl.h1` ~1673 | h1fast 32/0 + contract 35/0 + server |

```text
──── 当前 ────
NEXT = path-limited Landing residual STRUCT 2.4b（findings 不进 main；H3 Blocked）
```
