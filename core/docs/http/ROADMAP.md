...
**Updated**: 2026-07-26（Era P h2 DoS defense：rapid-reset + control-frame flood + CONTINUATION flood 连接级升级 + doc-truth；P-1..P-4 landed；NEXT=P-band 收口/HPACK bomb 评估）
...
| **NEXT** | **P-band 收口**——Era P（h2 DoS defense）P-1/P-2/P-3/P-4 已 landed（rapid-reset + control-frame flood + CONTINUATION flood 连接级升级）；剩余 DoS 向量评估（HPACK bomb / decompression 放大）；改方向先改本行 + §11 changelog |
