...
| Windows residual | **已清零**（2026-08-11）：IOCP 回调错误码语义漂移修复落地——`io.reactor.iocp` 新增 `IocpMapOsError` 全映射表（ERROR_*/WSAE* → errno，`IocpFail` 内 `SetLastError` 保留原始码），`IocpFail`/completion dispatch/`AsyncClose` **4 处**传错点统一交付 `-errno`（epoll/io_uring 契约；存档补丁 3 处 + 审计补第 4 处）。TDD：Wine 下 connect-refused RED `-1225` → GREEN `-111`（`test_reactor_iocp_wine` +1 用例，10/10 + 0 unfreed）；consumer audit 零行为变化（net.server.iocp 按符号、io.poller 透传）；Win64 交叉编译 210K 行通过 + `test_poller` Linux 回归绿 |
...
| 日期 | 变更 |
|------|------|
| 2026-08-11 | **M-band doc-truth + 跨 host 契约修复**（Windows residual 清零）：`io.reactor.iocp` 回调错误码从 `-Windows错误码` 对齐为 `-errno`（epoll/io_uring 契约）——`IocpMapOsError` 全映射表 + 4 处接线（存档补丁 3 处 + AsyncClose 审计补第 4 处）；TDD RED→GREEN（Wine connect-refused `-1225`→`-111`），`test_reactor_iocp_wine` 10/10 + 0 unfreed，consumer audit 零行为变化，Win64 交叉编译 + Linux `test_poller` 双端绿 |
| 2026-07-26 | **M-band heaptrc residual 清零**（http concurrent stress 的 ~15% 间歇泄漏根因）：8 线程共享 1 个 `IHttpClient` 时，连接池 host key 字符串 ~15% 概率泄漏（26 字节块）。逆推 FPC 64 位 AnsiString 块大小（`Len + SizeOf(TAnsiRec)=16 + 1`）锁定为 9 字符串，marker 判别法（临时给 `CanonicalPoolHostKey` 加 8 字符 → 块 26→34）坐实是池 host key。**根因非 http**：`platform_thread_create` 用裸 `pthread_create`/`CreateThread` 绕过 FPC `BeginThread`，从不置 `IsMultiThread:=True`（全仓 core/src 零处设置）→ FPC 对 managed 类型用**非原子** refcount → 真实并发下共享字符串的锁外 decref（调用方局部）与锁内 decref（池 RemoveAt）撞车丢递减。判别数据：per-thread-client 0/40 vs shared-client ~15%（独占不漏、共享才漏 = refcount 竞争特征）。修：两处 `platform_thread_create` 入口置 `IsMultiThread:=True`（`BeginThread` 同序）。附带 http 侧：`FPending` transport 字段 → 按请求 `var APending` 局部（共享 scratch 被并发 RoundTrip 覆写是独立逻辑竞争，违反 CONTRACT「同 transport 多线程各自 RoundTrip」）+ stress `StartServer` 就绪竞争改轮询真实端口。stress 泄漏 **0/80**；`test_platform_thread` 33/0、`test_http_server` 136/0、`test_http_threaded_host` 2/0 全 0 unfreed。**platform.thread 为跨 lane 进程级改动，main 收敛需 platform owner review** |
