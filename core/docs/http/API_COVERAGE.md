...
- `http.base` 现在也有 `HTTP_STATUS_NOT_IMPLEMENTED = 501 / "Not Implemented"` 的 focused proof。
...
- `TH1ResponseWriter` 边界覆盖现在包括预设 `Transfer-Encoding`、显式 `Content-Length` flush 路径、`204/304` no-body status 不注入 chunked、以及 no-body / chunked-finalized 两条拒绝继续写入路径。
...
| `IHttpResponseWriter` / `TH1ResponseWriter`    | write status, headers, body, flush, chunked default, explicit no-body status (`1xx/204/304`) does not auto-inject chunked framing and rejects body writes, no implicit close     | focused + integration | `test_http_h1writer`, `test_http_integration`, `test_http_server`                 | Add direct `1xx` focused proof later so the shared no-body predicate is covered beyond `204/304`.                        |
...
4. add direct `1xx` / `HEAD` response-body contract proof instead of only relying on the shared no-body predicate and current request-side coverage.
