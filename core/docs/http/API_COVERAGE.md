...
1. decide whether the current keep-alive request-tail transport truth should now be frozen as intentional transport policy: fixed-length / plain chunked / trailer-complete chunked 已覆盖 garbage-tail、partial-follow-up-line、partial-follow-up-headers、valid pipelined next-request isolation/completion，以及 “partial follow-up line can complete later” bridge proof；若仍要收紧成更早显式拒绝，则需要明确说明为何不破坏合法分段到达的第二请求。
