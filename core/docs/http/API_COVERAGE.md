...
1. decide whether the current keep-alive request-tail transport truth should remain “first request completes, malformed tail becomes follow-up `400`” for both `Content-Length` and chunked variants, now that fixed-length/chunked tail isolation plus fixed-length/chunked pipelined first-request isolation are all proven, or be tightened into an earlier explicit-rejection policy; then finish the remaining raw-wire malformed chunk framing audit, especially the trailer value-start / whitespace variants after the field-name and separator truncation variants.
