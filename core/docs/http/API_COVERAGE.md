...
- 2026-06-06 API 对标结论：
  ...
  - 继续收紧：client 现在也收口 close-capable request body ownership。
    `IHttpClient.Do_` 会在最终 round trip 返回或失败后关闭
    ...
    `Do_` success、transport error 和 reader-shortcut buffering 三条路径。
  ...
  - 继续补齐：新增 headers-level `SetBasicAuth(Headers, Username, Password)`
    ...
    锁住 facade 可见性，`test_http_client` 锁住 live `IHttpClient.Do_`
...
- `bench_http_server` 现在有 focused benchmark comparator smoke：测试会自动 build nextPas `bench_server`、Go `net/http` comparator、Rust std-only comparator 与可选 Hyper/Tokio comparator，用小规模 `--requests 32 --threads 2` 运行，并验证输出都包含 `operation=http.server.keepalive`、`workload=no_url` 或 `workload=url_path`、`impl`、`iterations`、`threads`、`completed`、`ns/op` 与 `req/s`，nextPas 行还会暴露 `nextpas_h1_path` marker，Rust std-only 行会暴露 `impl=rust_std` 与 `rust_profile=std_only` marker，Hyper/Tokio 行会暴露 `impl=rust_hyper` 与 `rust_profile=hyper_tokio` marker；direct nextPas / Go / Rust std-only / Hyper rows 现在还会显式给出 `requested_threads=` 与 `effective_threads=`，并有 focused clamp smoke 直接锁住 `--requests 3 --threads 5` 时仍保留 `requested_threads=5`、`effective_threads=3`，避免单体 comparator row 静默丢失调用方原始线程输入。针对 `response_1k` 这类 response-writer/drain workload，direct rows 现在还会显式给出 `client_read_mode=` 与 `response_body_bytes=`：nextPas / Rust std-only / Hyper 走 `header_plus_content_length`，Go comparator 则暴露 `http_client_body_drain`，并有 focused smoke 直接锁住 `response_body_bytes=1024`。`run_server_comparison.sh` 也有 focused smoke，会用 `--requests 8 --threads 1 --output ...`、`--workload url_path`、`--workload adapter_no_url`、`--workload response_1k` 与 `--include-hyper` 证明 comparison runner 可同时输出 stdout、报告文件、workload marker 和可选 Hyper/Tokio summary；同一 runner 现在还有 focused multi-run proof：`--runs N` 会保留逐次 `run=...` 原始输出并追加 nextPas / Go / Rust std-only 的 median `summary_impl` 行，且 runner 会拒绝 `iterations` / `completed` 未达到请求数的 incomplete rows，summary/report 也锁住 `median_completed`。`capture_server_comparison_snapshot.sh` 现在同样有 focused smoke，会生成包含 `git_head` / OS / FPC / Go / Rust 版本和 raw comparison output 的 Markdown snapshot，并锁住 snapshot 内不出现 FPC `Warning:` / `Note:`；snapshot 现在也会透传 `--workload`、`--runs N` 与 `--include-hyper`、记录 `workload=` / `runs=` / `include_hyper=` 并保留 summary rows，同时在环境块额外保留 `requested_threads=` / `effective_threads=`，避免 snapshot 自己静默吃掉 thread clamp truth。`bench_h1parser/run_flag_matrix.sh` 现在也有 focused multi-run smoke：`--runs N` 会生成逐次 `results.tsv`、聚合 `summary.tsv` 与 `env.txt`，用于更稳定地对比 Pascal raw llhttp 与 C llhttp 的 narrowed rows；自定义 `NEXTPAS_FLAG_MATRIX_OUTPUT_DIR` 若落在 `build/projects/nextpas.core.http/bench_h1parser/flag_matrix/...` 之外，现在也会 fail-fast，`test_http_benchmarks` 直接锁住这条 `unsafe output dir` contract。`bench_server` raw client 现在会按 response header boundary + expected body length 读取完整响应，避免大响应 workload 只读前缀；`docs/http/BENCHMARKS.md` 已记录本机 2026-06-05 `--runs 3` median snapshot，覆盖 `no_url` / `adapter_no_url` / `url_path` / `response_1k` 并标出下一批优先优化 llhttp adapter path；当前不声明永久性能排名，后续正式 benchmark 轮次应继续区分 std-only 与 Hyper/Tokio rows。
- `capture_server_comparison_snapshot.sh --include-hyper` 现在还会记录
...
| ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `nextpas.core.http` facade                     | type aliases, status constants, factory/forwarding functions; direct proof now includes facade consumption of `HTTP_STATUS_SWITCHING_PROTOCOLS` / `HTTP_STATUS_EARLY_HINTS` / `HTTP_STATUS_SEE_OTHER` / `HTTP_STATUS_PAYLOAD_TOO_LARGE` / `HTTP_STATUS_NOT_IMPLEMENTED` / `HTTP_STATUS_HEADER_TOO_LARGE` with correct status-text mapping, plus facade forwarding for status-class helpers, `ServeFile` / `ServeDir` / `UpgradeWebSocket`, `SetBasicAuth` / `SetBearerAuth`, `HttpGetToWriter`, `HttpWriteResponseString`, `IWebSocket`, `TWebSocketOpcode`, `TWebSocketFrame`, and `wsOp*` enum values | focused + integration | `test_http_contract`, `test_http_smoke`, `test_http_static`, `test_http_websocket`, `test_http_client` | Revisit later whether selected concrete implementation types should also enter the facade, or remain unit-local by design. |
| `http.base`                                    | `THttpVersion`, `THttpMethod`, `THttpStatus`, `EHttpError` message preservation + `ecNetwork` category, status text plus status-class helpers for `1xx` / `2xx` / `3xx` / `4xx` / `5xx`, `TUrl.Parse`, `TUrl.ParseRequestTarget` origin-form / path-only / absolute-form / asterisk-form / authority-form / scheme-like origin-form / empty-input behavior, `THttpClientOptions.Default`, `THttpServerOptions.Default`, `TTcpServerBackend` / backend constants re-export, `HTTP_STATUS_CONTINUE` / `HTTP_STATUS_EARLY_HINTS` / `HTTP_STATUS_NOT_IMPLEMENTED` text mapping | focused               | `test_http_base`, `test_http_contract`                                            | None in this phase.                                                                                                       |
| `IHttpHeaders` / `THttpHeaders`                | set/add/get/getall/has/del/clear/count/foreach/clone, validation, Clear 后可复用且不会暴露 stale entries, common `Authorization` helpers (`SetBasicAuth` / `SetBearerAuth`) with nil-header rejection and replacement semantics, concrete parser-trusted `AddParsed` / `AddParsedSpans` canonical lowercase insertion, header lookup hot-helper inline source contract (`FindFirst` / `NeedsNormalize` / `NormalizeIfNeeded`) | focused               | `test_http_headers`, `test_http_contract`, `test_http_integration`, `test_http_benchmarks` | Keep public `Add`/`Set_` validation for external input; use concrete trusted helpers only with parser-validated headers/spans.               |
...
| `IHttpClient` / `THttpClient`                  | do/get/post/put/delete/patch/head, `CloseIdleConnections` lifecycle seam with optional transport capability dispatch and H1 idle pool clear semantics, `Post` / `Put` / `Patch` shortcut body materialization through a shared bytes-buffer helper, close-capable request body release after `Do_` success/failure and after reader-shortcut buffering, direct `string` / `TBytes` shortcut body overloads with forwarded `content-type` and generated `content-length`, headers-level Basic/Bearer auth helper values are forwarded by `Do_`, explicit `Do_(nil)` `EArgumentError` rejection, stale pooled keep-alive retry narrowed to retry-safe plus replayable-body requests with non-idempotent or non-replayable fail-fast semantics, redirects including `301` / `302` / `303` replay-as-GET body drop, `307` / `308` method preservation with seekable body rewind/replay and non-replayable non-empty body fail-fast, redirect follow-up caller header preservation with cross-authority sensitive-header stripping and same-authority/custom `Host` preservation including omitted-port vs. default-port authority equivalence, relative `Location` path/query/fragment parsing, absolute `http` / `https` scheme case-insensitive matching with unsupported absolute scheme fail-fast, path-relative `Location` base-directory merge plus dot-segment normalization, fragment-only `Location` path/query preservation, and network-path `Location` scheme inheritance plus authority/path/query parsing before follow-up transport dispatch, timeout, negative `THttpClientOptions` validation, host header, pooling, chunked/EOF/truncated body handling, HEAD response header preservation with body suppression under explicit `Content-Length`, `HttpGetToWriter` / `HttpGetToFile` successful body copy, response body release on success/failure/non-2xx discard paths, non-2xx rejection, atomic final-file publish, and truncated-body temp cleanup, `HttpReadResponseBodyString` / `HttpReadResponseBodyBytes` full-body helpers with nil-body / nil-response boundaries, explicit transport injection, registry-backed default resolution | focused               | `test_http_client`, `test_http_smoke`, `test_http_contract`, `test_http_registry`, `test_http_examples` | Consider adding a same-client follow-up regression if later transport refactors reopen pooling behavior.                  |
