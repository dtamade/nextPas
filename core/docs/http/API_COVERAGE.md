...
- `IHttpServer` 现在也有 HEAD response suppression focused 覆盖：即使 handler 调用了 `Write`，raw-wire 响应也不注入 `Transfer-Encoding: chunked`、不写 chunk trailer、也不发 body bytes。
...
4. add direct HEAD explicit `Content-Length` preservation proof for shared GET/HEAD handlers.
5. add broader informational response proof only if runtime later needs `101` or multi-stage response handling.
6. continue malformed chunk/trailer audit only if still-unclassified terminal/trailer grammar subclasses are found during later protocol work.
