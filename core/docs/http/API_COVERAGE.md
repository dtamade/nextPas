...
1. decide whether the current keep-alive request-tail transport truth should remain “first request completes, malformed tail becomes follow-up `400`” for both `Content-Length` and chunked variants, now that fixed-length / plain chunked / trailer-complete chunked truth has covered garbage-tail, partial-follow-up-line, partial-follow-up-headers, and valid pipelined next-request isolation/completion, or be tightened into an earlier explicit-rejection policy; then continue the raw-wire malformed chunk framing audit only where still-unclassified terminal/trailer grammar subclasses remain.
2. decide whether a public trailer API is warranted; the current narrow contract is to preserve the `Trailer` declaration header but ignore trailer fields in ordinary headers.
...
4. future H2/H3 transport registration coverage on the landed internal registry.
