...
- `TH1ResponseWriter` 边界覆盖现在包括预设 `Transfer-Encoding`、显式 `Content-Length` flush 路径、`100/204/304` no-body status 不注入 chunked、HEAD-style suppress-body 路径不发 body bytes、以及 no-body / chunked-finalized 两条拒绝继续写入路径。
...
- `H1 parser` 现在也有 request-side unsupported transfer-coding focused 覆盖：`Transfer-Encoding: gzip, chunked` 不再被误当成可解码 chunked request，而是被直接判成 unsupported request transfer-coding。
...
| `IHttpResponseWriter` / `TH1ResponseWriter`    | write status, headers, body, flush, chunked default, explicit no-body status (`100` direct proof, shared `1xx/204/304` predicate) does not auto-inject chunked framing, HEAD-style suppress-body mode discards body output, no implicit close | focused + integration | `test_http_h1writer`, `test_http_integration`, `test_http_server`                 | Add direct `101` or broader informational proof later if server/runtime starts emitting more than `100`.                 |
...
4. add broader informational response proof only if runtime later needs `101` or multi-stage response handling.
5. continue malformed chunk/trailer audit only if still-unclassified terminal/trailer grammar subclasses are found during later protocol work.
