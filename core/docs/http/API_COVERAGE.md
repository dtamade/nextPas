...
- `IHttpServer` 现在也有 Linux `epoll` backend focused differential proof：`THttpServerOptions.Backend := TCP_SERVER_BACKEND_EPOLL` 时，simple GET、keep-alive 复用、fixed/chunked same-write pipelining、chunked trailer-complete keep-alive garbage tail / truncated follow-up line / truncated follow-up headers 的 follow-up 语义、chunked trailer-complete same-write pipelining、chunked trailer-complete partial follow-up line 后续可补全为合法第二请求、hijack ownership、以及 hijack 后异常不补写 `500` 的契约都与 threaded 路径保持一致。
