...
- `IHttpServer` 现在也有 Linux `epoll` backend focused differential proof：`THttpServerOptions.Backend := TCP_SERVER_BACKEND_EPOLL` 时，simple GET、keep-alive 复用、fixed/chunked same-write pipelining、hijack ownership、以及 hijack 后异常不补写 `500` 的契约都与 threaded 路径保持一致。
