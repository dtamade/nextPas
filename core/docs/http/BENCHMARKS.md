...
| **2026-07-21 E3s** | `no_url` 20k×4 epoll runs=3 | **50549** | 25655 | **1.97×** | 58127 / **155623** | 124993 / 678395 | **0.23×** |
| **2026-07-21 E3s** | `response_1k` 20k×4 epoll runs=3 | **48549** | 28177 | **1.72×** | 59968 / **162087** | 115513 / 596119 | **0.27×** |

**E3 / E3s gates**（相对 Go 同 harness）

| Gate | Threshold | 2026-07-20 E3 | 2026-07-21 E3s |
| ---- | --------- | ------------- | -------------- |
| RPS `no_url` | ≥ 0.80× | **2.21× Met** | **1.97× Met** |
| RPS `response_1k` | ≥ 0.80× | **2.22× Met** | **1.72× Met** |
| p99 `no_url` | ≤ 2.0× | **0.21× Met** | **0.23× Met** |
| p99 `response_1k` | ≤ 2.0× | **0.22× Met** | **0.27× Met** |

Artifacts: `build/projects/nextpas.core.http/server_comparison/e3s-20260721-epoll-*-runs3.md`

**E3 extras**

| Check | 2026-07-20 | 2026-07-21 E3s |
| ----- | ---------- | -------------- |
| Ladder 1k idle epoll probe | `stable=1` | **`stable=1`** open_ok=1000 probe_ok=1000 |
| Ladder 10k idle epoll probe | `stable=1` | **`stable=1`** open_ok=10000 probe_ok=10000 |
| `test_http_h2_facade` heaptrc | **0 unfreed** (5/5) | (not re-run this wave) |

**E3s verdict (2026-07-21)**：scale-ready RPS + p99 **仍 Met**；绝对值随主机噪声浮动（Go 行略升 → 比值从 ~2.2× 落到 ~1.7–2.0×）。**不**因比值下降收回宣称。
...
| **2026-07-21 E3s** | multiplex | epoll | 8×16×100 mid | 12800 | **2879** | 1 |
| **2026-07-21 E3s** | multiplex | epoll | 16×32×100 press | 51200 | **11469** | 1 |
...
row must report `iterations=<requests>` and `completed=<requests>`.
Raw row `operation` and `workload` markers must match the runner request before
...
## Static file / Range — frozen (2026-09-09 → 2026-09-02 frozen)

`ServeFile` / `ServeDir` / `ServeVfs` use **user-space streaming** (`static.pas:CopyRange` 32K buffer aligned `io.base:IO_COPY_BUF_SIZE` single source for Range segments, `io.Copy` via `IFile:IWriterTo` fast path for whole file; `VFS` embedded slices are zero-copy, file path 32K buffered copy — **not** kernel `CopyFileRange`/`sendfile` for generic `IWriter` socket path, honest residual pending `platform:sendfile` owner) with `Accept-Ranges: bytes`, single-range `206` + `Content-Range`, `416` on invalid/multi-range, and `304` conditional precedence. `source-contract` locks `IFile` + `io.Copy` / `CopyRange` and forbids `ReadFile`/`ReadAll` whole-file buffering; `try/finally`/`Close` release not lost, `heaptrc 0 unfreed`. `test_http_static` (304/Range/416/smoke) is **functional smoke + frozen regression** (32K `IO_COPY_BUF_SIZE` single source, `inline` probes zero-alloc via `bytes.ops:CompareMem` single source, VFS zero-copy retained); **bench_static fixed thresholds `≥0.80× Go` / `p99 ≤2×` same-harness frozen** (`benchmarks/nextpas.core.http/bench_static` + `compare_go`/`compare_rust` L3 aligned `bench_server` shape, `IWriterTo` already landed, four-piece/`inline`/zero-copy/`heaptrc 0` frozen). Scale gate: `bench_server` keepalive `≥0.80× Go` **plus** `bench_static` static/Range throughput `≥0.80× Go` / `p99 ≤2×` same-harness frozen (CONTRACT §1.1 §2). Cross-machine leaderboard claims are forbidden. Closure: `bench_static` + Go `net/http.ServeFile` / Rust `hyper_static` same-harness rows frozen median thresholds; see `CONTRACT.md:3.63`.
...
The same focused gate now also locks the narrower
`bench_filter=fast headers has accept` path. This keeps the presence-only raw
...
The focused benchmark gate now also locks the narrower
`bench_filter=request direct RawQuery access` path, so the direct query
projection row remains durable standalone microbenchmark evidence instead of
only appearing inside the broader `request ` filter group.

The focused benchmark gate now also locks the narrower
`bench_filter=request direct Path+RawQuery access` path, so the combined direct
path/query projection row remains durable standalone microbenchmark evidence
instead of only appearing inside the broader `request ` filter group.
...
The raw comparison runner now also locks `--include-hyper + --workload response_1k`,
