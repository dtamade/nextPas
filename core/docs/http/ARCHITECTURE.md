...
H2 已开始落内部 codec foundation（frame validation、HPACK Huffman、HPACK request sequence header block），
...
- 当前真实源码库存为 29 个 HTTP 单元，测试工程为 26 个；其中 H2 只有
  frame codec / validation、HPACK Huffman 和 HPACK request-sequence header-block 内部基础单元及
...
HPACK request-sequence header-block 和 frame codec / validation 内部基础，不对外声明 H2 可用。
