...
nextpas.core.respack.embed.pas          ← 嵌入工具链库：blob → .inc/.inc unit 纯内存生成（S4，阈值前置 4MiB 拒绝防超大临时分配，BytesCopy 单源零拷贝）
```
...
| `embed` | `nextpas.core.text.strings`/`text.char`/`text.conv` + `nextpas.core.bytes.ops` + `nextpas.core.encoding.hex`（纯 L1，零 FS/零 writer/零 dirsource） | **纯内存嵌入**：仅 `blob → .inc / .inc unit` 文本生成；glob 经 `text.strings.GlobMatch` + 标识符经 `text.char.IsAlpha/IsAlphaNum` + 数值经 `text.conv.IntToStr` 三单源（PChar 零拷贝视图 + *非 inline* 循环体守 design-conventions §2 红线2 + O(pat×name) 双追踪器）+ `bytes.ops.BytesCopy`/`BytesConcatMany` 单源（`WriteStr` `BytesCopy` inline 零拷贝 + 单次分配拼接，阈值前置 4MiB 拒绝防三段分配超大临时分配），`fs.glob` 为薄转发同源，无额外 L2 依赖；目录管线已收口 dirsource，修复 L1→L2 上行依赖，纯内存可复用 |
