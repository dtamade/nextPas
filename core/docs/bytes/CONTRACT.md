...
- **通用紧凑 Vec 注册表**：`TCompactLiveRegistry<T>`（`bytes.ops`）为 L1 通用紧凑注册表，已反哺落地（CONTRACT §1.2/§50 可抽候选）；`webview.live.TWebviewLiveRegistry<T>` 为其兼容薄别名（deprecated，过程式 `WebviewLiveAdd/Remove` 双形已收敛删除，现 `webview2` 直用 `VecGrow/VecRemoveSwap` 单源 inline 零额外调用，新代码直用 `bytes.ops.TCompactLiveRegistry<T>` 跨家族复用），`window.live` 同构已收敛至同源 Swap 语义；`Register` via `VecGrow` 0→4→2×，`Unregister` via `VecRemoveSwap` O(1) 零拷贝，`Snapshot` via `VecSnapshot` 单 SetLength + managed/blittable 分支零拷贝，`Clear` 逐槽 `Default(T)` 释放不丢，`Trim` 单源 `VecTrim`。
