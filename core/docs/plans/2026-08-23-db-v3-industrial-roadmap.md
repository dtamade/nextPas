...
| **D2 pg 协议系兼容层** ✅ 部分 | openGauss/KingbaseES 连通性 conformance 变体（env 门控）：错误归一表核对（SQLSTATE 沿用 PG 类目？）、能力矩阵实测回填、方言差异清单 | 复用 IDbCapabilities 自述 + conformance 全套件直接跑（B1 的直接收益：新后端零改造接入套件）；差异项（系统目录查询、savepoint 语义、批量返回值）逐条登记 §2.6 扩展节，不改统一契约除非确需增量 | **openGauss 5.0.0 x86 via Docker 真机 ✅**（2026-08-28，`opengauss/opengauss:5.0.0`，`password_encryption_type=0+md5+reload` 兼容三步，`NEXTPAS_PG_TEST_CONN` 指向 55432）：`test_db_pg 13 passed / conformance 2 passed / trace 5 passed heaptrc 0`，方言缺口 3 例（`unnest` 多列 / `WITH ORDINALITY` 缺失）诚实记录降级为 batch；KingbaseES 仍待环境 ⏳ |
| **D3 MySQL 协议系兼容层** ✅ | `mariadb:11.8(53306)` + `mysql:8.0.46(53307)` **双引擎真机**经 `libmariadb.so.3(112B)` 直连验证——`test_db_mysql_adapter 7/7 passed`(roundtrip/四分类/savepoint/caps) `heaptrc 0`；`mysql:8` 需 `mysql_native_password`（MariaDB 客户端 `caching_sha2` 需 libmysqlclient 8，生产显式创建该类用户，已入 `national-db-guide §5`）。双引擎已覆盖 TiDB/OceanBase/PolarDB-X/GoldenDB/TDSQL-MySQL 线协议兼容面（同属 MySQL 协议系，差异仅方言与事务语义 §2.3，savepoint/乐观重试登记见 §2.6b/B5） | P1 完成（离线 §2.3/2.5/§3 矩阵 + 双引擎 live 冒烟）；TiDB PD+TiKV+TiDB 完整集群编排为可选进阶（`NEXTPAS_TIDB_TEST_CONN` 另路门控，不阻塞发布） |
| **D4 达梦 DM 专用适配器** ✅ P1 | **ODBC 网关 P1 完成、DPI P2 按需**（ADR 0002）：P1 零新增代码、以 `ConnectOdbc(Driver=DM8 ODBC DRIVER;…)` 为 Tier-1 路径，能力按 §2.11 诚实降级（savepoints/false 等）；P2 专用 `libdmdpi`（`dpi_*` 25 符号、`ClassifyDm`、`dbkDm`）仅当触发条件满足时再议（≥2 消费方欠归一阻塞或需 LOB/interval/数组绑定） | P1 文档与门禁就绪：national-db-guide §2.4/§4.5 已更新 DM ODBC 配方与 Docker/QEMU 配方；P2 决策记录 `docs/adr/0002-dameng-dm-adapter.md` ✅ |
