...
| **D3 MySQL 协议系兼容层** | TiDB/OceanBase/PolarDB 经 ConnectMysql 的 conformance 变体；TiDB 的 savepoint/事务语义差异（乐观事务重试语义）与 B5 重试助手的组合验证 | TiDB 无真 savepoint（早期版本）/悲观事务模式差异登记；OceanBase MySQL 租户占位符与多语句支持核实；prepared stmt 二进制协议在 TiDB 的支持度实测（可能回落文本协议——诚实降级路径评估） | 离线预研完成（2026-08-28，§2.3/2.5 + §3 矩阵）；真机 NEXTPAS_TIDB_TEST_CONN 等门控 conformance 待环境 ⏳ |
