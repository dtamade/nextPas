...
| **B4 TLS 成文** | pg sslmode/sslrootcert 经 conninfo 透传已是事实，契约化 | CONTRACT §2.1 增 TLS 小节：证书校验责任表、各后端配置样例；mysql 适配器（A2）同步支持 --ssl-mode 透传 | 文档 + conformance 冒烟（本地带证书可选跑） |
...
| **B6 INC-4 异步挂载** | `nextpas.core.db.async`：TDbAsyncExecutor——把阻塞调用投递到专用执行线程组，返回 core.async channel/future | 硬规则落地：连接仍一连接一线程（G3），异步的是"等待"不是"并发复用"；取消经 IAsyncCancellationToken → pg PQcancel / sqlite progress handler | bench_db_async（吞吐/尾延迟）+ test_db_async 门禁 |
| **B7 LISTEN/NOTIFY（G9 收口）** | `nextpas.core.db.pg.listen`：订阅通道 → async.channel 投递；重连自动重新 LISTEN | 专用连接独占（LISTEN 会话不能跑普通查询——诚实约束入契约）；断线期间通知丢失如实上报告知消费方（at-most-once 是 pg 语义，不假装 at-least-once） | test_db_pg_listen 门禁（真机双进程自发自收）|
...
| **C2 参数级批量绑定（Array DML 对标）** | `IDbArrayBinder`（可选能力）：一行数据 N 组参数一次往返 | pg = unnest 数组展开或 binary COPY 二选一实测（INC-2 Tier2 正名立项）；sqlite = begin/exec N/commit 已够快，如实登记不硬造 | 三路基准：10K 行插入 vs 逐行 vs IDbBatchExecutor |
...
| **C4 基准门禁化** | 现有 4 个 bench 固化口径入册（数据集/迭代数/判据），加"裸驱动开销比"指标（统一层 vs 直调 ffi 的比值，健康线 ≤1.15×） | 口径写入 docs/db/benchmarks.md；Makefile 加 bench 目标但不进默认 verify（环境噪声隔离），手动门禁 | docs + Makefile 目标 + 一次全量采集入册 |
| **C5 sqlite 调优预设** | `TDbSqlitePragmas = record JournalMode/Synchronous/MmapSize/CacheSize...end` 进 ConnectSqlite options | 安全缺省 = WAL+synchronous=NORMAL（文件库）；:memory: 忽略 journal 类 PRAGMA（sqlite 会报错，需过滤——诚实处理非静默）；预设即注释即契约 | test_db_sqlite 扩组（PRAGMA 生效自证）|
...
| 分片 | 内容 | 关键设计点 | 门禁 |
|---|---|---|---|
| **D1 兼容性矩阵与连接指南** | openGauss / 人大金仓 KingbaseES / TiDB / OceanBase / PolarDB / TDSQL / GoldenDB 经现有 ConnectPostgres / ConnectMysql 的连通事实表（端口、方言开关、已知差异）；docs/db/domestic.md 成文 | 分类三档：①pg 协议系（openGauss/KingbaseES）②MySQL 协议系（TiDB/OceanBase MySQL 模式/PolarDB/TDSQL/GoldenDB）③私有协议（达梦 DM 等→D4/A3/A4 路径）；每库登记认证方式差异（如 openGauss 密码加密策略 sha256/md5）、SSL 要求、版本基线 | 文档门禁：连接指南进 docs/db/README 地图；无真机不写"已验证" |
...
- [x] `core/docs/db/README.md` 模块入口（本次新增：快速上手 + 特性矩阵）
- [ ] CONTRACT §2.x 随每片同步（纪律不变）
- [ ] `docs/db/benchmarks.md`（C4 产出：基准口径册）
- [ ] 各新后端一页指南（对齐 sqlite.md/pg.md 体例：A2/A4 产出）
- [ ] 本路线图每片 landing 回填状态 ✅
