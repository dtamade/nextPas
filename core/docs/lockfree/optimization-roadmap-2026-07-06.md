...
| 阶段 | 优先级 | 优化方向 | 工时 | 收益 |
|------|--------|---------|------|------|
| Phase 1 | P1+P3+P7 | 测试增强 | 8h | 质量保障 |
| Phase 2 | P4+P5 | 性能优化 | 12h | 延迟降低 |
| Phase 3 | P2+P6 | 数据结构增强 | 16h | 功能完善 |
| Phase 4 | P0 | 架构优化 | 16h | 读路径无锁 |

**总工时**: ~52h
**预期收益**:
- 测试覆盖: 246 → 280+ tests
- 读延迟: 降低 30-50% (RCU 风格)
- 内存效率: EBR batch retire 减少 50% contention
...
| 阶段 | 测试目标 | 验证方法 |
|------|---------|---------|
| Phase 1 | 280+ tests | make test |
| Phase 2 | 性能基准 | bench 工具 |
| Phase 3 | 并发测试 | stress 测试 |
| Phase 4 | 读路径测试 | 高并发读 |
...
| M1 | 测试增强完成 | 280+ tests |
| M2 | 性能优化完成 | 延迟降低 20% |
| M3 | 数据结构增强完成 | HashSet + Lock coupling |
| M4 | 架构优化完成 | 读路径无锁 |
