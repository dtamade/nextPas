...
**教学示例**：`t1_close_join_free/`（Channel）；`t1_segqueue_workers/`（SegQueue）；`t1_msqueue_close_join_free/`（MSQueue）；`t2_bag_close_join_free/`（Bag）；`t2_multimap_close_join_free/`（MultiMap）；`t2_hashmap_join_free/`（HashMap **无 Close**，stop→join→Free）。选型见 [`selection-guide.md`](selection-guide.md)「任务投递四选一」。
