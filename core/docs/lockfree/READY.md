...
生命周期示例：`t1_close_join_free/`（Channel）、`t1_segqueue_workers/`（SegQueue）、`t1_msqueue_close_join_free/`（MSQueue）、`t2_bag_close_join_free/`（Bag）、`t2_multimap_close_join_free/`（MultiMap）、`t2_hashmap_join_free/`（HashMap 无 Close）。
