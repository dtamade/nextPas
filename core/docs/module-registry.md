...
| `db` | L3 家族（L3 门面/适配 L3；base/intf/pool/migrate/batch/perf/trace 等 L2 基础设施严格单向 L0-L2，无上向；可裁剪边界=直连 adapter Connect* 或按需 factory.register.* 单后端注册单元，显式锁定于 `core/docs/db/CONTRACT.md §1/§2.14`，`db.pool`/`db.migrate` 已下沉 L2 无 L3→L3，见 design-conventions L890） | unified database family (IDbConnection over sqlite/pg/mysql/odbc/redis/dm, 6 backends; wallet now independent `nextpas.core.wallet` L3, db/billing thin compat physically deleted) | `nextpas.core.db` | L0-L2 (L3 family, L2 infra pool/migrate/batch/perf/base/intf) | focused-runtime |
| `wallet` | L3 | wallet ledger/balance/redeem (independent Owner=wallet lane, public facade `nextpas.core.wallet` pure aggregation; compat aliases `nextpas.core.db.wallet` / `billing.wallet` / `db.wallet.impl` physically deleted 2026-09-02, files removed, unified to `nextpas.core.wallet`, physical wallet.impl single source, Owner centralized见 wallet/CONTRACT.md §0) | `nextpas.core.wallet` | L0-L2 plus identity/time/text/bytes owners plus db infra `db.pool`/`db.migrate` (L2, strict downward, no L3→L3) | focused-runtime |
...
| `metrics` | L2 | generic observability (counter/threshold) | `nextpas.core.metrics` | L0-L1 | source-contract — `METRICS_MAX_FRAME_BYTES` 1 MiB 单源阈值 + `MetricsOversizedCount/Note/IsOversized*/ExpandedSize` plain UInt64 单调计数 `inline` 零拷贝视图（`bytes.ops` 单源，`Default` 释放不丢，UI 线程亲和）；L3 `webview.metrics` thin-forward alias 零双写，bridge 解析前/后双校验水位闭环 |
...
| `webview` | L3 | desktop app shell over system web engines (WebKitGTK/WebView2/WKWebView backends; unified IPC bridge) | `nextpas.core.webview` | L0-L2 plus json owner plus window (L2 has-a); platform.dl | focused-runtime, source-contract — S37 容量与 Fail-Fast 完整性（`IsValidWebviewSchemeToken` 复用 + Builder `GrowInvokes/GrowReady` 2× + Scheme/几何早筛 + `CONTRACT 1.31`）; L3→L2 has-a `nextpas.core.window` (factory/fake has-a `IWindow`, `WindowRunLoop` 单泵归 `window.factory`, `bytes.ops VecGrowCapacity` 单源 inline 零拷贝) |
| `window` | L2 | window shell + surface (nextpas.core.window family; first consumer webview/gpu/directui/game888; 11 backends incl. fake; 1.0 单源收口含 gtk3 Raw `WindowGtkRaw*` 12项) | `nextpas.core.window` | L0-L1 plus platform.dl seam plus one-way L2 `gtk2/gtk3/gtk4/qt5pas/qt` | focused-runtime — `test_window_*` 13 gates `heaptrc 0`, inline `WindowBackendAvailable/DefaultWindowKind/Post/GetWidth/IsClosed/GetDispatcher` 零拷贝薄转发, `bytes.ops VecGrowCapacity` 单源复用, `try-finally` 资源释放不丢, `WindowPumpOnce` 26ns/次早退 (`bench_dispatcher` 365µs PostSingle/271µs Zero 中位 5×方差<5%, O(1)+inline+共享队列) |
...
| `window` | L2 | window shell + surface (nextpas.core.window family; first consumer webview/gpu/directui/game888; 1.0 单源收口含 gtk3 Raw `WindowGtkRaw*` 12 项) | `nextpas.core.window` | L0-L1 plus `platform.dl` seam, plus one-way L2 `gtk2/gtk3/gtk4/qt5pas/qt` | ci-matrix (Linux 13门 runtime + Win/mac compile-only，残差诚实，见 window/FINAL_ROADMAP F3) — `test_window_*` 13 gates `heaptrc 0`, `bytes.ops` 单源 `WindowGrowCapacity` 0→32→2× / 0→64→2× inline 零额外调用 O(1)均摊不丢, `TWindowQueue` 环形 FIFO 32cap起步2×+锁外Drain/`TWindowLiveRegistry` 无锁原子 inline `Count` 16ns 零锁零拷贝 `Move`, 资源 `try-finally/FreeAndNil/finalization` 释放不丢 |
| `window.live` | L2 shard | window live registry (`nextpas.core.window.live`: `TWindowLiveRegistry`/`TWindowSdlLiveRegistry` + 无锁原子聚合计数) — 家族内特权共享，编译期需 `TWindowFamilyToken`（`window.impl` 单源 inline 零拷贝 IsValid，strict private sentinel）方可构造，不经公共门面 re-export，仅 `window.*` 后端 uses (owner `window.impl`, scope `window` family, base 仅纯数据类型) | no | L0-L1 plus `atomic` + `sync` + `bytes.ops` via `window.impl` | source-contract |
| `window.queue` | L2 shard | window work queue (`nextpas.core.window.queue`: `TWindowQueue` 环形 FIFO 32cap起步2×增长+锁外Drain) — 家族内特权共享，编译期需 `TWindowFamilyToken`（`window.impl` 单源 inline 零拷贝 IsValid，strict private sentinel）方可构造，不经公共门面 re-export，仅 `window.*` 后端 uses (owner `window.impl`, scope `window` family, base 仅纯数据类型) | no | L0-L1 plus `sync` + `bytes.ops` via `window.impl` | source-contract |
| `window.hash` | L2 shard | window hash helper (`nextpas.core.window.hash`: 开放寻址线性探测双哈希 Pointer/U32，负载≤0.5 Via WindowGrowCapacity 0→32→2× 幂二单源) — 家族内特权共享，编译期需 `TWindowFamilyToken`（`window.impl` 单源 inline 零拷贝 IsValid，strict private sentinel）方可调用，不经公共门面 re-export，仅 `window.live` uses (owner `window.impl`, scope `window` family, base 仅纯数据类型) | no | L0-L1 plus `bytes.ops` via `window.impl` | source-contract |
