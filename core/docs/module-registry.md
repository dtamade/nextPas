...
| --- | --- | --- | --- | --- | --- |
| `app` | L3 | app shell (Tauri App-level, thin wrapper over webview, `nextpas.core.app.*` 4件套) | `nextpas.core.app` | L0-L2 plus webview | focused-runtime — `TAppOptions=TWebviewOptions` 单源 |
| `args` | L3 | CLI surface | `nextpas.core.args` | L0-L2 | focused-runtime |
| `async` | L1 | async loop/runtime | `nextpas.core.async` | L0 plus `io/time` seams | focused-runtime |
...
| `bytes` | L1 | byte containers | `nextpas.core.bytes` | L0, documented text/encoding seam plus `simd` (impl-only) | focused-runtime |
...
| `io` | L1 | stream/poller/completion | `nextpas.core.io` | L0, platform | focused-runtime |
...
| `webview` | L3 | desktop app shell over system web engines (WebKitGTK/WebView2/WKWebView backends; unified IPC bridge) | `nextpas.core.webview` | L0-L2 plus json owner; platform.dl | focused-runtime, source-contract — S104 1.97 冻结（`WebviewGrowCapacity 0→4→2×` 单源 inline + `IsValidWebviewSchemeToken` 复用 + `FindMountByPrefix` inline + `CONTRACT 1.97`） |
| `websocket` | L3 | WebSocket | `nextpas.core.websocket` | L0-L2, HTTP/TLS seams | source-contract |
| `window` | L2 | window abstraction (11 backends×4件套, `nextpas.core.window.*` 4件套, `text.ansi` 单源, `platform.dl` 单源) | `nextpas.core.window` | L0-L1 plus platform.dl | focused-runtime, source-contract — 5.0 11后端完全体 |
