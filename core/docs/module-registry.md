...
| `archive` | L2 | archive shared helpers (tar/zip walk/sort/symlink/snapshot/zero-copy, federated via archive.fs) | `nextpas.core.archive.fs` | L0-L1 plus fs/io via platform.files (tar/zip owned, single seam) | focused-runtime — 几何扩容 `ArchiveEnsureWalkCapacity` 2×/16 + `ArchiveWalkAppend` inline + `ArchiveSortDirEntries` 零拷贝 pivot + `ArchiveEnsureNoSymlinkInPath` 增量前缀 + `ArchiveSnapshotStream` + `ArchiveWriteFileSlice` 零拷贝 PByte + `ArchiveRestoreDeferredDirs` 逆序 + `ArchiveCollectWalk` 单源递归（ReadDir 批量 + 单次 Stat/Entry 非 N+1） |
| `tar` | L2 | tar container (ustar/pax/GNU, block-aligned, sandboxed) | `nextpas.core.tar` | L0-L2; `tar.fs` is the single `fs` seam (pack/extract) plus `io`/`text` seams, federation via `archive.fs` (shared walk/sort/snapshot, L2 同层显式一-way) | focused-runtime, source-contract — `base` 单源 `IsSafeTarEntryName/Validate` + `C_TAR_BLOCK_SIZE=512` + `TarRegular/DirectoryMode` 薄封装；`common` 单点 `TarPadToBlock/GuardTarEntrySize/GuardTarTotalSize` + base-256 双路径校验和；`reader` 零拷贝切片 `EntryDataSlice/OpenEntryStream` + `PByte` 外部内存视图 + `pax x/g` 与 `GNU L/K` 长名单点覆盖 + `MaxEntry/MaxTotal` bomb 守卫 + `heaptrc 0`；`writer` `ustar` `prefix` 自动分割 + `pax x` 长名回退（>100 无切分/`linkpath>100` 前置 `x`，`bytes.ops` 单源 `StringToBytes` 一次 Move）+ `base-256` 溢出 + `IsSafeTarEntryName` 写端拒入 + 两零块收尾；`fs` `TarPackDirInto/TarExtractToDir` 确定性排序（委托 `ArchiveCollectWalk` 单源） + `deferred dir` 逆序定稿 + `EnsureNoSymlinkInPath` 二次拒绝对称 + `SkipSpecial` 默认跳特殊文件；`builder` `ITarBuilder` 链式薄门面委托 `TTarWriter` + `CreateBytesStream` 快照，`compress.tar` 为 deprecated 薄转发 |
| `vfs` | L2 | read-only virtual filesystem (memtree/embedded/os/sub/mount/overlay + facade + transform decorator) | `nextpas.core.vfs` | L0-L1; os seam is the single fs/path L2→L2; embedded adds respack.reader; mount/overlay pure composite zero extra deps; transform/compressed: L3 decorator hosted in L2 family (Registry white-listed豁免未拆分, L3→L2, `transform` 4K HeaderPred + `compressed` GZIP_MAX 32MiB 单源) | focused-runtime, source-contract — embedded `FPaths/FEntries/FETags/FLastMods` parallel cache (O(log n) index, O(1) ETag/Last-Modified, zero `DecodeWire`), `TEmbeddedSlice/TEmbeddedSliceStream` 零拷贝+`SpinLock` `EMBEDDED_POOL_SIZE` 16 槽池化 (10k 163ms, `heaptrc 0`)，`List` 零 `Stat` 直填 `FEntries`，`VfsNameCompare` 直通 `base.utils CompareBytesOrdered` + `VfsETagStrong/FNV` 单源消 `embedded/http` 字面量重复，`HasSubtreePath/IndexOfPath` 共用 `LowerBoundPath` + `CompareBytesOrdered` 显式字节序 + `CompareMem` 前缀 + `PChar→@S[1]` 显式化 + `LPtr/Llen` 缓存 + `inline` 热路径，`mount` 前缀最长匹配聚合+`overlay` 同根优先级叠加 patch>dlc>base（游戏热更，List去重合并，ETag优先透传）；`transform` 通用字节变换装饰器（`TVfsTransformFunc/TVfsShouldTransformFunc/TVfsHeaderPredicateFunc` 函数注入，L3，零拷贝按需变换：`Stat` 4K HeaderPred 快路径免大文件全量 `VfsReadAllBytes`（非变换直接回 `FInner.Stat`，小文件复用 Header 零二次 IO）、`OpenRead` HeaderPred 假时零物化直透 `FInner.OpenRead`（零拷贝无 materialize，4K peek ~972ns）、真时单次 `VfsReadAllBytes` 复用 `LData`+`Pointer` 去重、`TryGetETag` 禁用防旧指纹、`TryGetLastModified` 经 `QueryInterface` 透传，`inline` 热路径+`try-finally` Close 不丢；`compressed` 为 `transform` 薄门面仅保留策略 `VFS_DECOMPRESS_MAX_BYTES→GZIP_MAX_DECOMPRESS_BYTES` 单源与 `daAuto/daGzip` 语义（`IsGzipHeaderPred` 4K 头部谓词经 `bytes.ops` 单源魔数），`STORE` 零拷贝与 32MiB 防 bomb 由 `transform` 承载，`daAuto` 4K 头部预判避免 `Stat` 全量读取（非 gzip 直接返回内层 `Stat`，省一次 `VfsReadAllBytes`），`bench_transform` 4 项基准固化（`Stat/header-peek 972ns` 等），合规 `nextpas.core.exception` + `QueryInterface` 无 `SysUtils` 直引） |
