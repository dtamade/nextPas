...
| `tar` | L2 | tar container (ustar/pax/GNU, block-aligned, sandboxed) | `nextpas.core.tar` | L0-L2; `tar.fs` is the single `fs` seam (pack/extract), plus `io`/`text` seams | focused-runtime, source-contract — `base` 单源 `IsSafeTarEntryName/Validate` + `C_TAR_BLOCK_SIZE=512` + `TarRegular/DirectoryMode` 薄封装；`common` 单点 `TarPadToBlock/GuardTarEntrySize/GuardTarTotalSize` + base-256 双路径校验和；`reader` 零拷贝切片 `EntryDataSlice/OpenEntryStream` + `PByte` 外部内存视图 + `pax x/g` 与 `GNU L/K` 长名单点覆盖 + `MaxEntry/MaxTotal` bomb 守卫 + `heaptrc 0`；`writer` `ustar` `prefix` 自动分割 + `base-256` 溢出 + `IsSafeTarEntryName` 写端拒入 + 两零块收尾；`fs` `TarPackDirInto/TarExtractToDir` 确定性排序 + `deferred dir` 逆序定稿 + `EnsureNoSymlinkInPath` 二次拒绝对称 + `SkipSpecial` 默认跳特殊文件；`builder` `ITarBuilder` 链式薄门面委托 `TTarWriter` + `CreateBytesStream` 快照，`compress.tar` 为 deprecated 薄转发 |
