...
| `compress` | L2 | compression formats | `nextpas.core.compress` | L0-L1, provider FFI, plus L2 thin forward via nextpas.core.tar single source (deprecated, to be removed, bytes.ops 单源 inline/零拷贝) | focused-runtime — `compress.base GZIP_MAX_DECOMPRESS_BYTES=32MiB` 单源（门面重导出，`vfs.compressed VFS_DECOMPRESS_MAX_BYTES` 已收敛为薄别名/薄门面经 `transform` 承载）；`compress.tar` 薄转发 via nextpas.core.tar 门面单源（tar 已晋升独立 L2 nextpas.core.tar，新增请直接 uses nextpas.core.tar，本地薄别名复用 bytes.ops 单源 inline/零拷贝，资源 try-finally 不丢，待迁移后移除） |
