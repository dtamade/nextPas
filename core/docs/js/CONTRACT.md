...
| 单元 | 职责 | 允许 uses | 禁止 |
|------|------|-----------|------|
| `js.base` | `TJsBackendKind`、`TJsValueKind`、`TJsErrorCategory`、`TJsRuntimeOptions`、`EJsError` 族 + `JsTrimEquals`（零拷贝 `StringTrimEquals` 薄转发，`bytes.ops` 单源 `SpanTrim/SpanEqual`，`text.view` 同源复用，循环体留 owner，去 `inline` 解耦）+ `JS_QUICKJS_PROBE_NAMES[0..7]` 单源常量（收敛 loader 数组与 `EJsBackendUnavailable` prob 名表双写，`bytes.ops/text` 单源 comma-join via loader `TBufStringBuilder`，无 ifdef 分支，8 名跨平台单表） | `exception`、`base`（`interface` 仅 `base`/`exception` 纯 L0 类型载体零 L1 透出；`implementation` 单缝 `bytes.ops` `StringTrimEquals/SpanTrim*` 零拷贝视图单源，`L0-L1` 向下，`text.view` 同源复用，`CONTRACT §1` 单源下沉，不 `inline` 避 I-Cache/跨单元耦合，零拷贝 O(n) 单遍） | 任何 `js.*`、`platform`、`json`、`text.view`（禁止直引 `text.view`，经 `bytes.ops` 单源转发） |
...
| `js.quickjs.loader` | `platform.dl` 探测与符号装载（唯一可触 `platform.dl`，Vault 隔离幂等缓存，跨平台 8 名探测，热点 inline+零拷贝，资源 try-finally 幂等不丢，守 design-conventions §2 红线 2：循环/几何体 out-of-line） | `platform.dl`（唯一 `dl` 缝 L0 `platform.dl` 单源）、`js.base`（`JS_QUICKJS_PROBE_NAMES` 单源常量 8 名，`bytes.ops/text` 单源收敛，无 ifdef，双写消除 via `BuildProbeNames`）、`js.quickjs.ffi`、`bytes.ops`（`BytesZero` FillChar SIMD 单源 L1，`BytesCopy` Move 单源 inline 零拷贝，`BytesGrowCapacity` 几何）、`sync.vault`（`SyncVaultEnsureLock` 单源 lazy Exactly-Once out-of-line loop per §2，`VaultRef` inline 薄转发，`IMutex→platform.sync`）、`text.builder`（`TBufStringBuilder` 几何 `BytesNextCapacity` 单源 via `bytes.ops`，`BuildProbeNames` out-of-line 避 I-Cache 复制，`ProbeNames` comma-join 零拷贝 `try-finally Done` 不丢）、`atomic`（`atomic_load/store/compare_exchange` L0 单源，`GVaultInit` 64B 友好） | `DynLibs`、`Windows/BaseUnix` |
...
| `js.lifecycle` | 纯上下文生命周期 owner（`GPureClosed` 紧凑 4B `epoch*2+closed` generation-tagged atomic acquire/release + `atomic_fetch_add` lock-free id + 2^32 wrap freelist retry (long service not DoS, bounded recycling, generation-epoch protects ABA per INV-7) + `GPureFree` freelist bounded recycling via `bytes.ops`几何 `BytesNextCapacity` + `mem.dynarray` Exactly-Once poke amortized O(1) inline零拷贝 (freelist reuse candidate extractable to `nextpas.core.collections.freelist`) + shrink 4x阈值半衰半缩 (half-shrink vs exact, `bytes.ops`几何 + `mem.dynarray` poke avoids periodic large copy jitter), `GPureNextId/Len/Lock` plain (GPureClosed 紧凑4B/10k~40KB), IsAlive single acquire + relaxed Len (I-Cache/零拷贝, bulk IsValid zero barrier), spinlock resize with 5ms deadline backoff to avoid starvation, `bytes.ops` 几何 + `mem.dynarray` Exactly-Once poke，bulk IsValid 零原子 via FValid，幂等 `JsPureClose` 不丢+`atomic_compare_exchange` generation去重+`generation mismatch`强一致 per INV-7，L0 `atomic/bytes/platform.thread/platform.time` 单源，守 L0-L3，四件套 `base` 仅类型载体） | `base`、`atomic`、`bytes.ops`、`mem.dynarray`、`platform.thread`、`platform.time` | `json`、`platform.dl`、`*.ffi`、`webview.*` |
| `js.pure.base` | 纯族共享基座（标准子模块四件套 `js.pure.base` 纯类型载体 per four-piece `base←intf←impl←门面`，base零依赖 per design-conventions（无同模块 uses，lifecycle/Host/Heap/Value/IO 均 via owner `js.lifecycle`/`pure.host`/`pure.value`/`js.eval` 单源直引，无薄转发债务，`JS_PURE_HASH_THRESHOLD` 16 单源收敛至 `pure.hash`、`JS_PURE_EVAL_*` 5哨兵单源收敛至 `js.eval` 无双表重复，`bytes.ops` 几何 0→64→2×），纯类型载体无逻辑无可变全局，`JsPure*` 零分配视图经 owner 单源，零 FFI/零 `platform.dl`；L0 `platform.fs` 经`pure.host`直读 64MiB 限流，`bytes.ops` 零拷贝 via `BytesCopy`单源；`TJsPureHostState` 统一 `JsPureHostStateSet*` 经 `pure.host`，`JsPureClose` 由 `pure.impl` 组合 `lifecycle.Close`+`pure.host/hostStateClear`+`pure.value/valueStateClear` 幂等不丢，阈值 800 内（wc -l ~45 <800），守 L0–L3 与四件套，热点 inline+`BytesCopy` 零拷贝，资源幂等不丢） | `js.base/intf`、`text.view`、`json`（base零依赖，无同模块 uses） | `platform.dl`、`*.ffi`、`webview.*`、`js.lifecycle`/`js.pure.host`/`js.pure.value`/`js.eval` 同模块直引（base零依赖，via owner 单源直引禁止） |
| `js.pure.impl` | 纯模板组合（Runtime+Context，Host→`pure.host.TJsPureHostState` per-Context O(1) 桶、Value→`pure.value.TJsPureValueState`、IO via `js.eval` 单源（原 via `pure.base` 已收敛至 `js.eval` 直读，base零依赖），95% 复用 `js888/v8/chakra`，零 FFI/零 `platform.dl`，阈值 800 内，热点 FindHostView/Bind/DoEval/New* inline+`BytesCopy` 零拷贝，资源 `JsPureClose` 幂等不丢，守 L0–L3） | `js.base/intf`、`js.pure.base`、`js.pure.host`、`js.pure.value`、`js.eval`、`text.view`、`json`、`platform.thread/fs`（L0 直读） | `platform.dl`、`*.ffi`、`webview.*` |
...
| `js.registry` | 后端注册表：L2 唯一扇出 owner，5 后端工厂与探测单源（`O(1)` 枚举索引 `JsRegisterBackend` 扩展优雅，`CreateJsRuntime`/`JsBackendAvailable` 单源分发，内置 `fake/js888/v8/chakra=恒可用` + `jsbkQuickJs=JsQuickJsIsAvailable/Load 含探针名表 via loader `JsQuickJsProbeNames` 单源`js.base`），`bytes.ops` 单源 inline 零拷贝 SpanTrim/SpanEqual+BytesCopy + `pure.base` 几何 BytesNextCapacity，资源幂等 `JsPureClose/StoreClear` exactly-once，守 `base←registry` 零循环；**线程安全 vault 单 owner 模块化隔离（非裸全局，经 VaultRef inline 单源访问，`sync.vault SyncVaultEnsureLock` 单源 lazy Exactly-Once out-of-line loop per §2，GVaultInit 原子 Exactly-Once，IMutex→platform.sync acquire/release 原子保护 O(1) 快照零锁外派发，64B 友好，热点 inline+atomic_thread_fence 零拷贝，资源 try-finally/IMutex 不丢，loader/registry 共用 vault 单源消除克隆）**） | `js.base/intf` + `fake/js888/v8/chakra/quickjs/loader` + `sync.mutex/sync.vault→platform.sync`/`atomic`/`bytes.ops`（唯一扇出点 vault 单缝显式收敛，`sync.vault` 懒初始化单源，工厂传递扇出经 registry 单缝，L2→L1/L0 单向，非掩盖） | `json` 直接依赖（仅经 intf/pure.base 间接）、`platform.dl`（仅经 loader） |
| `js.factory` | 工厂：`CreateJsRuntime / JsBackendAvailable` 薄转发至注册表单源（自身零直接 uses，传递扇出经 registry 唯一扇出点显式，非掩盖；门面 inline 收益完整，`Registry O(1)` via VaultRef + `JsRegisterBackend` 优雅，`CheckJsRuntimeOptions` fail-closed exactly-once，阈值 800 内，wc -l ~50 <800） | `js.base/intf` + `js.registry`（唯一依赖注册表，零直接 `fake/js888/v8/chakra/quickjs` 扇出，传递扇出经 registry 单缝显式） | `json` 直接依赖、`platform.dl`、`fake/js888/v8/chakra/quickjs` 直接 `uses`（显式下沉至 registry 唯一扇出点，非掩盖） |
| `js.pas` | 门面：纯 re-export（`inline` 薄转发至 `js.factory`，零分支零探测，阈值 800 内，wc -l ~50 <800） | `js.base/intf/factory` | 逻辑（四件套纯聚合） |

```
base ← intf ← {fake, quickjs.ffi, value.store, quickjs.value, quickjs, lifecycle, pure.base ← pure.impl ← {js888, v8, chakra}} ← registry ← factory ← 门面 // 阈值800（单一阈值，历史 550/650 已收敛）
         ↑ 依赖闭包均 <800（hygiene 抽样 wc -l，阈值 800，模块 Makefile 显式锚定；pure.host ~400、pure.value ~490、pure.base ~45（wc -l ~45 <800，纯类型载体 base零依赖，threshold 16 via pure.hash单源, 哨兵5× via js.eval单源）、pure.impl ~440、lifecycle ~205、eval ~240、value.store ~120、quickjs.value ~390、registry ~210、factory ~50（wc -l ~50 <800）、门面 ~50（wc -l ~50 <800）、js888/v8/chakra ~30），单源 `bytes.ops` + `text.view` 零拷贝，热点 inline+BytesCopy，资源幂等不丢
```
...
**文件体积指引**：单单元 >800 行必拆；hygiene 自动化抽样 `wc -l core/src/nextpas.core.js*.pas`（阈值 800 告警，`make hygiene` 必过且 `core/tests/nextpas.core.js/test_js_source_contracts/Makefile` 显式锚定 `wc -l <800`，超出抽样即拆，无历史 550/650 反复调整，单一阈值 800）。实测：`js.intf` ~144 行、`js.base` ~147 行、`js.lifecycle` ~205 行、`js.eval` ~240 行、`js.pure.host` ~400 行、`js.pure.value` ~490 行、`js.pure.base` ~45 行（纯类型载体 base零依赖，threshold 16 via pure.hash单源, 哨兵5× via js.eval单源，wc -l ~45 <800）、`js.pure.impl` ~440 行、`js.value.store` ~120 行、`js.quickjs.value` ~390 行、`js.registry` ~210 行、`js.factory` ~50 行、`js.js888/v8/chakra` 各 ~30 行、`js.quickjs.ffi/loader` <50 行，`js.fake` ~380 行，`host/value` 薄别名 <50 行，均 <800（wc -l 抽样阈值 800，见模块 Makefile 锚定）。热点 `inline` + `bytes.ops`/`BytesCopy`/`text.view` 零拷贝单源，资源 `try-finally` 幂等不丢，守四件套 `base←intf←impl←门面` 与 L0–L3，单一阈值 800。
...
- 允许：`base`、`errors`、`exception`、`json`、`text.view/builder`、`bytes.ops`（`SpanEqual/BytesCopy/SpanIndexOf/BytesGrowCapacity` 零拷贝 single source via `bytes.ops` single source, `bytes.ops` 单源 table-driven predicates + `TStringView.Equals/Slice` 零拷贝 `inline`）、`text.scan`（`ScanPredicateTable` 通用谓词+字面量 VecWidth 表驱动 single source via `bytes.ops`/`simd.vec` single source, L1 复用 `VecWidth/VecCmpEq/VecCtz` 单遍, 零拷贝, `js.eval` ScanEvalPredicates 薄委托至 text.scan 复用, 尾部 `VecWidth` overlapping 单遍, json 字面量共享同谓词表）、`simd.vec/simd.base`（仅 text.scan 内聚 via text.scan 复用, L0→L1→L2 单向 `text.scan → js.eval`）、`mem`、`platform.dl`（仅 loader）、`platform.fs`（`TryEvalFile` L0直读，`bytes.ops BytesCopy` 零拷贝单源 `JS_PURE_FILE_MAX_BYTES` 64MiB canonical via `bytes.ops BYTES_BULK_PARSE_MAX_BYTES` L1 single source, `FORMAT_BULK_PARSE_MAX_BYTES` 同源 alias, 无 L2→L2, `try-finally` 释放不丢）、`encoding`（`TBytes` base64 若涉二进制）—— **L2 js 禁止直接 `simd.vec` 硬耦合扫描, 一律经 `text.scan ScanPredicateTable` 单源复用, 守 module-registry `json/text.view/bytes.ops+text.scan` 单缝单向, `bytes.ops` 单源 `SpanEqual/BytesCopy` 零拷贝 `inline` + `text.scan` 单遍 `VecWidth` 谓词表 single source**
...
| 2026-09-02 | 2.4 | 匠心修复lifecycle三痛·紧凑与强一致：`GPureClosed` 64B填充槽(10k~640KB)→紧凑4B `epoch*2+closed`(10k~40KB, L1/L2友好, `bytes.ops`几何紧凑, `inline`零拷贝)；`GPureFree` 仅几何增长不收缩→4x阈值半缩 `TryShrinkGPureFreeLocked` (长服短生命周期后内存回落)；`freelist`复用清零误判存活→generation-tagged Id `(epoch<<32|idx)` + `mismatch即closed` 强一致 per INV-7 (`atomic_compare_exchange`幂等, `acquire`单源), 守L0-L3与四件套, `bytes.ops`单源, 18份对齐, CONTRACT为准 | codex/core-js |
