...
**Owner**：`codex/core-js`
**最后更新**：2026-09-02
**版本**：1.5（匠心修复 INV-1 单源：js.intf 实现段仅 json.types/writer 单缝 去 json.value/text.escape 双路径归一 TJsonWriter，IsValid 热路径 inline+lock-free 零拷贝 via bytes.ops 几何，intf 240行 <500 阈值内极简奢华 四件套 base←intf←实现←门面 守 L0-L3，pure.base 501行 JsPureToJsonString 单源化，wc -l 501 阈值650内<800必拆 18 份对齐）
...
```
base ← intf ← {fake, quickjs.ffi, value.store(Pure Heap/Global via pure.base), quickjs.value(Pure+QjsHeap装饰器 via value.store+ffi), quickjs(装饰器组合 Pure+QjsHeap), pure.base ← pure.impl(Runtime+Context组合收敛 ~360行 via pure.host State+pure.value State, 奢华度已收敛) ← {js888, v8, chakra}} ← factory ← 门面
         ↑ pure.base 501 行 + pure.impl ~360 行(HostState+ValueState组合, wc -l ~360, <650/<800 必拆, 奢华度已收敛 Host→pure.host+Value→pure.value) + value.store ~70 行 + quickjs.value ~250 行 (Pure+QjsHeap装饰器单Store, wc -l ~320, <650/<800 必拆, 匠心双堆耦合收敛至 js.value.store 单源)；js888/v8/chakra 各 ~29 行零 FFI/零 dl，纯族阈值内与 quickjs 平级；factory 单源分支/探测，门面 inline 零拷贝薄转发
```
...
**文件体积指引**：单单元 >800 行必拆；`js.intf` 240行含值+宿主+运行时三职责，>500 行即拆 `js.value.pas`/`js.host.pas`（wc -l 240实测 <500 阈值内极简）；`js.fake` 含 `platform.thread/platform.fs` 集成与三形态宿主，阈值放宽至 550（`design-conventions §2` 加严；见 `SIXDIM_REVIEW M-1/M-2`）。纯族 `pure.base` 501 行四职责聚合 Host/Heap/Value/IO（IO 仅 L0 `platform.fs` 直读，非 dl，64MiB限流）+ `pure.impl` ~360 行 Runtime+Context组合收敛（奢华度已收敛：Host→pure.host.TJsPureHostState per-Context O(1)桶 inline+bytes.ops单源 + Value→pure.value.TJsPureValueState Heap/Global bytes.ops+mem.dynarray, <650/<800 必拆，wc -l ~360 实测，已落地收敛）+ `js.js888/v8/chakra` 各 ~29 行，单单元均 <650（<800 必拆，wc -l 501/~360 实测），阈值内（`wc -l` 实测 501+~360+29×3≈948，共享后单源 pure.base 501 + pure.impl ~360 各阈值内，已落地 HostState+ValueState组合）。`make hygiene` 抽样 `wc -l core/src/nextpas.core.js*.pas` 告警阈值 650/800（`js.fake` 146，其余 500，纯族 501/~360 行体量同步 BENCHMARKS 1.7，复用 `bytes.ops` 单源与 `text.view` 零拷贝视图，热点 `JsPureHostStateFindView/JsPureValueState*` inline + `BytesCopy` 零拷贝，资源 `try-finally/JsPureClose/FreeAndNil` 不丢，守 L0–L3 与四件套例外，`JsPureDoEval` 单遍谓词缓存避免重复 SIMD 扫面，常驻 hash 桶索引已落地 HostState，魔串常量化+单加号 parser 候选）。
