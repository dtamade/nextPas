...
**Owner**：`codex/core-js`
**最后更新**：2026-09-02
**版本**：2.0（匠心修复单源收敛：Host/Heap/Value inline thin-forward至pure.host/pure.value/js.eval/platform.fs单源，js.value已移除双入口（空deprecated shim零能力，pure.value永久单源 owner，无阈值迁移），消费者导入粒度收敛至pure.value或pure.base聚合 单入口, 消除同能力双入口混乱, 四件套base←intf←impl←门面单源 owner+ L0-L3守分层，wc -l ~390 <650(<800必拆) 18份对齐，复用bytes.ops单源，inline零拷贝, 资源try-finally不丢, CONTRACT为准）
...
| `js.lifecycle` | 纯上下文生命周期 owner（`GPureClosed` 64B padded atomic acquire/release cache-line隔离 + atomic_fetch_add lock-free id + spinlock resize 实例隔离线程高级感，`bytes.ops` 几何 + `mem.dynarray` Exactly-Once poke，bulk IsValid 零原子 via FValid，强一致 acquire，幂等 `JsPureClose` 不丢，L0 `atomic/bytes` 单源，守 L0-L3，四件套 `base`仅类型载体解耦） | `base`、`atomic`、`bytes.ops`、`mem.dynarray` | `json`、`platform.dl`、`*.ffi`、`webview.*` |
| `js.pure.base` | 纯后端共享基座薄门面（`JsPure*` 零分配视图，零 FFI/零 `platform.dl`（L0 `platform.fs` 仅 `TryEvalFile` 直读，非 dl，64MiB限流，`bytes.ops Move`零拷贝），**非标准四件套命名显式例外**：`pure.base` 薄转发 single source 复用 `js888/v8/chakra` 三纯后端，lifecycle 已抽 `js.lifecycle` 单源（GPureClosed 64B padded atomic + spinlock几何），Host/Heap/Value/IO仅inline thin-forward至`pure.host/pure.value/js.eval/platform.fs/js.lifecycle`单源（pure.host/pure.value永久单源 owner，无阈值迁移，js.value已移除为零能力空shim，消费者经pure.value/pure.base单入口，Close双重载奢华收敛至`_JsPureHostsClear`单源模板），阈值 650 内（<800 必拆，wc -l ~320 实测，无双入口，pure.value单源），`pure` 为纯族聚合前缀、`base` 为共享基座后缀，非独立模块，守 L0–L3，复用 `bytes.ops` 单源与 `text.view` 零拷贝，热点 inline+`BytesCopy` 零拷贝，资源幂等不丢） | `js.base/intf`、`text.view`、`json`、`platform.fs`（L0 直读）、`js.lifecycle` | `platform.dl`、`*.ffi`、`webview.*` |
| `js.pure.impl` | 纯模板组合收敛（Runtime+Context，Host/Heap/Value/IO 四职责已落地组合：`TJsPureHostState via pure.host`(Host/Call, per-Context桶 O(1) inline+bytes.ops单源)+`TJsPureValueState via pure.value`(Heap/Value, bytes.ops+mem.dynarray inline+零拷贝)+IO via pure.base直读，阈值 650 内（<800 必拆，wc -l ~360 实测，奢华度已收敛），`js888/v8/chakra` 三纯后端 95% 复用，零 FFI/零 `platform.dl`，`bytes.ops` 几何扩容与 `text.view` 零拷贝直通，热点 FindHostView/Bind/DoEval/New* inline+`BytesCopy` 零拷贝，资源 `JsPureClose` 幂等不丢，守 L0–L3） | `js.base/intf`、`js.pure.base`、`js.pure.host`、`js.pure.value`、`text.view`、`json`、`platform.thread/fs`（L0 直读） | `platform.dl`、`*.ffi`、`webview.*` |
...
```
base ← intf ← {fake, quickjs.ffi, value.store(Pure Heap/Global via pure.base), quickjs.value(Pure+QjsHeap装饰器 via value.store+ffi), quickjs(装饰器组合 Pure+QjsHeap), js.lifecycle(lifecycle 64B padded atomic+spinlock单源), pure.base(thin facade via lifecycle/host/value/eval, ~320行+Close单源收敛) ← pure.impl(Runtime+Context组合收敛 ~360行 via pure.host State+pure.value State, 奢华度已收敛) ← {js888, v8, chakra}} ← registry(5 后端单源 O(1) 扇出收敛显式，JsRegisterBackend 扩展优雅，VaultRef+GVaultInit 原子 Exactly-Once，bytes.ops 单源) ← factory(传递扇出经 registry 单缝显式 inline 薄转发，非掩盖) ← 门面
         ↑ js.lifecycle ~70 行(lifecycle 64B padded+spinlock几何 single source, base仅类型解耦) + pure.base ~320 行(Close双重载收敛至_JsPureHostsClear单源, wc -l ~320, <650/<800 必拆, 奢华收敛 Host→pure.host+Value→pure.value+lifecycle) + pure.impl ~360 行 + value.store ~70 行 + quickjs.value ~250 行 + registry ~110 行(vault VaultRef+GVaultInit 原子 lazy)；js888/v8/chakra 各 ~29 行零 FFI/零 dl，纯族阈值内与 quickjs 平级；registry 单源分支/探测 O(1) 显式 + 工厂传递扇出经 registry 单缝显式 inline 薄转发，JsRegisterBackend 扩展优雅，门面 inline 零拷贝
```
...
**文件体积指引**：单单元 >800 行必拆；`js.intf` 240行含值+宿主+运行时三职责，>500 行即拆纯族 host/value 已收敛至 pure.host/pure.value 单源（wc -l 240实测 <500 阈值内极简，无js.value/js.host双入口）；`js.fake` 含 `platform.thread/platform.fs` 集成与三形态宿主，阈值放宽至 550（`design-conventions §2` 加严；见 `SIXDIM_REVIEW M-1/M-2`）。纯族 `js.lifecycle` ~70 行 lifecycle 64B padded atomic cache-line隔离+atomic_fetch_add lock-free+spinlock resize实例隔离（`bytes.ops`几何+`mem.dynarray` poke single source, base仅类型解耦）+ `pure.base` ~320 行薄门面Host/Heap/Value/IO仅thin-forward至`pure.host/pure.value/js.eval/js.lifecycle`单源（Close双重载奢华收敛至`_JsPureHostsClear`单源模板，`bytes.ops`+`text.view`零拷贝, 热点 inline+`BytesCopy`零拷贝, 资源幂等不丢, pure.host/pure.value永久单源，无阈值迁移）+ `pure.impl` ~360 行 Runtime+Context组合收敛（Host→pure.host.TJsPureHostState per-Context O(1)桶+Value→pure.value.TJsPureValueState, <650/<800 必拆，wc -l ~320/~360 实测，pure.value单源已收敛无js.value双入口）+ `js.js888/v8/chakra` 各 ~29 行，单单元均 <650（<800 必拆），阈值内。`make hygiene` 抽样 `wc -l core/src/nextpas.core.js*.pas` 告警阈值 650/800，保持（lifecycle单源+Close单源 18份对齐，复用 `bytes.ops`+`text.view`零拷贝）。
...
| 2026-09-02 | 1.8 | 匠心修复 pure.base：lifecycle 64B padded atomic cache-line隔离+atomic_fetch_add lock-free+spinlock resize实例隔离线程高级感，Host/Heap/Value/IO仅inline thin-forward至pure.host/pure.value/js.eval/platform.fs单源并落地js.host/js.value独立门面复用下沉，四件套base←intf←实现←门面单职责范式+ L0-L3守分层，wc -l ~390 <650(<800必拆) 18份对齐 | codex/core-js |
| 2026-09-02 | 1.9 | 匠心修复双入口收敛：js.host/js.value降级为deprecated兼容薄别名 (thin alias to pure.host/value, inline零拷贝 via text.view+bytes.ops FNV1a/bytes.ops+mem.dynarray单源, 资源幂等不丢), 消费者导入粒度收敛至pure.host/pure.value或pure.base聚合 单入口, 消除同能力双入口混乱 (bytes.ops FNV1a/BytesCopy/BytesNextCapacity单源, 热点 inline+零拷贝, 资源 try-finally/Done幂等不丢, 守四件套base←intf←impl←门面与L0-L3, CONTRACT为准, 缺能力反哺owner), wc -l ~390 <650(<800必拆) 18份对齐 | codex/core-js |
| 2026-09-02 | 2.0 | 匠心修复单源收敛：js.value双入口彻底移除（空deprecated shim零能力，pure.value永久单源，无阈值迁移），消费者单入口 pure.value/pure.base聚合，pure.value Heap/Value via bytes.ops+mem.dynarray几何 inline零拷贝，js.lifecycle 64B padded atomic, 四件套base←intf←impl←门面单源owner+L0-L3, CONTRACT为准, 资源try-finally不丢, wc -l ~390 <650(<800必拆) 18份对齐 | codex/core-js |
