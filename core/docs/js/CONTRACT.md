...
**Owner**：`codex/core-js`
**最后更新**：2026-09-02
**版本**：1.9（匠心修复双入口收敛：Host/Heap/Value仍inline thin-forward至pure.host/pure.value/js.eval/platform.fs单源，js.host/js.value降级为deprecated兼容薄别名 (thin alias to pure.host/value, inline零拷贝 via text.view+bytes.ops FNV1a/bytes.ops+mem.dynarray单源, 资源幂等不丢), 消费者导入粒度收敛至pure.host/pure.value或pure.base聚合 单入口, 消除同能力双入口混乱, 四件套base←intf←实现←门面单职责+ L0-L3守分层，wc -l ~390 <650(<800必拆) 18份对齐）
...
| `js.pure.base` | 纯后端共享基座薄门面（`JsPure*` 零分配视图，零 FFI/零 `platform.dl`（L0 `platform.fs` 仅 `TryEvalFile` 直读，非 dl，64MiB限流，`bytes.ops Move`零拷贝），**非标准四件套命名显式例外**：`pure.base` 薄转发 single source 复用 `js888/v8/chakra` 三纯后端，lifecycle 已抽 `js.lifecycle` 单源（GPureClosed 64B padded atomic + spinlock几何），Host/Heap/Value/IO仅inline thin-forward至`pure.host/pure.value/js.eval/platform.fs/js.lifecycle`单源（`js.host/js.value`为deprecated兼容薄别名 thin alias to pure.host/value, 消费者应 import pure.host/value 或 pure.base 聚合 不新增 js.host/value 入口, 阈值>800时职责可迁, Close双重载奢华收敛至`_JsPureHostsClear`单源模板），阈值 650 内（<800 必拆，wc -l ~320 实测，超阈预案 `js.host`+`js.value` deprecated alias ready），`pure` 为纯族聚合前缀、`base` 为共享基座后缀，非独立模块，守 L0–L3，复用 `bytes.ops` 单源与 `text.view` 零拷贝，热点 inline+`BytesCopy` 零拷贝，资源幂等不丢） | `js.base/intf`、`text.view`、`json`、`platform.fs`（L0 直读）、`js.lifecycle` | `platform.dl`、`*.ffi`、`webview.*` |
