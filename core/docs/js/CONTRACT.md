...
**Owner**：`codex/core-js`
**最后更新**：2026-09-02
**版本**：2.1（单源收敛：pure.host/pure.value/js.eval 单源，js.value 已删除（无空shim残留，pure.value永久单源，无阈值迁移，设计规范§2禁止空文件凑四件套），消费者经 pure.value/pure.base 单入口；体积与阈值见 §1 体积指引，守四件套与 L0-L3，热点 inline+bytes.ops 零拷贝，资源 try-finally 幂等不丢，业务以 CONTRACT 为准）
...
| `js.pure.base` | 纯后端共享基座薄门面（`JsPure*` 零分配视图，零 FFI/零 `platform.dl`；L0 `platform.fs` 仅 `TryEvalFile` 直读 64MiB 限流，`bytes.ops` 零拷贝；非标准四件套命名显式例外，薄转发复用 `js888/v8/chakra`，Host→`pure.host`、Value→`pure.value`、lifecycle→`js.lifecycle` 单源，`_JsPureHostsClear` 单源 Close，阈值 800 内，`pure` 为聚合前缀/`base` 为基座后缀，守 L0–L3，热点 inline+`BytesCopy` 零拷贝，资源幂等不丢） | `js.base/intf`、`text.view`、`json`、`platform.fs`（L0 直读）、`js.lifecycle` | `platform.dl`、`*.ffi`、`webview.*` |
