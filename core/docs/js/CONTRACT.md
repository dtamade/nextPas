...
| `js.lifecycle` | 纯上下文生命周期 owner（`GPureClosed` 紧凑 4B `epoch*2+closed` generation-tagged atomic acquire/release + `atomic_fetch_add` lock-free id + `GPureFree` freelist bounded recycling + shrink (4x阈值半缩, `bytes.ops` 几何 `BytesNextCapacity` 单源 + `mem.dynarray` Exactly-Once poke), `GPureNextId/Len/Lock` 各64B VARMIN isolated (GPureClosed 紧凑4B/10k~40KB, L1/L2友好), spinlock resize with 5ms deadline backoff to avoid starvation, `bytes.ops` 几何 + `mem.dynarray` Exactly-Once poke，bulk IsValid 零原子 via FValid，幂等 `JsPureClose` 不丢+`atomic_compare_exchange` generation去重+`generation mismatch`强一致 per INV-7，L0 `atomic/bytes/platform.thread/platform.time` 单源，守 L0-L3，四件套 `base` 仅类型载体） | `base`、`atomic`、`bytes.ops`、`mem.dynarray`、`platform.thread`、`platform.time` | `json`、`platform.dl`、`*.ffi`、`webview.*` |
...
base ← intf ← {fake, quickjs.ffi, value.store, quickjs.value, quickjs, lifecycle, pure.base ← pure.impl ← {js888, v8, chakra}} ← registry ← factory ← 门面 // 阈值800（单一阈值，历史 550/650 已收敛）
         ↑ 依赖闭包均 <800（hygiene 抽样 wc -l，阈值 800，模块 Makefile 显式锚定；pure.host ~400、pure.value ~490、pure.base ~360（wc -l ~360 <800）、pure.impl ~440、lifecycle ~205、eval ~240、value.store ~120、quickjs.value ~390、registry ~210、factory ~50（wc -l ~50 <800）、门面 ~50（wc -l ~50 <800）、js888/v8/chakra ~30），单源 `bytes.ops` + `text.view` 零拷贝，热点 inline+BytesCopy，资源幂等不丢
```
