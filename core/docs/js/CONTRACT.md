...
| `js.pure.base` | 纯后端共享基座（`JsPure*` 零分配视图，零 FFI/零 `platform.dl`（L0 `platform.fs` 仅 `TryEvalFile` 直读，非 dl，64MiB限流，`bytes.ops Move`零拷贝），**非标准四件套命名显式例外**：`pure.base` lifecycle单职责+单源复用 `js888/v8/chakra` 三纯后端，GPureClosed 64B padded atomic acquire/release cache-line隔离 + atomic_fetch_add lock-free id + spinlock resize 实例隔离线程高级感，Host/Heap/Value/IO仅inline thin-forward至`pure.host/pure.value/js.eval/platform.fs`单源（已落地`js.host/js.value`独立门面复用下沉），阈值 650 内（<800 必拆，wc -l ~390 实测，超阈预案 `js.host`+`js.value` ready），`pure` 为纯族聚合前缀、`base` 为共享基座后缀，非独立模块，守 L0–L3，复用 `bytes.ops` 单源与 `text.view` 零拷贝） | `js.base/intf`、`text.view`、`json`、`platform.fs`（L0 直读） | `platform.dl`、`*.ffi`、`webview.*` |
...
```
base ← intf ← {fake, quickjs.ffi, value.store(Pure Heap/Global via pure.base), quickjs.value(Pure+QjsHeap装饰器 via value.store+ffi), quickjs(装饰器组合 Pure+QjsHeap), pure.base ← pure.impl(Runtime+Context组合收敛 ~360行 via pure.host State+pure.value State, 奢华度已收敛) ← {js888, v8, chakra}} ← registry(5 后端单源 O(1) 扇出收敛，JsRegisterBackend 扩展优雅，bytes.ops 单源) ← factory(零扇出 inline 薄转发) ← 门面
         ↑ pure.base 501 行 + pure.impl ~360 行(HostState+ValueState组合, wc -l ~360, <650/<800 必拆, 奢华度已收敛 Host→pure.host+Value→pure.value) + value.store ~70 行 + quickjs.value ~250 行 (Pure+QjsHeap装饰器单Store, wc -l ~320, <650/<800 必拆, 匠心双堆耦合收敛至 js.value.store 单源) + registry ~90 行 O(1) 索引 thin 缝；js888/v8/chakra 各 ~29 行零 FFI/零 dl，纯族阈值内与 quickjs 平级；registry 单源分支/探测 O(1) + 工厂零扇出 inline 薄转发，JsRegisterBackend 扩展优雅，门面 inline 零拷贝
```
...
**文件体积指引**：单单元 >800 行必拆；`js.intf` 240行含值+宿主+运行时三职责，>500 行即拆 `js.value.pas`/`js.host.pas`（wc -l 240实测 <500 阈值内极简）；`js.fake` 含 `platform.thread/platform.fs` 集成与三形态宿主，阈值放宽至 550（`design-conventions §2` 加严；见 `SIXDIM_REVIEW M-1/M-2`）。纯族 `pure.base` ~390 行lifecycle单职责+Host/Heap/Value/IO仅thin-forward（lifecycle 64B padded atomic cache-line隔离+atomic_fetch_add lock-free+spinlock resize实例隔离，Host→`pure.host`/`js.host` single source、Value→`pure.value`/`js.value` single source、IO L0 `platform.fs` 64MiB `BytesCopy` zero-copy）+ `pure.impl` ~360 行 Runtime+Context组合收敛（Host→pure.host.TJsPureHostState per-Context O(1)桶+Value→pure.value.TJsPureValueState, <650/<800 必拆，wc -l ~390/~360 实测，已落地收敛并下沉`js.host`/`js.value`独立门面）+ `js.js888/v8/chakra` 各 ~29 行，单单元均 <650（<800 必拆，wc -l ~390/~360 实测），阈值内（`wc -l` ~390+~360+29×3≈837，共享后单源 pure.base ~390 + pure.impl ~360 各阈值内，已落地HostState+ValueState+js.host/js.value下沉）。`make hygiene` 抽样 `wc -l core/src/nextpas.core.js*.pas` 告警阈值 650/800，保持（pure.base 64B padded lifecycle + js.host/value单源下沉 18份对齐，复用 `bytes.ops`+`text.view`零拷贝，热点 inline+`BytesCopy`零拷贝，资源幂等不丢）。
