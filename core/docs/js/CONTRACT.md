...
| `js.pure.impl` | 纯模板单源（Runtime+Context，Host/Heap/Value/IO 四职责聚合，`TJsPureContext` 分组标记阈值 650 内（<800 必拆，wc -l 407 实测，奢华度临界→分组收敛，超阈预案 `js.value`(Heap/Value)+`js.host`(Host/Call)），`js888/v8/chakra` 三纯后端 95% 复用，零 FFI/零 `platform.dl`，`bytes.ops` 几何扩容与 `text.view` 零拷贝直通，热点 inline+`Move` 零拷贝，资源 `JsPureClose` 幂等不丢，守 L0–L3） | `js.base/intf`、`js.pure.base`、`text.view`、`json`、`platform.thread/fs`（L0 直读） | `platform.dl`、`*.ffi`、`webview.*` |
...
```
base ← intf ← {fake, quickjs.ffi, value.store(Pure Heap/Global via pure.base), quickjs.value(Pure+QjsHeap装饰器 via value.store+ffi), quickjs(装饰器组合 Pure+QjsHeap), pure.base ← pure.impl(Runtime+Context四职责分组 407行) ← {js888, v8, chakra}} ← factory ← 门面
         ↑ pure.base 501 行 + pure.impl 407 行 + value.store ~70 行 + quickjs.value ~250 行 (Pure+QjsHeap装饰器单Store, wc -l ~320, <650/<800 必拆, 匠心双堆耦合收敛至 js.value.store 单源)；js888/v8/chakra 各 ~29 行零 FFI/零 dl，纯族阈值内与 quickjs 平级；factory 单源分支/探测，门面 inline 零拷贝薄转发
```
...
**文件体积指引**：单单元 >800 行必拆；`js.intf` 240行含值+宿主+运行时三职责，>500 行即拆 `js.value.pas`/`js.host.pas`（wc -l 240实测 <500 阈值内极简）；`js.fake` 含 `platform.thread/platform.fs` 集成与三形态宿主，阈值放宽至 550（`design-conventions §2` 加严；见 `SIXDIM_REVIEW M-1/M-2`）。纯族 `pure.base` 501 行四职责聚合 Host/Heap/Value/IO（IO 仅 L0 `platform.fs` 直读，非 dl，64MiB限流）+ `pure.impl` 407 行 Runtime+Context四职责分组（奢华度临界已收敛分组标记，<650/<800 必拆，wc -l 407 实测，超阈预案 `js.value`(Heap/Value)+`js.host`(Host/Call) 就绪）+ `js.js888/v8/chakra` 各 ~29 行，单单元均 <650（<800 必拆，wc -l 501/407 实测），阈值内（`wc -l` 实测 501+407+29×3≈995，共享后单源 pure.base 501 + pure.impl 407 各阈值内，超阈即拆 `js.value`+`js.host`）。`make hygiene` 抽样 `wc -l core/src/nextpas.core.js*.pas` 告警阈值 650/800（`js.fake` 146，其余 500，纯族 501/407 行体量同步 BENCHMARKS 1.7，复用 `bytes.ops` 单源与 `text.view` 零拷贝视图，热点 `JsPureFindHostView/JsPureNew*` inline + `Move` 零拷贝，资源 `try-finally/JsPureClose/FreeAndNil` 不丢，守 L0–L3 与四件套例外，`JsPureDoEval` 单遍谓词缓存避免重复 SIMD 扫面，常驻 hash 桶索引预案就绪，魔串常量化+单加号 parser 候选）。
