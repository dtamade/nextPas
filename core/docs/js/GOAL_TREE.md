...
| 级别 | 条件 | 证据 |
|------|------|------|
| `source-contract` | 四件套门面纯聚合 + L2 只依赖 L0-L1：`base/intf` 不含后端符号（含纯后端预留不暴露 `JSValue`），`ffi` 零逻辑仅 `cdecl`，`loader` 唯一 `platform.dl`（禁 `DynLibs`），纯族 `pure.base` 单源 352行阈值550内 + `js.js888/v8/chakra` 零FFI/零dl恒可用 | `check_source_contracts.py` pass + `wc -l` 阈值550 + `grep platform.dl` 0 |
...
| `Production Ready` | `S2` 联动全绿 + `bench_eval` 5后端基线落库（含纯/QuickJS 双基线，BENCHMARKS 1.3 均值 ~660ns） + `hygiene/source-contract` 双 pass + 稳定性资源可释放 `Close` 幂等 + 文档 18份对齐 | 13+3 门 + bench |
