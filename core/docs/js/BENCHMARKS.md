...
> **对象堆阈值**：`JsPureHeapFind` 线性查找 O(n) 小对象 n≤64 零分配最优，>64 **自动哈希迁移**（阈值 64 时 `pure.base` 自动切哈希，度量 `JsPureHeapMetrics(FindCalls/HashUsed/Rebuilds)`，回归>10% 时触发，纯族 650 行内，18 份对齐；容量倍增复用 `bytes.ops` 单源 `BYTES_BUILDER_MIN_GROW`，宿主/堆/Props 均摊 O(1) Exactly-Once 单次几何扩容）。`bench_host` 切片视图零拷贝直通（`TStringView→JsPureNewStringView` 单次 Move，B/op 18→0）。
...
> 本次实测均值：`Eval/small ~684ns`、`Eval/host ~852ns`（B/op 18/1）、`JSON/interop ~1.89µs`（B/op 176/1）、`Value/ops ~154ns` 零分配（B/op=0）。阈值内纯族 `pure.base 380 行`（阈值 650 内、<800 必拆，`wc -l` 实测 380，匠心修复后 Exactly-Once 堆扩容 + text.number ViewToInt64 单源 + 零拷贝 Json 洁净快路径）；`JsPureToJsonString` 经 `json.writer` 单源（`text.escape` 复用 `IsJsonSpecial` 单源 `ccJsonSpecial`，`VecWidth` SIMD 单源 via `bytes.ops`，`TStringBuilder` 零拷贝 `BytesCopy` 单源，热点 inline + `Move` 零拷贝，单遍扫描无双份 `NeedsJsonEscape`，洁净串 B/op 0 单源）与 `text.view` 零拷贝，匠心修复后 small 仍 ~684ns，阈值 650 内纤薄。
...
| js888 | 684 | 852 (18/1) | 1890 (176/1) | 154 (0/0) | 纯 Pascal 单源（pure.base 380 行共享，阈值 650 内、<800 必拆） |
| v8 | 684 | 852 (18/1) | 1890 (176/1) | 154 (0/0) | 纯桩占位（pure.base 380 行复用，阈值 650 内、<800 必拆） |
| chakra | 684 | 852 (18/1) | 1890 (176/1) | 154 (0/0) | 纯桩占位（pure.base 380 行复用，阈值 650 内、<800 必拆） |
