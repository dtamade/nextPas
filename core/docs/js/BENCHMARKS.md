...
> **对象堆阈值**：`JsPureHeapFind` 线性查找 O(n)，小对象 n≤32 零分配最优，>64 建议哈希迁移（阈值 >64 实测或回归>10% 时迁移至哈希，纯族 481 行阈值550内，18 份对齐）。
...
> 本次实测均值：`Eval/small ~684ns`、`Eval/host ~852ns`（B/op 18/1）、`JSON/interop ~1.89µs`（B/op 176/1）、`Value/ops ~154ns` 零分配（B/op=0）。阈值内纯族 `pure.base 481 行`（阈值 550 内，`wc -l` 实测）；`JsPureToJsonString` 快路径（无转义 `'"'+S+'"'`）使 small 由 716ns 回落至 684ns（-4.5%），`r8` 回归已收敛。
...
| js888 | 684 | 852 (18/1) | 1890 (176/1) | 154 (0/0) | 纯 Pascal 单源（pure.base 481 行共享，阈值 550 内） |
| v8 | 684 | 852 (18/1) | 1890 (176/1) | 154 (0/0) | 纯桩占位（pure.base 481 行复用，阈值 550 内） |
| chakra | 684 | 852 (18/1) | 1890 (176/1) | 154 (0/0) | 纯桩占位（pure.base 481 行复用，阈值 550 内） |
| quickjs | SKIP | SKIP | SKIP | SKIP | 无 `libquickjs.so`（探针 8 名完整，`NEXTPAS_JS_QUICKJS_REQUIRED=1` 时 fail-closed） |

> 纯族 `Value/ops` 零分配符合 `B/op=0` 断言（r9 同步，4 后端 `B/op=0` 恒成立）；`Eval/host` 的 18 B/op / 1 alloc 为宿主参 `TStringView→string` 单次分配，`JSON/interop` 176 B/op / 1 alloc 为 `JsonParse` 单次分配（B/op 18/176 对齐）。`Eval/small` 5 后端矩阵已按 r9 `bench_eval` 均值同步（均值 ~684ns；host ~852ns，JSON ~1.89µs，均含快路径）。详见 `build/bench-eval-*.json` 落盘，`pure.base 481 行`阈值 550 内（`wc -l core/src/nextpas.core.js.pure.base.pas`），`JsPureToJsonString` 快路径覆盖 `\b\f\n\r\t\"\\` 及 `\u0000-\u001F`，无转义时零 `TStringBuilder` 分配。
