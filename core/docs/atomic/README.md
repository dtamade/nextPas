...
Signed integer atomic arithmetic is defined as two-complement wrapping for the atomic RMW surface: `TAtomicInt32` uses modulo-2^32 arithmetic for `Increment`/`Decrement` and `FetchAdd`/`FetchSub`; root `atomic_increment`/`atomic_decrement`/`atomic_fetch_add`/`atomic_fetch_sub` on `Int32` follow the same contract. `FetchAdd` and `FetchSub` return the previous value, while `Increment` and `Decrement` return the new wrapped value.
...
`TAtomicInt64` and root `atomic_increment_64`/`atomic_decrement_64`/`atomic_fetch_add_64`/`atomic_fetch_sub_64` on `Int64` use modulo-2^64 two-complement arithmetic when that public 64-bit surface is compiled. Passing `Low(Int64)` to signed `FetchSub` is valid and does not become a checked-overflow exception under `{$Q+}`.
...
`TAtomicISize` and root `atomic_increment`/`atomic_decrement`/`atomic_fetch_add`/`atomic_fetch_sub` on `PtrInt` use modulo pointer-width two-complement arithmetic. The main-facade pointer byte-offset overload `atomic_fetch_sub(var Pointer; PtrInt)` uses the same checked-safe pointer-width subtraction delta; pointer bitwise overloads remain compat-only.
...
- On the **FPC host**, scalar RMW routes through the backend seam in `nextpas.core.atomic.core.pas` (`_backend_cmpxchg/xchg/xadd_i32/i64` + `_backend_read/write/full_barrier`, all `inline`), whose bodies delegate to FPC `Interlocked*` / barrier intrinsics. The LL/SC weak-CAS asm (AArch64/ARM/RISC-V), the lightweight `_backend_compiler_barrier`, and the `cpu_pause`/`cpu_prefetch_nta` hints also live in the seam (`cpu_prefetch_nta` wraps the FPC `Prefetch` intrinsic — PREFETCHNTA on x86, inlinable, no-op elsewhere; it replaced a hand-written asm block in `lockfree.base` that could not inline and prefetched its own argument slot instead of the target). The lockfree production surface is asm-free (line-level source-contract pinned in `test_lockfree`). `atomic.pas` / `atomic.types` no longer touch those intrinsics directly and define no assembler routines (source-contract pinned in `test_atomic`); the only backend residue outside atomic.core is the i386-only CMPXCHG8B/cpuid inline asm in `atomic.pas` (no i386 cross compiler available to verify a move). That is **not** a `uses SysUtils` violation, but it **is** a dual-compiler backend dependency until nextpas replaces the seam bodies (LLVM atomics / asm) with its own atomic lowering.
