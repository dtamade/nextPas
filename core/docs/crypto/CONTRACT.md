...
# bench 吞吐回归（默认 HEAPTRC_GATE=0 防失真，BENCH_HEAPTRC=1 强制 heaptrc0 闭环；泄漏由 test_aesgcm/test_tls13_* production gates + bench 泄漏变体覆盖：inline/零拷贝 bytes.ops 单源，O3 -Xs，±5% env noise，零豁免统一门禁）
make focused FOCUS=core/tests/nextpas.core.crypto/bench_tls13_record
# bench 泄漏变体：make -C core/tests/nextpas.core.crypto/bench_tls13_record clean test BENCH_HEAPTRC=1  # heaptrc 0
```
