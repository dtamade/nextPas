...
- S44 Solinas 终局：`nextpas.core.crypto.bigint` Solinas 归约 `p=2^384-2^128-2^96+2^32-1`（`GetP384BigNat/IsP384Modulus/BigNatShiftLeft/SolinasReduceP384`，`Low+High+P<<32-High<<32+High<<96+High<<128` 递归至 12 limbs，`Montgomery` 主路径保持零回退 `766ms`，`Solinas` 单独验证 `1.22s` 仍 77 全绿作为可验证备用；`IsP384Modulus` 分流备用，预留无分配定长优化空间）。
