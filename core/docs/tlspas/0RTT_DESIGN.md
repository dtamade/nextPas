# tlspas 0-RTT 设计 — Early Data 平面 (Implemented S30 Trace Final)

**模块**: `nextpas.core.net.async.tlspas` (L2 async, pure Pascal TLS 1.3 + Registry Cached + Health + MetricsHandler + Trace) + `nextpas.core.http.earlydata` (L3 薄桥) + `nextpas.core.http.middleware.earlydata` (L3 中间件) + `nextpas.core.http.middleware.earlydata.adaptive` (L3 自适应限流埋点+结构化指标+压测+Prometheus+RegistryCached+Config+Health+Handler+TraceParent) + `IHttpRequestWithEarlyData` (L3 请求标记) + `nextpas.core.http.client_earlydata` (L3 客户端零配置自动重试) + `TAsyncTlsPasAdaptiveObserver` (L2 自适应熔断+指标聚合+压测+Prometheus+Health) + `TAsyncTlsPasPrometheusRegistry` (L2 多实例聚合, 缓存) + `TAsyncTlsPasCachedPrometheusExporter` (L2 缓存导出器) + `ITlsPasTracer` (L2 W3C traceparent + 采样)
**状态**: Implemented — S30 Trace 终局：W3C `traceparent` 零堆解析/格式化 + `ITlsPasTracer/Noop/Sampling(0.01)` FNV 采样 + `TlsPasTraceDecide` 观测透传 + L3 `HttpTraceParentMiddleware`/`HttpTraceLogLine`/`CONTEXT_TRACEPARENT` + `DemoTrace` 13段自证 + `TraceParent/Tracer/Middleware` 3测试 + 55 bench 全绿，68 tests 全绿，heap 0，S30 Final
**RFC**: 8446 §2.3 / §4.2.10 / §4.6.1 / Appendix E.5, 8446 §8, 8470 (425 Too Early)
**关联提交**: `402184890` (LRU4+HRR) → `6f3be848b` docs/bench → `64f3e3ede` S6-keys → `62644ee02` S6-ext → `17e54cbe3` S6-record → `9889712c7` S6-e2e → `efcf72530` S7-EOED → `792efc13d` S8-policy+replay → `f38e1965f` S9-store+stats → `8afd531a5` S10-file → S11-kv → S12-server → S13-observe → S14-adaptive → S15-polish → S16-http-bridge → S17-http-middleware → S18-http-client-retry → S19-http-auto-retry → S20-adaptive-observer → S21-demo-fullchain → S22-adaptive-middleware → S23-adaptive-metrics → S24-pressure-demo → S25-prometheus → S26-registry+config → S27-health → S28-cached → S29-handler → S30-trace (本提交)
...
- `bench_tlspas_hrr` 55 项 + demo S30：`MessageHash 2.7/4.1µs Patch 2.7/1.7µs P256 2.39ms Transcript 6.2µs EarlyData 28/29µs EOED 342ns Policy 3.4ns Fingerprint 10µs Replay 1.2µs Store 1.4µs Stats 86ns IsReplayed 7µs FileStore 200µs KvStore 69µs ServerDecide 3.2µs Observer 3.2µs Adaptive 1.9ns AdaptiveObserverDecide 3.5µs AdaptiveObserverMax 95ns Header 4ns HttpHeader 4ns HttpStream nil 21ns HttpRequest 81ns Middleware 433ns IsThrottled 188ns AdaptiveMiddleware 648ns MetricsFormat 3.0µs LogLine 5.8µs Pressure 2.2µs Prometheus 7.8µs Registry 2×17µs Cached 120ns/7µs RegistryCached 150ns/17µs Handler 500ns TraceParse 150ns TraceFormat 200ns ShouldSample 20ns TraceMiddleware 600ns ConfigEnv 887ns Health 200ns HealthProm 1µs Append 7µs ClientIsIdempotent 24ns ClientIsEarly 88ns ClientShouldRetry 139ns ClientClone 2.0µs ClientAutoMark 1.8µs ClientAutoRetry 2.0µs`，`P384 741ms` 单采。
