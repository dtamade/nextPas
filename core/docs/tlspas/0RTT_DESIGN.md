...
- `bench_tlspas_hrr` 64 项 + demo S42：`MessageHash 2.6/3.4µs Patch 2.2/1.2µs P256 1.9ms Transcript 4.7µs EarlyData 24/26µs EOED 354ns Policy 3.5ns Fingerprint 11µs Replay 1.2µs Store 1.2µs Stats 33ns IsReplayed 3.1µs FileStore 191ms KvStore 78µs ServerDecide 4.6µs Observer 3.6µs Adaptive 2.6ns AdaptiveObserverDecide 3.4µs AdaptiveObserverMax 99ns Header 4ns HttpHeader 4ns HttpStream nil 23ns HttpRequest 157ns Middleware 427ns IsThrottled 181ns AdaptiveMiddleware 630ns MetricsFormat 2.9µs LogLine 8.6µs Pressure 2.0µs Prometheus 7.5µs Registry 2×17µs Cached 348ns/10µs RegistryCached 764ns/18µs Handler 1.1µs TraceParse 328ns TraceFormat 2.1µs ShouldSample 26ns TraceMiddleware 3.4µs SpanExport 117ns SpanJSON 583µs Tracez 566µs AdaptiveSampling 4ns AdaptiveTracer 3ns OTLP 689µs Gauge 1.3µs SetRate 36ns Snapshot 1.4µs ConfigEnv 885ns Health 1.4µs HealthProm 2.9µs Append 8.4µs ClientIsIdempotent 25ns ClientIsEarly 146ns ClientShouldRetry 134ns ClientClone 2.2µs ClientAutoMark 2.3µs ClientAutoRetry 2.0µs`，`P384 windowed ~500ms` 估（`HRR 421ms` 已含双次握手，单次标量乘 `~500ms`，累计 `12.4s→0.5s 24×` 趋近 P-256 的 `1.9ms` 仍 260×，纯 Pascal BigInt 极限，生产建议 P-256 优先）。
