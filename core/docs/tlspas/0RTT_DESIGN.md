# tlspas 0-RTT 设计 — Early Data 平面 (Implemented S45 Solinas-Fast Final)

**模块**: `nextpas.core.net.async.tlspas` (L2 async, pure Pascal TLS 1.3 + Registry Cached + Health + MetricsHandler + Trace + Span + AdaptiveSampling + PrometheusGauge + SetRate + Snapshot + P-384 Jacobian/Windowed/Fixed-Base + Montgomery Cache + Solinas-Fast) + `nextpas.core.http.earlydata` (L3 薄桥) + `nextpas.core.http.middleware.earlydata` (L3 中间件) + `nextpas.core.http.middleware.earlydata.adaptive` (L3 自适应限流埋点+结构化指标+压测+Prometheus+RegistryCached+Config+Health+Handler+TraceParent+Tracez+OTLP+IsProm) + `IHttpRequestWithEarlyData` (L3 请求标记) + `nextpas.core.http.client_earlydata` (L3 客户端零配置自动重试) + `TAsyncTlsPasAdaptiveObserver` (L2 自适应熔断+指标聚合+压测+Prometheus+Health+Snapshot) + `TAsyncTlsPasPrometheusRegistry` (L2 多实例聚合, 缓存) + `TAsyncTlsPasCachedPrometheusExporter` (L2 缓存导出器) + `ITlsPasTracer` (L2 W3C traceparent + 采样) + `ITlsPasSpanExporter` (L2 OTEL Span 环形) + `TAsyncTlsPasAdaptiveTracer` (L2 自适应采样+快照单锁) + `nextpas.core.crypto.p384` (L2 P-384 Jacobian/Windowed/Fixed-Base) + `nextpas.core.crypto.bigint` (L0 Montgomery Cache + Solinas-Fast P-384)
**状态**: Implemented — S45 Solinas-Fast 收敛：`demo S36 16段自证闭环` + `P-384 Jacobian 单逆元` + `BigInt 别名安全` + `Montgomery threadvar 缓存` + `4-bit 窗口化` + `G 固定基点预计算 1P..15P` + `Solinas-Fast p=2^384-2^128-2^96+2^32-1 归约` (Montgomery 主路径保持 `HRR 397ms` 零回退，Solinas-Fast 零分配 `AddShifted/SubShifted` 单缓冲原地 `1.22s→770ms` 与 Montgomery `766ms` 对等，`High<<32/96/128` 零分配，主路径切 Solinas-Fast `770ms` 稳定) + `0RTT body S45 同步` + `77 tests` 全绿，`64 bench` 全绿，heap 0，S45 Final
**RFC**: 8446 §2.3 / §4.2.10 / §4.6.1 / Appendix E.5, 8446 §8, 8470 (425 Too Early)
**关联提交**: `402184890` (LRU4+HRR) → `6f3be848b` docs/bench → `64f3e3ede` S6-keys → `62644ee02` S6-ext → `17e54cbe3` S6-record → `9889712c7` S6-e2e → `efcf72530` S7-EOED → `792efc13d` S8-policy+replay → `f38e1965f` S9-store+stats → `8afd531a5` S10-file → S11-kv → S12-server → S13-observe → S14-adaptive → S15-polish → S16-http-bridge → S17-http-middleware → S18-http-client-retry → S19-http-auto-retry → S20-adaptive-observer → S21-demo-fullchain → S22-adaptive-middleware → S23-adaptive-metrics → S24-pressure-demo → S25-prometheus → S26-registry+config → S27-health → S28-cached → S29-handler → S30-trace → S31-span → S32-adaptive-sampling → S33-polish → S34-consistent → S35-snapshot → S36-landing-sync → S39-cache → S40-jacobian → S41-mont-cache → S42-windowed → S43-fixed-base → S44-solinas → S45-solinas-fast (本提交)
...
## 2. 落地状态（截至 S36 Landing Sync Final）
...
- S33 Polish 终局：抽取 `TlsPasPrometheusGauge` 统一 gauge 复用 + `TlsPasTryExportSpansToFile` 原子清理无 `.tmp` 泄漏 + `HttpIsPromRequest` 去重分支 + `S33Polish` 边界全覆盖（gauge/clamp/空 OTLP/nil path/tmp 清理）+ `Prometheus Gauge 62` 项，`75 tests` 全绿。
- S34 Consistent 终局：`SetRate/UpdateConfig` 同步 `Inner.Rate` 避免漂移 + `Thr floor 0.05` 防止小 `BaseRate` 抖动误触发 + `OTLP resource.service.name=nextpas` 完整 + `C_TLSPAS_PROM_PREFIX` 常量收敛 + `S34Consistency` fuzz/边界覆盖，`76 tests/63 bench` 全绿。
- S35 Snapshot 终局：新增 `TTlsPasAdaptiveSnapshot{Metrics,Health}` + `TAsyncTlsPasAdaptiveObserver.GetSnapshot` 单锁原子快照，`GetAdaptiveRate` 由双锁 5 次收敛为快照单锁 2 次，采样热点降 60%；`S35Snapshot` 原子一致性单测 + `Snapshot 2ns` bench，`77 tests/64 bench` 全绿，`demo S35` 自证 `Polish+S34+S35` 收敛。
- S39 Cache 终局：`nextpas.core.crypto.p384` 常量缓存（`GP384_P/N/B/GX/GY/PMinus2/NMinus2` 惰性缓存，`P384_GX_HEX/GY_HEX` 预计算）+ `BigInt Montgomery` 复用，`P384 single 1001ms→760ms` 4.6% 提升，`77 tests` 全绿。
- S40 Jacobian 终局：`nextpas.core.crypto.p384` Jacobian 坐标单逆元（`TJac384{X,Y,Z}` + `JacIsInfinity/FromAffine/Double/Add/ToAffine` + `Montgomery ladder CTSwapJac`，384 位标量 1 逆元归一，384 次 `Double/Add` 零逆元，末尾单 `ModExp(P-2)` + `Zinv2/Zinv3` 归一；修复 `BigInt out 别名清零` 根因（`out` 参数同变量传参时 `FPC` 在入口清零导致 `Z3*2` 归零至 `len1 0`，改 `BigInt` 读参先于清零 + `p384` 调用点去别名 `Tmp` 中转）+ `TryP384ScalarMult→Jacobian` 主路径，`P384 single 760ms→739ms` 再提 2.8%（累计 26% vs 1001ms 基线），`affine vs jacobian 标量 1CF7…` 一致性自证，`77 tests` 全绿，`64 bench` 全绿。
- S41 Montgomery Cache 终局：`nextpas.core.crypto.bigint` `threadvar` Montgomery 上下文缓存（`GLatestMontModulus/Ctx/Valid` + `TryGetCachedMontCtx` 命中复用 `R/R2/NPrime`，`BigNatCompare` 命中判定，`TryBigIntModMul/ModExp/RSA` 三处复用；`Init` 原 768 次 `DoubleMod` 每 `ModMul` 重算，现单次缓存后零重算，`P384KeyPairRoundtrip 9.11s→1.33s` 6.8×，`HRRP384Live 4.51s→0.66s` 6.7×，`77 tests` 双跑 heap 0，`bench 64` 全绿，`P384 single` 稳定 743ms 实验性）。
- S42 Windowed 终局：`nextpas.core.crypto.p384` 4-bit 窗口化 Jacobian 标量乘（`Table[0..15]` 预计算 `1P..15P` 经 14 次 `JacAdd`，左到右 96 窗口：首窗零 `Double`，后 95 窗口各 4 次 `Double` + 条件 `Add(Table[w])`，`try` 别名安全 `Tmp` 中转；`384 Doubles + ≤96 Adds` 替代 ladder `384 Doubles +384 Adds`，`P384KeyPairRoundtrip 1.33s→0.78s 1.7×`、`HRRP384Live 669ms→421ms 1.6×`，累计 `12.4s→0.78s 15.9×` vs 基线，`affine/ladder/window` 三路 `1CF7…/1/2` 一致性自证，`77 tests` heap 0，`bench 64` 全绿，窗口表 16×48B 零堆外常驻）。
- S43 Fixed-Base 终局：`nextpas.core.crypto.p384` 固定基点 `G` 预计算（`GP384BaseTable[0..15]` 全局 `1P..15P` 惰性一次 `EnsureP384BaseTable` 经 14 次 `JacAdd`，`IsP384BasePoint` 分流：`G` 走缓存表、任意点走即时表；`P384KeyPair 780ms→764ms 1.02×`、`HRRP384 421ms→397ms 1.06×`，累计 `12.4s→0.76s 16.3×`，`77 tests` heap 0，`bench 64` 全绿，表 16×48B 常驻零分配）。
- S44 Solinas 终局：`nextpas.core.crypto.bigint` Solinas 归约
- S45 Solinas-Fast 终局：`nextpas.core.crypto.bigint` 零分配 Solinas-Fast（`BigNatAddShifted/SubShifted` 单缓冲原地，`Low+High+P<<32-High<<32+High<<96+High<<128` 零 `ShiftLeft` 分配，递归至12 limbs，`Montgomery 766ms→Solinas 770ms` 对等，主路径切 Solinas-Fast，`77 tests` 全绿） `p=2^384-2^128-2^96+2^32-1`（`GetP384BigNat/IsP384Modulus/BigNatShiftLeft/SolinasReduceP384`，`Low+High+P<<32-High<<32+High<<96+High<<128` 递归至 12 limbs，`Montgomery` 主路径保持零回退 `766ms`，`Solinas` 单独验证 `1.22s` 仍 77 全绿作为可验证备用；`IsP384Modulus` 分流备用，预留无分配定长优化空间）。
- `bench_tlspas_hrr` 64 项 + demo S43：`MessageHash 2.6/3.4µs Patch 2.2/1.2µs P256 1.9ms Transcript 4.7µs EarlyData 24/26µs EOED 354ns Policy 3.5ns Fingerprint 11µs Replay 1.2µs Store 1.2µs Stats 33ns IsReplayed 3.1µs FileStore 191ms KvStore 78µs ServerDecide 4.6µs Observer 3.6µs Adaptive 2.6ns AdaptiveObserverDecide 3.4µs AdaptiveObserverMax 99ns Header 4ns HttpHeader 4ns HttpStream nil 23ns HttpRequest 157ns Middleware 427ns IsThrottled 181ns AdaptiveMiddleware 630ns MetricsFormat 2.9µs LogLine 8.6µs Pressure 2.0µs Prometheus 7.5µs Registry 2×17µs Cached 348ns/10µs RegistryCached 764ns/18µs Handler 1.1µs TraceParse 328ns TraceFormat 2.1µs ShouldSample 26ns TraceMiddleware 3.4µs SpanExport 117ns SpanJSON 583µs Tracez 566µs AdaptiveSampling 4ns AdaptiveTracer 3ns OTLP 689µs Gauge 1.3µs SetRate 36ns Snapshot 1.4µs ConfigEnv 885ns Health 1.4µs HealthProm 2.9µs Append 8.4µs ClientIsIdempotent 25ns ClientIsEarly 146ns ClientShouldRetry 134ns ClientClone 2.2µs ClientAutoMark 2.3µs ClientAutoRetry 2.0µs`，`P384 windowed+fixed-base ~500ms` 估（`HRR 397ms` 双握手，单次 `~500ms`，累计 `12.4s→0.76s 16.3×`，纯 Pascal 极限，生产 P-256 优先）。
...
| P384 single (outside) | 739ms | 26% vs 1001ms 基线 | Jacobian 单逆元 experimental |
