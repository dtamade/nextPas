# tlspas 0-RTT 设计 — Early Data 平面 (Implemented S9)

**模块**: `nextpas.core.net.async.tlspas` (L2 async, pure Pascal TLS 1.3)
**状态**: Implemented — S9 可观测与可注入收口：重放存储接口化 + Stats + 本地去重回退，默认 1-RTT 零开销，early_data 显式幂等启用，服务端 LRU64/10min 可配
**RFC**: 8446 §2.3 / §4.2.10 / §4.6.1 / Appendix E.5, 8446 §8
**关联提交**: `402184890` (LRU4+HRR) → `6f3be848b` docs/bench → `64f3e3ede` S6-keys → `62644ee02` S6-ext → `17e54cbe3` S6-record → `9889712c7` S6-e2e → `efcf72530` S7-EOED → `792efc13d` S8-policy+replay → S9-store+stats (本提交)
...
- S7 完整度：`EndOfEarlyData` (`0x05` 4B) 在 `EarlyDataAccepted` 时纳入 Finished 转录哈希并与 Finished 同记录单刷，`BuildTLS13EndOfEarlyDataHandshake`/`TryParse` 单测与 `FinishedWithEOEDDiffers` 合成自证已覆盖，`bench EOED build 229ns`。
- S8 策略与重放：`TlsPasIsEarlyDataAllowed` 纯函数零分支准入（`HasMaxEarlyData && 0<Size<=16384 && 0<Len<=Size && Len<=16384`）供 `AllocHsCtx` 与上层复用；`TlsPasComputeEarlyDataFingerprint:=SHA256(ticket||early)` 32B 稳定指纹；`TAsyncTlsPasReplayCache` LRU64 窗口 10min、Mutex 保护、`CheckAndAdd` 命中即重放、过期清扫与 `SecureZero`，`bench Fingerprint 2.97µs / ReplayCache 894ns`。
- S9 可观测与可注入：`ITlsPasReplayStore` 接口化 + `TAsyncTlsPasReplayStats{ Hits/Misses/Evictions/Expiries/Current }` + `TlsPasIsEarlyDataReplayed` 帮手；`TAsyncTlsPasReplayCache` 改 `TInterfacedObject` 实现接口，`Clear` 重置 Stats，`CheckAndAdd` 维护 Hits/Misses/Evictions/Expiries；`TAsyncTlsPasClientOptions.ReplayStore` 可选注入（nil 零开销），命中则 `AllocHsCtx` 本地回退为 1-RTT（防同进程误重放），`bench ReplayStore interface 873ns (≈零开销) / GetStats 34ns / IsReplayed 3.19µs`。
- `bench_tlspas_hrr` 15 项：`MessageHash 2.75/3.35µs Patch 1.33/2.37µs P256 1.90ms Transcript 2.89µs EarlyData 14.8/19.5µs EOED 229ns Policy 2.2ns Fingerprint 2.97µs Replay 894ns Store 873ns Stats 34ns IsReplayed 3.19µs`，`P384 767ms` 实验性单采明示。
...
- S8 前本层不提供去重票据 (single-use ticket 需服务端配合)；客户端侧仅保证：同一 PSK 的 0-RTT 在一次 `AsyncTlsPasConnect` 中最多发送一次，超时/失败不自动重放。S8 新增 `TAsyncTlsPasReplayCache`（LRU64/10min 窗口、指纹 `SHA256(ticket||early)`、Mutex）供服务端去重与客户端单票单用检测，命中即重放、不持有密钥。S9 起接口化 `ITlsPasReplayStore` 支持服务端全局/跨实例注入，Stats 可观测，客户端可选本地去重（`ReplayStore` 非 nil 时 `AllocHsCtx` 命中即回退 1-RTT，防同进程误重放，nil 时零开销）。
...
## 8. 实测性能（bench_tlspas_hrr 15 项，120ms×3，2026-08-28）

| 项 | ns/op | 吞吐 | 备注 |
|----|-------|------|------|
| MessageHash SHA256 | 2754 | 363K ops/s | |
| MessageHash SHA384 | 3356 | 297K ops/s | |
| Patch X25519→P384 | 2372 | 421K ops/s | |
| Patch P384→P256 | 1332 | 750K ops/s | |
| P256 ECDHE keypair | 1.90ms | 525 ops/s | |
| Transcript | 2891 | 345K ops/s | |
| EarlyData SHA256 | 14862 | 67K ops/s | 单次 |
| EarlyData SHA384 | 19598 | 51K ops/s | 单次 |
| EOED build (4B) | 229 | 4.35M ops/s | S7 |
| Policy allowed | 2.2 | 451M ops/s | S8 纯分支 |
| Fingerprint | 2972 | 336K ops/s | SHA256 |
| ReplayCache | 893 | 1.11M ops/s | LRU64 |
| ReplayStore interface | 873 | 1.14M ops/s | ≈零开销 |
| ReplayStats GetStats | 34 | 29M ops/s | Mutex 拷贝 |
| IsEarlyDataReplayed | 3195 | 312K ops/s | 指纹+窗口 |
| P384 single (outside) | 767ms | — | experimental |

`EarlyData`/`EOED`/`Policy`/`Fingerprint`/`ReplayCache`/`Store`/`Stats` 均仅 0-RTT 路径单次或按需触发，不入 1-RTT 热路径；`Policy 2.2ns`/`Stats 34ns` 零堆、接口派发与类直调差 `<3%`、1-RTT `AsyncWrite` 差异 `<1%`。

## 9. 风险与回滚

- 重放：S8 前默认关闭 + 文档强约束 + `Idempotency-Key` 建议；S8 后 `ReplayCache` 指纹窗口去重，不持密钥，窗口 10min 可配，单 PSK 0-RTT 最多一次不重放。S9 后接口化 `ITlsPasReplayStore` 支持跨进程/全局注入与 Stats 观测，`ReplayStore` nil 时零开销，命中本地回退 1-RTT。
- 扩展错位：复用 `PSK 尾部` 扫描，`EarlyDataExtension` 单测覆盖 binder；`TlsPasIsEarlyDataAllowed` 四重限幅防超限，`TlsPasIsEarlyDataReplayed` 指纹封装。
- 回滚：任一 S6-S9 切片可独立 revert，`AllowEarlyData` 默认为关，`ReplayStore` 默认为 nil，revert 后行为与 `bcdc562` 一致；`P384` 仍实验性。

---

*本设计遵循 `core/AGENTS.md` 与 `core/docs/design-conventions.md`：L2 仅依赖 L0-L1 与 `tls13.*` 原语，零 OpenSSL，不引入新分层。S6-S9 已闭环（S7 EOED + S8 策略与重放 + S9 接口/Stats/注入），后续可按需接持久化重放存储（文件/外部 KV）与服务端 `EarlyData` 限额动态。*
