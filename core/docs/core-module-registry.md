...
| `git` | L2 | git/libgit2 backend | yes | L0-L1 plus same-layer one-way `fs`/`compress`(`Deflate*`)/`hash`/`zlib`/`checksum`(`Adler32Update`) — `git.native.zlib` only `DeflateCompress`/`DeflateDecompressWithEndPos`/`DeflateDecompressPtrWithEndPos`/`DeflateDecompressPtrWithEndPosToBuffer`/`DeflateDecompressPtrPrefix` + `Adler32Update` single-source passthrough, zero handwritten deflate/adler loop, anchor `git-native-zlib-l2-exempt` (C5) plus libgit2 FFI owner | draft |
...
| `js` | L2 | JS execution engine (QuickJS FFI plus pure Pascal backends `nextpas.core.js.*`) | yes | L0-L1 plus json/text/mem owners; platform.dl (loader only); same-layer one-way `json` (single-point `js.intf` → `json.types` + `js.pure.value` → `json.writer`/`json.value`, cycle-gated, no reverse `json`→`js`) plus `text.view`/`text.builder`/`text.escape`/`text.number` via `pure.value` single source and `mem.dynarray` owner; `platform.fs` (`TryEvalFile` L0 direct 64MiB); `pure` ~40 standard facade (`pure.runtime`~45+`pure.context`~360) `pure.base←runtime/context←门面` + `pure.impl` compat alias | source-contract + focused-runtime |
...
| `ssh` | L2 | SSH-2 client protocol stack (`nextpas.core.ssh.*`; RFC 4251/4253 transport+cipher/kex/hostkey/channel/session+SFTP/agent/proxyjump async, pure Pascal, no C lib; 46 units, four-piece `base←intf/ffi←impl←facade`, `transport.core` single source, `bytes.ops` single source, `inline`/`zero-copy` preserved) | yes | L0-L1 plus L2 crypto/hash/compress/net owners (bytes/text/time seam; net via `net.ffi+intf` seam, compress via `compress.zlib.ffi` single entry, async peer `net.async.tcp` `IAsyncTcpStream/AsyncTcpDial(RFC8305)`, same-layer one-way, no L3) | focused-runtime |
...
| `text.wildmatch` | L0 | segment-oriented wildcard engine (`WildSegment/WildSegmentRange/WildSegmentsMatch/HasUnescapedSlash`, single source consumed via `text.pas` thin wrappers + `git.native.attributes` direct; plain-string `*`/`?` path glob single source is `text.strings.GlobMatch`, consumed by `bench.base`/`fs.glob`; `ssh.hostkey` keeps a private OpenSSH-semantics matcher since `[host]:port` brackets are literal there) | yes | L0 only (`base`) | source-contract |
