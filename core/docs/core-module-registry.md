...
| `vfs` | L2 | read-only virtual filesystem (`nextpas.core.vfs.*`, memtree/embedded/os/sub/mount/overlay + transform/compressed L3 单缝装饰器寄居，13 units + facade) | yes | L0-L1; os seam single fs/path L2→L2; embedded adds respack.reader; mount/overlay pure composite; transform/compressed: L3 单缝装饰器寄居 L2 家族（Registry 单缝白名单过渡，L7 到期拆分为 nextpas.core.vfs.decorator 独立 L3 族后移除白名单，单源决策器单流 4K HeaderPred + 大文件栈上 2 字节轻量预判零堆分配免 4K + GZIP_MAX 32MiB 对齐 canonical（base 为唯一字面量，compressed 经 vfs.base 单源别名复用不再双写，无直接 compress.base 依赖）+ bytes.ops inline 零拷贝单源、直接复用 transform.TRANSFORM_HEADER_PEEK 无本地别名，泛型路径输入/输出双 32MiB 防 bomb 统一，Stat/OpenRead 大文件解压一致性） | focused-runtime |
| `vfs` | L2 | read-only virtual filesystem (`nextpas.core.vfs.*`, memtree/embedded/os/sub/mount/overlay + transform/compressed L3 单缝装饰器寄居，13 units + facade) | yes | L0-L1; os seam single fs/path L2→L2; embedded adds respack.reader; mount/overlay pure composite; transform/compressed: L3 单缝装饰器寄居 L2 家族（Registry 单缝白名单过渡，L7 到期拆分为 nextpas.core.vfs.decorator 独立 L3 族后移除白名单，单源决策器单流 4K HeaderPred + 大文件栈上 2 字节轻量预判零堆分配免 4K + GZIP_MAX 32MiB 字面量数值对齐 compress.base canonical 单源、接口层无直接 compress.base 依赖 + bytes.ops inline 零拷贝单源、直接复用 transform.TRANSFORM_HEADER_PEEK 无本地别名，泛型路径输入/输出双 32MiB 防 bomb 统一，Stat/OpenRead 大文件解压一致性） | focused-runtime |
