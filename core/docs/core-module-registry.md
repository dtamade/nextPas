...
| `vfs` | L2 | read-only virtual filesystem (`nextpas.core.vfs.*`, memtree/embedded/os/sub/mount/overlay + transform/compressed L3 单缝装饰器寄居，13 units + facade) | yes | L0-L1; os seam single fs/path L2→L2; embedded adds respack.reader; mount/overlay pure composite; transform/compressed: L3 单缝装饰器寄居 L2 家族（Registry 单缝白名单过渡，L7 到期拆分为 nextpas.core.vfs.decorator 独立 L3 族后移除白名单，单源决策器单流 4K HeaderPred + 大文件 2 字节轻量预判免 4K + GZIP_MAX 32MiB via compress.base + bytes.ops 零拷贝单源，Stat/OpenRead 大文件解压一致性） | focused-runtime |
