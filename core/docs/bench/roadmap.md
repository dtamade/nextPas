...
## Phase 5: 统计能力深化 (2026-07-06)

**目标**: 在统计方法上显著超越 Go/Rust，成为最严谨的基准测试框架。

### 5.1 K-S 检验 (P1)

| ID | 功能 | 理由 | 对标 |
|----|------|------|------|
| P5-1 | 单样本 K-S 检验 | 通用分布检验，比 Shapiro-Wilk 更灵活 | 无（超越） |
| P5-2 | 两样本 K-S 检验 | 比较两个基准的分布是否相同 | 无（超越） |

### 5.2 Bootstrap 改进 (P2)

| ID | 功能 | 理由 | 对标 |
|----|------|------|------|
| P5-3 | Xoroshiro128+ PRNG | 替换 LCG，提升统计质量 | 无 |
| P5-4 | BCa Bootstrap | 偏差修正加速，小样本更准确 | R boot package |
| P5-5 | Bootstrap 假设检验 | 直接用 bootstrap 检验差异 | 无（超越） |

### 5.3 贝叶斯估计 (P2)

| ID | 功能 | 理由 | 对标 |
|----|------|------|------|
| P5-6 | 正态-正态共轭模型 | 贝叶斯均值估计 | 无（超越） |
| P5-7 | 可信区间 | 比置信区间更直观 | 无（超越） |
| P5-8 | 先验融合 | 融合历史数据改进估计 | 无（超越） |
