...
- **覆盖**：`Oid/Same:inline`（`inline` + `Move` 零拷贝，复用 `bytes.ops.SpanEqual` 单源，20-byte `TByteSpan` 三 QWord 对比）/ `Oid/IsValidHex|FromHex|ToHex`（not inline per red line 2, 40× loop/alloc+table exceeds I-Cache, zero-copy via `encoding.hex` HexVal/HexDecode single source + `bytes.ops` SpanCopy single source）/ `Kind/FromMode`（`inline`）/ `Zlib/Compress1K|Decompress1K`（`native.zlib → compress.Deflate*` 透传，`PByte+Len` 零拷贝）/ `Adler32/PByte64K:zero-copy|Bytes64K`（`PByte` 零拷贝单源 `checksum.adler32.Adler32Update(PByte,Len)`，`ADLER32_MOD/NMAX` 单源，不自建 65521 循环）/ `Wild/*`（`wildmatch` 单源 `inline WildSegment* / WildSegmentsMatch` via `text.wildmatch` + `bytes.ops` 零拷贝，`**` 目录通配）/ `Delta/Apply|ApplyReuse`（`TByteSpan` 零拷贝 + `GReuseBuf` 复用单源 `GitApplyDeltaInto` via `bytes.ops`）
- **阈值 SLO（绝对红线，ns/op / ops/sec，同机 `-O3 -Xs` 无 heaptrc 中位数，含 10-15% 抖动余量，`inline/零拷贝` 路径不回退，单源复用 `bytes.ops`/`checksum.adler32`/`compress`/`wildmatch`）**：`Oid/Same:inline` ≤ 80 ns/op（≥12.5 Mops/sec）；`Oid/IsValidHex|FromHex|ToHex` ≤ 150 ns/op（≥6.6 Mops/sec, not inline per red line 2 — 40× HexVal/alloc+table loop exceeds inline I-Cache benefit, bench阈与实现策略已对齐）；`Kind/FromMode:inline` ≤ 30 ns/op（≥33 Mops/sec）；`Zlib/Compress1K|Decompress1K` ≤ 15 µs/op（≥66 Kop/sec）；`Adler32/PByte64K:zero-copy|Bytes64K` ≤ 3 µs/op（≥333 Kop/sec, ~21 GB/s，零拷贝 `PByte+Len` 单源，`Bytes64K` 仅薄封装同阈）；`Wild/Segment:inline|Class|SegmentsMatch:**` ≤ 100 ns/op（≥10 Mops/sec）；`Delta/Apply|ApplyReuse:inline` ≤ 5 µs/op（≥200 Kop/sec）
