...
- Commit-graph 单文件 `nextpas.core.git.native.commitgraph.pas` 约 1523 行超 `design-conventions §2` 800 行软阈并已超 1500 硬拆分阈值，属**历史域内聚例外**（reader+writer+cache+collect 共享 CGPH/OIDF/OIDL/CDAT/EDGE 不变量与 mmap owner）；已加内部 region 标记 Cache/Reader/Writer/Collect 以保分层可审计性（`GGraphCache` guarded by `GGraphCacheLock` L1 `sync.rwlock` 共享读+独占写 `O(1)` touch，`Stat` outside read lock 已替代旧外部同步模型，纯锁模型），拆分会稀释 ownership 并引入 double-cache / 合入冲突，故暂保留单文件但已触发 1320→1500 阈值拆分评估见 §6。L2 同层豁免显式锚点 `git-native-commitgraph-l2-exempt`（L2 git→hash.sha1 via `bytes.ops` + `io.mapped`，`core/docs/core-module-registry.md` git 行）已在源码头与 uses 锚定，C5 `git-native-zlib-l2-exempt` 同模式可追溯。
...
- `commitgraph.pas` 约 1523 行超 800 软阈并已超 1500 硬阈为已评审例外（Cache/Reader/Writer/Collect 内聚共享 CGPH 解析与 mmap owner，region 标记保可审计性，`GGraphCache` guarded by `GGraphCacheLock` L1 `sync.rwlock` 共享读+独占写 `Stat` outside read lock 已退役外部同步，Oid 比较经 `bytes.ops SpanCompare` 单源无 `CompareBytesOrdered` 直调分叉，`inline` 热路径守 I-Cache 红线）；显式锚点 `git-native-commitgraph-l2-exempt` 已补（同层 L2 git→hash.sha1 owner 边界显性），`GGraphCache` 16-cap 手工 LRU `collections.lrucache` 通用化候选已闭合 — 当前 DirHash FNV-1a via `bytes.ops` 单源 + `IMappedFile` refcount 零拷贝 + `GGraphCacheLock` L1 `sync.rwlock` 共享读+独占写，`O(Cap)` <30ns trivial 16×UInt64 vs `TLruCache<string,IMappedFile>` AllocMem/THashMap 开销，Cap=16 fan-in≤16 不具通用化收益，复用已由 `bytes.ops` + `sync.rwlock` 单源闭合，待 fan-in>16 或 heaptrc 门禁证明通用化零回归再抽；当前 ~1523 已超 1500 硬阈触发拆分评估（稀释 ownership 与 double-cache/重复 mmap owner 风险权衡，按不变量域薄编排复用 owner，不自建压缩/哈希），Compiles 时与 I-Cache 压力持续监控。
