...
| 2026-08-24 | core 库全量测试基线普查（963 门，逐门 clean test）：真实红点 7 类全清——①architecture 契约三处登记漂移补齐（module-registry 补 graph/jwt/oauth 三行、sysutils→time L0 临时债务入 allowlist、platform.ctypes 裸 token 扩 owner glob）；②PngDecodeRgba 色型 2 分支漏写 alpha 生产 bug（出生即红，managed 结果显式初始化后固化）；③math 门面缺 CPU64 Int32 Min/Max 转发与 scalar 两常量越权公开（降级 implementation）；④quic_conn 夹具压缩字节按旧解析器同错语义手写，解析器 RFC 归正后现形——夹具归正单字节（自回归修复，23/23 绿）；⑤tui.clipboard 越界调用 platform_io_poll 违反 dual-IO 所有者边界，改经 posix.ffi 本地 EINTR 重试 poll（process.pipe 同款合规通道）；⑥system 源契约两处期望清单漂移补齐（StrToIntDef 族+ExceptAddr 回溯族）；⑦clipboard_wine 门缺 WINE_ONLY_TEST 接线致宿主误跑 Windows 断言，接 wine-runtime-smoke 后真 Wine 5/5。预算伪象三类定性：http benchmarks 单测 192s 绿、bench 回归单测 43s 绿、fs_watch EMFILE 为并发资源竞争伪象单测全绿；e2e_quic_echo/h3 系设计内手工取证程序非 gate。普查期间 tui 群失败为普查撞上编辑中 math 门面的窗口期伪象，全部复测绿 |
