...
- `nextpas.core.js.pure.base` 为纯后端共享基座 facade，非标准四件套命名（`pure.base` 薄转发复用 `js888/v8/chakra`，零 FFI/零 `platform.dl`（L0 `platform.fs` 仅 `TryEvalFile` 直读，非 dl，64MiB限流，`bytes.ops Move`零拷贝），四职责聚合 Host/Heap/Value/IO 阈值650内（<800 必拆，wc -l 501 实测，超阈预案 `js.value`+`js.host`），见 `core/docs/js/CONTRACT.md §1`；`pure` 为纯族前缀，`base` 为共享后缀，复用 `bytes.ops` 单源几何扩容与 `text.view` 零拷贝，热点 inline+`BytesCopy` 零拷贝，单遍谓词缓存+常驻桶阈值64，魔串常量化/parser候选 js.eval，资源 `try-finally/JsPureClose` 不丢，守 L0–L3）
