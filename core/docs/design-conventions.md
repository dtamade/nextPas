...
- `nextpas.core.js.lifecycle` 为纯上下文生命周期 owner（GPureClosed 紧凑4B `epoch*2+closed` generation-tagged atomic acquire/release + `GPureFree` freelist bounded recycling + 4x阈值半缩 via `bytes.ops`几何 + `mem.dynarray` Exactly-Once poke single source, GPureNextId/Len/Lock 各64B VARMIN隔离 (GPureClosed 紧凑4B 10k~40KB L1/L2友好), `atomic_fetch_add` lock-free id + spinlock resize 实例隔离线程高级感, generation mismatch即强一致closed per INV-7, bulk IsValid 零原子 via FValid, `base` 仅类型载体解耦, 四件套单职责, 阈值800内（<800 必拆, wc -l ~150）, 守 L0-L3, 见 `core/docs/js/CONTRACT.md §1`）
- `nextpas.core.js.pure.base` 为纯族共享基座（标准子模块四件套 `js.pure.base`：类型载体 + inline 薄转发 per four-piece `base←intf←impl←门面`，lifecycle 已抽 `js.lifecycle` 单源，Host/Heap/Value/IO inline thin-forward至`pure.host/pure.value/js.eval/js.lifecycle`单源（pure.host/pure.value永久单源，无阈值迁移，`JsPureHostsClear`+`JsPureTryReadFileText`+`JsPureCall` PBuckets nil单模板 已下沉至`pure.host` via `platform.fs/bytes.ops` inline零拷贝 single source，`TJsPureHostState` 统一 `JsPureHostStateSet*` 3+3 inline零拷贝 无 legacy shim 奢华薄零逻辑），零 FFI/零 `platform.dl`（L0 `platform.fs` 经`pure.host`直读，64MiB限流，`bytes.ops`零拷贝 via `BytesCopy`单源），阈值800内（<800 必拆，wc -l ~230 实测），见 `core/docs/js/CONTRACT.md §1`；守四件套与 L0–L3，复用 `bytes.ops` 单源与 `text.view` 零拷贝，热点 inline+`BytesCopy` 零拷贝，资源 `JsPureClose` 幂等不丢）
...
- 软性指引，内聚性强的代码可以例外
- 显式例外 `nextpas.core.js.pure.impl` / `js.lifecycle` 硬门禁 800 行（`core/docs/js/CONTRACT.md §1` 阈值 800，hygiene 抽样 `wc -l core/src/nextpas.core.js*.pas` 告警，超阈必拆，无 650 反复调整），`js.pure.base` 已收敛为标准子模块四件套 `js.pure.base`（类型载体 + inline 薄转发，守 L0–L3 与四件套，阈值 800 内）
