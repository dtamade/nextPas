...
- `nextpas.core.js.pure.base` 为纯后端共享基座薄门面，非标准四件套命名（`pure.base` 薄转发：lifecycle 已抽 `js.lifecycle` 单源，Host/Heap/Value/IO仅inline thin-forward至`pure.host/pure.value/js.eval/js.lifecycle`单源（pure.host/pure.value永久单源，无阈值迁移，`JsPureHostsClear`+`JsPureTryReadFileText` 单源已下沉至`pure.host` via `platform.fs/bytes.ops`，`HostSet` 12 thin-forwards已deprecated兼容保留直达`pure.host`无dispatcher内联膨胀（首选 `TJsPureHostState` 统一门面 `JsPureHostStateSet*` 3+3 单选型 via `pure.host.TJsPureHostState` inline零拷贝），`JsPureCall` 2重载+State统一收敛至`PBuckets nil`单模板，`JsPureClose` 2 legacy deprecated+State统一），零 FFI/零 `platform.dl`（L0 `platform.fs` 经`pure.host`直读，64MiB限流，`bytes.ops`零拷贝 via `BytesCopy`单源），阈值800内（<800 必拆，wc -l ~310 实测），见 `core/docs/js/CONTRACT.md §1`；`pure` 为纯族前缀，`base` 为共享后缀，复用 `bytes.ops` 单源与 `text.view` 零拷贝，热点 inline+`BytesCopy` 零拷贝，资源 `JsPureClose` 幂等不丢，守 L0–L3）
