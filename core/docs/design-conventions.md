...
- `nextpas.core.js.pure.base` 为纯后端共享基座 facade，非标准四件套命名（`pure.base` lifecycle单职责 + 薄转发复用 `js888/v8/chakra`，零 FFI/零 `platform.dl`（L0 `platform.fs` 仅 `TryEvalFile` 直读，非 dl，64MiB限流，`bytes.ops Move`零拷贝），生命周期 GPureClosed 64B padded atomic acquire/release cache-line隔离 + atomic_fetch_add lock-free id + spinlock resize，实例隔离线程高级感，Host/Heap/Value/IO仅inline thin-forward至`pure.host/pure.value/js.eval/platform.fs`单源（已落地`js.host/js.value`独立门面复用下沉，见`nextpas.core.js.host/value.pas`），阈值650内（<800 必拆，wc -l ~390 实测，超阈预案 `js.host`+`js.value` ready），见 `core/docs/js/CONTRACT.md §1`；`pure` 为纯族前缀，`base` 为共享后缀，复用 `bytes.ops` 单源几何扩容与 `text.view` 零拷贝，热点 inline+`BytesCopy` 零拷贝，单遍谓词缓存+常驻桶阈值64，魔串常量化/parser候选 js.eval，资源 `try-finally/JsPureClose` 不丢，守 L0–L3）
