...
- **禁止 C 风格赋值操作符**（`+=`、`-=`、`*=`、`/=`）：其合法性取决于宿主
  `fpc.cfg` 是否开 `-Sc`（`settings.inc` 不开 `{$COPERATORS ON}`），同一份
  源码在不同机器/CI host 上编译结果不同；且 stage0 编译器不支持该语法，
  写入即埋自举雷。统一写 `X := X + Y`（或 `Inc(X, Y)`）。
...
- `nextpas.core.js.lifecycle` 为纯上下文生命周期 owner（GPureClosed 紧凑4B `epoch*2+closed` generation-tagged atomic acquire/release + `GPureFree` freelist bounded recycling + 4x阈值半缩 via `bytes.ops`几何 + `mem.dynarray` Exactly-Once poke single source, GPureNextId/Len/Lock plain (GPureClosed 紧凑4B 10k~40KB), IsAlive single acquire + relaxed Len (I-Cache/零拷贝, bulk IsValid zero barrier), `atomic_fetch_add` lock-free id + 2^32 wrap detect + spinlock resize 实例隔离, generation mismatch即强一致closed per INV-7, bulk IsValid 零原子 via FValid, `base` 仅类型载体解耦, 四件套单职责, 阈值800内（<800 必拆, wc -l ~150）, 守 L0-L3, 见 `core/docs/js/CONTRACT.md §1`）
- `nextpas.core.js.pure.base` 为纯族共享基座（标准子模块四件套 `js.pure.base`：纯类型载体 per four-piece `base←intf←impl←门面`，lifecycle 已抽 `js.lifecycle` 单源 inline薄转发（仅 lifecycle，Host/Heap/Value/IO via `pure.host/pure.value/js.eval` 单源直引，base零依赖同模块 per design-conventions — 已收敛 former Host/Value/Eval thin-forward豁免债务，`JS_PURE_HASH_THRESHOLD` 16 单源收敛至 `pure.hash`，无字面量重复，`JsPureHostsClear`+`JsPureTryReadFileText`+`JsPureCall` PBuckets nil单模板 已下沉至`pure.host` via `platform.fs/bytes.ops` inline零拷贝 single source，`TJsPureHostState` 统一 `JsPureHostStateSet*` 经 `pure.host`），零 FFI/零 `platform.dl`（L0 `platform.fs` 经`pure.host`直读，64MiB限流，`bytes.ops`零拷贝 via `BytesCopy`单源），阈值800内（<800 必拆，wc -l ~80 实测），见 `core/docs/js/CONTRACT.md §1`；守四件套与 L0–L3，复用 `bytes.ops` 单源与 `text.view` 零拷贝，热点 inline+`BytesCopy` 零拷贝，资源 `JsPureClose` 直接清 Hosts/Heap 幂等不丢）
...
- 软性指引，内聚性强的代码可以例外
- 显式例外 `nextpas.core.js.pure.impl` / `js.lifecycle` 硬门禁 800 行（`core/docs/js/CONTRACT.md §1` 阈值 800，hygiene 抽样 `wc -l core/src/nextpas.core.js*.pas` 告警，超阈必拆，无 650 反复调整），`js.pure.base` 已收敛为标准子模块四件套 `js.pure.base`（纯类型载体 + lifecycle 仅，Host/Value/Eval via owner 直引，base零依赖，阈值 16 via `pure.hash` 单源，守 L0–L3 与四件套，阈值 800 内）
...
host ABI 声明的取证入口集中记录在 `docs/platform-ffi-source-evidence-index.md`。该索引说明
FPC source family、OS SDK/header 补证、nextPas-owned host base/ffi owner 与证据边界之间的关系。
它必须继续接入 `build/verify_local.sh` 的
`core-platform-ffi-source-evidence-index-check` focused route，并在最终 verification envelope 中保留
...
- `nextpas.core.platform.<host>.ffi`：宿主平台 API 绑定，如 `cdecl/stdcall external`
  声明、libc/kernel32/pthread/mach 入口，以及 raw `syscall` 入口声明。这里不放 wrapper /
  projection / convenience helper。
...
shared `posix.ffi` 同样必须保持 raw-only：只声明 `pthread_self`、`pthread_create/join/detach`、
`sched_yield`、TLS key 读写、`sysconf`、`clock_gettime` 等 POSIX external 入口，不放 token 投影、
retry、errno 读取、sysconf 正数投影或 pthread lifecycle glue。`EINTR`、`_SC_NPROCESSORS_ONLN`
这类编号 token 属于 host base；跨宿主复用的纯数学换算，例如 `timespec -> ns`、
`timespec + ns` 与 deadline remaining 算术，归 `nextpas.core.platform.posix.math`。
...
generic Unix fallback 既然已经通过 `nextpas.core.platform.unix.base` / `.ffi` 承载
`clock_gettime` / `clock_getres` raw declarations 与 timeout clock token，就必须把
`NEXTPAS_POSIX_CLOCK` 视为这条 fallback contract 的一部分。也就是说，generic Unix 不能一边暴露
POSIX clock raw entry，一边让 `platform.time` 因为缺少 `NEXTPAS_POSIX_CLOCK` 而退化成 unsupported。
...
`nextpas.core.platform.windows.base` 这类 host base owner 单元提供；`platform.time` 只消费这些 token，
不能把裸魔数常量继续埋在实现里。
...
Windows `Sleep`、`WaitOnAddress`、`SleepConditionVariableSRW` 这类 API 的超时换算 policy
（纳秒到毫秒的向上取整、`INFINITE` sentinel、最大有限超时截断）不属于 `.ffi`。raw API
声明和 wait/error 常量分别放在 `windows.ffi` / `windows.base`；`platform.thread`、`platform.sync`
直接消费这些 raw declaration + token，并在统一函数层完成 timeout/result 语义。

Darwin `mach_timebase_info` 的 cache / sanitize policy，以及 Windows
`QueryPerformanceFrequency` / `QueryPerformanceCounter` / `GetSystemTimeAsFileTime`
这些宿主时钟 API 的 stateful 处理，属于 `platform.time.host` 统一函数层。host `.ffi` 只声明
`mach_*` / QPC / FILETIME raw entry；host `.base` 只提供必要的 ABI record / constant truth。

POSIX 的 `clock_gettime` / `clock_getres` 也遵循同一条 owner boundary。shared
`nextpas.core.platform.posix.base` 保留 `timespec` 等共享 ABI 形状；shared
`nextpas.core.platform.posix.ffi` 只保留 raw `clock_gettime` / `clock_getres` declaration；饱和
`timespec -> ns`、`timespec + ns` 与 deadline remaining 算术归 `posix.math`；
`platform.time.host` 直接消费这些 raw declaration、host clock id token 与 math helper，形成统一
monotonic/realtime/resolution API。
...
Windows `SRWLOCK`、`CONDITION_VARIABLE`、`WaitOnAddress` 的 raw ABI 声明属于
`nextpas.core.platform.windows.ffi`，对应 record / constant truth 属于 `windows.base`。
init/lock/trylock/unlock/destroy/wait/wake 的 wrapper、timeout/error mapping 和 public result 选择
属于 `platform.sync` 统一函数层，不再放回 `.ffi`。

同样，Linux futex 的 raw syscall 编号、`FUTEX_*` opcode 属于 `linux.base`，raw `syscall`
declaration 属于 `linux.ffi`。`platform.sync` 直接消费这些 raw declaration + token，保留
nil/value mismatch、timeout/error mapping、public result 常量选择和跨平台 wait/wake contract。
...
`PLATFORM_ERR_AGAIN`。timeout / last-error / raw wait success 由 `platform.sync` 基于 host
raw return token 统一投影，不能把 `PLATFORM_ERR_AGAIN` 硬编码进 `windows.ffi` 或其他 host ffi owner。

POSIX pthread mutex / rwlock / condvar / timeout-clock 的 raw declarations 属于 shared
`posix.ffi`，宿主 capability / kind / errno / timeout clock token 属于各 host `.base`。mutex attr /
cond attr 初始化、public mutex kind 映射、deadline 计算、errno/result 分类和 unsupported fallback
属于 `platform.sync`，不属于 `posix.ffi` 或 host `.ffi`。

因此，`platform.sync` 继续保留 NextPas 的 public result adapter、wait-bucket 策略
...
record、host size alias 或 public error constant。
...
WaitOnAddress 绑定，不能反向拥有 wait-bucket 策略或创建
...
当前线程 token、native thread id、TLS key create/free/set/get、CPU count、yield/sleep 这类 raw
entry 分别由 host/shared `.ffi` 声明：Windows 使用 `GetCurrentThreadId` / `Tls*` / `GetSystemInfo` /
`CreateThread` / `WaitForSingleObject` / `CloseHandle` / `Sleep`，POSIX 使用 `pthread_self`、
`gettid` / `pthread_threadid_np` / `pthread_getthreadid_np`、`sysconf(_SC_NPROCESSORS_ONLN)`、
`pthread_create/join/detach`、`pthread_key_*`、`sched_yield`、`nanosleep`。thread state、join/detach
收口、TLS key carrier、sleep retry、CPU fallback 和 public handle/token 语义属于
`platform.thread` 统一函数层，不属于 `.ffi` helper。
