...
- A minimal live `nextpas.core.system.sysutils` unit exists — **40+ functions** (see § Current live minimum; `core/src/nextpas.core.system.sysutils.pas` ~620 lines, `Format`/`SameText`/`IntToStr`/`Trim` + `StrToInt`/`FloatToStr`/`FileExists`/`ExtractFile*`/`Now`/`Sleep`/`SysErrorMessage` + `BytesOf`/`StringOf` via `text` -> `bytes` single source (INV-5, inline thin-forward zero-copy), all delegating to `text`/`bytes`/`platform.files`/`platform.fs`/`platform.path`/`platform.env`/`platform.error` — L0→L2 fs/path/os.env debt converged to platform abstraction, `bytes.ops` single source via `text`/`bytes` facade retained).
- **L0 debt note (2026-09-02, L7 converged 2026-09-03)**: `system.sysutils` holds **2 L0 allowlist entries (text + bytes) single source** — 5 text sub-units converged to `nextpas.core.text` single source + `nextpas.core.bytes` single source, thin facade `inline` zero-copy via `bytes.ops` single-source (`TByteSpan` view, single `Move` in owner), `try-finally`/`Close` not lost; file/path/env slices converged to `platform.files`/`platform.fs`/`platform.path`/`platform.env` single source (inline zero-copy, bytes.ops single source in owner, try-finally not lost); text domain retained in L1 as single facade (not a violation, L7 debt converged) — see `core/tests/architecture/source_contracts/architecture_contract_registry.json:139-142` (single source converged via `text`/`bytes` facade; stub elegance via `units/<target>/` no IFDEF fork, ~800 lines soft-800 pure aggregation like http umbrella, 40+ inline thin forwards, single source in owner).
