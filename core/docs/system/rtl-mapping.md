...
| Heap manager contract | `compiler/HIR contract live; no public facade` | `np.system.heap_alloc` / `np.system.heap_free` are production typed HIR for GetMem/FreeMem (`sckHeapAlloc` / `sckHeapFree`); LLVM maps to `@np_alloc` / `@np_free`. `arr_alloc`/`class_alloc` stay implementation intrinsics. Owner remains `nextpas.core.mem`. |
