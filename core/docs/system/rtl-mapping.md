...
| `SameText` for case-insensitive comparison | `system facade delegating to owner` | Live re-export surface in `nextpas.core.system.sysutils` only; **implementation owner is `nextpas.core.text.conv`** (ASCII fold lives there). System does not own text comparison. |
