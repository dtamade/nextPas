...
- `uses nextpas.core.tui.ext`
  稳定增强入口。需要 `TApp`、theme、task、panel、scrollview、modal、dialog、split_pane、select、focus、interaction、frame budget 时用它。
...
**Auto-wrap / DECAWM**（DECSET 7）：`EnterAlternate` 发 `CSI ? 7 l`，`LeaveAlternate`
恢复 `? 7 h`——避免终端自动换行破坏 cell 格网（crossterm `DisableLineWrap`）。
