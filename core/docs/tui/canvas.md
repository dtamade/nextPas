...
**门禁**：`heaptrc 0`（`IAllocator` 下传 buffer 不丢；`common.mk HEAPTRC_GATE=1 haltonnotreleased,log`，`focused FOCUS=core/tests/nextpas.core.tui.canvas/test_tui_canvas_doc` 等 8 域 + 主包聚合路径双门禁；主包不再聚合 canvas，按需 `uses tui.canvas` 子facade，零分配由子家族独立承载）
