...
| Windows readiness poller | wine-runtime-smoke + **ci-matrix** (24 **platform** gates) | Durable on `windows-latest` for platform matrix modules (+which +dl +args +pipe). Wine 24 secondary. Job `total` may be **25–26** when mem.host and/or resource candidate listed (not platform facade promote). |
...
| Windows documented facade matrix | **ci-matrix** (24 platform) | Suite dirs through +info +which +dl +args +pipe + iocp + 3 real gates. Scripts may list **resource candidate** (Batch-19) and mem.host. Watch S1 not matrix-gated. |
