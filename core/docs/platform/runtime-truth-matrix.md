...
| Windows readiness poller | wine-runtime-smoke + **ci-matrix** (GHA documented 20-gate set) | Durable on `windows-latest` for matrix modules; not full-host Windows parity. Wine matrix 22 modules. Promoted GHA pass=20 (run 29718874441 @ `534d5e7c4`, +info). |
