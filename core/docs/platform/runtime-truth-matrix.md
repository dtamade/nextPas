...
| Windows readiness poller | wine-runtime-smoke + **ci-matrix** (21 **platform** gates) | Durable on `windows-latest` for platform matrix modules (+which). Wine 24 secondary. Job `total` may be **22** when `mem.host_runtime` is listed (not a platform facade gate). |
...
| Windows documented facade matrix | **ci-matrix** (21 platform) | Suite dirs through +info +which + iocp + 3 real gates; GHA run 29721371136. Scripts may list **dl candidate** (Batch-16) and mem.host. |
