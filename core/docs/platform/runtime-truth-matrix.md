...
| Windows readiness poller | wine-runtime-smoke + **ci-matrix** (23 **platform** gates) | Durable on `windows-latest` for platform matrix modules (+which +dl +args). Wine 24 secondary. Job `total` may be **24–25** when mem.host and/or pipe candidate listed (not platform facade promote). |
...
| Windows documented facade matrix | **ci-matrix** (23 platform) | Suite dirs through +info +which +dl +args + iocp + 3 real gates. GHA 29728715160 pass=24 fail=0. Scripts may list **pipe candidate** (Batch-18) and mem.host. |
