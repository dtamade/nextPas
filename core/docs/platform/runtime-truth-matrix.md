...
| Windows readiness poller | wine-runtime-smoke + **ci-matrix** (24 **platform** gates) | Durable on `windows-latest` for platform matrix modules (+which +dl +args +pipe). Wine 24 secondary. Job `total` may be **25** when `mem.host_runtime` is listed (not a platform facade gate). |
...
| Windows documented facade matrix | **ci-matrix** (24 platform) | Suite dirs through +info +which +dl +args +pipe + iocp + 3 real gates. pipe PASS on GHA 29729281116; promote lands with this batch. |
