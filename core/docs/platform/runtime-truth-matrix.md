...
| Windows readiness poller | wine-runtime-smoke + **ci-matrix** (20 **platform** gates) | Durable on `windows-latest` for platform matrix modules. Wine 24 secondary. Job `total` may be **21** when `mem.host_runtime` is listed (not a platform facade gate). |
...
| Windows documented facade matrix | **ci-matrix** (20 platform) | 17 suite dirs (+error +fmt +info) + 3 real gates; GHA pass=20 (run 29718874441). Script may also list mem.host and **which candidate** (Batch-14; not promoted). |
