...
| Windows readiness poller | wine-runtime-smoke + **ci-matrix** (22 **platform** gates) | Durable on `windows-latest` for platform matrix modules (+which +dl). Wine 24 secondary. Job `total` may be **23** when `mem.host_runtime` is listed (not a platform facade gate). |
...
| Windows documented facade matrix | **ci-matrix** (22 platform) | Suite dirs through +info +which +dl + iocp + 3 real gates. dl PASS on GHA 29725431946; poller empty-case Win64 fix lands with promote. |
