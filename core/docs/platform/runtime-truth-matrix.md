...
| Windows readiness poller | wine-runtime-smoke + **ci-matrix** (GHA documented 19-gate set) | Durable on `windows-latest` for matrix modules; not full-host Windows parity. Wine matrix 17 modules (+error +fmt +info). |
