...
| Windows readiness poller | wine-runtime-smoke + **ci-matrix** (GHA documented 20-gate set) | Durable on `windows-latest` for matrix modules; not full-host Windows parity. Wine matrix 24 modules (+watch UNSUPPORTED smoke +pty). Promoted GHA pass=20 (run 29718874441 @ `534d5e7c4`, +info); Windows job pass=21 with mem.host. |
...
| Windows documented facade matrix | **ci-matrix** (20) | 17 suite dirs (+error +fmt +info) + 3 real gates; GHA pass=20 (run 29718874441 @ `534d5e7c4`). |
