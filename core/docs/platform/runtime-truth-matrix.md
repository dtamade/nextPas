...
| Windows readiness poller | wine-runtime-smoke + **ci-matrix** (26 **platform** gates) | Durable on `windows-latest` for platform matrix modules (+… +pipe +resource +pty). Wine 24 secondary. Job `total` may be **27–28** when mem.host and/or watch candidate listed (not platform facade promote). |
...
| Windows documented facade matrix | **ci-matrix** (26 platform) | Suite dirs through +info +which +dl +args +pipe +resource +pty + iocp + 3 real gates. Scripts may list **watch candidate** (Batch-21 S1–S3). |
