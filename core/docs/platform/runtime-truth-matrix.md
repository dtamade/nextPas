...
| Windows readiness poller | wine-runtime-smoke + **ci-matrix** (22 **platform** gates) | Durable on `windows-latest` for platform matrix modules (+which +dl). Wine 24 secondary. Job `total` may be **23–24** when mem.host and/or args candidate listed (not platform facade promote). |
...
| Windows documented facade matrix | **ci-matrix** (22 platform) | Suite dirs through +info +which +dl + iocp + 3 real gates. GHA 29727006733 pass=23 fail=0. Scripts may list **args candidate** (Batch-17) and mem.host. |
