...
| Windows readiness poller | wine-runtime-smoke + **ci-matrix** (25 **platform** gates) | Durable on `windows-latest` for platform matrix modules (+… +pipe +resource). Wine 24 secondary. Job `total` may be **26** when `mem.host_runtime` is listed (not a platform facade gate). |
...
| Windows documented facade matrix | **ci-matrix** (25 platform) | Suite dirs through +info +which +dl +args +pipe +resource + iocp + 3 real gates. resource PASS on GHA 29730911054. Watch S1 not matrix-gated. |
