...
| Windows readiness poller | wine-runtime-smoke + **ci-matrix** (GHA documented 17-gate set) | Durable on `windows-latest` for matrix modules; not full-host Windows parity. Wine matrix now 15 modules (+error). Windows script 18-gate candidate (+error) pending GHA green. |
...
| Windows documented facade matrix | **ci-matrix** (17 promoted) | Promoted set: 14 wine-suite dirs + 3 real gates. Script now lists 15 wine-suite dirs (+error) + 3 real = 18-gate candidate; promote only after GHA green. |
