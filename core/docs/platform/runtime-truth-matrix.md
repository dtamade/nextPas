...
| Windows readiness poller | wine-runtime-smoke + **ci-matrix** (25 **platform** gates) | Durable on `windows-latest` for platform matrix modules (+… +pipe +resource). Wine 24 secondary. Job `total` may be **26–27** when mem.host and/or pty candidate listed (not platform facade promote). |
...
| Windows documented facade matrix | **ci-matrix** (25 platform) | Suite dirs through +info +which +dl +args +pipe +resource + iocp + 3 real gates. Scripts may list **pty candidate** (Batch-20). Watch S1+S2 not matrix-gated. |
