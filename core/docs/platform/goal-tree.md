...
- **Windows x86_64**: durable **`ci-matrix`** for documented **18-gate** set on GHA
  ...
  `platform.fmt`); Wine runtime smoke secondary (**18** matrix modules, +info +which).
...
| Windows x86_64 | **ci-matrix** 19-gate set; wine 18-module secondary | expand matrix; keep GHA+wine green |
...
**Wine smoke** (Win64 PE under Wine; secondary; **18** matrix modules): time,
memory, sync, thread, io, process, files, fs, path, env, mmap, random, socket,
error, fmt, info, which, io.reactor.iocp. Suites exist but are not matrix-gated
yet: dl, pipe. Not covered: signal, console, args, freetype/net, pty.
