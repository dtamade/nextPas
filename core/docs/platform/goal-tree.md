...
- **Windows x86_64**: durable **`ci-matrix`** for documented **18-gate** set on GHA
  ...
  `platform.fmt`); Wine runtime smoke secondary (**19** matrix modules, +info +which +dl).
...
**Wine smoke** (Win64 PE under Wine; secondary; **19** matrix modules): time,
memory, sync, thread, io, process, files, fs, path, env, mmap, random, socket,
error, fmt, info, which, dl, io.reactor.iocp. Suites exist but are not
matrix-gated yet: pipe. Not covered: signal, console, args, freetype/net, pty.
