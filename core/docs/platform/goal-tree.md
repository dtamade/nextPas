...
- **Windows x86_64**: durable **`ci-matrix`** for documented **18-gate** set on GHA
  `test-windows-runtime` (15 wine-suite dirs including `platform.error` +
  poller/io/socket real); Wine runtime smoke secondary (**15** matrix modules).
...
| Windows x86_64 | **ci-matrix** 18-gate set; wine 15-module secondary | expand matrix; keep GHA+wine green |
...
**Real Windows ci-matrix (18)** via `platform-windows-ci-matrix.sh`: time, memory,
...
platform.socket.windows_real. Promoted after GHA `test-windows-runtime`
pass=18 (run 29683362919, HEAD `4b831227f`). Not full-host parity outside that list.
