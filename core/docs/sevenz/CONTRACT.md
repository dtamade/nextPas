...
**真相**：`focused-runtime` 166 用例 + `bench_sevenz` + `README` 单源真相同步（`filters.DeltaApply` 外联 / `reader.Sort` + `EnsureSortedGeneric` / `writer.pathvalid`）
...
## 线程与资源
- reader `2-entry LRU 64MiB` 缓存（大 solid 单 MRU 保留）+ `Sort` 单源 `collections.algorithms.Sort`（`TSevenZSortCtx`）+ `EnsureSortedGeneric` 懒加载，`ReverseStr/LowerBoundGeneric` 外联避热点膨胀，writer `platform.thread` 并行（`IsMultiThread` 门控，`BytesReplicateCopy` 零拷贝，Move 语义去深拷），`CopyMatch` 倍增 Move，`ValidateEntryName` 复用 `bytes.pathvalid` 单源
