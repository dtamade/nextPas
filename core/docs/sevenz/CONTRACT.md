...
**真相**：`focused-runtime` 166 用例 + `bench_sevenz` + `README` 单源真相同步（`header IBytesBuilder ToBuilder 单源 + CRC 预计数两遍单分配` + `FEntries/LFolders 几何扩容 + writer SpanClone 单源` / `filters.DeltaApply` 外联 / `reader.Sort` + `EnsureSortedGeneric` / `writer.pathvalid`）
