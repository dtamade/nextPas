...
| `nextpas.core.sevenz.writer` | archive serialization (single/multi-folder `IsMultiThread` 并行), `FEntries/LFolders` 几何扩容（复用 `bytes.ops.BytesNextCapacity` 单源 `BYTES_BUILDER_MIN_GROW×2` 均摊，条目表与文件夹表均 O(n²)→O(n)）+ `ValidateEntryName` 复用 `bytes.pathvalid.BytesValidPath` 单源 (零拷 `UTF8IsValid`), `AddFile` 单源 `SpanClone`, 单遍 `Move+CRC`, header/FilesInfo 全路径 `IBytesBuilder` 单源（外层 `LPlainBuilder/LBlockBuilder` + 5 段 `LPayloadBuilder` 均按载荷预估 `CreateBytesBuilder(N)`，`Name` 精确 UTF16LE 总量，均摊 O(n) 近零 Grow） |
...
- Focused gate: `make -C core/tests/nextpas.core.sevenz/test_sevenz clean test`
  (172 tests: UTF conversion edge cases, FILETIME, LZMA2 round trips incl. stored-fallback
