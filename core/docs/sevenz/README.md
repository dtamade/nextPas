...
| `nextpas.core.sevenz.reader` | signature header validation, 2-entry LRU folder decode cache, O(1) name hash index (`TSwissTableStr`, 1M entries) + lexicographic sorted index for `EntriesByPrefix` O(log N+M), `EntriesBySuffix` suffix retrieval, zero-alloc ASCII `FindIgnoreCase` fast path, CRC checks, `ESevenZLimitError` bomb gates (header 64MiB, pack 64MiB, `SEVENZ_MAX_*` via `limits`) |
...
- Focused gate: `make -C core/tests/nextpas.core.sevenz/test_sevenz clean test`
  (149 tests: UTF conversion edge cases, FILETIME, LZMA2 round trips incl. stored-fallback
