...
| `nextpas.core.sevenz.fs` | filesystem federation: `SevenZAddTree`/`SevenZAddFileFromFs` and `SevenZExtractAllToFs` (grouped `ExtractAll` single decode per folder) + bulk `SevenZExtractByPrefix/Suffix/GlobToFs` + `IgnoreCase` bulk `SevenZExtractByPrefix/Suffix/GlobIgnoreCaseToFs` (`Try*`/`Try*IgnoreCase` variants, shared `FlushExtractedToFs` dedup) |
...
| FS federation | `SevenZAddTree` / `SevenZAddFileFromFs` / `SevenZExtractToFs` / `SevenZExtractAllToFs` (grouped) / `SevenZExtractByPrefix/Suffix/GlobToFs` + `SevenZExtractBy*IgnoreCaseToFs` / `SevenZTryExtractByGlob*ToFs` |
...
- Focused gate: `make -C core/tests/nextpas.core.sevenz/test_sevenz clean test`
  (163 tests: UTF conversion edge cases, FILETIME, LZMA2 round trips incl. stored-fallback
