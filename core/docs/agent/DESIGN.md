...
### D14 并行工具经底座 async 任务设施执行
- **决定**：loop 并行批次 = taskgroup 提交 + 轮线程汇合；子令牌隔离批内失败；
  不自建线程池，不做全局并发上限（v1，inbox 记录）。
- **备选**：自研线程池；串行 only（code888 对非并行批的保守路径保留为子集）。
- **理由**：复用底座原语避免第二套并发设施；子令牌树精确表达"单工具失败不
  连坐兄弟"；loop 保持单线程编排心智，事件回调无锁。详见 LIFECYCLE §5。
