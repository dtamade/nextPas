...
| **W1 协议与传输** | base/errors/intf 全量词表；fold.pas；agent.sse 增量解析器；transport.http；provider.common + provider.openai + provider.fake；clock | test_protocol / test_errors / test_sse / test_transport_stream / test_provider_openai / test_fake_provider 绿 |
| **W2 可靠性** | retry 装饰器（含 fake clock 生产单元）；anthropic 适配器；取消贯穿 transport/provider；usage 六字段全量映射 | test_retry / test_provider_anthropic 绿 |
