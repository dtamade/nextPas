...
# 26 platform gates promoted (+… +pipe +resource +pty). Evidence:
# - pty: GHA 29734704405 @ d2eb8f890 PASS platform.pty (pass=27 fail=0).
# Batch-21 candidate: +platform.watch (S1–S3 RDCW; not promoted until GHA).
...
echo "truth=ci-matrix-candidate; 26 platform gates promoted + watch candidate; mem.host optional; not full-host Windows parity"
echo "core=$CORE_ROOT"
...
echo "truth=ci-matrix; gates_passed=$pass_count; gates_failed=$fail_count; scope=documented-26-platform-gate-set-plus-watch-candidate-plus-mem-host"
