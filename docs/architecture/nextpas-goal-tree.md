...
- 最小 source-backed `System.pas` / `TObject` truth 已落地：implicit runtime 语义层会读取
  ...
  `done:`。当前这仍只是最小 ownership contract，不是 allocator free、diagnostics/trap failure
  path 或完整 validation runtime。
...
- 继续扩展 `System` source truth：把 `np_object_alloc` / `np_object_free_release` helper 从 header
  ownership + magic validation contract 接到 allocator free、diagnostics/trap、完整 dynamic dispatch 与
