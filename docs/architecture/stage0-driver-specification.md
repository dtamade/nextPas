...
`env sync` 当前是 `stage0` 上第二条 workspace-local mutation surface。它的职责边界固定如下：

- 只接受 `--target`、可选 `--toolchain-binding` 与必须的 `--workspace`
- 在没有显式 `--toolchain-binding` 时复用同一份 workspace selection
...
- verify gate 还必须固定当前最小 `member-call` query truth：
  `Worker.Run;` / `Worker.SetValue(7);` / `Self.SetValue(9)` /
  `Halt(Worker.Add(1, 2));` / `TWorker.Create(42)` 会出现在 `queryBindings`，
  并通过 `queryDefinitions` join 到 `TClass.Method` method symbol；这仍不表示完整 LSP、
  inherited member lookup、runtime constructor lowering 或 type-based overload dispatch
...
- 当前 failure projection 至少要能暴露 `diagnostic-code=sema.duplicate-declaration` /
  `sema.ambiguous-overload` / `sema.wrong-argument-count` 与 `diagnostic-phase=sema`
