...
4. 第一阶段 runtime contracts 均有 compiler consumer 和 executable failure/success evidence。
5. stage A、B、C 都是实际执行的 compiler，并重建同一份 compiler-system bundle。
6. focused gates、`git diff --check` 与 `make hygiene` 在最新-main landing candidate 上通过。
7. 文档只声明 fresh evidence 能证明的能力。
