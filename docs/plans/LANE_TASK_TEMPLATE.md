...
> 日常用法：给 AI 发一句话
> `读 docs/plans/LANE_TASK_TEMPLATE.md，按 landing 流程把 <lane分支名> 合进 main`
...
## 2. Landing 详细流程与安全规范（main 的 worktree 执行，一次只合一个 lane）

### 2.1 起点检查（不干净就停）

```bash
git worktree list --porcelain   # 只能有 main 这一个（加正在合的 lane）
git status --short --branch     # 必须是 ## main...origin/main，且无改动
git pull --ff-only               # 拉到远端最新；失败说明有人动过 main，先查清再继续
scripts/worktree-audit.sh        # 确认目标 lane 存在、无违规 worktree
```

### 2.2 确认要合的内容

```bash
git rev-parse --short <lane分支名>   # 取 lane 顶端，报给总控确认
git log --oneline main..<lane分支名>  # 列出待合 commits，与 lane 的 Ready 报告对一遍
```

对不上的（多了/少了 commit）就停，找 lane owner 问清再动手。
...
- 有冲突：解完 `git add` + `git cherry-pick --continue`；解不了或拿不准语义就
  `git cherry-pick --abort`，把冲突文件和双方改动原样报给总控，**不要自己选边**。
...
红了就停：把报错原样报给总控，**不要自己修 lane 的代码**，修好重跑再继续。
...
git push origin main                           # 总控明确说"推"才执行
```
