...
| 子任务 | 内容 | 预估 |
|--------|------|------|
| D2.1 LICM (循环不变量外提) | 识别循环不变量，提升到循环外 | 2 天 |
| D2.2 逃逸分析 | 栈分配优化，减少堆分配 | 2 天 |
| D2.3 尾调用优化 | 尾递归 → 循环转换 | 1.5 天 |
| D2.4 去虚拟化 | 静态分派替换虚调用 | 1.5 天 |
| D2.5 内联策略增强 | 启发式内联决策（当前是简单内联） | 1.5 天 |
| D2.6 向量化识别 | LLVM 自动向量化标注 | 1.5 天 |

**验证**: 12 pass 全部注册, compiler-pass 34/34, MIR 单元测试 ≥ 60

### AL2-D3: 增量编译稳定性（预估 5 天，P2 优先级）

**为什么第三**: 已有框架（PrepareIncrementalBuild/FinalizeIncrementalBuild），需要验证和修复。

| 子任务 | 内容 | 预估 |
|--------|------|------|
| D3.1 文件级增量正确性 | 全量 vs 增量产物 diff 一致，覆盖所有 compiler-pass 用例 | 2 天 |
| D3.2 查询依赖图精确失效 | 函数级增量：只重编译受影响的查询 | 2 天 |
| D3.3 冷编译性能基线 | 确认增量框架不增加冷编译开销 | 1 天 |

**验证**: 增量 vs 全量 diff 100% 一致, 冷编译性能不退化

### AL2-D4: 诊断增强（预估 7 天，P3 优先级）

**为什么第四**: 有基础（JSON 输出 + 错误恢复），需要增强到 rustc 水平。

| 子任务 | 内容 | 预估 |
|--------|------|------|
| D4.1 多错误报告 | 错误恢复后继续编译，报告所有错误（当前有 MaxErrors=100 框架） | 2 天 |
| D4.2 修复建议 | help: did you mean... 基于编辑距离的标识符建议 | 2 天 |
| D4.3 错误代码体系 | E0001-E9999 分类，每个诊断有唯一错误代码 | 2 天 |
| D4.4 LSP 诊断推送 | diagnostics streaming 接口 | 1 天 |

**验证**: 多错误文件报告 ≥5 错误, 修复建议覆盖 typo/缺失导入/类型不匹配

### AL2-D5: 并行编译稳定性（预估 5 天，P4 优先级）

**为什么第五**: 已有 TParallelScheduler 框架，需要验证确定性。

| 子任务 | 内容 | 预估 |
|--------|------|------|
| D5.1 单元级并行确定性 | stress test 100 次一致 | 2 天 |
| D5.2 阶段级并行 | 查询无依赖并行调度 | 2 天 |
| D5.3 性能 scaling | 4 核 → ~2.5x 加速验证 | 1 天 |

**验证**: stress test 100/100 一致, 4 核加速比 > 2.0x

### AL2-D6: 编译器标准库化（预估 3 天，P5 优先级）

**为什么最后**: 依赖其他工作稳定后进行。

| 子任务 | 内容 | 预估 |
|--------|------|------|
| D6.1 审计自实现数据结构 | 消除编译器自己实现的数据结构，全部迁移到 core/ | 1 天 |
| D6.2 扩展标准库引用 | 编译器引用标准库模块 4→50 | 2 天 |

**验证**: 编译器无自己实现的数据结构, 标准库引用 ≥ 50 模块

### AL2-D7: 规范覆盖验证（预估 2 天，P6 优先级）

| 子任务 | 内容 | 预估 |
|--------|------|------|
| D7.1 4 份核心规范落地审计 | compiler-pipeline/semantic-model/ir-architecture/backend-specification | 1 天 |
| D7.2 规范-实现差距报告 | 列出每份规范中未实现的项目 | 1 天 |
