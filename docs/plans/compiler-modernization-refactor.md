...
创建：2026-08-23　最后更新：2026-08-25（v2.37：tree-exit139 同族解剖前三层
修复——①记录返回方法 sret 分类补齐（seeder 方法分支+ProcessMethodBegin
'r' 后缀，Collect 类 define void @(ptr sret,ptr self) 归正）；②语句位调用
by-ref 点访问/Result 实参地址化（TryPointerFieldAccess/TryByRefFieldAddressBlob
从 FoldCore 嵌套局部函数提升为 TSemanticAnalyzer 方法双通道共享，
EncodeCallStatementArgs 接入 varref+field_ref 地址通道并补 Result→retvar
别名；种子体记录局部经 LookupRecordVar 回退解析）；③try 帧早退 pop 纪律
（发射器 FTryFrameDepth 深度跟踪，htkReturn/ret_str/ret_str_owned 与
except-handler 路径按深度补发 np_try_pop，根除悬垂 jmpbuf 野跳）；
v2.36：llvm 执行面 stub SysUtils
Result[I] 循环机件打通——FillHirLoweringContext 统一构造根除降级上下文
漏线崩溃（iso_a 编译期 AV@$1）、Length(Result) 变量优先守卫根除自调用
递归（mini_up exit139）、集合成员测试折叠按栈机真实语义重写、setchar
写分支补 Result→retvar 别名，mini_up up=[HELLO]/lo=[world] 全链归正；
v2.35：字符索引读写修复——
S[I] 读经 np_tstring_copy 物化 1 字符切片、S[I]:=X 写经新增运行时
np_tstring_setchar（CoW ensure-unique），mc_c C0=2/C1=X 家族归正，
cow1 共享缓冲对照实证别名安全；v2.34：值位置 concat 写实参修复——
...
正确性    residual 0/0 ✅(0823全量复跑)   compiler-pass 58/58 ✅   opt 首错支配性违规已修✅(v2.25 三层修复)   concat-swap b4b-i16 已修✅(v2.29 tryenv len=7)   residual 标识符类 by-ref 调用点地址化 ✅(v2.30·字面0消灭·DotAccess 类挂账)   DotAccess 字段链地址通道 ✅(v2.33 双编码器就绪·overload 契约断点挂账)   字符串全局 24B 内联存储 ✅(v2.32·mc_e 循环变量踩踏根除)   值位置 concat 写实参 ✅(v2.34·mini_cat2 四形状归正)   字符索引读写 ✅(v2.35·mc_c 归正+CoW setchar 别名安全)   stub SysUtils Result[I] 循环机件 ✅(v2.36·上下文统一构造+集合折叠重写·up=[HELLO]/lo=[world])   tree-exit139 前三层修复 ✅(v2.37·方法 sret 分类+语句位 by-ref 点访问/Result 地址化+try 帧早退 pop 纪律；第四层=种子链托管字符串记录字段未初始化挂账)
...
下一口    tree-exit139 第四层=种子链托管字符串记录字段未初始化（valgrind 实证链：RegisterScalarBackend memset 表→RegisterBackend 718 槽拷贝→EnsureUniqueBackendInfoText 读 Name/Description 触 np_tstring_assign/np_str_cmp 未初始化条件跳转→IsBackendRegistered 未初值→jump 0x0；伴随缺陷=语句位双层字段写 `X.F.G := v` 与 WriteLn(单层字段读) 静默丢弃（v237matrix IR 实证：G.Info.A:=22 无 store、WriteLn(G.Count) 只剩换行）、@g_G 全局记录 i64 尺寸不足、APool var 形参基址 field_ref 无类型注册可查）；修通即八探针（cmpbyte/cmpmid/cmpnocall/generic/hashmap/sync/tree/tvec）预期 exit0；residual 战役阶段A 续刀主靶=overload-aware 结构化调用契约（atomic_* 族 `overload;` 声明使 HasOverload→TryGetDirectCallContract 即退，结构化 shekCall 无 byref 'r' ops 全参值语义——FindSpanOwnerThreadId 类 DotAccess 现场的真正根因）+ deref 基值位置语句吞失修复（m2_mini_dotref 复现）+ SSE2Shift*Raw/Copy/Delete/SpanInit 多形状家族解剖 + L3 1893 存量新缓存复测；阶段B=方法重载坍缩（ENextPasError.CreateFmt 8 形参裸 define）→ strict 计量器归零后常开；llvm 执行面验证 lane 余项另含 cap/cmpgen/puny 构建期挂账；或 swiss 接线 → P1 刀⑨静窗复测
```
...
| v2.37 | （本提交） | tree-exit139 同族八探针解剖·前三层修复：①缺陷实证链=八探针（cmpbyte/cmpmid/cmpnocall/generic/hashmap/sync/tree/tvec）运行期全部 SIGSEGV，gdb 逐层解剖出三层独立缺陷叠加——**第一层**记录返回方法无 sret 分类（seeder 方法分支缺 IsRecReturn case→签名串无 ':rN' 后缀；ProcessMethodBegin 缺 'r' 分支→FSretValueId/FPendingParamLlvmIdx 不接 sret_ptr 形参→retvar 落 EnsureAlloca(name) 未初始化槽 Result.X 野写；修复后 IR 实证 define void @TAllocStatsCollector.Collect(ptr sret, ptr self) 且字段 GEP 直写首参）；**第二层**语句位调用 by-ref 点访问/Result 实参传值不传址（EncodeRuntimeIntExprFoldCore 内 TryByRefFieldAddressBlob 仅表达式位接线、语句位 EncodeCallStatementArgs 无点访问分支→EnsureUniqueBackendInfoText(LCanonicalTable.BackendInfo) 类现场发 call @...(i64 值载) vs define (ptr)——类型失配 UB 经 opt -O2 折叠 call(undef)+unreachable 截断函数体致执行跌入下一函数；诊断实证 [byref-meta]/[byref-field] 零命中证明语句位根本不走 FoldCore）；**第三层**try 帧早退 pop 纪律缺失（RebindSimdPublicApi 两早退分支 ret 无配对 np_try_pop→悬垂 jmpbuf 滞留 @__np_exc_stack→后续 raise longjmp 入死帧野跳栈地址/0x0）；②修复=第一层 seeder 方法分支补 ':r'+TypeMetaSize 槽数与 ProcessMethodBegin 'r' 后缀三分支（sret_ptr 形参/FSretValueId 选择/FPendingParamLlvmIdx 取位）；第二层 TryPointerFieldAccess/TryByRefFieldAddressBlob 从 FoldCore 嵌套局部函数提升为 TSemanticAnalyzer 方法（类声明+方法实现双迁，双编码器单源共享）+EncodeCallStatementArgs 新增 by-ref 点访问分支（IsVarParamAtPosition 门控+TryByRefFieldAddressBlob→varref+field_ref 地址 blob）+裸 Result 标识符分支（SameText('Result')→FCurrentRetVarName varref）+blob 基址 LookupRecordVar 回退（种子体局部只走 RegisterRecordVar 注册表无模型符号，FindSymbolByName=0 场景实证）+blob 点访问基座 Result→retvar 归一；第三层发射器 FTryFrameDepth 深度跟踪（sckExceptionTryPush/Pop 与 legacy hikTryBegin/End 双路径计数+EmitTryFramePops 助手接线 htkReturn/ret_str/ret_str_owned 三位点——except-handler 路径 longjmp 不消耗帧同需补 pop）；③IR 实证=v237var TouchVar(G.Info) 发 getelementptr i64, ptr @g_G, i64 1 +call(ptr) 全 ptr 化；cmpnocall EnsureUniqueBackendInfoText$r 四处调用点全 ptr 化；RebindSimdPublicApi np_try_pop 1→3 处；④验证=rebuild pass×2+v237var/v237repro/v237matrix 最小探针 exit0+24 探针全量回归 14 绿+8 已知挂账+3 已知构建期挂账与 v2.36 基线逐项一致零劣化+compiler-pass 组 passed+contract pass+hygiene pass+diff-check(本批十文件)；MIXUSE-AUDIT.md 并行会话文件按 D28 纪律继续排除 |
| v2.36 | （本提交） | llvm 执行面·stub SysUtils Result[I] 循环机件打通（UpperCase/LowerCase 全链归正）+HIR 降级上下文构造根治：①三缺陷实证=iso_a `S[1] in ['a'..'z']` 编译期 EAccessViolation@$1（EncodeRuntimeBoolExprFold 包装器栈上 Ctx 只接 6 字段，本批集合折叠触达未接线 EvaluateIntegerConstant=栈垃圾函数指针，Assigned 守卫拦不住非 nil 垃圾）；mini_up exit139（Length(Result) 经 v2.35 Result→retvar 重映射后 ReceiverVarName='UpperCase' 同时命中 IsRuntimeStrVar 与 LookupProcedureBody→循环界物化成 @UpperCase 自调用无限递归，IR 直接证据 call void @UpperCase(ptr sret %len_tmp) 单参错配三参签名）；up/lo 恒等（赋值分派 setchar 分支基座文本 'Result' 未做 retvar 别名→IsRuntimeStrVar('Result')=False 静默丢弃写，bb13 空块 IR 实证）；②修复=FillHirLoweringContext 统一构造（FillChar 清零+数据成员+12 回调全量接线；BuildRuntimeScalarHirExpr 与 EncodeRuntimeBoolExprFold 两构造点共用；清零保证未来漏线=nil 可守卫而非垃圾跳转）+Length 标识符分支 retvar 名跳过 LookupProcedureBody（变量读优先原则）+setchar 写分支补 SameText('Result')→FCurrentRetVarName 别名（读链已有写链补齐对称）；③集合成员测试折叠重写=栈机无 pick/swap 埋藏副本不可复用，每初等比较独立重折左操作数（范围=(x≥lo)*(x≤hi) mul 合取、单项=(x=c)、项间 add 合取且 add 后置于项 blob 之后、逐 cmp 补 zext），iso_d 单范围/混合/全 miss-else 三形状 1/10/7 实证；④附带修复=builder EmitTStringInit 改 FindLocalAlloca 本地限定（stub 形参 S 曾被外单元同名全局投毒跳过 alloca 创建，跨单元名冲突缺陷 c）+str_chr 字节 gep_i8 令牌接入表达式位读链（1 基减一在栈上完成，gep_i64 会按 8 字节缩放读错位）；⑤验证=rebuild pass×3+mini_up up=[HELLO]/lo=[world]+lo=[world] exit0+iso_a/b/c/d 四探针全对+微家族 cow1/mc_a-e/mg1/mini_cat 族 11/11 与基线一致+24 探针全量回归与 HEAD 基线逐项一致（临时 detached worktree 对照法零脏文件扰动：14 绿+8 tree-exit139 同族先在挂账+cap/cmpgen/puny 构建期已知挂账）+compiler-pass 58/58+contract pass+hygiene pass+diff-check(本批九文件)；MIXUSE-AUDIT.md 并行会话文件按 D28 纪律继续排除 |
| v2.35 | （本提交） | 字符索引读写修复（mc_c 家族归正）+CoW setchar：①缺陷实证=S[I] 读折叠为裸索引值（写实参链无 ArrayAccess-over-strvar 分支→漏入 write-int 打印索引，C0=2）；S[I]:=X 写退化为整串字面量替换毁串（赋值分派无 strvar 索引左值分支→落普通 assign-tstring-literal，C1=X）——两路径均对 gnkArrayAccess 基座静默丢弃索引语义；②修复读=写实参链新增分支：基座 identifier+IsRuntimeStrVar 且下标可折→物化 $chr_tmp_N 经既有 tstring-copy-runtime 全链（np_tstring_copy dst#9src#9idx#9int 1，运行时自带界检查与 create 语义）+write-str-var+QueuePendingStringTempRelease/冲刷（owned 写临时同模式），零新增运行时；③修复写=新增 tstring-setchar-runtime 节点族（types 枚举 hnkTStringSetCharRuntime+令牌映射+process 分派+builder ProcessTStringSetChar：FindAlloca 槽解析兼局部/全局+LowerNodeExprOrBlobTyped/ParseIntExprArg 双路降级+发射器调用分支与 declare），SEMA 赋值分派在数组元素存储分支后加 strvar ArrayAccess 分支，RHS 三形状=char 字面量/EvaluateStringConstant 首字节/int 数值码；④受控跨模块 rtl/runtime=新增 np_tstring_setchar(ptr,i64 1基idx,i64 ch)：界检查越界静默忽略；SSO 内联字节写（数据 offset 2）；heap ensure-unique——ref≠1 或 literal 负引用一律 np_alloc 同容量新块（总分配 8+cap+1 与 fini 释放尺寸一致）memcpy payload+refcount=1+旧头原子 decr 归零释放+换头，绝不原地改共享 payload（np_tstring_assign incr+memcpy 共享语义确证必须设防）；理由=无助手则写只能裸 store 腐坏别名串，风险=纯新增函数零 ABI 变化；⑤实证=mc_c C0=b/C1=Xbc+cow1 共享缓冲对照（S 改 Jello world/T 保持 hello world）+IR setchar×1/copy 切片形状正确；边界=Ord(S[I])/比较位字符读未接（现只 WriteLn 实参位），stub SysUtils Result[I] 循环机件另刀（原语本批已备）；验证=rebuild pass+runtime libnprt.a rebuild+微家族全绿+tryenv len=7 保持+23 探针（含 globstr/valcat 新守卫）逐项一致仅 cap/cmpgen/puny 已知挂账+compiler-pass 58/58+contract pass+hygiene pass+diff-check(本批八文件) |
