...
以 `task_plan.md` / `progress.md` 的最新完成批次和 fresh verification evidence 为准。全局能力目标和每轮批次归属现在由
...
- 当前 freshest verification evidence 是 fresh `bash build/verify_local.sh`，
  ...
  target snapshot missing blocked path、manifest-lock out-of-sync blocked path 及其 expected/actual detail；
  workspace member fixture 也已让
...
- 当前滚动批次继续建立在已经存在的
  ...
  `Batch 76` 把同一条 ambiguity diagnostic 扩展到 direct member-call：compact signature
