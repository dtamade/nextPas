...
- 当前 freshest verification evidence 是 fresh `bash build/verify_local.sh`，
  ...
  side tables，`pkg inspect / pkg graph` 也已投影
  ...
  dependencies fixture 也已让 `doctor` / `pkg inspect / pkg graph` 同时冻结 package manifest root 与
  workspace descriptor root + member package 的 dependency intent 投影。
- 当前 rolling window 已推进到 Batch 52：`pkg graph` read-only surface。
- 这份计划从现在起接管”当前主线的批次顺序”。
- `Batch 1` 到 `Batch 52` 已完成。
- 当前滚动批次继续建立在已经存在的
