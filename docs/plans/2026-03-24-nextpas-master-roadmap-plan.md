...
- 当前 freshest verification evidence 是 fresh `bash build/verify_local.sh`，
  ...
  side tables，`pkg inspect / pkg plan / pkg graph` 也已投影
...
- 当前 rolling window 已推进到 Batch 61：target snapshot preflight + imported call binding closure。
- 这份计划从现在起接管”当前主线的批次顺序”。
- `Batch 1` 到 `Batch 61` 已完成。
- 当前滚动批次继续建立在已经存在的
