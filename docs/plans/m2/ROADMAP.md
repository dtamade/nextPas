...
| 指标 | 值 | 轨迹 |
|------|-----|------|
| tree mini 相位耗时（P0 探针，NEXTPAS_PHASE_TIMING=1，两轮方差 <2%） | **sema 296s（99%）·seed 235s（79%）·resolution 4.7s·syntax ~0** → P1 刀②后 seed **232s**（-1.5%/-2.4%）→ 刀⑤后 seed **219-222s**、sema **279-281s**（累计 -5.9%/-8.3%，2026-08-23）→ **刀⑥后 seed 58.0/60.0s、sema 75.6/78.0s（A/B 同日 stash 基线腿 220.5/280.1s ⇒ seed -73%，2026-08-24）** → **刀⑦后 seed 17.9/18.2s、sema 36.2s（基线腿 59.1/76.9s ⇒ 再 -69%；今日累计 -91.8%）** → **刀⑧（泛型实例化 O(N²) 清扫）噪声级记零 18.06/18.04s（D22：debug 画像≠release 现实，选靶以 release A/B 为裁判）** → **刀⑨（体查找接刀② 索引+前缀扫机械化）静窗疑似 -6%（16.79/16.77s 对基线带 17.9-18.2s），确认腿遇 core-db 并发高载窗口（load≈31）待静窗复测（D23：高载窗口 ±10% 级效应不可测）**；seed 子相：encode ~96%（**D21：分母仅 SeedFunctionBodies 内部**，声明注册区 ~163s 未探针被掩盖，刀⑥ 已削）；刀⑥ 后复剖析（49 样本）=GetItem 按值返回 14.3%+SymbolAt 家族 33%（TypeSymbolForTypeId 头号）→ 刀⑦ 削；gdb 剖析（stage0-debug -g 版+135 样本）=fpc_copy 38.5%+ansistr_assign 16.3% | b4b-i17 LookupProcedureBody 开销 A/B 实测 **+4.8s（~1.6%）非主要矛盾**；主战场=播种路径本身（3.3 体秒）；探针=`nextpas.compiler.frontend.phase_timing`，TSV `/tmp/m2-phase-timing.tsv`；perf top-10 受阻已由 gdb 批采样法解除（详见主文档 §2.3） |
