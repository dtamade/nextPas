...
| opt 首错 | `use of undefined value '@Pos'`（.ll 362582 行，Pos 内建无运行时方法体——pos-intrinsic 口，project-helper 桶 total 68 占残留 92%）；上一口为 `@IInterface.QueryInterface`（已修，注³⁴） | 本轮实测（探针 2/74；接口 dispatch 元数据 seed 后 opt 进入 Pos 内建） |
