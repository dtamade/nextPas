...
begin
  ...
  Msg := StringToUTF8Bytes('Hello, nextPas!');
...
begin
  ...
  Key := StringToUTF8Bytes('my-secret-key');
  Msg := StringToUTF8Bytes('important data');
  Mac := HMAC_SHA256(Key, Msg);
  WriteLn('  HMAC: ', DigestToHex(Mac[0], 32));

  // Verify: recompute and compare
  if BytesEqual(Mac, HMAC_SHA256(Key, Msg)) then
...
begin
  ...
  IKM := StringToUTF8Bytes('input keying material');
  Salt := StringToUTF8Bytes('optional salt');
  Info := StringToUTF8Bytes('context info');
...
begin
  ...
  Part1 := StringToUTF8Bytes('common prefix | ');

  H := NewSHA256;
  H.Write(Part1[0], Length(Part1));
  H2 := H.Clone;

  Part1 := StringToUTF8Bytes('branch A');
  H.Write(Part1[0], Length(Part1));
  WriteLn('  Branch A: ', Copy(DigestToHex(H.SumBytes[0], 32), 1, 16), '...');

  Part1 := StringToUTF8Bytes('branch B');
