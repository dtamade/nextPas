#!/usr/bin/env bash
# hash-contract-check.sh — hash 模块契约完备性门禁
set -euo pipefail
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
CONTRACT="$REPO_ROOT/core/docs/hash/CONTRACT.md"
SRC_DIR="$REPO_ROOT/core/src"
TEST_DIR="$REPO_ROOT/core/tests/nextpas.core.hash"
...
printf "\n${BOLD}C4: 门面+测试${NC}\n"
[ -f "$SRC_DIR/nextpas.core.hash.pas" ] && ok "门面文件存在" || warn_check "hash.pas 门面缺失"
