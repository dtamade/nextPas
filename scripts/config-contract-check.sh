...
# config-contract-check.sh — config 模块契约完备性门禁
set -euo pipefail

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
CONTRACT="$REPO_ROOT/core/docs/config/CONTRACT.md"
SRC_DIR="$REPO_ROOT/core/src"
TEST_DIR="$REPO_ROOT/core/tests/nextpas.core.config"
...
printf "\n${BOLD}C2: 源文件完备性${NC}\n"
CFG_FILES=("nextpas.core.config.pas" "nextpas.core.config.builder.pas" "nextpas.core.config.env.pas"
           "nextpas.core.config.export.pas" "nextpas.core.config.flatten.pas" "nextpas.core.config.watcher.pas")
...
printf "\n${BOLD}C4: 格式支持${NC}\n"
for fmt in "JSON" "YAML" "TOML" "INI"; do
  if grep -rql "\b$fmt\b" "$SRC_DIR"/nextpas.core.config*.pas 2>/dev/null; then ok "格式: $fmt"; else warn_check "格式未发现: $fmt"; fi
done

printf "\n${BOLD}C5: 门面单元${NC}\n"
FACADE="$SRC_DIR/nextpas.core.config.pas"
if [ -f "$FACADE" ]; then ok "门面文件存在"; else fail_check "config.pas 门面缺失"; fi
