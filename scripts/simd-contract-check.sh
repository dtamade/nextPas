#!/usr/bin/env bash
# simd-contract-check.sh — simd 模块契约完备性门禁
set -euo pipefail
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
CONTRACT="$REPO_ROOT/core/docs/simd/CONTRACT.md"
SRC_DIR="$REPO_ROOT/core/src"
TEST_DIR="$REPO_ROOT/core/tests/nextpas.core.simd"
...
printf "\n${BOLD}C2: 源文件规模${NC}\n"
PAS_COUNT=$(find "$SRC_DIR" -name 'nextpas.core.simd*.pas' 2>/dev/null | wc -l)
INC_COUNT=$(find "$SRC_DIR" -name 'nextpas.core.simd*.inc' 2>/dev/null | wc -l)
