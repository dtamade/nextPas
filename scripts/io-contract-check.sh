#!/usr/bin/env bash
# io-contract-check.sh — io 模块契约完备性门禁
set -euo pipefail
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
CONTRACT="$REPO_ROOT/core/docs/io/CONTRACT.md"
SRC_DIR="$REPO_ROOT/core/src"
TEST_DIR="$REPO_ROOT/core/tests/nextpas.core.io"
...
printf "\n${BOLD}C2: 源文件完备性${NC}\n"
PAS_COUNT=$(find "$SRC_DIR" -name 'nextpas.core.io*.pas' 2>/dev/null | wc -l)
ok "源文件: $PAS_COUNT 个"
printf "\n${BOLD}C3: 核心接口${NC}\n"
for iface in "IStream" "IReader" "IWriter" "IReadWriteCloser"; do
  if grep -rql "\b$iface\b" "$SRC_DIR"/nextpas.core.io*.pas 2>/dev/null; then ok "接口: $iface"; else warn_check "接口未发现: $iface"; fi
done
printf "\n${BOLD}C4: 门面+测试${NC}\n"
[ -f "$SRC_DIR/nextpas.core.io.pas" ] && ok "门面文件存在" || warn_check "io.pas 门面缺失"
