...
printf "\n${BOLD}C2: 源文件完备性${NC}\n"
for f in nextpas.core.bytes.pas nextpas.core.bytes.base.pas nextpas.core.bytes.span.pas nextpas.core.bytes.builder.pas nextpas.core.bytes.endian.pas; do
  if [ -f "$SRC_DIR/$f" ]; then ok "文件: $f"; else warn_check "文件缺失: $f"; fi
done
printf "\n${BOLD}C3: 核心类型${NC}\n"
for type in "TBytesBuilder" "TSpan" "TByteOrder"; do
