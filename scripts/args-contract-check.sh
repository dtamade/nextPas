#!/usr/bin/env bash
# args-contract-check.sh — args 模块契约完备性门禁
set -euo pipefail
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
CONTRACT="$REPO_ROOT/core/docs/args/CONTRACT.md"
SRC_DIR="$REPO_ROOT/core/src"
TEST_DIR="$REPO_ROOT/core/tests/nextpas.core.args"
...
printf "\n${BOLD}C2: 源文件完备性${NC}\n"
for f in nextpas.core.args.pas nextpas.core.args.base.pas; do
  if [ -f "$SRC_DIR/$f" ]; then ok "文件: $f"; else fail_check "文件缺失: $f"; fi
done
printf "\n${BOLD}C3: 核心类型${NC}\n"
for type in "TArgParser" "TArgApp"; do
  if grep -rql "\b$type\b" "$SRC_DIR"/nextpas.core.args*.pas 2>/dev/null; then ok "类型: $type"; else warn_check "类型未发现: $type"; fi
done
printf "\n${BOLD}C4: 门面+测试${NC}\n"
[ -f "$SRC_DIR/nextpas.core.args.pas" ] && ok "门面文件存在" || fail_check "args.pas 门面缺失"
