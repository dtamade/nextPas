#!/usr/bin/env bash
# test-contract-check.sh — test 模块契约完备性门禁
set -euo pipefail
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
CONTRACT="$REPO_ROOT/core/docs/test/CONTRACT.md"
SRC_DIR="$REPO_ROOT/core/src"
TEST_DIR="$REPO_ROOT/core/tests/nextpas.core.test"
...
printf "\n${BOLD}C5: 门面+测试${NC}\n"
[ -f "$SRC_DIR/nextpas.core.test.pas" ] && ok "门面文件存在" || warn_check "test.pas 门面缺失"
