#!/usr/bin/env bash
# id-contract-check.sh — id 模块契约完备性门禁
set -euo pipefail
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
CONTRACT="$REPO_ROOT/core/docs/id/CONTRACT.md"
SRC_DIR="$REPO_ROOT/core/src"
TEST_DIR="$REPO_ROOT/core/tests/nextpas.core.id"
...
printf "\n${BOLD}C2: 源文件完备性${NC}\n"
PAS_COUNT=$(find "$SRC_DIR" -name 'nextpas.core.id*.pas' 2>/dev/null | wc -l)
ok "源文件: $PAS_COUNT 个"
printf "\n${BOLD}C3: ID 算法${NC}\n"
for alg in "UUID" "ULID" "NanoId" "Snowflake" "XID" "KSUID"; do
  if grep -rql "\b$alg\b" "$SRC_DIR"/nextpas.core.id*.pas 2>/dev/null; then ok "算法: $alg"; else warn_check "算法未发现: $alg"; fi
done
printf "\n${BOLD}C4: 门面+测试${NC}\n"
[ -f "$SRC_DIR/nextpas.core.id.pas" ] && ok "门面文件存在" || fail_check "id.pas 门面缺失"
