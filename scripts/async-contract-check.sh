#!/usr/bin/env bash
# async-contract-check.sh — async 模块契约完备性门禁
set -euo pipefail
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
CONTRACT="$REPO_ROOT/core/docs/async/CONTRACT.md"
SRC_DIR="$REPO_ROOT/core/src"
TEST_DIR="$REPO_ROOT/core/tests/nextpas.core.async"
...
printf "\n${BOLD}C2: 源文件完备性${NC}\n"
for f in nextpas.core.async.pas nextpas.core.async.base.pas nextpas.core.async.loop.pas nextpas.core.async.task.pas nextpas.core.async.timer.pas; do
  if [ -f "$SRC_DIR/$f" ]; then ok "文件: $f"; else warn_check "文件缺失: $f"; fi
done
printf "\n${BOLD}C3: 核心类型${NC}\n"
for type in "TAsyncLoop" "TAsyncPendingItem" "TTimeoutCtx"; do
  if grep -rql "\b$type\b" "$SRC_DIR"/nextpas.core.async*.pas 2>/dev/null; then ok "类型: $type"; else warn_check "类型未发现: $type"; fi
done
printf "\n${BOLD}C4: 门面+测试${NC}\n"
[ -f "$SRC_DIR/nextpas.core.async.pas" ] && ok "门面文件存在" || fail_check "async.pas 门面缺失"
