...
printf "\n${BOLD}C2: 源文件完备性${NC}\n"
for f in nextpas.core.async.pas nextpas.core.async.base.pas nextpas.core.async.future.pas nextpas.core.async.task.pas nextpas.core.async.scheduler.pas; do
  if [ -f "$SRC_DIR/$f" ]; then ok "文件: $f"; else warn_check "文件缺失: $f"; fi
done
printf "\n${BOLD}C3: 核心类型${NC}\n"
for type in "TFuture" "TTask" "TAsync"; do
