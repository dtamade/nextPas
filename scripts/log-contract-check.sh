#!/usr/bin/env bash
# log-contract-check.sh — log 模块契约完备性门禁
set -euo pipefail
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
CONTRACT="$REPO_ROOT/core/docs/log/CONTRACT.md"
SRC_DIR="$REPO_ROOT/core/src"
TEST_DIR="$REPO_ROOT/core/tests/nextpas.core.log"
...
printf "\n${BOLD}C2: 源文件完备性${NC}\n"
for f in nextpas.core.log.pas nextpas.core.log.intf.pas; do
...
printf "\n${BOLD}C4: 门面+测试${NC}\n"
[ -f "$SRC_DIR/nextpas.core.log.pas" ] && ok "门面文件存在" || fail_check "log.pas 门面缺失"
