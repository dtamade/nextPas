#!/usr/bin/env bash
# lockfree-contract-check.sh — lockfree 模块契约完备性门禁
set -euo pipefail
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
CONTRACT="$REPO_ROOT/core/docs/lockfree/CONTRACT.md"
SRC_DIR="$REPO_ROOT/core/src"
TEST_DIR="$REPO_ROOT/core/tests/nextpas.core.lockfree"
...
printf "\n${BOLD}C4: 内存回收${NC}\n"
for type in "EBR\|HazardPointer"; do
  if grep -rql "$type" "$SRC_DIR"/nextpas.core.lockfree*.pas 2>/dev/null; then ok "内存回收: $type"; else warn_check "内存回收未发现"; fi
done
printf "\n${BOLD}C5: 门面+测试${NC}\n"
[ -f "$SRC_DIR/nextpas.core.lockfree.pas" ] && ok "门面文件存在" || fail_check "lockfree.pas 门面缺失"
