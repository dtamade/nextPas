...
# toml-contract-check.sh — toml 模块契约完备性门禁
set -euo pipefail

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
CONTRACT="$REPO_ROOT/core/docs/toml/CONTRACT.md"
SRC_DIR="$REPO_ROOT/core/src"
TEST_DIR="$REPO_ROOT/core/tests/nextpas.core.toml"
...
printf "\n${BOLD}C2: 源文件完备性${NC}\n"
TOML_FILES=("nextpas.core.toml.pas" "nextpas.core.toml.base.pas" "nextpas.core.toml.value.pas"
            "nextpas.core.toml.parser.pas" "nextpas.core.toml.writer.pas" "nextpas.core.toml.builder.pas")
...
if grep -rql "IAllocator\|ResolveAllocator\|FAllocator" "$SRC_DIR"/nextpas.core.toml*.pas 2>/dev/null; then
  ok "IAllocator 集成"
else
...
printf "\n${BOLD}C5: 门面单元${NC}\n"
FACADE="$SRC_DIR/nextpas.core.toml.pas"
if [ -f "$FACADE" ]; then ok "门面文件存在"; else fail_check "toml.pas 门面缺失"; fi
