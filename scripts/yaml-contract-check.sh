...
# yaml-contract-check.sh — yaml 模块契约完备性门禁
set -euo pipefail

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
CONTRACT="$REPO_ROOT/core/docs/yaml/CONTRACT.md"
SRC_DIR="$REPO_ROOT/core/src"
TEST_DIR="$REPO_ROOT/core/tests/nextpas.core.yaml"
...
printf "\n${BOLD}C2: 源文件完备性${NC}\n"
YAML_FILES=("nextpas.core.yaml.pas" "nextpas.core.yaml.types.pas" "nextpas.core.yaml.value.pas"
            "nextpas.core.yaml.scanner.pas" "nextpas.core.yaml.parser.pas"
            "nextpas.core.yaml.writer.pas" "nextpas.core.yaml.builder.pas")
...
if grep -rql "IAllocator\|ResolveAllocator\|FAllocator" "$SRC_DIR"/nextpas.core.yaml*.pas 2>/dev/null; then
  ok "IAllocator 集成"
else
...
printf "\n${BOLD}C5: 门面单元${NC}\n"
FACADE="$SRC_DIR/nextpas.core.yaml.pas"
if [ -f "$FACADE" ]; then ok "门面文件存在"; else fail_check "yaml.pas 门面缺失"; fi
