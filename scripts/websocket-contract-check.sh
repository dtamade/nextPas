...
# websocket-contract-check.sh — websocket 模块契约完备性门禁
set -euo pipefail

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
CONTRACT="$REPO_ROOT/core/docs/websocket/CONTRACT.md"
SRC_DIR="$REPO_ROOT/core/src"
TEST_DIR="$REPO_ROOT/core/tests/nextpas.core.websocket"
...
printf "\n${BOLD}C2: 源文件完备性${NC}\n"
WS_FILES=("nextpas.core.websocket.pas" "nextpas.core.websocket.base.pas")
...
printf "\n${BOLD}C4: 门面单元${NC}\n"
FACADE="$SRC_DIR/nextpas.core.websocket.pas"
if [ -f "$FACADE" ]; then ok "门面文件存在"; else fail_check "websocket.pas 门面缺失"; fi
