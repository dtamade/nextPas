#!/usr/bin/env bash
# xml-contract-check.sh — xml 模块契约完备性门禁
set -euo pipefail
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
CONTRACT="$REPO_ROOT/core/docs/xml/CONTRACT.md"
SRC_DIR="$REPO_ROOT/core/src"
TEST_DIR="$REPO_ROOT/core/tests/nextpas.core.xml"
...
printf "\n${BOLD}C2: 源文件完备性${NC}\n"
PAS_COUNT=$(find "$SRC_DIR" -name 'nextpas.core.xml*.pas' 2>/dev/null | wc -l)
ok "源文件: $PAS_COUNT 个"
printf "\n${BOLD}C3: 核心类型${NC}\n"
for type in "TXmlNode" "TXmlParser" "TXmlWriter" "TXmlBuilder"; do
  if grep -rql "\b$type\b" "$SRC_DIR"/nextpas.core.xml*.pas 2>/dev/null; then ok "类型: $type"; else warn_check "类型未发现: $type"; fi
done
printf "\n${BOLD}C4: 门面+测试${NC}\n"
[ -f "$SRC_DIR/nextpas.core.xml.pas" ] && ok "门面文件存在" || fail_check "xml.pas 门面缺失"
