...
`failure-kind=...`、`diagnostic-code=parser.syntax-error|resolver.unit-not-found|resolver.ambiguous-unit-source|resolver.unit-cycle-detected|sema.duplicate-declaration|sema.ambiguous-overload|sema.wrong-argument-count|sema.unknown-callable|toolchain.host-compiler-exec-failed`、
...
`semantic-analysis-failed` + `sema.unknown-callable`，再对
