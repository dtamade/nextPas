...
最小 `doctor`、最小 `query symbols` 与只读 `pkg inspect / pkg graph`，而不是从大而全的工具集开始。
...
`pkg inspect / pkg graph` 只复用 workspace model 与 package manifest truth 做只读 package workflow
投影，不提前执行 resolver、fetch/install 或 lockfile mutation。
