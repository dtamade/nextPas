...
最小 `doctor`、最小 `query symbols` 与只读 `pkg inspect / pkg plan / pkg graph`，而不是从大而全的工具集开始。
...
package workflow 投影，不提前执行 resolver、fetch/install 或 lockfile mutation。canonical
`nextpas.lock` 当前只读解析最小 v1 detail，公开 `missing|ready|invalid`、format version、
...
lock-invalid、lock-out-of-sync、lock-missing 六类 preflight 终止原因；out-of-sync blocker
还会公开 expected package 与 actual lock entries detail。`pkg graph` 则把同一份 truth 再展开成 root/dependency
nodes 与 edges。
