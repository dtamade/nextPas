...
type
  ...
  TDiagnosticByteCountResolver = function(
...
procedure TDiagnosticsSink.BindByteCountResolver(
  const AResolver: TDiagnosticByteCountResolver
);
begin
  FByteCountResolver := AResolver;
end;

function TDiagnosticsSink.ResolveByteCount(
  const AFileId: TCoreId;
  const AByteOffset: LongInt
): LongInt;
begin
